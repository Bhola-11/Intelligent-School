/**
 * EduFlow Enterprise Operations Controller: Assignments (Assignment & Homework Management)
...
class AssignmentsOperationsController {
    constructor(config = {}) {
        this.app = 'assignments';
        this.domain = 'Assignments';
        this.codePrefix = 'ASSI';
        this.apiBase = config.apiBase || '/api/v1/assignments/assignments/';
    ...
    init() {
        document.addEventListener('DOMContentLoaded', () => {
            ...
            console.log('[EduFlow] Assignments Controller initialized successfully.');
        });
    }

    bindEventHandlers() {
        const selectAllBox = document.getElementById('select-all-assignments');
        if (selectAllBox) {
            selectAllBox.addEventListener('change', (e) => this.toggleSelectAll(e.target.checked));
        }

        const itemBoxes = document.querySelectorAll('.assignments-select-item');
        ...
        const refreshBtn = document.getElementById('btn-refresh-assignments-telemetry');
        if (refreshBtn) {
            refreshBtn.addEventListener('click', () => this.fetchTelemetryData());
        }

        const bulkActionBtn = document.getElementById('btn-bulk-action-assignments');
        if (bulkActionBtn) {
            bulkActionBtn.addEventListener('click', () => this.executeSelectedBulkAction());
        }

        const exportCsvBtn = document.getElementById('btn-export-assignments-csv');
        if (exportCsvBtn) {
            exportCsvBtn.addEventListener('click', () => this.exportToCSV());
        }

        const exportJsonBtn = document.getElementById('btn-export-assignments-json');
        if (exportJsonBtn) {
            exportJsonBtn.addEventListener('click', () => this.exportToJSON());
        }

        const printBtn = document.getElementById('btn-print-assignments');
        if (printBtn) {
            printBtn.addEventListener('click', () => this.triggerPrintView());
        }

        const prevPageBtn = document.getElementById('btn-prev-page-assignments');
        if (prevPageBtn) {
            prevPageBtn.addEventListener('click', () => this.changePage(-1));
        }

        const nextPageBtn = document.getElementById('btn-next-page-assignments');
        if (nextPageBtn) {
            nextPageBtn.addEventListener('click', () => this.changePage(1));
        }

        const pageSizeSelector = document.getElementById('select-pagesize-assignments');
        if (pageSizeSelector) {
        ...
        const resetFilterBtn = document.getElementById('btn-reset-filters-assignments');
        if (resetFilterBtn) {
            resetFilterBtn.addEventListener('click', () => this.resetAllFilters());
        }

        const sortHeaders = document.querySelectorAll('.th-sortable-assignments');
    ...
    initDataCacheFromDOM() {
        const rows = document.querySelectorAll('#tbody-assignments tr');
        this.records = [];
        rows.forEach(row => {
    ...
    updateBulkActionBar() {
        const countSpan = document.getElementById('selected-assignments-count');
        const bulkBar = document.getElementById('bulk-action-bar-assignments');
        if (countSpan) {
    ...
    async performLiveSearch(query) {
        const tableBody = document.getElementById('tbody-assignments');
        if (!tableBody) return;
    ...
    updateSortHeaderIndicators() {
        document.querySelectorAll('.th-sortable-assignments').forEach(th => {
            const col = th.getAttribute('data-sort-col');
    ...
    renderFilteredPage() {
        ...
        if (pageItems.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="8" class="text-center py-4 text-muted">No Assignments records match search criteria.</td></tr>';
            this.updatePaginationDisplay();
            return;
        }

        tableBody.innerHTML = pageItems.map(r => '<tr>' +
            '<td><input type="checkbox" class="assignments-select-item" value="' + r.id + '"' + (this.selectedIds.has(r.id) ? ' checked' : '') + '></td>' +
            '<td class="font-mono text-primary font-bold col-code">' + (r.code || ('ASSI-' + r.id)) + '</td>' +
            '<td class="col-name"><a href="/assignments/assignments/' + r.id + '/" class="hover:underline font-semibold">' + (r.name || ('Record #' + r.id)) + '</a></td>' +
            ...
            '<td class="text-end">' +
                '<button type="button" class="btn btn-sm btn-outline-info me-1" onclick="window.AssignmentsControllerInstance.openPreviewDrawer(\'' + r.id + '\')">Preview</button>' +
                '<a href="/assignments/assignments/' + r.id + '/" class="btn btn-sm btn-outline-secondary me-1">View</a>' +
                '<a href="/assignments/assignments/' + r.id + '/update/" class="btn btn-sm btn-outline-primary">Edit</a>' +
            '</td>' +
        '</tr>').join('');
    ...
    updatePaginationDisplay() {
        const pageSpan = document.getElementById('page-info-assignments');
        const totalPages = Math.ceil(this.filteredRecords.length / this.pageSize) || 1;
        if (pageSpan) {
            pageSpan.textContent = 'Page ' + this.currentPage + ' of ' + totalPages + ' (' + this.filteredRecords.length + ' items)';
        }
        const prevBtn = document.getElementById('btn-prev-page-assignments');
        const nextBtn = document.getElementById('btn-next-page-assignments');
        if (prevBtn) prevBtn.disabled = this.currentPage <= 1;
        if (nextBtn) nextBtn.disabled = this.currentPage >= totalPages;
    }

    async fetchTelemetryData() {
        const metricContainer = document.getElementById('assignments-telemetry-cards');
        ...
        try {
            ...
            console.warn('[EduFlow] Telemetry poll failed for Assignments:', err);
        }
    }

    updateMetricCard(cardKey, value) {
        const el = document.getElementById('assignments-metric-' + cardKey);
        if (el) {
    ...
    async executeSelectedBulkAction() {
        const selectAction = document.getElementById('bulk-action-select-assignments');
    ...
    exportToCSV() {
        ...
        link.setAttribute('download', 'assignments_export_' + new Date().toISOString().slice(0, 10) + '.csv');
    ...
    exportToJSON() {
        ...
        link.setAttribute('download', 'assignments_export_' + new Date().toISOString().slice(0, 10) + '.json');
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    ...
    openPreviewDrawer(recordId) {
        ...
        let drawer = document.getElementById('preview-drawer-assignments');
        if (!drawer) {
            drawer = document.createElement('div');
            drawer.id = 'preview-drawer-assignments';
        ...
        drawer.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <button type="button" class="btn btn-sm btn-outline-secondary" onclick="window.AssignmentsControllerInstance.closePreviewDrawer()">✕</button>
            </div>
            <hr class="my-2">
            <dl class="row">
            ...
            <div class="mt-4 d-flex gap-2">
                <a href="/assignments/assignments/${rec.id}/" class="btn btn-primary btn-sm flex-grow-1">Full Details</a>
                <a href="/assignments/assignments/${rec.id}/update/" class="btn btn-outline-primary btn-sm flex-grow-1">Edit</a>
            </div>
        `;
    ...
    closePreviewDrawer() {
        const drawer = document.getElementById('preview-drawer-assignments');
        if (drawer) {
    ...
    renderMetricsCanvas() {
        const canvas = document.getElementById('chart-assignments-utilization');
    ...
    initFilterBar() {
        const statusFilter = document.getElementById('filter-assignments-status');
        if (statusFilter) {
        ...
        const tierFilter = document.getElementById('filter-assignments-tier');
        if (tierFilter) {
...
window.AssignmentsController = AssignmentsOperationsController;
window.AssignmentsControllerInstance = new AssignmentsOperationsController();
