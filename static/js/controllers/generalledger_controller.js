/**
 * EduFlow Enterprise Operations Controller: GeneralLedger (General Ledger & Double-Entry Journals)
...
class GeneralLedgerOperationsController {
    constructor(config = {}) {
        this.app = 'accounting';
        this.domain = 'GeneralLedger';
        this.codePrefix = 'GENE';
        this.apiBase = config.apiBase || '/api/v1/accounting/generalledger/';
    ...
    init() {
        document.addEventListener('DOMContentLoaded', () => {
            ...
            console.log('[EduFlow] GeneralLedger Controller initialized successfully.');
        });
    }

    bindEventHandlers() {
        const selectAllBox = document.getElementById('select-all-generalledger');
        if (selectAllBox) {
            selectAllBox.addEventListener('change', (e) => this.toggleSelectAll(e.target.checked));
        }

        const itemBoxes = document.querySelectorAll('.generalledger-select-item');
        itemBoxes.forEach(box => {
            box.addEventListener('change', (e) => this.toggleItemSelect(box.value, e.target.checked));
        });

        const searchInput = document.getElementById('search-generalledger');
        if (searchInput) {
            let debounceTimer;
        ...
        const refreshBtn = document.getElementById('btn-refresh-generalledger-telemetry');
        if (refreshBtn) {
            refreshBtn.addEventListener('click', () => this.fetchTelemetryData());
        }

        const bulkActionBtn = document.getElementById('btn-bulk-action-generalledger');
        if (bulkActionBtn) {
            bulkActionBtn.addEventListener('click', () => this.executeSelectedBulkAction());
        }

        const exportCsvBtn = document.getElementById('btn-export-generalledger-csv');
        if (exportCsvBtn) {
            exportCsvBtn.addEventListener('click', () => this.exportToCSV());
        }

        const exportJsonBtn = document.getElementById('btn-export-generalledger-json');
        if (exportJsonBtn) {
            exportJsonBtn.addEventListener('click', () => this.exportToJSON());
        }

        const printBtn = document.getElementById('btn-print-generalledger');
        if (printBtn) {
            printBtn.addEventListener('click', () => this.triggerPrintView());
        }

        const prevPageBtn = document.getElementById('btn-prev-page-generalledger');
        if (prevPageBtn) {
            prevPageBtn.addEventListener('click', () => this.changePage(-1));
        }

        const nextPageBtn = document.getElementById('btn-next-page-generalledger');
        if (nextPageBtn) {
            nextPageBtn.addEventListener('click', () => this.changePage(1));
        }

        const pageSizeSelector = document.getElementById('select-pagesize-generalledger');
        if (pageSizeSelector) {
        ...
        const resetFilterBtn = document.getElementById('btn-reset-filters-generalledger');
        if (resetFilterBtn) {
            resetFilterBtn.addEventListener('click', () => this.resetAllFilters());
        }

        const sortHeaders = document.querySelectorAll('.th-sortable-generalledger');
    ...
    initDataCacheFromDOM() {
        const rows = document.querySelectorAll('#tbody-generalledger tr');
        this.records = [];
        rows.forEach(row => {
    ...
    updateBulkActionBar() {
        const countSpan = document.getElementById('selected-generalledger-count');
        const bulkBar = document.getElementById('bulk-action-bar-generalledger');
        if (countSpan) {
    ...
    async performLiveSearch(query) {
        const tableBody = document.getElementById('tbody-generalledger');
        if (!tableBody) return;
    ...
    updateSortHeaderIndicators() {
        document.querySelectorAll('.th-sortable-generalledger').forEach(th => {
            const col = th.getAttribute('data-sort-col');
    ...
    renderFilteredPage() {
        ...
        if (pageItems.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="8" class="text-center py-4 text-muted">No GeneralLedger records match search criteria.</td></tr>';
            this.updatePaginationDisplay();
            return;
        }

        tableBody.innerHTML = pageItems.map(r => '<tr>' +
            '<td><input type="checkbox" class="generalledger-select-item" value="' + r.id + '"' + (this.selectedIds.has(r.id) ? ' checked' : '') + '></td>' +
            '<td class="font-mono text-primary font-bold col-code">' + (r.code || ('GENE-' + r.id)) + '</td>' +
            '<td class="col-name"><a href="/accounting/generalledger/' + r.id + '/" class="hover:underline font-semibold">' + (r.name || ('Record #' + r.id)) + '</a></td>' +
            ...
            '<td class="text-end">' +
                '<button type="button" class="btn btn-sm btn-outline-info me-1" onclick="window.GeneralLedgerControllerInstance.openPreviewDrawer(\'' + r.id + '\')">Preview</button>' +
                '<a href="/accounting/generalledger/' + r.id + '/" class="btn btn-sm btn-outline-secondary me-1">View</a>' +
                '<a href="/accounting/generalledger/' + r.id + '/update/" class="btn btn-sm btn-outline-primary">Edit</a>' +
            '</td>' +
        '</tr>').join('');
    ...
    updatePaginationDisplay() {
        const pageSpan = document.getElementById('page-info-generalledger');
        const totalPages = Math.ceil(this.filteredRecords.length / this.pageSize) || 1;
        if (pageSpan) {
            pageSpan.textContent = 'Page ' + this.currentPage + ' of ' + totalPages + ' (' + this.filteredRecords.length + ' items)';
        }
        const prevBtn = document.getElementById('btn-prev-page-generalledger');
        const nextBtn = document.getElementById('btn-next-page-generalledger');
        if (prevBtn) prevBtn.disabled = this.currentPage <= 1;
        if (nextBtn) nextBtn.disabled = this.currentPage >= totalPages;
    }

    async fetchTelemetryData() {
        const metricContainer = document.getElementById('generalledger-telemetry-cards');
        ...
        try {
            ...
            console.warn('[EduFlow] Telemetry poll failed for GeneralLedger:', err);
        }
    }

    updateMetricCard(cardKey, value) {
        const el = document.getElementById('generalledger-metric-' + cardKey);
        if (el) {
    ...
    async executeSelectedBulkAction() {
        const selectAction = document.getElementById('bulk-action-select-generalledger');
    ...
    exportToCSV() {
        ...
        link.setAttribute('download', 'generalledger_export_' + new Date().toISOString().slice(0, 10) + '.csv');
    ...
    exportToJSON() {
        ...
        link.setAttribute('download', 'generalledger_export_' + new Date().toISOString().slice(0, 10) + '.json');
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    ...
    openPreviewDrawer(recordId) {
        ...
        let drawer = document.getElementById('preview-drawer-generalledger');
        if (!drawer) {
            drawer = document.createElement('div');
            drawer.id = 'preview-drawer-generalledger';
        ...
        drawer.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <button type="button" class="btn btn-sm btn-outline-secondary" onclick="window.GeneralLedgerControllerInstance.closePreviewDrawer()">✕</button>
            </div>
            <hr class="my-2">
            <dl class="row">
            ...
            <div class="mt-4 d-flex gap-2">
                <a href="/accounting/generalledger/${rec.id}/" class="btn btn-primary btn-sm flex-grow-1">Full Details</a>
                <a href="/accounting/generalledger/${rec.id}/update/" class="btn btn-outline-primary btn-sm flex-grow-1">Edit</a>
            </div>
        `;
    ...
    closePreviewDrawer() {
        const drawer = document.getElementById('preview-drawer-generalledger');
        if (drawer) {
    ...
    renderMetricsCanvas() {
        const canvas = document.getElementById('chart-generalledger-utilization');
    ...
    initFilterBar() {
        const statusFilter = document.getElementById('filter-generalledger-status');
        if (statusFilter) {
        ...
        const tierFilter = document.getElementById('filter-generalledger-tier');
        if (tierFilter) {
...
window.GeneralLedgerController = GeneralLedgerOperationsController;
window.GeneralLedgerControllerInstance = new GeneralLedgerOperationsController();
