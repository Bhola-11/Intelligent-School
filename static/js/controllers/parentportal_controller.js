/**
 * EduFlow Enterprise Operations Controller: ParentPortal (Parent & Guardian Multi-Child Portal)
 * Module: apps/portals
...
class ParentPortalOperationsController {
    constructor(config = {}) {
        this.app = 'portals';
        this.domain = 'ParentPortal';
        this.codePrefix = 'PARE';
        this.apiBase = config.apiBase || '/api/v1/portals/parentportal/';
    ...
    init() {
        document.addEventListener('DOMContentLoaded', () => {
            ...
            console.log('[EduFlow] ParentPortal Controller initialized successfully.');
        });
    }

    bindEventHandlers() {
        const selectAllBox = document.getElementById('select-all-parentportal');
        if (selectAllBox) {
            selectAllBox.addEventListener('change', (e) => this.toggleSelectAll(e.target.checked));
        }

        const itemBoxes = document.querySelectorAll('.parentportal-select-item');
        itemBoxes.forEach(box => {
            box.addEventListener('change', (e) => this.toggleItemSelect(box.value, e.target.checked));
        });

        const searchInput = document.getElementById('search-parentportal');
        if (searchInput) {
            let debounceTimer;
        ...
        const refreshBtn = document.getElementById('btn-refresh-parentportal-telemetry');
        if (refreshBtn) {
            refreshBtn.addEventListener('click', () => this.fetchTelemetryData());
        }

        const bulkActionBtn = document.getElementById('btn-bulk-action-parentportal');
        if (bulkActionBtn) {
            bulkActionBtn.addEventListener('click', () => this.executeSelectedBulkAction());
        }

        const exportCsvBtn = document.getElementById('btn-export-parentportal-csv');
        if (exportCsvBtn) {
            exportCsvBtn.addEventListener('click', () => this.exportToCSV());
        }

        const exportJsonBtn = document.getElementById('btn-export-parentportal-json');
        if (exportJsonBtn) {
            exportJsonBtn.addEventListener('click', () => this.exportToJSON());
        }

        const printBtn = document.getElementById('btn-print-parentportal');
        if (printBtn) {
            printBtn.addEventListener('click', () => this.triggerPrintView());
        }

        const prevPageBtn = document.getElementById('btn-prev-page-parentportal');
        if (prevPageBtn) {
            prevPageBtn.addEventListener('click', () => this.changePage(-1));
        }

        const nextPageBtn = document.getElementById('btn-next-page-parentportal');
        if (nextPageBtn) {
            nextPageBtn.addEventListener('click', () => this.changePage(1));
        }

        const pageSizeSelector = document.getElementById('select-pagesize-parentportal');
        if (pageSizeSelector) {
        ...
        const resetFilterBtn = document.getElementById('btn-reset-filters-parentportal');
        if (resetFilterBtn) {
            resetFilterBtn.addEventListener('click', () => this.resetAllFilters());
        }

        const sortHeaders = document.querySelectorAll('.th-sortable-parentportal');
    ...
    initDataCacheFromDOM() {
        const rows = document.querySelectorAll('#tbody-parentportal tr');
        this.records = [];
        rows.forEach(row => {
    ...
    updateBulkActionBar() {
        const countSpan = document.getElementById('selected-parentportal-count');
        const bulkBar = document.getElementById('bulk-action-bar-parentportal');
        if (countSpan) {
    ...
    async performLiveSearch(query) {
        const tableBody = document.getElementById('tbody-parentportal');
        if (!tableBody) return;
    ...
    updateSortHeaderIndicators() {
        document.querySelectorAll('.th-sortable-parentportal').forEach(th => {
            const col = th.getAttribute('data-sort-col');
    ...
    renderFilteredPage() {
        ...
        if (pageItems.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="8" class="text-center py-4 text-muted">No ParentPortal records match search criteria.</td></tr>';
            this.updatePaginationDisplay();
            return;
        }

        tableBody.innerHTML = pageItems.map(r => '<tr>' +
            '<td><input type="checkbox" class="parentportal-select-item" value="' + r.id + '"' + (this.selectedIds.has(r.id) ? ' checked' : '') + '></td>' +
            '<td class="font-mono text-primary font-bold col-code">' + (r.code || ('PARE-' + r.id)) + '</td>' +
            '<td class="col-name"><a href="/portals/parentportal/' + r.id + '/" class="hover:underline font-semibold">' + (r.name || ('Record #' + r.id)) + '</a></td>' +
            ...
            '<td class="text-end">' +
                '<button type="button" class="btn btn-sm btn-outline-info me-1" onclick="window.ParentPortalControllerInstance.openPreviewDrawer(\'' + r.id + '\')">Preview</button>' +
                '<a href="/portals/parentportal/' + r.id + '/" class="btn btn-sm btn-outline-secondary me-1">View</a>' +
                '<a href="/portals/parentportal/' + r.id + '/update/" class="btn btn-sm btn-outline-primary">Edit</a>' +
            '</td>' +
        '</tr>').join('');
    ...
    updatePaginationDisplay() {
        const pageSpan = document.getElementById('page-info-parentportal');
        const totalPages = Math.ceil(this.filteredRecords.length / this.pageSize) || 1;
        if (pageSpan) {
            pageSpan.textContent = 'Page ' + this.currentPage + ' of ' + totalPages + ' (' + this.filteredRecords.length + ' items)';
        }
        const prevBtn = document.getElementById('btn-prev-page-parentportal');
        const nextBtn = document.getElementById('btn-next-page-parentportal');
        if (prevBtn) prevBtn.disabled = this.currentPage <= 1;
        if (nextBtn) nextBtn.disabled = this.currentPage >= totalPages;
    }

    async fetchTelemetryData() {
        const metricContainer = document.getElementById('parentportal-telemetry-cards');
        ...
        try {
            ...
            console.warn('[EduFlow] Telemetry poll failed for ParentPortal:', err);
        }
    }

    updateMetricCard(cardKey, value) {
        const el = document.getElementById('parentportal-metric-' + cardKey);
        if (el) {
    ...
    async executeSelectedBulkAction() {
        const selectAction = document.getElementById('bulk-action-select-parentportal');
    ...
    exportToCSV() {
        ...
        link.setAttribute('download', 'parentportal_export_' + new Date().toISOString().slice(0, 10) + '.csv');
    ...
    exportToJSON() {
        ...
        link.setAttribute('download', 'parentportal_export_' + new Date().toISOString().slice(0, 10) + '.json');
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    ...
    openPreviewDrawer(recordId) {
        ...
        let drawer = document.getElementById('preview-drawer-parentportal');
        if (!drawer) {
            drawer = document.createElement('div');
            drawer.id = 'preview-drawer-parentportal';
        ...
        drawer.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <button type="button" class="btn btn-sm btn-outline-secondary" onclick="window.ParentPortalControllerInstance.closePreviewDrawer()">✕</button>
            </div>
            <hr class="my-2">
            <dl class="row">
            ...
            <div class="mt-4 d-flex gap-2">
                <a href="/portals/parentportal/${rec.id}/" class="btn btn-primary btn-sm flex-grow-1">Full Details</a>
                <a href="/portals/parentportal/${rec.id}/update/" class="btn btn-outline-primary btn-sm flex-grow-1">Edit</a>
            </div>
        `;
    ...
    closePreviewDrawer() {
        const drawer = document.getElementById('preview-drawer-parentportal');
        if (drawer) {
    ...
    renderMetricsCanvas() {
        const canvas = document.getElementById('chart-parentportal-utilization');
    ...
    initFilterBar() {
        const statusFilter = document.getElementById('filter-parentportal-status');
        if (statusFilter) {
        ...
        const tierFilter = document.getElementById('filter-parentportal-tier');
        if (tierFilter) {
...
window.ParentPortalController = ParentPortalOperationsController;
window.ParentPortalControllerInstance = new ParentPortalOperationsController();
