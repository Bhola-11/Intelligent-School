/**
 * EduFlow Enterprise Operations Controller: RBAC (Role-Based Access Control & Permissions)
...
class RBACOperationsController {
    constructor(config = {}) {
        this.app = 'accounts';
        this.domain = 'RBAC';
        this.codePrefix = 'RBAC';
        this.apiBase = config.apiBase || '/api/v1/accounts/rbac/';
    ...
    init() {
        document.addEventListener('DOMContentLoaded', () => {
            ...
            console.log('[EduFlow] RBAC Controller initialized successfully.');
        });
    }

    bindEventHandlers() {
        const selectAllBox = document.getElementById('select-all-rbac');
        if (selectAllBox) {
            selectAllBox.addEventListener('change', (e) => this.toggleSelectAll(e.target.checked));
        }

        const itemBoxes = document.querySelectorAll('.rbac-select-item');
        itemBoxes.forEach(box => {
            box.addEventListener('change', (e) => this.toggleItemSelect(box.value, e.target.checked));
        });

        const searchInput = document.getElementById('search-rbac');
        if (searchInput) {
            let debounceTimer;
        ...
        const refreshBtn = document.getElementById('btn-refresh-rbac-telemetry');
        if (refreshBtn) {
            refreshBtn.addEventListener('click', () => this.fetchTelemetryData());
        }

        const bulkActionBtn = document.getElementById('btn-bulk-action-rbac');
        if (bulkActionBtn) {
            bulkActionBtn.addEventListener('click', () => this.executeSelectedBulkAction());
        }

        const exportCsvBtn = document.getElementById('btn-export-rbac-csv');
        if (exportCsvBtn) {
            exportCsvBtn.addEventListener('click', () => this.exportToCSV());
        }

        const exportJsonBtn = document.getElementById('btn-export-rbac-json');
        if (exportJsonBtn) {
            exportJsonBtn.addEventListener('click', () => this.exportToJSON());
        }

        const printBtn = document.getElementById('btn-print-rbac');
        if (printBtn) {
            printBtn.addEventListener('click', () => this.triggerPrintView());
        }

        const prevPageBtn = document.getElementById('btn-prev-page-rbac');
        if (prevPageBtn) {
            prevPageBtn.addEventListener('click', () => this.changePage(-1));
        }

        const nextPageBtn = document.getElementById('btn-next-page-rbac');
        if (nextPageBtn) {
            nextPageBtn.addEventListener('click', () => this.changePage(1));
        }

        const pageSizeSelector = document.getElementById('select-pagesize-rbac');
        if (pageSizeSelector) {
        ...
        const resetFilterBtn = document.getElementById('btn-reset-filters-rbac');
        if (resetFilterBtn) {
            resetFilterBtn.addEventListener('click', () => this.resetAllFilters());
        }

        const sortHeaders = document.querySelectorAll('.th-sortable-rbac');
    ...
    initDataCacheFromDOM() {
        const rows = document.querySelectorAll('#tbody-rbac tr');
        this.records = [];
        rows.forEach(row => {
    ...
    updateBulkActionBar() {
        const countSpan = document.getElementById('selected-rbac-count');
        const bulkBar = document.getElementById('bulk-action-bar-rbac');
        if (countSpan) {
    ...
    async performLiveSearch(query) {
        const tableBody = document.getElementById('tbody-rbac');
        if (!tableBody) return;
    ...
    updateSortHeaderIndicators() {
        document.querySelectorAll('.th-sortable-rbac').forEach(th => {
            const col = th.getAttribute('data-sort-col');
    ...
    renderFilteredPage() {
        ...
        if (pageItems.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="8" class="text-center py-4 text-muted">No RBAC records match search criteria.</td></tr>';
            this.updatePaginationDisplay();
            return;
        }

        tableBody.innerHTML = pageItems.map(r => '<tr>' +
            '<td><input type="checkbox" class="rbac-select-item" value="' + r.id + '"' + (this.selectedIds.has(r.id) ? ' checked' : '') + '></td>' +
            '<td class="font-mono text-primary font-bold col-code">' + (r.code || ('RBAC-' + r.id)) + '</td>' +
            '<td class="col-name"><a href="/accounts/rbac/' + r.id + '/" class="hover:underline font-semibold">' + (r.name || ('Record #' + r.id)) + '</a></td>' +
            ...
            '<td class="text-end">' +
                '<button type="button" class="btn btn-sm btn-outline-info me-1" onclick="window.RBACControllerInstance.openPreviewDrawer(\'' + r.id + '\')">Preview</button>' +
                '<a href="/accounts/rbac/' + r.id + '/" class="btn btn-sm btn-outline-secondary me-1">View</a>' +
                '<a href="/accounts/rbac/' + r.id + '/update/" class="btn btn-sm btn-outline-primary">Edit</a>' +
            '</td>' +
        '</tr>').join('');
    ...
    updatePaginationDisplay() {
        const pageSpan = document.getElementById('page-info-rbac');
        const totalPages = Math.ceil(this.filteredRecords.length / this.pageSize) || 1;
        if (pageSpan) {
            pageSpan.textContent = 'Page ' + this.currentPage + ' of ' + totalPages + ' (' + this.filteredRecords.length + ' items)';
        }
        const prevBtn = document.getElementById('btn-prev-page-rbac');
        const nextBtn = document.getElementById('btn-next-page-rbac');
        if (prevBtn) prevBtn.disabled = this.currentPage <= 1;
        if (nextBtn) nextBtn.disabled = this.currentPage >= totalPages;
    }

    async fetchTelemetryData() {
        const metricContainer = document.getElementById('rbac-telemetry-cards');
        ...
        try {
            ...
            console.warn('[EduFlow] Telemetry poll failed for RBAC:', err);
        }
    }

    updateMetricCard(cardKey, value) {
        const el = document.getElementById('rbac-metric-' + cardKey);
        if (el) {
    ...
    async executeSelectedBulkAction() {
        const selectAction = document.getElementById('bulk-action-select-rbac');
    ...
    exportToCSV() {
        ...
        link.setAttribute('download', 'rbac_export_' + new Date().toISOString().slice(0, 10) + '.csv');
    ...
    exportToJSON() {
        ...
        link.setAttribute('download', 'rbac_export_' + new Date().toISOString().slice(0, 10) + '.json');
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    ...
    openPreviewDrawer(recordId) {
        ...
        let drawer = document.getElementById('preview-drawer-rbac');
        if (!drawer) {
            drawer = document.createElement('div');
            drawer.id = 'preview-drawer-rbac';
        ...
        drawer.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <button type="button" class="btn btn-sm btn-outline-secondary" onclick="window.RBACControllerInstance.closePreviewDrawer()">✕</button>
            </div>
            <hr class="my-2">
            <dl class="row">
            ...
            <div class="mt-4 d-flex gap-2">
                <a href="/accounts/rbac/${rec.id}/" class="btn btn-primary btn-sm flex-grow-1">Full Details</a>
                <a href="/accounts/rbac/${rec.id}/update/" class="btn btn-outline-primary btn-sm flex-grow-1">Edit</a>
            </div>
        `;
    ...
    closePreviewDrawer() {
        const drawer = document.getElementById('preview-drawer-rbac');
        if (drawer) {
    ...
    renderMetricsCanvas() {
        const canvas = document.getElementById('chart-rbac-utilization');
    ...
    initFilterBar() {
        const statusFilter = document.getElementById('filter-rbac-status');
        if (statusFilter) {
        ...
        const tierFilter = document.getElementById('filter-rbac-tier');
        if (tierFilter) {
...
window.RBACController = RBACOperationsController;
window.RBACControllerInstance = new RBACOperationsController();
