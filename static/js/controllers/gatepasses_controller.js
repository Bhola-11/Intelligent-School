/**
 * EduFlow Enterprise Operations Controller: GatePasses (Hostel Gate Passes & Resident Discipline)
 * Module: apps/hostel
...
class GatePassesOperationsController {
    constructor(config = {}) {
        this.app = 'hostel';
        this.domain = 'GatePasses';
        this.codePrefix = 'GATE';
        this.apiBase = config.apiBase || '/api/v1/hostel/gatepasses/';
    ...
    init() {
        document.addEventListener('DOMContentLoaded', () => {
            ...
            console.log('[EduFlow] GatePasses Controller initialized successfully.');
        });
    }

    bindEventHandlers() {
        const selectAllBox = document.getElementById('select-all-gatepasses');
        if (selectAllBox) {
            selectAllBox.addEventListener('change', (e) => this.toggleSelectAll(e.target.checked));
        }

        const itemBoxes = document.querySelectorAll('.gatepasses-select-item');
        itemBoxes.forEach(box => {
            box.addEventListener('change', (e) => this.toggleItemSelect(box.value, e.target.checked));
        });

        const searchInput = document.getElementById('search-gatepasses');
        if (searchInput) {
            let debounceTimer;
        ...
        const refreshBtn = document.getElementById('btn-refresh-gatepasses-telemetry');
        if (refreshBtn) {
            refreshBtn.addEventListener('click', () => this.fetchTelemetryData());
        }

        const bulkActionBtn = document.getElementById('btn-bulk-action-gatepasses');
        if (bulkActionBtn) {
            bulkActionBtn.addEventListener('click', () => this.executeSelectedBulkAction());
        }

        const exportCsvBtn = document.getElementById('btn-export-gatepasses-csv');
        if (exportCsvBtn) {
            exportCsvBtn.addEventListener('click', () => this.exportToCSV());
        }

        const exportJsonBtn = document.getElementById('btn-export-gatepasses-json');
        if (exportJsonBtn) {
            exportJsonBtn.addEventListener('click', () => this.exportToJSON());
        }

        const printBtn = document.getElementById('btn-print-gatepasses');
        if (printBtn) {
            printBtn.addEventListener('click', () => this.triggerPrintView());
        }

        const prevPageBtn = document.getElementById('btn-prev-page-gatepasses');
        if (prevPageBtn) {
            prevPageBtn.addEventListener('click', () => this.changePage(-1));
        }

        const nextPageBtn = document.getElementById('btn-next-page-gatepasses');
        if (nextPageBtn) {
            nextPageBtn.addEventListener('click', () => this.changePage(1));
        }

        const pageSizeSelector = document.getElementById('select-pagesize-gatepasses');
        if (pageSizeSelector) {
        ...
        const resetFilterBtn = document.getElementById('btn-reset-filters-gatepasses');
        if (resetFilterBtn) {
            resetFilterBtn.addEventListener('click', () => this.resetAllFilters());
        }

        const sortHeaders = document.querySelectorAll('.th-sortable-gatepasses');
    ...
    initDataCacheFromDOM() {
        const rows = document.querySelectorAll('#tbody-gatepasses tr');
        this.records = [];
        rows.forEach(row => {
    ...
    updateBulkActionBar() {
        const countSpan = document.getElementById('selected-gatepasses-count');
        const bulkBar = document.getElementById('bulk-action-bar-gatepasses');
        if (countSpan) {
    ...
    async performLiveSearch(query) {
        const tableBody = document.getElementById('tbody-gatepasses');
        if (!tableBody) return;
    ...
    updateSortHeaderIndicators() {
        document.querySelectorAll('.th-sortable-gatepasses').forEach(th => {
            const col = th.getAttribute('data-sort-col');
    ...
    renderFilteredPage() {
        ...
        if (pageItems.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="8" class="text-center py-4 text-muted">No GatePasses records match search criteria.</td></tr>';
            this.updatePaginationDisplay();
            return;
        }

        tableBody.innerHTML = pageItems.map(r => '<tr>' +
            '<td><input type="checkbox" class="gatepasses-select-item" value="' + r.id + '"' + (this.selectedIds.has(r.id) ? ' checked' : '') + '></td>' +
            '<td class="font-mono text-primary font-bold col-code">' + (r.code || ('GATE-' + r.id)) + '</td>' +
            '<td class="col-name"><a href="/hostel/gatepasses/' + r.id + '/" class="hover:underline font-semibold">' + (r.name || ('Record #' + r.id)) + '</a></td>' +
            ...
            '<td class="text-end">' +
                '<button type="button" class="btn btn-sm btn-outline-info me-1" onclick="window.GatePassesControllerInstance.openPreviewDrawer(\'' + r.id + '\')">Preview</button>' +
                '<a href="/hostel/gatepasses/' + r.id + '/" class="btn btn-sm btn-outline-secondary me-1">View</a>' +
                '<a href="/hostel/gatepasses/' + r.id + '/update/" class="btn btn-sm btn-outline-primary">Edit</a>' +
            '</td>' +
        '</tr>').join('');
    ...
    updatePaginationDisplay() {
        const pageSpan = document.getElementById('page-info-gatepasses');
        const totalPages = Math.ceil(this.filteredRecords.length / this.pageSize) || 1;
        if (pageSpan) {
            pageSpan.textContent = 'Page ' + this.currentPage + ' of ' + totalPages + ' (' + this.filteredRecords.length + ' items)';
        }
        const prevBtn = document.getElementById('btn-prev-page-gatepasses');
        const nextBtn = document.getElementById('btn-next-page-gatepasses');
        if (prevBtn) prevBtn.disabled = this.currentPage <= 1;
        if (nextBtn) nextBtn.disabled = this.currentPage >= totalPages;
    }

    async fetchTelemetryData() {
        const metricContainer = document.getElementById('gatepasses-telemetry-cards');
        ...
        try {
            ...
            console.warn('[EduFlow] Telemetry poll failed for GatePasses:', err);
        }
    }

    updateMetricCard(cardKey, value) {
        const el = document.getElementById('gatepasses-metric-' + cardKey);
        if (el) {
    ...
    async executeSelectedBulkAction() {
        const selectAction = document.getElementById('bulk-action-select-gatepasses');
    ...
    exportToCSV() {
        ...
        link.setAttribute('download', 'gatepasses_export_' + new Date().toISOString().slice(0, 10) + '.csv');
    ...
    exportToJSON() {
        ...
        link.setAttribute('download', 'gatepasses_export_' + new Date().toISOString().slice(0, 10) + '.json');
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    ...
    openPreviewDrawer(recordId) {
        ...
        let drawer = document.getElementById('preview-drawer-gatepasses');
        if (!drawer) {
            drawer = document.createElement('div');
            drawer.id = 'preview-drawer-gatepasses';
        ...
        drawer.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <button type="button" class="btn btn-sm btn-outline-secondary" onclick="window.GatePassesControllerInstance.closePreviewDrawer()">✕</button>
            </div>
            <hr class="my-2">
            <dl class="row">
            ...
            <div class="mt-4 d-flex gap-2">
                <a href="/hostel/gatepasses/${rec.id}/" class="btn btn-primary btn-sm flex-grow-1">Full Details</a>
                <a href="/hostel/gatepasses/${rec.id}/update/" class="btn btn-outline-primary btn-sm flex-grow-1">Edit</a>
            </div>
        `;
    ...
    closePreviewDrawer() {
        const drawer = document.getElementById('preview-drawer-gatepasses');
        if (drawer) {
    ...
    renderMetricsCanvas() {
        const canvas = document.getElementById('chart-gatepasses-utilization');
    ...
    initFilterBar() {
        const statusFilter = document.getElementById('filter-gatepasses-status');
        if (statusFilter) {
        ...
        const tierFilter = document.getElementById('filter-gatepasses-tier');
        if (tierFilter) {
...
window.GatePassesController = GatePassesOperationsController;
window.GatePassesControllerInstance = new GatePassesOperationsController();
