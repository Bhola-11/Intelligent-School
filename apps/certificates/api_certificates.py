"""
Extended REST API Controllers for EduFlow Certificates Generation & QR Verification (Certificates).
...
class CertificatesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(CertificatesMaster, pk=pk)
        data = master.to_dict()
...
class CertificatesAPISearchView(LoginRequiredMixin, View):
...
class CertificatesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class CertificatesAPITelemetryView(LoginRequiredMixin, View):
