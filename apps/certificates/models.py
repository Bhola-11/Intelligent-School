"""
Models for EduFlow Certificates Generation & QR Verification (Certificates) in apps.certificates.
...
# -----------------------------------------------------------------------------
class CertificatesStatusChoices(models.TextChoices):
...
class CertificatesPriorityChoices(models.TextChoices):
...
class CertificatesTierChoices(models.TextChoices):
...
class CertificatesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class CertificatesMasterQuerySet(models.QuerySet):
    def active(self):
...
class CertificatesMasterManager(models.Manager):
    def get_queryset(self):
        return CertificatesMasterQuerySet(self.model, using=self._db)
...
class CertificatesMaster(models.Model):
    """
    Primary master configuration entity for Certificates Generation & QR Verification.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = CertificatesStatusChoices
    PriorityChoices = CertificatesPriorityChoices
    TierChoices = CertificatesTierChoices
    ScopeChoices = CertificatesScopeChoices
    ...
    objects = CertificatesMasterManager()

    class Meta:
        db_table = 'eduflow_certificates_certificates_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Certificates Master")
        verbose_name_plural = _("Certificates Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_certi_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_certi_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_certi_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_certi_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'certificates:certificates_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/certificates/certificates/{self.pk}/"
...
# -----------------------------------------------------------------------------
class CertificatesItem(models.Model):
    """Subordinate component or line transaction item for Certificates."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Certificates Item")
        verbose_name_plural = _("Certificates Items")
...
# -----------------------------------------------------------------------------
class CertificatesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Certificates."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_allocation'
        ordering = ['-start_time']
        verbose_name = _("Certificates Allocation")
        verbose_name_plural = _("Certificates Allocations")
...
# -----------------------------------------------------------------------------
class CertificatesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Certificates."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Certificates Metric Record")
        verbose_name_plural = _("Certificates Metric Records")
...
# -----------------------------------------------------------------------------
class CertificatesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Certificates."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_rule'
        ordering = ['rule_code']
        verbose_name = _("Certificates Policy Rule")
        verbose_name_plural = _("Certificates Policy Rules")
...
# -----------------------------------------------------------------------------
class CertificatesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Certificates."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Certificates Schedule Period")
        verbose_name_plural = _("Certificates Schedule Periods")
...
# -----------------------------------------------------------------------------
class CertificatesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Certificates."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_review'
        ordering = ['-submitted_at']
        verbose_name = _("Certificates Review")
        verbose_name_plural = _("Certificates Reviews")
...
# -----------------------------------------------------------------------------
class CertificatesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Certificates Workflow Transition")
        verbose_name_plural = _("Certificates Workflow Transitions")
...
# -----------------------------------------------------------------------------
class CertificatesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_access'
        verbose_name = _("Certificates Access Rule")
        verbose_name_plural = _("Certificates Access Rules")
...
# -----------------------------------------------------------------------------
class CertificatesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_config'
        verbose_name = _("Certificates Config Parameter")
        verbose_name_plural = _("Certificates Config Parameters")
...
# -----------------------------------------------------------------------------
class CertificatesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Certificates Document Attachment")
        verbose_name_plural = _("Certificates Document Attachments")
...
# -----------------------------------------------------------------------------
class CertificatesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Certificates."""
    master = models.ForeignKey(CertificatesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_certificates_certificates_audit'
        ordering = ['-timestamp']
        verbose_name = _("Certificates Audit Entry")
        verbose_name_plural = _("Certificates Audit Entries")
