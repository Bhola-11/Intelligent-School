"""
Models for EduFlow Global Search, Audit Logs, Master Reports & CLI (MasterReports) in apps.reports.
...
# -----------------------------------------------------------------------------
class MasterReportsStatusChoices(models.TextChoices):
...
class MasterReportsPriorityChoices(models.TextChoices):
...
class MasterReportsTierChoices(models.TextChoices):
...
class MasterReportsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class MasterReportsMasterQuerySet(models.QuerySet):
    def active(self):
...
class MasterReportsMasterManager(models.Manager):
    def get_queryset(self):
        return MasterReportsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class MasterReportsMaster(models.Model):
    """
    Primary master configuration entity for Global Search, Audit Logs, Master Reports & CLI.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = MasterReportsStatusChoices
    PriorityChoices = MasterReportsPriorityChoices
    TierChoices = MasterReportsTierChoices
    ScopeChoices = MasterReportsScopeChoices
    ...
    objects = MasterReportsMasterManager()

    class Meta:
        db_table = 'eduflow_reports_masterreports_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("MasterReports Master")
        verbose_name_plural = _("MasterReports Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_maste_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_maste_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_maste_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_maste_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'reports:masterreports_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/reports/masterreports/{self.pk}/"
...
# -----------------------------------------------------------------------------
class MasterReportsItem(models.Model):
    """Subordinate component or line transaction item for MasterReports."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        ...
        verbose_name = _("MasterReports Item")
        verbose_name_plural = _("MasterReports Items")
...
# -----------------------------------------------------------------------------
class MasterReportsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for MasterReports."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        ...
        ordering = ['-start_time']
        verbose_name = _("MasterReports Allocation")
        verbose_name_plural = _("MasterReports Allocations")
...
# -----------------------------------------------------------------------------
class MasterReportsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for MasterReports."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        ...
        verbose_name = _("MasterReports Metric Record")
        verbose_name_plural = _("MasterReports Metric Records")
...
# -----------------------------------------------------------------------------
class MasterReportsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for MasterReports."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        ...
        ordering = ['rule_code']
        verbose_name = _("MasterReports Policy Rule")
        verbose_name_plural = _("MasterReports Policy Rules")
...
# -----------------------------------------------------------------------------
class MasterReportsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for MasterReports."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        ...
        verbose_name = _("MasterReports Schedule Period")
        verbose_name_plural = _("MasterReports Schedule Periods")
...
# -----------------------------------------------------------------------------
class MasterReportsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for MasterReports."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        ...
        verbose_name = _("MasterReports Review")
        verbose_name_plural = _("MasterReports Reviews")
...
# -----------------------------------------------------------------------------
class MasterReportsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        ...
        verbose_name = _("MasterReports Workflow Transition")
        verbose_name_plural = _("MasterReports Workflow Transitions")
...
# -----------------------------------------------------------------------------
class MasterReportsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        ...
        verbose_name = _("MasterReports Access Rule")
        verbose_name_plural = _("MasterReports Access Rules")
...
# -----------------------------------------------------------------------------
class MasterReportsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        ...
        verbose_name = _("MasterReports Config Parameter")
        verbose_name_plural = _("MasterReports Config Parameters")
...
# -----------------------------------------------------------------------------
class MasterReportsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        ...
        verbose_name = _("MasterReports Document Attachment")
        verbose_name_plural = _("MasterReports Document Attachments")
...
# -----------------------------------------------------------------------------
class MasterReportsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on MasterReports."""
    master = models.ForeignKey(MasterReportsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        ...
        ordering = ['-timestamp']
        verbose_name = _("MasterReports Audit Entry")
        verbose_name_plural = _("MasterReports Audit Entries")
