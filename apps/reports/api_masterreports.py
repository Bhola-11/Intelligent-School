"""
Extended REST API Controllers for EduFlow Global Search, Audit Logs, Master Reports & CLI (MasterReports).
...
class MasterReportsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(MasterReportsMaster, pk=pk)
        data = master.to_dict()
...
class MasterReportsAPISearchView(LoginRequiredMixin, View):
...
class MasterReportsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class MasterReportsAPITelemetryView(LoginRequiredMixin, View):
