"""
Models for EduFlow Learning Outcomes & Bloom's Taxonomy (Outcomes) in apps.academics.
...
# -----------------------------------------------------------------------------
class OutcomesStatusChoices(models.TextChoices):
...
class OutcomesPriorityChoices(models.TextChoices):
...
class OutcomesTierChoices(models.TextChoices):
...
class OutcomesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class OutcomesMasterQuerySet(models.QuerySet):
    def active(self):
...
class OutcomesMasterManager(models.Manager):
    def get_queryset(self):
        return OutcomesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class OutcomesMaster(models.Model):
    """
    Primary master configuration entity for Learning Outcomes & Bloom's Taxonomy.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = OutcomesStatusChoices
    PriorityChoices = OutcomesPriorityChoices
    TierChoices = OutcomesTierChoices
    ScopeChoices = OutcomesScopeChoices
    ...
    objects = OutcomesMasterManager()

    class Meta:
        db_table = 'eduflow_academics_outcomes_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Outcomes Master")
        verbose_name_plural = _("Outcomes Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_outco_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_outco_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_outco_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_outco_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'academics:outcomes_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/academics/outcomes/{self.pk}/"
...
# -----------------------------------------------------------------------------
class OutcomesItem(models.Model):
    """Subordinate component or line transaction item for Outcomes."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Outcomes Item")
        verbose_name_plural = _("Outcomes Items")
...
# -----------------------------------------------------------------------------
class OutcomesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Outcomes."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_allocation'
        ordering = ['-start_time']
        verbose_name = _("Outcomes Allocation")
        verbose_name_plural = _("Outcomes Allocations")
...
# -----------------------------------------------------------------------------
class OutcomesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Outcomes."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Outcomes Metric Record")
        verbose_name_plural = _("Outcomes Metric Records")
...
# -----------------------------------------------------------------------------
class OutcomesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Outcomes."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_rule'
        ordering = ['rule_code']
        verbose_name = _("Outcomes Policy Rule")
        verbose_name_plural = _("Outcomes Policy Rules")
...
# -----------------------------------------------------------------------------
class OutcomesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Outcomes."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Outcomes Schedule Period")
        verbose_name_plural = _("Outcomes Schedule Periods")
...
# -----------------------------------------------------------------------------
class OutcomesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Outcomes."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_review'
        ordering = ['-submitted_at']
        verbose_name = _("Outcomes Review")
        verbose_name_plural = _("Outcomes Reviews")
...
# -----------------------------------------------------------------------------
class OutcomesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Outcomes Workflow Transition")
        verbose_name_plural = _("Outcomes Workflow Transitions")
...
# -----------------------------------------------------------------------------
class OutcomesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_access'
        verbose_name = _("Outcomes Access Rule")
        verbose_name_plural = _("Outcomes Access Rules")
...
# -----------------------------------------------------------------------------
class OutcomesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_config'
        verbose_name = _("Outcomes Config Parameter")
        verbose_name_plural = _("Outcomes Config Parameters")
...
# -----------------------------------------------------------------------------
class OutcomesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Outcomes Document Attachment")
        verbose_name_plural = _("Outcomes Document Attachments")
...
# -----------------------------------------------------------------------------
class OutcomesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Outcomes."""
    master = models.ForeignKey(OutcomesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_academics_outcomes_audit'
        ordering = ['-timestamp']
        verbose_name = _("Outcomes Audit Entry")
        verbose_name_plural = _("Outcomes Audit Entries")
