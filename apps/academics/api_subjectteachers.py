"""
Extended REST API Controllers for EduFlow Teacher-Class-Subject Allocation Matrix (SubjectTeachers).
...
class SubjectTeachersAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(SubjectTeachersMaster, pk=pk)
        data = master.to_dict()
...
class SubjectTeachersAPISearchView(LoginRequiredMixin, View):
...
class SubjectTeachersAPIBatchUpdateView(LoginRequiredMixin, View):
...
class SubjectTeachersAPITelemetryView(LoginRequiredMixin, View):
