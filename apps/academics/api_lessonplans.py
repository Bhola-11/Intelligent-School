"""
Extended REST API Controllers for EduFlow Lesson Plans & Daily Teaching Logs (LessonPlans).
...
class LessonPlansAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(LessonPlansMaster, pk=pk)
        data = master.to_dict()
...
class LessonPlansAPISearchView(LoginRequiredMixin, View):
...
class LessonPlansAPIBatchUpdateView(LoginRequiredMixin, View):
...
class LessonPlansAPITelemetryView(LoginRequiredMixin, View):
