"""
Views for EduFlow Learning Outcomes & Bloom's Taxonomy (Outcomes).
...
try:
    ...
    from .forms import (
        OutcomesMasterForm, OutcomesItemForm, OutcomesAllocationForm,
        OutcomesMetricRecordForm, OutcomesPolicyRuleForm, OutcomesSchedulePeriodForm,
        OutcomesFeedbackReviewForm, OutcomesWorkflowTransitionForm, OutcomesAccessRuleForm,
        OutcomesConfigurationParameterForm, OutcomesDocumentAttachmentForm, OutcomesFilterForm,
        OutcomesBulkActionForm
    )
except ImportError:
    try:
...
class OutcomesListView(LoginRequiredMixin, ListView):
    template_name = 'academics/outcomes_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = OutcomesMaster.objects.all() if 'OutcomesMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = OutcomesFilterForm(self.request.GET) if 'OutcomesFilterForm' in globals() else None
        ctx['bulk_form'] = OutcomesBulkActionForm() if 'OutcomesBulkActionForm' in globals() else None
        ctx['total_count'] = OutcomesMaster.objects.count() if 'OutcomesMaster' in globals() else 0
        ctx['active_count'] = OutcomesMaster.objects.filter(status='ACTIVE').count() if 'OutcomesMaster' in globals() else 0
        ctx['pending_count'] = OutcomesMaster.objects.filter(status='PENDING_REVIEW').count() if 'OutcomesMaster' in globals() else 0
        ctx['page_title'] = "Learning Outcomes & Bloom's Taxonomy Directory"
        ctx['domain_name'] = "Outcomes"
        return ctx

class OutcomesDetailView(LoginRequiredMixin, DetailView):
    template_name = 'academics/outcomes_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(OutcomesMaster, pk=self.kwargs.get('pk'))
...
class OutcomesCreateView(LoginRequiredMixin, CreateView):
    template_name = 'academics/outcomes_form.html'
    form_class = OutcomesMasterForm
    success_url = reverse_lazy('academics:outcomes_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Outcomes record '{form.instance.name}' created successfully.")
        return super().form_valid(form)

class OutcomesUpdateView(LoginRequiredMixin, UpdateView):
    template_name = 'academics/outcomes_form.html'
    form_class = OutcomesMasterForm
    success_url = reverse_lazy('academics:outcomes_list')
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Outcomes record updated successfully.")
        return super().form_valid(form)

class OutcomesDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'academics/outcomes_confirm_delete.html'
    success_url = reverse_lazy('academics:outcomes_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class OutcomesItemCreateView(LoginRequiredMixin, CreateView):
    form_class = OutcomesItemForm
    template_name = 'academics/outcomes_form.html'

    def form_valid(self, form):
        master = get_object_or_404(OutcomesMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('academics:outcomes_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class OutcomesItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = OutcomesItemForm
    template_name = 'academics/outcomes_form.html'

    def get_object(self):
        return get_object_or_404(OutcomesItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('academics:outcomes_detail', kwargs={'pk': self.object.master_id})

class OutcomesItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'academics/outcomes_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class OutcomesAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = OutcomesAllocationForm
    template_name = 'academics/outcomes_form.html'
...
class OutcomesAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(OutcomesAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('academics:outcomes_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class OutcomesMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = OutcomesMetricRecordForm
    template_name = 'academics/outcomes_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class OutcomesPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = OutcomesPolicyRuleForm
    template_name = 'academics/outcomes_form.html'
...
# -----------------------------------------------------------------------------
class OutcomesSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = OutcomesSchedulePeriodForm
    template_name = 'academics/outcomes_form.html'
...
# -----------------------------------------------------------------------------
class OutcomesFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = OutcomesFeedbackReviewForm
    template_name = 'academics/outcomes_form.html'
...
# -----------------------------------------------------------------------------
class OutcomesWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = OutcomesWorkflowTransitionForm
    template_name = 'academics/outcomes_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class OutcomesAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = OutcomesAccessRuleForm
    template_name = 'academics/outcomes_form.html'
...
# -----------------------------------------------------------------------------
class OutcomesConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = OutcomesConfigurationParameterForm
    template_name = 'academics/outcomes_form.html'
...
# -----------------------------------------------------------------------------
class OutcomesDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = OutcomesDocumentAttachmentForm
    template_name = 'academics/outcomes_form.html'
...
# -----------------------------------------------------------------------------
class OutcomesBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = OutcomesMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('academics:outcomes_list')

class OutcomesExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="outcomes_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
...
class OutcomesExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = OutcomesMaster.objects.all() if 'OutcomesMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class OutcomesAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = OutcomesMaster.objects.all() if 'OutcomesMaster' in globals() else []
...
class OutcomesAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(OutcomesMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
