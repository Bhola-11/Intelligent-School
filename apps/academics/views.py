"""
Views for EduFlow Academic Curriculum & Syllabus Topics (Curriculum).
...
try:
    ...
    from .forms import (
        CurriculumMasterForm, CurriculumItemForm, CurriculumAllocationForm,
        CurriculumMetricRecordForm, CurriculumPolicyRuleForm, CurriculumSchedulePeriodForm,
        CurriculumFeedbackReviewForm, CurriculumWorkflowTransitionForm, CurriculumAccessRuleForm,
        CurriculumConfigurationParameterForm, CurriculumDocumentAttachmentForm, CurriculumFilterForm,
        CurriculumBulkActionForm
    )
except ImportError:
    try:
        from .models import (
...
class CurriculumListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        qs = CurriculumMaster.objects.all() if 'CurriculumMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = CurriculumFilterForm(self.request.GET) if 'CurriculumFilterForm' in globals() else None
        ctx['bulk_form'] = CurriculumBulkActionForm() if 'CurriculumBulkActionForm' in globals() else None
        ctx['total_count'] = CurriculumMaster.objects.count() if 'CurriculumMaster' in globals() else 0
        ctx['active_count'] = CurriculumMaster.objects.filter(status='ACTIVE').count() if 'CurriculumMaster' in globals() else 0
        ctx['pending_count'] = CurriculumMaster.objects.filter(status='PENDING_REVIEW').count() if 'CurriculumMaster' in globals() else 0
        ctx['page_title'] = "Academic Curriculum & Syllabus Topics Directory"
        ctx['domain_name'] = "Curriculum"
        return ctx
...
class CurriculumDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_object(self):
        return get_object_or_404(CurriculumMaster, pk=self.kwargs.get('pk'))
...
class CurriculumCreateView(LoginRequiredMixin, CreateView):
    ...
    form_class = CurriculumMasterForm
    success_url = reverse_lazy('academics:curriculum_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Curriculum record '{form.instance.name}' created successfully.")
...
class CurriculumUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Curriculum record updated successfully.")
        return super().form_valid(form)

class CurriculumDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'academics/curriculum_confirm_delete.html'
    success_url = reverse_lazy('academics:curriculum_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class CurriculumItemCreateView(LoginRequiredMixin, CreateView):
    form_class = CurriculumItemForm
    template_name = 'academics/curriculum_form.html'

    def form_valid(self, form):
        master = get_object_or_404(CurriculumMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('academics:curriculum_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class CurriculumItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = CurriculumItemForm
    template_name = 'academics/curriculum_form.html'

    def get_object(self):
        return get_object_or_404(CurriculumItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('academics:curriculum_detail', kwargs={'pk': self.object.master_id})

class CurriculumItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'academics/curriculum_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class CurriculumAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = CurriculumAllocationForm
    template_name = 'academics/curriculum_form.html'
...
class CurriculumAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(CurriculumAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('academics:curriculum_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class CurriculumMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = CurriculumMetricRecordForm
    template_name = 'academics/curriculum_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class CurriculumPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = CurriculumPolicyRuleForm
    template_name = 'academics/curriculum_form.html'
...
# -----------------------------------------------------------------------------
class CurriculumSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = CurriculumSchedulePeriodForm
    template_name = 'academics/curriculum_form.html'
...
# -----------------------------------------------------------------------------
class CurriculumFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = CurriculumFeedbackReviewForm
    template_name = 'academics/curriculum_form.html'
...
# -----------------------------------------------------------------------------
class CurriculumWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = CurriculumWorkflowTransitionForm
    template_name = 'academics/curriculum_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class CurriculumAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = CurriculumAccessRuleForm
    template_name = 'academics/curriculum_form.html'
...
# -----------------------------------------------------------------------------
class CurriculumConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = CurriculumConfigurationParameterForm
    template_name = 'academics/curriculum_form.html'
...
# -----------------------------------------------------------------------------
class CurriculumDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = CurriculumDocumentAttachmentForm
    template_name = 'academics/curriculum_form.html'
...
# -----------------------------------------------------------------------------
class CurriculumBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = CurriculumMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
...
class CurriculumExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="curriculum_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        writer = csv.writer(response)
        writer.writerow(['Code', 'Name', 'Category', 'Tier', 'Priority', 'Status', 'Capacity', 'Occupancy', 'Budget ($)', 'Incurred ($)', 'Start Date', 'End Date'])
        records = CurriculumMaster.objects.all() if 'CurriculumMaster' in globals() else []
        for r in records:
...
class CurriculumExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = CurriculumMaster.objects.all() if 'CurriculumMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class CurriculumAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = CurriculumMaster.objects.all() if 'CurriculumMaster' in globals() else []
...
class CurriculumAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(CurriculumMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
