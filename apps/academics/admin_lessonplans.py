"""Django Admin configuration for EduFlow Lesson Plans & Daily Teaching Logs (LessonPlans)."""
...
if 'LessonPlansMaster' in globals():
    @admin.register(LessonPlansMaster)
    class LessonPlansMasterAdmin(admin.ModelAdmin):
...
if 'LessonPlansItem' in globals():
    @admin.register(LessonPlansItem)
    class LessonPlansItemAdmin(admin.ModelAdmin):
...
if 'LessonPlansAllocation' in globals():
    @admin.register(LessonPlansAllocation)
    class LessonPlansAllocationAdmin(admin.ModelAdmin):
...
if 'LessonPlansMetricRecord' in globals():
    @admin.register(LessonPlansMetricRecord)
    class LessonPlansMetricRecordAdmin(admin.ModelAdmin):
        list_display = ['master', 'metric_name', 'target_value', 'actual_value', 'score_percentage', 'is_passing']
        list_filter = ['is_passing']

if 'LessonPlansPolicyRule' in globals():
    @admin.register(LessonPlansPolicyRule)
    class LessonPlansPolicyRuleAdmin(admin.ModelAdmin):
        list_display = ['master', 'rule_code', 'rule_name', 'threshold_value', 'is_enforced']
        list_filter = ['is_enforced']

if 'LessonPlansSchedulePeriod' in globals():
    @admin.register(LessonPlansSchedulePeriod)
    class LessonPlansSchedulePeriodAdmin(admin.ModelAdmin):
        list_display = ['master', 'period_title', 'day_of_week', 'start_time', 'end_time', 'room_number']
        list_filter = ['day_of_week', 'is_break_period']

if 'LessonPlansFeedbackReview' in globals():
    @admin.register(LessonPlansFeedbackReview)
    class LessonPlansFeedbackReviewAdmin(admin.ModelAdmin):
        list_display = ['master', 'reviewer_name', 'reviewer_role', 'rating', 'is_resolved', 'submitted_at']
        list_filter = ['rating', 'is_resolved']

if 'LessonPlansWorkflowTransition' in globals():
    @admin.register(LessonPlansWorkflowTransition)
    class LessonPlansWorkflowTransitionAdmin(admin.ModelAdmin):
        list_display = ['master', 'from_stage', 'to_stage', 'actor_username', 'is_approved', 'transition_timestamp']
        list_filter = ['is_approved']

if 'LessonPlansAccessRule' in globals():
    @admin.register(LessonPlansAccessRule)
    class LessonPlansAccessRuleAdmin(admin.ModelAdmin):
        list_display = ['master', 'role_allowed', 'can_read', 'can_write', 'can_delete', 'can_export']
        list_filter = ['role_allowed']

if 'LessonPlansConfigurationParameter' in globals():
    @admin.register(LessonPlansConfigurationParameter)
    class LessonPlansConfigurationParameterAdmin(admin.ModelAdmin):
        list_display = ['master', 'param_key', 'param_value', 'data_type', 'is_editable']

if 'LessonPlansDocumentAttachment' in globals():
    @admin.register(LessonPlansDocumentAttachment)
    class LessonPlansDocumentAttachmentAdmin(admin.ModelAdmin):
        list_display = ['master', 'title', 'mime_type', 'file_size_bytes', 'uploaded_by', 'uploaded_at']

if 'LessonPlansAuditTrail' in globals():
    @admin.register(LessonPlansAuditTrail)
    class LessonPlansAuditTrailAdmin(admin.ModelAdmin):
