"""
Models for EduFlow Lesson Plans & Daily Teaching Logs (LessonPlans) in apps.academics.
...
# -----------------------------------------------------------------------------
class LessonPlansStatusChoices(models.TextChoices):
...
class LessonPlansPriorityChoices(models.TextChoices):
...
class LessonPlansTierChoices(models.TextChoices):
...
class LessonPlansScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class LessonPlansMasterQuerySet(models.QuerySet):
    def active(self):
...
class LessonPlansMasterManager(models.Manager):
    def get_queryset(self):
        return LessonPlansMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class LessonPlansMaster(models.Model):
    """
    Primary master configuration entity for Lesson Plans & Daily Teaching Logs.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = LessonPlansStatusChoices
    PriorityChoices = LessonPlansPriorityChoices
    TierChoices = LessonPlansTierChoices
    ScopeChoices = LessonPlansScopeChoices
    ...
    objects = LessonPlansMasterManager()

    class Meta:
        db_table = 'eduflow_academics_lessonplans_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("LessonPlans Master")
        verbose_name_plural = _("LessonPlans Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_lesso_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_lesso_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_lesso_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_lesso_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'academics:lessonplans_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/academics/lessonplans/{self.pk}/"
...
# -----------------------------------------------------------------------------
class LessonPlansItem(models.Model):
    """Subordinate component or line transaction item for LessonPlans."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("LessonPlans Item")
        verbose_name_plural = _("LessonPlans Items")
...
# -----------------------------------------------------------------------------
class LessonPlansAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for LessonPlans."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_allocation'
        ordering = ['-start_time']
        verbose_name = _("LessonPlans Allocation")
        verbose_name_plural = _("LessonPlans Allocations")
...
# -----------------------------------------------------------------------------
class LessonPlansMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for LessonPlans."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("LessonPlans Metric Record")
        verbose_name_plural = _("LessonPlans Metric Records")
...
# -----------------------------------------------------------------------------
class LessonPlansPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for LessonPlans."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_rule'
        ordering = ['rule_code']
        verbose_name = _("LessonPlans Policy Rule")
        verbose_name_plural = _("LessonPlans Policy Rules")
...
# -----------------------------------------------------------------------------
class LessonPlansSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for LessonPlans."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("LessonPlans Schedule Period")
        verbose_name_plural = _("LessonPlans Schedule Periods")
...
# -----------------------------------------------------------------------------
class LessonPlansFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for LessonPlans."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_review'
        ordering = ['-submitted_at']
        verbose_name = _("LessonPlans Review")
        verbose_name_plural = _("LessonPlans Reviews")
...
# -----------------------------------------------------------------------------
class LessonPlansWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("LessonPlans Workflow Transition")
        verbose_name_plural = _("LessonPlans Workflow Transitions")
...
# -----------------------------------------------------------------------------
class LessonPlansAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_access'
        verbose_name = _("LessonPlans Access Rule")
        verbose_name_plural = _("LessonPlans Access Rules")
...
# -----------------------------------------------------------------------------
class LessonPlansConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_config'
        verbose_name = _("LessonPlans Config Parameter")
        verbose_name_plural = _("LessonPlans Config Parameters")
...
# -----------------------------------------------------------------------------
class LessonPlansDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("LessonPlans Document Attachment")
        verbose_name_plural = _("LessonPlans Document Attachments")
...
# -----------------------------------------------------------------------------
class LessonPlansAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on LessonPlans."""
    master = models.ForeignKey(LessonPlansMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_academics_lessonplans_audit'
        ordering = ['-timestamp']
        verbose_name = _("LessonPlans Audit Entry")
        verbose_name_plural = _("LessonPlans Audit Entries")
