"""
Extended REST API Controllers for EduFlow Academic Curriculum & Syllabus Topics (Curriculum).
...
class CurriculumAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(CurriculumMaster, pk=pk)
        data = master.to_dict()
...
class CurriculumAPISearchView(LoginRequiredMixin, View):
...
class CurriculumAPIBatchUpdateView(LoginRequiredMixin, View):
...
class CurriculumAPITelemetryView(LoginRequiredMixin, View):
