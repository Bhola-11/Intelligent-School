"""
Models for EduFlow Teacher-Class-Subject Allocation Matrix (SubjectTeachers) in apps.academics.
...
# -----------------------------------------------------------------------------
class SubjectTeachersStatusChoices(models.TextChoices):
...
class SubjectTeachersPriorityChoices(models.TextChoices):
...
class SubjectTeachersTierChoices(models.TextChoices):
...
class SubjectTeachersScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class SubjectTeachersMasterQuerySet(models.QuerySet):
    def active(self):
...
class SubjectTeachersMasterManager(models.Manager):
    def get_queryset(self):
        return SubjectTeachersMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class SubjectTeachersMaster(models.Model):
    """
    Primary master configuration entity for Teacher-Class-Subject Allocation Matrix.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = SubjectTeachersStatusChoices
    PriorityChoices = SubjectTeachersPriorityChoices
    TierChoices = SubjectTeachersTierChoices
    ScopeChoices = SubjectTeachersScopeChoices
    ...
    objects = SubjectTeachersMasterManager()

    class Meta:
        db_table = 'eduflow_academics_subjectteachers_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("SubjectTeachers Master")
        verbose_name_plural = _("SubjectTeachers Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_subje_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_subje_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_subje_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_subje_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'academics:subjectteachers_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/academics/subjectteachers/{self.pk}/"
...
# -----------------------------------------------------------------------------
class SubjectTeachersItem(models.Model):
    """Subordinate component or line transaction item for SubjectTeachers."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("SubjectTeachers Item")
        verbose_name_plural = _("SubjectTeachers Items")
...
# -----------------------------------------------------------------------------
class SubjectTeachersAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for SubjectTeachers."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_allocation'
        ordering = ['-start_time']
        verbose_name = _("SubjectTeachers Allocation")
        verbose_name_plural = _("SubjectTeachers Allocations")
...
# -----------------------------------------------------------------------------
class SubjectTeachersMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for SubjectTeachers."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("SubjectTeachers Metric Record")
        verbose_name_plural = _("SubjectTeachers Metric Records")
...
# -----------------------------------------------------------------------------
class SubjectTeachersPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for SubjectTeachers."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_rule'
        ordering = ['rule_code']
        verbose_name = _("SubjectTeachers Policy Rule")
        verbose_name_plural = _("SubjectTeachers Policy Rules")
...
# -----------------------------------------------------------------------------
class SubjectTeachersSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for SubjectTeachers."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("SubjectTeachers Schedule Period")
        verbose_name_plural = _("SubjectTeachers Schedule Periods")
...
# -----------------------------------------------------------------------------
class SubjectTeachersFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for SubjectTeachers."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_review'
        ordering = ['-submitted_at']
        verbose_name = _("SubjectTeachers Review")
        verbose_name_plural = _("SubjectTeachers Reviews")
...
# -----------------------------------------------------------------------------
class SubjectTeachersWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("SubjectTeachers Workflow Transition")
        verbose_name_plural = _("SubjectTeachers Workflow Transitions")
...
# -----------------------------------------------------------------------------
class SubjectTeachersAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_access'
        verbose_name = _("SubjectTeachers Access Rule")
        verbose_name_plural = _("SubjectTeachers Access Rules")
...
# -----------------------------------------------------------------------------
class SubjectTeachersConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_config'
        verbose_name = _("SubjectTeachers Config Parameter")
        verbose_name_plural = _("SubjectTeachers Config Parameters")
...
# -----------------------------------------------------------------------------
class SubjectTeachersDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("SubjectTeachers Document Attachment")
        verbose_name_plural = _("SubjectTeachers Document Attachments")
...
# -----------------------------------------------------------------------------
class SubjectTeachersAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on SubjectTeachers."""
    master = models.ForeignKey(SubjectTeachersMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_academics_subjectteachers_audit'
        ordering = ['-timestamp']
        verbose_name = _("SubjectTeachers Audit Entry")
        verbose_name_plural = _("SubjectTeachers Audit Entries")
