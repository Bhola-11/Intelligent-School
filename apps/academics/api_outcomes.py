"""
Extended REST API Controllers for EduFlow Learning Outcomes & Bloom's Taxonomy (Outcomes).
...
class OutcomesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(OutcomesMaster, pk=pk)
        data = master.to_dict()
...
class OutcomesAPISearchView(LoginRequiredMixin, View):
...
class OutcomesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class OutcomesAPITelemetryView(LoginRequiredMixin, View):
