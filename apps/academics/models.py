"""
Models for EduFlow Academic Curriculum & Syllabus Topics (Curriculum) in apps.academics.
...
# -----------------------------------------------------------------------------
class CurriculumStatusChoices(models.TextChoices):
...
class CurriculumPriorityChoices(models.TextChoices):
...
class CurriculumTierChoices(models.TextChoices):
...
class CurriculumScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class CurriculumMasterQuerySet(models.QuerySet):
    def active(self):
...
class CurriculumMasterManager(models.Manager):
    def get_queryset(self):
        return CurriculumMasterQuerySet(self.model, using=self._db)
...
class CurriculumMaster(models.Model):
    """
    Primary master configuration entity for Academic Curriculum & Syllabus Topics.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = CurriculumStatusChoices
    PriorityChoices = CurriculumPriorityChoices
    TierChoices = CurriculumTierChoices
    ScopeChoices = CurriculumScopeChoices
    ...
    objects = CurriculumMasterManager()

    class Meta:
        db_table = 'eduflow_academics_curriculum_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Curriculum Master")
        verbose_name_plural = _("Curriculum Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_curri_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_curri_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_curri_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_curri_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'academics:curriculum_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/academics/curriculum/{self.pk}/"
...
class CurriculumItem(models.Model):
    """Subordinate component or line transaction item for Curriculum."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_item'
...
class CurriculumAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Curriculum."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_allocation'
        ordering = ['-start_time']
        verbose_name = _("Curriculum Allocation")
        verbose_name_plural = _("Curriculum Allocations")
...
# -----------------------------------------------------------------------------
class CurriculumMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Curriculum."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Curriculum Metric Record")
        verbose_name_plural = _("Curriculum Metric Records")
...
# -----------------------------------------------------------------------------
class CurriculumPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Curriculum."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_rule'
        ordering = ['rule_code']
        verbose_name = _("Curriculum Policy Rule")
        verbose_name_plural = _("Curriculum Policy Rules")
...
# -----------------------------------------------------------------------------
class CurriculumSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Curriculum."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Curriculum Schedule Period")
        verbose_name_plural = _("Curriculum Schedule Periods")
...
# -----------------------------------------------------------------------------
class CurriculumFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Curriculum."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_review'
        ordering = ['-submitted_at']
        verbose_name = _("Curriculum Review")
        verbose_name_plural = _("Curriculum Reviews")
...
# -----------------------------------------------------------------------------
class CurriculumWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Curriculum Workflow Transition")
        verbose_name_plural = _("Curriculum Workflow Transitions")
...
# -----------------------------------------------------------------------------
class CurriculumAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_access'
        verbose_name = _("Curriculum Access Rule")
        verbose_name_plural = _("Curriculum Access Rules")
...
# -----------------------------------------------------------------------------
class CurriculumConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_config'
        verbose_name = _("Curriculum Config Parameter")
        verbose_name_plural = _("Curriculum Config Parameters")
...
# -----------------------------------------------------------------------------
class CurriculumDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Curriculum Document Attachment")
        verbose_name_plural = _("Curriculum Document Attachments")
...
# -----------------------------------------------------------------------------
class CurriculumAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Curriculum."""
    master = models.ForeignKey(CurriculumMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_academics_curriculum_audit'
        ordering = ['-timestamp']
        verbose_name = _("Curriculum Audit Entry")
        verbose_name_plural = _("Curriculum Audit Entries")
