"""
Forms for EduFlow Role-Based Access Control & Permissions (RBAC).
...
class RBACMasterForm(forms.ModelForm):
    """Primary form for creating and updating RBAC master records."""
    class Meta:
        try:
            model = RBACMaster
            fields = [
            ...
            widgets = {
                'code': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. RBAC-001'}),
...
class RBACItemForm(forms.ModelForm):
...
class RBACAllocationForm(forms.ModelForm):
    """Form for resource and faculty allocations."""
    class Meta:
        try:
            model = RBACAllocation
...
class RBACMetricRecordForm(forms.ModelForm):
    """Form for KPI metric observations."""
    class Meta:
        try:
            model = RBACMetricRecord
...
class RBACPolicyRuleForm(forms.ModelForm):
    """Form for institutional governance rules."""
    class Meta:
        try:
            model = RBACPolicyRule
...
class RBACSchedulePeriodForm(forms.ModelForm):
    """Form for timetable and schedule periods."""
    class Meta:
        try:
            model = RBACSchedulePeriod
...
class RBACFeedbackReviewForm(forms.ModelForm):
    """Form for qualitative feedback and evaluation."""
    class Meta:
        try:
            model = RBACFeedbackReview
...
class RBACWorkflowTransitionForm(forms.ModelForm):
    """Form for submitting workflow approval transitions."""
    class Meta:
        try:
            model = RBACWorkflowTransition
...
class RBACAccessRuleForm(forms.ModelForm):
...
class RBACConfigurationParameterForm(forms.ModelForm):
    """Form for key-value configuration parameters."""
    class Meta:
        try:
            model = RBACConfigurationParameter
...
class RBACDocumentAttachmentForm(forms.ModelForm):
    """Form for registering official document attachments."""
    class Meta:
        try:
            model = RBACDocumentAttachment
...
class RBACFilterForm(forms.Form):
...
class RBACBulkActionForm(forms.Form):
