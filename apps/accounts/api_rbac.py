"""
Extended REST API Controllers for EduFlow Role-Based Access Control & Permissions (RBAC).
...
class RBACAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(RBACMaster, pk=pk)
        data = master.to_dict()
...
class RBACAPISearchView(LoginRequiredMixin, View):
...
class RBACAPIBatchUpdateView(LoginRequiredMixin, View):
...
class RBACAPITelemetryView(LoginRequiredMixin, View):
