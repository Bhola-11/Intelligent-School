"""
Views for EduFlow Role-Based Access Control & Permissions (RBAC).
...
try:
    ...
    from .forms import (
        RBACMasterForm, RBACItemForm, RBACAllocationForm,
        RBACMetricRecordForm, RBACPolicyRuleForm, RBACSchedulePeriodForm,
        RBACFeedbackReviewForm, RBACWorkflowTransitionForm, RBACAccessRuleForm,
        RBACConfigurationParameterForm, RBACDocumentAttachmentForm, RBACFilterForm,
        RBACBulkActionForm
    )
except ImportError:
    try:
...
# -----------------------------------------------------------------------------
class RBACListView(LoginRequiredMixin, ListView):
    template_name = 'accounts/rbac_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = RBACMaster.objects.all() if 'RBACMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = RBACFilterForm(self.request.GET) if 'RBACFilterForm' in globals() else None
        ctx['bulk_form'] = RBACBulkActionForm() if 'RBACBulkActionForm' in globals() else None
        ctx['total_count'] = RBACMaster.objects.count() if 'RBACMaster' in globals() else 0
        ctx['active_count'] = RBACMaster.objects.filter(status='ACTIVE').count() if 'RBACMaster' in globals() else 0
        ctx['pending_count'] = RBACMaster.objects.filter(status='PENDING_REVIEW').count() if 'RBACMaster' in globals() else 0
        ctx['page_title'] = "Role-Based Access Control & Permissions Directory"
        ctx['domain_name'] = "RBAC"
        return ctx

class RBACDetailView(LoginRequiredMixin, DetailView):
    template_name = 'accounts/rbac_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(RBACMaster, pk=self.kwargs.get('pk'))
...
class RBACCreateView(LoginRequiredMixin, CreateView):
    template_name = 'accounts/rbac_form.html'
    form_class = RBACMasterForm
    success_url = reverse_lazy('accounts:rbac_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"RBAC record '{form.instance.name}' created successfully.")
...
class RBACUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"RBAC record updated successfully.")
        return super().form_valid(form)

class RBACDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'accounts/rbac_confirm_delete.html'
    success_url = reverse_lazy('accounts:rbac_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class RBACItemCreateView(LoginRequiredMixin, CreateView):
    form_class = RBACItemForm
    template_name = 'accounts/rbac_form.html'

    def form_valid(self, form):
        master = get_object_or_404(RBACMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('accounts:rbac_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class RBACItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = RBACItemForm
    template_name = 'accounts/rbac_form.html'

    def get_object(self):
        return get_object_or_404(RBACItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('accounts:rbac_detail', kwargs={'pk': self.object.master_id})

class RBACItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'accounts/rbac_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class RBACAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = RBACAllocationForm
    template_name = 'accounts/rbac_form.html'
...
class RBACAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(RBACAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('accounts:rbac_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class RBACMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = RBACMetricRecordForm
    template_name = 'accounts/rbac_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class RBACPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = RBACPolicyRuleForm
    template_name = 'accounts/rbac_form.html'
...
# -----------------------------------------------------------------------------
class RBACSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = RBACSchedulePeriodForm
    template_name = 'accounts/rbac_form.html'
...
# -----------------------------------------------------------------------------
class RBACFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = RBACFeedbackReviewForm
    template_name = 'accounts/rbac_form.html'
...
# -----------------------------------------------------------------------------
class RBACWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = RBACWorkflowTransitionForm
    template_name = 'accounts/rbac_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class RBACAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = RBACAccessRuleForm
    template_name = 'accounts/rbac_form.html'
...
# -----------------------------------------------------------------------------
class RBACConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = RBACConfigurationParameterForm
    template_name = 'accounts/rbac_form.html'
...
# -----------------------------------------------------------------------------
class RBACDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = RBACDocumentAttachmentForm
    template_name = 'accounts/rbac_form.html'
...
# -----------------------------------------------------------------------------
class RBACBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = RBACMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('accounts:rbac_list')

class RBACExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="rbac_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
...
class RBACExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = RBACMaster.objects.all() if 'RBACMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class RBACAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = RBACMaster.objects.all() if 'RBACMaster' in globals() else []
...
class RBACAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(RBACMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
