"""
Models for EduFlow Role-Based Access Control & Permissions (RBAC) in apps.accounts.
...
# -----------------------------------------------------------------------------
class RBACStatusChoices(models.TextChoices):
...
class RBACPriorityChoices(models.TextChoices):
...
class RBACTierChoices(models.TextChoices):
...
class RBACScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class RBACMasterQuerySet(models.QuerySet):
    def active(self):
...
class RBACMasterManager(models.Manager):
    def get_queryset(self):
        return RBACMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class RBACMaster(models.Model):
    """
    Primary master configuration entity for Role-Based Access Control & Permissions.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = RBACStatusChoices
    PriorityChoices = RBACPriorityChoices
    TierChoices = RBACTierChoices
    ScopeChoices = RBACScopeChoices
    ...
    objects = RBACMasterManager()

    class Meta:
        db_table = 'eduflow_accounts_rbac_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("RBAC Master")
        verbose_name_plural = _("RBAC Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_rbac_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_rbac_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_rbac_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_rbac_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'accounts:rbac_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/accounts/rbac/{self.pk}/"
...
class RBACItem(models.Model):
    """Subordinate component or line transaction item for RBAC."""
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("RBAC Item")
        verbose_name_plural = _("RBAC Items")
...
# -----------------------------------------------------------------------------
class RBACAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for RBAC."""
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_allocation'
        ordering = ['-start_time']
        verbose_name = _("RBAC Allocation")
        verbose_name_plural = _("RBAC Allocations")
...
# -----------------------------------------------------------------------------
class RBACMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for RBAC."""
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("RBAC Metric Record")
        verbose_name_plural = _("RBAC Metric Records")
...
# -----------------------------------------------------------------------------
class RBACPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for RBAC."""
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_rule'
        ordering = ['rule_code']
        verbose_name = _("RBAC Policy Rule")
        verbose_name_plural = _("RBAC Policy Rules")
...
# -----------------------------------------------------------------------------
class RBACSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for RBAC."""
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("RBAC Schedule Period")
        verbose_name_plural = _("RBAC Schedule Periods")
...
# -----------------------------------------------------------------------------
class RBACFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for RBAC."""
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_review'
        ordering = ['-submitted_at']
        verbose_name = _("RBAC Review")
        verbose_name_plural = _("RBAC Reviews")
...
# -----------------------------------------------------------------------------
class RBACWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("RBAC Workflow Transition")
        verbose_name_plural = _("RBAC Workflow Transitions")
...
class RBACAccessRule(models.Model):
    ...
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_access'
        verbose_name = _("RBAC Access Rule")
        verbose_name_plural = _("RBAC Access Rules")
...
# -----------------------------------------------------------------------------
class RBACConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_config'
        verbose_name = _("RBAC Config Parameter")
        verbose_name_plural = _("RBAC Config Parameters")
...
# -----------------------------------------------------------------------------
class RBACDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("RBAC Document Attachment")
        verbose_name_plural = _("RBAC Document Attachments")
...
# -----------------------------------------------------------------------------
class RBACAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on RBAC."""
    master = models.ForeignKey(RBACMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_accounts_rbac_audit'
        ordering = ['-timestamp']
        verbose_name = _("RBAC Audit Entry")
        verbose_name_plural = _("RBAC Audit Entries")
