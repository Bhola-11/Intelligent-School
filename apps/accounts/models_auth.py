"""
Models for EduFlow Authentication Workflows & Profile Management (Auth) in apps.accounts.
...
class AuthPriorityChoices(models.TextChoices):
...
class AuthTierChoices(models.TextChoices):
...
class AuthScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class AuthMasterQuerySet(models.QuerySet):
    def active(self):
...
class AuthMasterManager(models.Manager):
    def get_queryset(self):
        return AuthMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class AuthMaster(models.Model):
    """
    Primary master configuration entity for Authentication Workflows & Profile Management.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = AuthStatusChoices
    PriorityChoices = AuthPriorityChoices
    TierChoices = AuthTierChoices
    ScopeChoices = AuthScopeChoices
    ...
    objects = AuthMasterManager()

    class Meta:
        db_table = 'eduflow_accounts_auth_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Auth Master")
        verbose_name_plural = _("Auth Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_auth_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_auth_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_auth_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_auth_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'accounts:auth_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/accounts/auth/{self.pk}/"
...
class AuthItem(models.Model):
    """Subordinate component or line transaction item for Auth."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Auth Item")
        verbose_name_plural = _("Auth Items")
...
# -----------------------------------------------------------------------------
class AuthAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Auth."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_allocation'
        ordering = ['-start_time']
        verbose_name = _("Auth Allocation")
        verbose_name_plural = _("Auth Allocations")
...
# -----------------------------------------------------------------------------
class AuthMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Auth."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Auth Metric Record")
        verbose_name_plural = _("Auth Metric Records")
...
class AuthPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Auth."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_rule'
        ordering = ['rule_code']
        verbose_name = _("Auth Policy Rule")
        verbose_name_plural = _("Auth Policy Rules")
...
# -----------------------------------------------------------------------------
class AuthSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Auth."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Auth Schedule Period")
        verbose_name_plural = _("Auth Schedule Periods")
...
# -----------------------------------------------------------------------------
class AuthFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Auth."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_review'
        ordering = ['-submitted_at']
        verbose_name = _("Auth Review")
        verbose_name_plural = _("Auth Reviews")
...
# -----------------------------------------------------------------------------
class AuthWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Auth Workflow Transition")
        verbose_name_plural = _("Auth Workflow Transitions")
...
# -----------------------------------------------------------------------------
class AuthAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_access'
        verbose_name = _("Auth Access Rule")
        verbose_name_plural = _("Auth Access Rules")
...
# -----------------------------------------------------------------------------
class AuthConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_config'
        verbose_name = _("Auth Config Parameter")
        verbose_name_plural = _("Auth Config Parameters")
...
# -----------------------------------------------------------------------------
class AuthDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Auth Document Attachment")
        verbose_name_plural = _("Auth Document Attachments")
...
# -----------------------------------------------------------------------------
class AuthAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Auth."""
    master = models.ForeignKey(AuthMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_accounts_auth_audit'
        ordering = ['-timestamp']
        verbose_name = _("Auth Audit Entry")
        verbose_name_plural = _("Auth Audit Entries")
