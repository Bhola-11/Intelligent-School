"""
Extended REST API Controllers for EduFlow Authentication Workflows & Profile Management (Auth).
...
class AuthAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(AuthMaster, pk=pk)
        data = master.to_dict()
...
class AuthAPISearchView(LoginRequiredMixin, View):
...
class AuthAPIBatchUpdateView(LoginRequiredMixin, View):
...
class AuthAPITelemetryView(LoginRequiredMixin, View):
