"""
Extended REST API Controllers for EduFlow Library Book Catalog & ISBN Management (BookCatalog).
...
class BookCatalogAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(BookCatalogMaster, pk=pk)
        data = master.to_dict()
...
class BookCatalogAPISearchView(LoginRequiredMixin, View):
...
class BookCatalogAPIBatchUpdateView(LoginRequiredMixin, View):
...
class BookCatalogAPITelemetryView(LoginRequiredMixin, View):
