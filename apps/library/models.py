"""
Models for EduFlow Library Book Catalog & ISBN Management (BookCatalog) in apps.library.
...
# -----------------------------------------------------------------------------
class BookCatalogStatusChoices(models.TextChoices):
...
class BookCatalogPriorityChoices(models.TextChoices):
...
class BookCatalogTierChoices(models.TextChoices):
...
class BookCatalogScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class BookCatalogMasterQuerySet(models.QuerySet):
    def active(self):
...
class BookCatalogMasterManager(models.Manager):
    def get_queryset(self):
        return BookCatalogMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class BookCatalogMaster(models.Model):
    """
    Primary master configuration entity for Library Book Catalog & ISBN Management.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = BookCatalogStatusChoices
    PriorityChoices = BookCatalogPriorityChoices
    TierChoices = BookCatalogTierChoices
    ScopeChoices = BookCatalogScopeChoices
    ...
    objects = BookCatalogMasterManager()

    class Meta:
        db_table = 'eduflow_library_bookcatalog_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("BookCatalog Master")
        verbose_name_plural = _("BookCatalog Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_bookc_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_bookc_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_bookc_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_bookc_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'library:bookcatalog_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/library/bookcatalog/{self.pk}/"
...
# -----------------------------------------------------------------------------
class BookCatalogItem(models.Model):
    """Subordinate component or line transaction item for BookCatalog."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("BookCatalog Item")
        verbose_name_plural = _("BookCatalog Items")
...
# -----------------------------------------------------------------------------
class BookCatalogAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for BookCatalog."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_allocation'
        ordering = ['-start_time']
        verbose_name = _("BookCatalog Allocation")
        verbose_name_plural = _("BookCatalog Allocations")
...
# -----------------------------------------------------------------------------
class BookCatalogMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for BookCatalog."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("BookCatalog Metric Record")
        verbose_name_plural = _("BookCatalog Metric Records")
...
# -----------------------------------------------------------------------------
class BookCatalogPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for BookCatalog."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_rule'
        ordering = ['rule_code']
        verbose_name = _("BookCatalog Policy Rule")
        verbose_name_plural = _("BookCatalog Policy Rules")
...
# -----------------------------------------------------------------------------
class BookCatalogSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for BookCatalog."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("BookCatalog Schedule Period")
        verbose_name_plural = _("BookCatalog Schedule Periods")
...
# -----------------------------------------------------------------------------
class BookCatalogFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for BookCatalog."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_review'
        ordering = ['-submitted_at']
        verbose_name = _("BookCatalog Review")
        verbose_name_plural = _("BookCatalog Reviews")
...
# -----------------------------------------------------------------------------
class BookCatalogWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("BookCatalog Workflow Transition")
        verbose_name_plural = _("BookCatalog Workflow Transitions")
...
# -----------------------------------------------------------------------------
class BookCatalogAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_access'
        verbose_name = _("BookCatalog Access Rule")
        verbose_name_plural = _("BookCatalog Access Rules")
...
# -----------------------------------------------------------------------------
class BookCatalogConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_config'
        verbose_name = _("BookCatalog Config Parameter")
        verbose_name_plural = _("BookCatalog Config Parameters")
...
# -----------------------------------------------------------------------------
class BookCatalogDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("BookCatalog Document Attachment")
        verbose_name_plural = _("BookCatalog Document Attachments")
...
# -----------------------------------------------------------------------------
class BookCatalogAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on BookCatalog."""
    master = models.ForeignKey(BookCatalogMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcatalog_audit'
        ordering = ['-timestamp']
        verbose_name = _("BookCatalog Audit Entry")
        verbose_name_plural = _("BookCatalog Audit Entries")
