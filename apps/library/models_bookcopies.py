"""
Models for EduFlow Book Copies, Barcodes & Shelf Tracking (BookCopies) in apps.library.
...
# -----------------------------------------------------------------------------
class BookCopiesStatusChoices(models.TextChoices):
...
class BookCopiesPriorityChoices(models.TextChoices):
...
class BookCopiesTierChoices(models.TextChoices):
...
class BookCopiesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class BookCopiesMasterQuerySet(models.QuerySet):
    def active(self):
...
class BookCopiesMasterManager(models.Manager):
    def get_queryset(self):
        return BookCopiesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class BookCopiesMaster(models.Model):
    """
    Primary master configuration entity for Book Copies, Barcodes & Shelf Tracking.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = BookCopiesStatusChoices
    PriorityChoices = BookCopiesPriorityChoices
    TierChoices = BookCopiesTierChoices
    ScopeChoices = BookCopiesScopeChoices
    ...
    objects = BookCopiesMasterManager()

    class Meta:
        db_table = 'eduflow_library_bookcopies_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("BookCopies Master")
        verbose_name_plural = _("BookCopies Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'library:bookcopies_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/library/bookcopies/{self.pk}/"
...
# -----------------------------------------------------------------------------
class BookCopiesItem(models.Model):
    """Subordinate component or line transaction item for BookCopies."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("BookCopies Item")
        verbose_name_plural = _("BookCopies Items")
...
# -----------------------------------------------------------------------------
class BookCopiesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for BookCopies."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_allocation'
        ordering = ['-start_time']
        verbose_name = _("BookCopies Allocation")
        verbose_name_plural = _("BookCopies Allocations")
...
# -----------------------------------------------------------------------------
class BookCopiesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for BookCopies."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("BookCopies Metric Record")
        verbose_name_plural = _("BookCopies Metric Records")
...
# -----------------------------------------------------------------------------
class BookCopiesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for BookCopies."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_rule'
        ordering = ['rule_code']
        verbose_name = _("BookCopies Policy Rule")
        verbose_name_plural = _("BookCopies Policy Rules")
...
# -----------------------------------------------------------------------------
class BookCopiesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for BookCopies."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("BookCopies Schedule Period")
        verbose_name_plural = _("BookCopies Schedule Periods")
...
# -----------------------------------------------------------------------------
class BookCopiesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for BookCopies."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_review'
        ordering = ['-submitted_at']
        verbose_name = _("BookCopies Review")
        verbose_name_plural = _("BookCopies Reviews")
...
# -----------------------------------------------------------------------------
class BookCopiesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("BookCopies Workflow Transition")
        verbose_name_plural = _("BookCopies Workflow Transitions")
...
# -----------------------------------------------------------------------------
class BookCopiesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_access'
        verbose_name = _("BookCopies Access Rule")
        verbose_name_plural = _("BookCopies Access Rules")
...
# -----------------------------------------------------------------------------
class BookCopiesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_config'
        verbose_name = _("BookCopies Config Parameter")
        verbose_name_plural = _("BookCopies Config Parameters")
...
# -----------------------------------------------------------------------------
class BookCopiesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("BookCopies Document Attachment")
        verbose_name_plural = _("BookCopies Document Attachments")
...
# -----------------------------------------------------------------------------
class BookCopiesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on BookCopies."""
    master = models.ForeignKey(BookCopiesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_library_bookcopies_audit'
        ordering = ['-timestamp']
        verbose_name = _("BookCopies Audit Entry")
        verbose_name_plural = _("BookCopies Audit Entries")
