"""
Models for EduFlow Book Reservations & Overdue Fines (Reservations) in apps.library.
...
# -----------------------------------------------------------------------------
class ReservationsStatusChoices(models.TextChoices):
...
class ReservationsPriorityChoices(models.TextChoices):
...
class ReservationsTierChoices(models.TextChoices):
...
class ReservationsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ReservationsMasterQuerySet(models.QuerySet):
    def active(self):
...
class ReservationsMasterManager(models.Manager):
    def get_queryset(self):
        return ReservationsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ReservationsMaster(models.Model):
    """
    Primary master configuration entity for Book Reservations & Overdue Fines.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ReservationsStatusChoices
    PriorityChoices = ReservationsPriorityChoices
    TierChoices = ReservationsTierChoices
    ScopeChoices = ReservationsScopeChoices
    ...
    objects = ReservationsMasterManager()

    class Meta:
        db_table = 'eduflow_library_reservations_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Reservations Master")
        verbose_name_plural = _("Reservations Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_reser_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_reser_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_reser_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_reser_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'library:reservations_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/library/reservations/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ReservationsItem(models.Model):
    """Subordinate component or line transaction item for Reservations."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Reservations Item")
        verbose_name_plural = _("Reservations Items")
...
# -----------------------------------------------------------------------------
class ReservationsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Reservations."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_allocation'
        ordering = ['-start_time']
        verbose_name = _("Reservations Allocation")
        verbose_name_plural = _("Reservations Allocations")
...
# -----------------------------------------------------------------------------
class ReservationsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Reservations."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Reservations Metric Record")
        verbose_name_plural = _("Reservations Metric Records")
...
# -----------------------------------------------------------------------------
class ReservationsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Reservations."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_rule'
        ordering = ['rule_code']
        verbose_name = _("Reservations Policy Rule")
        verbose_name_plural = _("Reservations Policy Rules")
...
# -----------------------------------------------------------------------------
class ReservationsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Reservations."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Reservations Schedule Period")
        verbose_name_plural = _("Reservations Schedule Periods")
...
# -----------------------------------------------------------------------------
class ReservationsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Reservations."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_review'
        ordering = ['-submitted_at']
        verbose_name = _("Reservations Review")
        verbose_name_plural = _("Reservations Reviews")
...
# -----------------------------------------------------------------------------
class ReservationsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Reservations Workflow Transition")
        verbose_name_plural = _("Reservations Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ReservationsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_access'
        verbose_name = _("Reservations Access Rule")
        verbose_name_plural = _("Reservations Access Rules")
...
# -----------------------------------------------------------------------------
class ReservationsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_config'
        verbose_name = _("Reservations Config Parameter")
        verbose_name_plural = _("Reservations Config Parameters")
...
# -----------------------------------------------------------------------------
class ReservationsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Reservations Document Attachment")
        verbose_name_plural = _("Reservations Document Attachments")
...
# -----------------------------------------------------------------------------
class ReservationsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Reservations."""
    master = models.ForeignKey(ReservationsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_library_reservations_audit'
        ordering = ['-timestamp']
        verbose_name = _("Reservations Audit Entry")
        verbose_name_plural = _("Reservations Audit Entries")
