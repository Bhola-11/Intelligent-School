"""
Extended REST API Controllers for EduFlow Book Reservations & Overdue Fines (Reservations).
...
class ReservationsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ReservationsMaster, pk=pk)
        data = master.to_dict()
...
class ReservationsAPISearchView(LoginRequiredMixin, View):
...
class ReservationsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ReservationsAPITelemetryView(LoginRequiredMixin, View):
