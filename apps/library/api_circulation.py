"""
Extended REST API Controllers for EduFlow Library Circulation, Borrow & Returns (Circulation).
...
class CirculationAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(CirculationMaster, pk=pk)
        data = master.to_dict()
...
class CirculationAPISearchView(LoginRequiredMixin, View):
...
class CirculationAPIBatchUpdateView(LoginRequiredMixin, View):
...
class CirculationAPITelemetryView(LoginRequiredMixin, View):
