"""
Views for EduFlow Book Reservations & Overdue Fines (Reservations).
...
try:
    ...
    from .forms import (
        ReservationsMasterForm, ReservationsItemForm, ReservationsAllocationForm,
        ReservationsMetricRecordForm, ReservationsPolicyRuleForm, ReservationsSchedulePeriodForm,
        ReservationsFeedbackReviewForm, ReservationsWorkflowTransitionForm, ReservationsAccessRuleForm,
        ReservationsConfigurationParameterForm, ReservationsDocumentAttachmentForm, ReservationsFilterForm,
        ReservationsBulkActionForm
    )
except ImportError:
    try:
        ...
        from .forms_reservations import (
            ReservationsMasterForm, ReservationsItemForm, ReservationsAllocationForm,
            ReservationsMetricRecordForm, ReservationsPolicyRuleForm, ReservationsSchedulePeriodForm,
            ReservationsFeedbackReviewForm, ReservationsWorkflowTransitionForm, ReservationsAccessRuleForm,
            ReservationsConfigurationParameterForm, ReservationsDocumentAttachmentForm, ReservationsFilterForm,
...
class ReservationsListView(LoginRequiredMixin, ListView):
    template_name = 'library/reservations_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = ReservationsMaster.objects.all() if 'ReservationsMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = ReservationsFilterForm(self.request.GET) if 'ReservationsFilterForm' in globals() else None
        ctx['bulk_form'] = ReservationsBulkActionForm() if 'ReservationsBulkActionForm' in globals() else None
        ctx['total_count'] = ReservationsMaster.objects.count() if 'ReservationsMaster' in globals() else 0
        ctx['active_count'] = ReservationsMaster.objects.filter(status='ACTIVE').count() if 'ReservationsMaster' in globals() else 0
        ctx['pending_count'] = ReservationsMaster.objects.filter(status='PENDING_REVIEW').count() if 'ReservationsMaster' in globals() else 0
        ctx['page_title'] = "Book Reservations & Overdue Fines Directory"
        ctx['domain_name'] = "Reservations"
        return ctx

class ReservationsDetailView(LoginRequiredMixin, DetailView):
    template_name = 'library/reservations_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(ReservationsMaster, pk=self.kwargs.get('pk'))
...
class ReservationsCreateView(LoginRequiredMixin, CreateView):
    template_name = 'library/reservations_form.html'
    form_class = ReservationsMasterForm
    success_url = reverse_lazy('library:reservations_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Reservations record '{form.instance.name}' created successfully.")
...
class ReservationsUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Reservations record updated successfully.")
        return super().form_valid(form)

class ReservationsDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'library/reservations_confirm_delete.html'
    success_url = reverse_lazy('library:reservations_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class ReservationsItemCreateView(LoginRequiredMixin, CreateView):
    form_class = ReservationsItemForm
    template_name = 'library/reservations_form.html'

    def form_valid(self, form):
        master = get_object_or_404(ReservationsMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('library:reservations_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class ReservationsItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = ReservationsItemForm
    template_name = 'library/reservations_form.html'

    def get_object(self):
        return get_object_or_404(ReservationsItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('library:reservations_detail', kwargs={'pk': self.object.master_id})

class ReservationsItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'library/reservations_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class ReservationsAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = ReservationsAllocationForm
    template_name = 'library/reservations_form.html'
...
class ReservationsAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(ReservationsAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('library:reservations_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class ReservationsMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = ReservationsMetricRecordForm
    template_name = 'library/reservations_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class ReservationsPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = ReservationsPolicyRuleForm
    template_name = 'library/reservations_form.html'
...
# -----------------------------------------------------------------------------
class ReservationsSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = ReservationsSchedulePeriodForm
    template_name = 'library/reservations_form.html'
...
# -----------------------------------------------------------------------------
class ReservationsFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = ReservationsFeedbackReviewForm
    template_name = 'library/reservations_form.html'
...
# -----------------------------------------------------------------------------
class ReservationsWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = ReservationsWorkflowTransitionForm
    template_name = 'library/reservations_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class ReservationsAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = ReservationsAccessRuleForm
    template_name = 'library/reservations_form.html'
...
# -----------------------------------------------------------------------------
class ReservationsConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = ReservationsConfigurationParameterForm
    template_name = 'library/reservations_form.html'
...
# -----------------------------------------------------------------------------
class ReservationsDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = ReservationsDocumentAttachmentForm
    template_name = 'library/reservations_form.html'
...
# -----------------------------------------------------------------------------
class ReservationsBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = ReservationsMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('library:reservations_list')

class ReservationsExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="reservations_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
...
class ReservationsExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = ReservationsMaster.objects.all() if 'ReservationsMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class ReservationsAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = ReservationsMaster.objects.all() if 'ReservationsMaster' in globals() else []
...
class ReservationsAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(ReservationsMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
