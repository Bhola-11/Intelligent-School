"""Django Admin configuration for EduFlow Book Copies, Barcodes & Shelf Tracking (BookCopies)."""
...
if 'BookCopiesMaster' in globals():
    @admin.register(BookCopiesMaster)
    class BookCopiesMasterAdmin(admin.ModelAdmin):
...
if 'BookCopiesItem' in globals():
    @admin.register(BookCopiesItem)
    class BookCopiesItemAdmin(admin.ModelAdmin):
...
if 'BookCopiesAllocation' in globals():
    @admin.register(BookCopiesAllocation)
    class BookCopiesAllocationAdmin(admin.ModelAdmin):
...
if 'BookCopiesMetricRecord' in globals():
    @admin.register(BookCopiesMetricRecord)
    class BookCopiesMetricRecordAdmin(admin.ModelAdmin):
        list_display = ['master', 'metric_name', 'target_value', 'actual_value', 'score_percentage', 'is_passing']
        list_filter = ['is_passing']

if 'BookCopiesPolicyRule' in globals():
    @admin.register(BookCopiesPolicyRule)
    class BookCopiesPolicyRuleAdmin(admin.ModelAdmin):
        list_display = ['master', 'rule_code', 'rule_name', 'threshold_value', 'is_enforced']
        list_filter = ['is_enforced']

if 'BookCopiesSchedulePeriod' in globals():
    @admin.register(BookCopiesSchedulePeriod)
    class BookCopiesSchedulePeriodAdmin(admin.ModelAdmin):
        list_display = ['master', 'period_title', 'day_of_week', 'start_time', 'end_time', 'room_number']
        list_filter = ['day_of_week', 'is_break_period']

if 'BookCopiesFeedbackReview' in globals():
    @admin.register(BookCopiesFeedbackReview)
    class BookCopiesFeedbackReviewAdmin(admin.ModelAdmin):
        list_display = ['master', 'reviewer_name', 'reviewer_role', 'rating', 'is_resolved', 'submitted_at']
        list_filter = ['rating', 'is_resolved']

if 'BookCopiesWorkflowTransition' in globals():
    @admin.register(BookCopiesWorkflowTransition)
    class BookCopiesWorkflowTransitionAdmin(admin.ModelAdmin):
        list_display = ['master', 'from_stage', 'to_stage', 'actor_username', 'is_approved', 'transition_timestamp']
        list_filter = ['is_approved']

if 'BookCopiesAccessRule' in globals():
    @admin.register(BookCopiesAccessRule)
    class BookCopiesAccessRuleAdmin(admin.ModelAdmin):
        list_display = ['master', 'role_allowed', 'can_read', 'can_write', 'can_delete', 'can_export']
        list_filter = ['role_allowed']

if 'BookCopiesConfigurationParameter' in globals():
    @admin.register(BookCopiesConfigurationParameter)
    class BookCopiesConfigurationParameterAdmin(admin.ModelAdmin):
        list_display = ['master', 'param_key', 'param_value', 'data_type', 'is_editable']

if 'BookCopiesDocumentAttachment' in globals():
    @admin.register(BookCopiesDocumentAttachment)
    class BookCopiesDocumentAttachmentAdmin(admin.ModelAdmin):
        list_display = ['master', 'title', 'mime_type', 'file_size_bytes', 'uploaded_by', 'uploaded_at']

if 'BookCopiesAuditTrail' in globals():
    @admin.register(BookCopiesAuditTrail)
    class BookCopiesAuditTrailAdmin(admin.ModelAdmin):
