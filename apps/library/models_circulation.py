"""
Models for EduFlow Library Circulation, Borrow & Returns (Circulation) in apps.library.
...
# -----------------------------------------------------------------------------
class CirculationStatusChoices(models.TextChoices):
...
class CirculationPriorityChoices(models.TextChoices):
...
class CirculationTierChoices(models.TextChoices):
...
class CirculationScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class CirculationMasterQuerySet(models.QuerySet):
    def active(self):
...
class CirculationMasterManager(models.Manager):
    def get_queryset(self):
        return CirculationMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class CirculationMaster(models.Model):
    """
    Primary master configuration entity for Library Circulation, Borrow & Returns.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = CirculationStatusChoices
    PriorityChoices = CirculationPriorityChoices
    TierChoices = CirculationTierChoices
    ScopeChoices = CirculationScopeChoices
    ...
    objects = CirculationMasterManager()

    class Meta:
        db_table = 'eduflow_library_circulation_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Circulation Master")
        verbose_name_plural = _("Circulation Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_circu_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_circu_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_circu_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_circu_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'library:circulation_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/library/circulation/{self.pk}/"
...
# -----------------------------------------------------------------------------
class CirculationItem(models.Model):
    """Subordinate component or line transaction item for Circulation."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Circulation Item")
        verbose_name_plural = _("Circulation Items")
...
# -----------------------------------------------------------------------------
class CirculationAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Circulation."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_allocation'
        ordering = ['-start_time']
        verbose_name = _("Circulation Allocation")
        verbose_name_plural = _("Circulation Allocations")
...
# -----------------------------------------------------------------------------
class CirculationMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Circulation."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Circulation Metric Record")
        verbose_name_plural = _("Circulation Metric Records")
...
# -----------------------------------------------------------------------------
class CirculationPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Circulation."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_rule'
        ordering = ['rule_code']
        verbose_name = _("Circulation Policy Rule")
        verbose_name_plural = _("Circulation Policy Rules")
...
# -----------------------------------------------------------------------------
class CirculationSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Circulation."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Circulation Schedule Period")
        verbose_name_plural = _("Circulation Schedule Periods")
...
# -----------------------------------------------------------------------------
class CirculationFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Circulation."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_review'
        ordering = ['-submitted_at']
        verbose_name = _("Circulation Review")
        verbose_name_plural = _("Circulation Reviews")
...
# -----------------------------------------------------------------------------
class CirculationWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Circulation Workflow Transition")
        verbose_name_plural = _("Circulation Workflow Transitions")
...
# -----------------------------------------------------------------------------
class CirculationAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_access'
        verbose_name = _("Circulation Access Rule")
        verbose_name_plural = _("Circulation Access Rules")
...
# -----------------------------------------------------------------------------
class CirculationConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_config'
        verbose_name = _("Circulation Config Parameter")
        verbose_name_plural = _("Circulation Config Parameters")
...
# -----------------------------------------------------------------------------
class CirculationDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Circulation Document Attachment")
        verbose_name_plural = _("Circulation Document Attachments")
...
# -----------------------------------------------------------------------------
class CirculationAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Circulation."""
    master = models.ForeignKey(CirculationMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_library_circulation_audit'
        ordering = ['-timestamp']
        verbose_name = _("Circulation Audit Entry")
        verbose_name_plural = _("Circulation Audit Entries")
