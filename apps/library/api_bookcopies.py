"""
Extended REST API Controllers for EduFlow Book Copies, Barcodes & Shelf Tracking (BookCopies).
...
class BookCopiesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(BookCopiesMaster, pk=pk)
        data = master.to_dict()
...
class BookCopiesAPISearchView(LoginRequiredMixin, View):
...
class BookCopiesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class BookCopiesAPITelemetryView(LoginRequiredMixin, View):
