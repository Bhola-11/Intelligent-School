"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Book Copies, Barcodes & Shelf Tracking (BookCopies).
...
class BookCopiesDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for BookCopies."""
    template_name = 'library/bookcopies_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = BookCopiesAuditTrail.objects.all().order_by('-timestamp')[:10] if 'BookCopiesAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Book Copies, Barcodes & Shelf Tracking Operations Dashboard",
            'domain_name': 'BookCopies',
            'app_title': 'Library',
...
class BookCopiesAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'library/bookcopies_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Book Copies, Barcodes & Shelf Tracking Analytical Insights",
