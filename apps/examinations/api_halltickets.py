"""
Extended REST API Controllers for EduFlow Exam Hall Tickets & Roll Number Generation (HallTickets).
...
class HallTicketsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(HallTicketsMaster, pk=pk)
        data = master.to_dict()
...
class HallTicketsAPISearchView(LoginRequiredMixin, View):
...
class HallTicketsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class HallTicketsAPITelemetryView(LoginRequiredMixin, View):
