"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Exam Hall Tickets & Roll Number Generation (HallTickets).
...
class HallTicketsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for HallTickets."""
    template_name = 'examinations/halltickets_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = HallTicketsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'HallTicketsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Exam Hall Tickets & Roll Number Generation Operations Dashboard",
            'domain_name': 'HallTickets',
            'app_title': 'Examinations',
...
class HallTicketsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'examinations/halltickets_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Exam Hall Tickets & Roll Number Generation Analytical Insights",
