"""
Extended REST API Controllers for EduFlow Exam Schedules & Timetables (ExamSchedules).
...
class ExamSchedulesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ExamSchedulesMaster, pk=pk)
        data = master.to_dict()
...
class ExamSchedulesAPISearchView(LoginRequiredMixin, View):
...
class ExamSchedulesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ExamSchedulesAPITelemetryView(LoginRequiredMixin, View):
