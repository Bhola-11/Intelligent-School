"""
Models for EduFlow Exam Rooms & Invigilator Assignments (Invigilation) in apps.examinations.
...
# -----------------------------------------------------------------------------
class InvigilationStatusChoices(models.TextChoices):
...
class InvigilationPriorityChoices(models.TextChoices):
...
class InvigilationTierChoices(models.TextChoices):
...
class InvigilationScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class InvigilationMasterQuerySet(models.QuerySet):
    def active(self):
...
class InvigilationMasterManager(models.Manager):
    def get_queryset(self):
        return InvigilationMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class InvigilationMaster(models.Model):
    """
    Primary master configuration entity for Exam Rooms & Invigilator Assignments.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = InvigilationStatusChoices
    PriorityChoices = InvigilationPriorityChoices
    TierChoices = InvigilationTierChoices
    ScopeChoices = InvigilationScopeChoices
    ...
    objects = InvigilationMasterManager()

    class Meta:
        db_table = 'eduflow_examinations_invigilation_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Invigilation Master")
        verbose_name_plural = _("Invigilation Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_invig_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_invig_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_invig_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_invig_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'examinations:invigilation_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/examinations/invigilation/{self.pk}/"
...
# -----------------------------------------------------------------------------
class InvigilationItem(models.Model):
    """Subordinate component or line transaction item for Invigilation."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Invigilation Item")
        verbose_name_plural = _("Invigilation Items")
...
class InvigilationAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Invigilation."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_allocation'
        ordering = ['-start_time']
        verbose_name = _("Invigilation Allocation")
        verbose_name_plural = _("Invigilation Allocations")
...
# -----------------------------------------------------------------------------
class InvigilationMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Invigilation."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Invigilation Metric Record")
        verbose_name_plural = _("Invigilation Metric Records")
...
# -----------------------------------------------------------------------------
class InvigilationPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Invigilation."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_rule'
        ordering = ['rule_code']
        verbose_name = _("Invigilation Policy Rule")
        verbose_name_plural = _("Invigilation Policy Rules")
...
# -----------------------------------------------------------------------------
class InvigilationSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Invigilation."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Invigilation Schedule Period")
        verbose_name_plural = _("Invigilation Schedule Periods")
...
# -----------------------------------------------------------------------------
class InvigilationFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Invigilation."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_review'
        ordering = ['-submitted_at']
        verbose_name = _("Invigilation Review")
        verbose_name_plural = _("Invigilation Reviews")
...
# -----------------------------------------------------------------------------
class InvigilationWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Invigilation Workflow Transition")
        verbose_name_plural = _("Invigilation Workflow Transitions")
...
# -----------------------------------------------------------------------------
class InvigilationAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_access'
        verbose_name = _("Invigilation Access Rule")
        verbose_name_plural = _("Invigilation Access Rules")
...
# -----------------------------------------------------------------------------
class InvigilationConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_config'
        verbose_name = _("Invigilation Config Parameter")
        verbose_name_plural = _("Invigilation Config Parameters")
...
# -----------------------------------------------------------------------------
class InvigilationDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Invigilation Document Attachment")
        verbose_name_plural = _("Invigilation Document Attachments")
...
# -----------------------------------------------------------------------------
class InvigilationAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Invigilation."""
    master = models.ForeignKey(InvigilationMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_examinations_invigilation_audit'
        ordering = ['-timestamp']
        verbose_name = _("Invigilation Audit Entry")
        verbose_name_plural = _("Invigilation Audit Entries")
