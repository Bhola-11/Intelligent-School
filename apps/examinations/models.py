"""
Models for EduFlow Examination Sessions & Exam Types (ExamSessions) in apps.examinations.
...
# -----------------------------------------------------------------------------
class ExamSessionsStatusChoices(models.TextChoices):
...
class ExamSessionsPriorityChoices(models.TextChoices):
...
class ExamSessionsTierChoices(models.TextChoices):
...
class ExamSessionsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ExamSessionsMasterQuerySet(models.QuerySet):
    def active(self):
...
class ExamSessionsMasterManager(models.Manager):
    def get_queryset(self):
        return ExamSessionsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ExamSessionsMaster(models.Model):
    """
    Primary master configuration entity for Examination Sessions & Exam Types.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ExamSessionsStatusChoices
    PriorityChoices = ExamSessionsPriorityChoices
    TierChoices = ExamSessionsTierChoices
    ScopeChoices = ExamSessionsScopeChoices
    ...
    objects = ExamSessionsMasterManager()

    class Meta:
        db_table = 'eduflow_examinations_examsessions_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("ExamSessions Master")
        verbose_name_plural = _("ExamSessions Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'examinations:examsessions_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/examinations/examsessions/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ExamSessionsItem(models.Model):
    """Subordinate component or line transaction item for ExamSessions."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("ExamSessions Item")
        verbose_name_plural = _("ExamSessions Items")
...
# -----------------------------------------------------------------------------
class ExamSessionsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for ExamSessions."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_allocation'
        ordering = ['-start_time']
        verbose_name = _("ExamSessions Allocation")
        verbose_name_plural = _("ExamSessions Allocations")
...
# -----------------------------------------------------------------------------
class ExamSessionsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for ExamSessions."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("ExamSessions Metric Record")
        verbose_name_plural = _("ExamSessions Metric Records")
...
# -----------------------------------------------------------------------------
class ExamSessionsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for ExamSessions."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_rule'
        ordering = ['rule_code']
        verbose_name = _("ExamSessions Policy Rule")
        verbose_name_plural = _("ExamSessions Policy Rules")
...
# -----------------------------------------------------------------------------
class ExamSessionsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for ExamSessions."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("ExamSessions Schedule Period")
        verbose_name_plural = _("ExamSessions Schedule Periods")
...
# -----------------------------------------------------------------------------
class ExamSessionsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for ExamSessions."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_review'
        ordering = ['-submitted_at']
        verbose_name = _("ExamSessions Review")
        verbose_name_plural = _("ExamSessions Reviews")
...
# -----------------------------------------------------------------------------
class ExamSessionsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("ExamSessions Workflow Transition")
        verbose_name_plural = _("ExamSessions Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ExamSessionsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_access'
        verbose_name = _("ExamSessions Access Rule")
        verbose_name_plural = _("ExamSessions Access Rules")
...
# -----------------------------------------------------------------------------
class ExamSessionsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_config'
        verbose_name = _("ExamSessions Config Parameter")
        verbose_name_plural = _("ExamSessions Config Parameters")
...
# -----------------------------------------------------------------------------
class ExamSessionsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("ExamSessions Document Attachment")
        verbose_name_plural = _("ExamSessions Document Attachments")
...
# -----------------------------------------------------------------------------
class ExamSessionsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on ExamSessions."""
    master = models.ForeignKey(ExamSessionsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examsessions_audit'
        ordering = ['-timestamp']
        verbose_name = _("ExamSessions Audit Entry")
        verbose_name_plural = _("ExamSessions Audit Entries")
