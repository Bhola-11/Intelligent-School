"""
Extended REST API Controllers for EduFlow Exam Seating Arrangement & Desk Planner (SeatingPlan).
...
class SeatingPlanAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(SeatingPlanMaster, pk=pk)
        data = master.to_dict()
...
class SeatingPlanAPISearchView(LoginRequiredMixin, View):
...
class SeatingPlanAPIBatchUpdateView(LoginRequiredMixin, View):
...
class SeatingPlanAPITelemetryView(LoginRequiredMixin, View):
