"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Exam Schedules & Timetables (ExamSchedules).
...
class ExamSchedulesDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for ExamSchedules."""
    template_name = 'examinations/examschedules_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = ExamSchedulesAuditTrail.objects.all().order_by('-timestamp')[:10] if 'ExamSchedulesAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Exam Schedules & Timetables Operations Dashboard",
            'domain_name': 'ExamSchedules',
...
class ExamSchedulesAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'examinations/examschedules_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Exam Schedules & Timetables Analytical Insights",
