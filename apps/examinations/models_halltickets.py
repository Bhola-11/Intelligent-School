"""
Models for EduFlow Exam Hall Tickets & Roll Number Generation (HallTickets) in apps.examinations.
...
# -----------------------------------------------------------------------------
class HallTicketsStatusChoices(models.TextChoices):
...
class HallTicketsPriorityChoices(models.TextChoices):
...
class HallTicketsTierChoices(models.TextChoices):
...
class HallTicketsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class HallTicketsMasterQuerySet(models.QuerySet):
    def active(self):
...
class HallTicketsMasterManager(models.Manager):
    def get_queryset(self):
        return HallTicketsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class HallTicketsMaster(models.Model):
    """
    Primary master configuration entity for Exam Hall Tickets & Roll Number Generation.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = HallTicketsStatusChoices
    PriorityChoices = HallTicketsPriorityChoices
    TierChoices = HallTicketsTierChoices
    ScopeChoices = HallTicketsScopeChoices
    ...
    objects = HallTicketsMasterManager()

    class Meta:
        db_table = 'eduflow_examinations_halltickets_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("HallTickets Master")
        verbose_name_plural = _("HallTickets Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_hallt_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_hallt_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_hallt_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_hallt_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'examinations:halltickets_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/examinations/halltickets/{self.pk}/"
...
# -----------------------------------------------------------------------------
class HallTicketsItem(models.Model):
    """Subordinate component or line transaction item for HallTickets."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("HallTickets Item")
        verbose_name_plural = _("HallTickets Items")
...
# -----------------------------------------------------------------------------
class HallTicketsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for HallTickets."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_allocation'
        ordering = ['-start_time']
        verbose_name = _("HallTickets Allocation")
        verbose_name_plural = _("HallTickets Allocations")
...
# -----------------------------------------------------------------------------
class HallTicketsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for HallTickets."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("HallTickets Metric Record")
        verbose_name_plural = _("HallTickets Metric Records")
...
# -----------------------------------------------------------------------------
class HallTicketsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for HallTickets."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_rule'
        ordering = ['rule_code']
        verbose_name = _("HallTickets Policy Rule")
        verbose_name_plural = _("HallTickets Policy Rules")
...
# -----------------------------------------------------------------------------
class HallTicketsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for HallTickets."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("HallTickets Schedule Period")
        verbose_name_plural = _("HallTickets Schedule Periods")
...
# -----------------------------------------------------------------------------
class HallTicketsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for HallTickets."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_review'
        ordering = ['-submitted_at']
        verbose_name = _("HallTickets Review")
        verbose_name_plural = _("HallTickets Reviews")
...
# -----------------------------------------------------------------------------
class HallTicketsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("HallTickets Workflow Transition")
        verbose_name_plural = _("HallTickets Workflow Transitions")
...
# -----------------------------------------------------------------------------
class HallTicketsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_access'
        verbose_name = _("HallTickets Access Rule")
        verbose_name_plural = _("HallTickets Access Rules")
...
# -----------------------------------------------------------------------------
class HallTicketsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_config'
        verbose_name = _("HallTickets Config Parameter")
        verbose_name_plural = _("HallTickets Config Parameters")
...
# -----------------------------------------------------------------------------
class HallTicketsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("HallTickets Document Attachment")
        verbose_name_plural = _("HallTickets Document Attachments")
...
# -----------------------------------------------------------------------------
class HallTicketsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on HallTickets."""
    master = models.ForeignKey(HallTicketsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_examinations_halltickets_audit'
        ordering = ['-timestamp']
        verbose_name = _("HallTickets Audit Entry")
        verbose_name_plural = _("HallTickets Audit Entries")
