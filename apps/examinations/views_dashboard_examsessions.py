"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Examination Sessions & Exam Types (ExamSessions).
...
class ExamSessionsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for ExamSessions."""
    template_name = 'examinations/examsessions_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = ExamSessionsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'ExamSessionsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Examination Sessions & Exam Types Operations Dashboard",
            'domain_name': 'ExamSessions',
...
class ExamSessionsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'examinations/examsessions_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Examination Sessions & Exam Types Analytical Insights",
