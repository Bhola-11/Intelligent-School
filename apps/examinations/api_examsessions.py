"""
Extended REST API Controllers for EduFlow Examination Sessions & Exam Types (ExamSessions).
...
class ExamSessionsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ExamSessionsMaster, pk=pk)
        data = master.to_dict()
...
class ExamSessionsAPISearchView(LoginRequiredMixin, View):
...
class ExamSessionsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ExamSessionsAPITelemetryView(LoginRequiredMixin, View):
