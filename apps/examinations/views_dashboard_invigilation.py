"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Exam Rooms & Invigilator Assignments (Invigilation).
...
class InvigilationDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Invigilation."""
    template_name = 'examinations/invigilation_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = InvigilationAuditTrail.objects.all().order_by('-timestamp')[:10] if 'InvigilationAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Exam Rooms & Invigilator Assignments Operations Dashboard",
            'domain_name': 'Invigilation',
...
class InvigilationAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'examinations/invigilation_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Exam Rooms & Invigilator Assignments Analytical Insights",
