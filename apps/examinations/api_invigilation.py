"""
Extended REST API Controllers for EduFlow Exam Rooms & Invigilator Assignments (Invigilation).
...
class InvigilationAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(InvigilationMaster, pk=pk)
        data = master.to_dict()
...
class InvigilationAPISearchView(LoginRequiredMixin, View):
...
class InvigilationAPIBatchUpdateView(LoginRequiredMixin, View):
...
class InvigilationAPITelemetryView(LoginRequiredMixin, View):
