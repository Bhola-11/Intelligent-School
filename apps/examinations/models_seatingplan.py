"""
Models for EduFlow Exam Seating Arrangement & Desk Planner (SeatingPlan) in apps.examinations.
...
# -----------------------------------------------------------------------------
class SeatingPlanStatusChoices(models.TextChoices):
...
class SeatingPlanPriorityChoices(models.TextChoices):
...
class SeatingPlanTierChoices(models.TextChoices):
...
class SeatingPlanScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class SeatingPlanMasterQuerySet(models.QuerySet):
    def active(self):
...
class SeatingPlanMasterManager(models.Manager):
    def get_queryset(self):
        return SeatingPlanMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class SeatingPlanMaster(models.Model):
    """
    Primary master configuration entity for Exam Seating Arrangement & Desk Planner.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = SeatingPlanStatusChoices
    PriorityChoices = SeatingPlanPriorityChoices
    TierChoices = SeatingPlanTierChoices
    ScopeChoices = SeatingPlanScopeChoices
    ...
    objects = SeatingPlanMasterManager()

    class Meta:
        db_table = 'eduflow_examinations_seatingplan_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("SeatingPlan Master")
        verbose_name_plural = _("SeatingPlan Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_seati_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_seati_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_seati_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_seati_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'examinations:seatingplan_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/examinations/seatingplan/{self.pk}/"
...
# -----------------------------------------------------------------------------
class SeatingPlanItem(models.Model):
    """Subordinate component or line transaction item for SeatingPlan."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("SeatingPlan Item")
        verbose_name_plural = _("SeatingPlan Items")
...
# -----------------------------------------------------------------------------
class SeatingPlanAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for SeatingPlan."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_allocation'
        ordering = ['-start_time']
        verbose_name = _("SeatingPlan Allocation")
        verbose_name_plural = _("SeatingPlan Allocations")
...
# -----------------------------------------------------------------------------
class SeatingPlanMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for SeatingPlan."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("SeatingPlan Metric Record")
        verbose_name_plural = _("SeatingPlan Metric Records")
...
# -----------------------------------------------------------------------------
class SeatingPlanPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for SeatingPlan."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_rule'
        ordering = ['rule_code']
        verbose_name = _("SeatingPlan Policy Rule")
        verbose_name_plural = _("SeatingPlan Policy Rules")
...
# -----------------------------------------------------------------------------
class SeatingPlanSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for SeatingPlan."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("SeatingPlan Schedule Period")
        verbose_name_plural = _("SeatingPlan Schedule Periods")
...
# -----------------------------------------------------------------------------
class SeatingPlanFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for SeatingPlan."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_review'
        ordering = ['-submitted_at']
        verbose_name = _("SeatingPlan Review")
        verbose_name_plural = _("SeatingPlan Reviews")
...
# -----------------------------------------------------------------------------
class SeatingPlanWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("SeatingPlan Workflow Transition")
        verbose_name_plural = _("SeatingPlan Workflow Transitions")
...
# -----------------------------------------------------------------------------
class SeatingPlanAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_access'
        verbose_name = _("SeatingPlan Access Rule")
        verbose_name_plural = _("SeatingPlan Access Rules")
...
# -----------------------------------------------------------------------------
class SeatingPlanConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_config'
        verbose_name = _("SeatingPlan Config Parameter")
        verbose_name_plural = _("SeatingPlan Config Parameters")
...
# -----------------------------------------------------------------------------
class SeatingPlanDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("SeatingPlan Document Attachment")
        verbose_name_plural = _("SeatingPlan Document Attachments")
...
# -----------------------------------------------------------------------------
class SeatingPlanAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on SeatingPlan."""
    master = models.ForeignKey(SeatingPlanMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_examinations_seatingplan_audit'
        ordering = ['-timestamp']
        verbose_name = _("SeatingPlan Audit Entry")
        verbose_name_plural = _("SeatingPlan Audit Entries")
