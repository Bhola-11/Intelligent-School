"""
Models for EduFlow Exam Schedules & Timetables (ExamSchedules) in apps.examinations.
...
# -----------------------------------------------------------------------------
class ExamSchedulesStatusChoices(models.TextChoices):
...
class ExamSchedulesPriorityChoices(models.TextChoices):
...
class ExamSchedulesTierChoices(models.TextChoices):
...
class ExamSchedulesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ExamSchedulesMasterQuerySet(models.QuerySet):
    def active(self):
...
class ExamSchedulesMasterManager(models.Manager):
    def get_queryset(self):
        return ExamSchedulesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ExamSchedulesMaster(models.Model):
    """
    Primary master configuration entity for Exam Schedules & Timetables.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ExamSchedulesStatusChoices
    PriorityChoices = ExamSchedulesPriorityChoices
    TierChoices = ExamSchedulesTierChoices
    ScopeChoices = ExamSchedulesScopeChoices
    ...
    objects = ExamSchedulesMasterManager()

    class Meta:
        db_table = 'eduflow_examinations_examschedules_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("ExamSchedules Master")
        verbose_name_plural = _("ExamSchedules Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_exams_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_exams_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_exams_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_exams_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'examinations:examschedules_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/examinations/examschedules/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ExamSchedulesItem(models.Model):
    """Subordinate component or line transaction item for ExamSchedules."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("ExamSchedules Item")
        verbose_name_plural = _("ExamSchedules Items")
...
# -----------------------------------------------------------------------------
class ExamSchedulesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for ExamSchedules."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_allocation'
        ordering = ['-start_time']
        verbose_name = _("ExamSchedules Allocation")
        verbose_name_plural = _("ExamSchedules Allocations")
...
# -----------------------------------------------------------------------------
class ExamSchedulesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for ExamSchedules."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("ExamSchedules Metric Record")
        verbose_name_plural = _("ExamSchedules Metric Records")
...
# -----------------------------------------------------------------------------
class ExamSchedulesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for ExamSchedules."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_rule'
        ordering = ['rule_code']
        verbose_name = _("ExamSchedules Policy Rule")
        verbose_name_plural = _("ExamSchedules Policy Rules")
...
# -----------------------------------------------------------------------------
class ExamSchedulesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for ExamSchedules."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("ExamSchedules Schedule Period")
        verbose_name_plural = _("ExamSchedules Schedule Periods")
...
# -----------------------------------------------------------------------------
class ExamSchedulesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for ExamSchedules."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_review'
        ordering = ['-submitted_at']
        verbose_name = _("ExamSchedules Review")
        verbose_name_plural = _("ExamSchedules Reviews")
...
# -----------------------------------------------------------------------------
class ExamSchedulesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("ExamSchedules Workflow Transition")
        verbose_name_plural = _("ExamSchedules Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ExamSchedulesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_access'
        verbose_name = _("ExamSchedules Access Rule")
        verbose_name_plural = _("ExamSchedules Access Rules")
...
# -----------------------------------------------------------------------------
class ExamSchedulesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_config'
        verbose_name = _("ExamSchedules Config Parameter")
        verbose_name_plural = _("ExamSchedules Config Parameters")
...
# -----------------------------------------------------------------------------
class ExamSchedulesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("ExamSchedules Document Attachment")
        verbose_name_plural = _("ExamSchedules Document Attachments")
...
# -----------------------------------------------------------------------------
class ExamSchedulesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on ExamSchedules."""
    master = models.ForeignKey(ExamSchedulesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_examinations_examschedules_audit'
        ordering = ['-timestamp']
        verbose_name = _("ExamSchedules Audit Entry")
        verbose_name_plural = _("ExamSchedules Audit Entries")
