"""
Models for EduFlow Student Medical & Health Records (Health) in apps.students.
...
class HealthPriorityChoices(models.TextChoices):
...
class HealthTierChoices(models.TextChoices):
...
class HealthScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class HealthMasterQuerySet(models.QuerySet):
    def active(self):
...
class HealthMasterManager(models.Manager):
    def get_queryset(self):
        return HealthMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class HealthMaster(models.Model):
    """
    Primary master configuration entity for Student Medical & Health Records.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = HealthStatusChoices
    PriorityChoices = HealthPriorityChoices
    TierChoices = HealthTierChoices
    ScopeChoices = HealthScopeChoices
    ...
    objects = HealthMasterManager()

    class Meta:
        db_table = 'eduflow_students_health_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Health Master")
        verbose_name_plural = _("Health Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_healt_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_healt_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_healt_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_healt_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'students:health_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/students/health/{self.pk}/"
...
class HealthItem(models.Model):
    """Subordinate component or line transaction item for Health."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_students_health_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Health Item")
        verbose_name_plural = _("Health Items")
...
# -----------------------------------------------------------------------------
class HealthAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Health."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_students_health_allocation'
        ordering = ['-start_time']
        verbose_name = _("Health Allocation")
        verbose_name_plural = _("Health Allocations")
...
class HealthMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Health."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_students_health_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Health Metric Record")
        verbose_name_plural = _("Health Metric Records")
...
# -----------------------------------------------------------------------------
class HealthPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Health."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_health_rule'
        ordering = ['rule_code']
        verbose_name = _("Health Policy Rule")
        verbose_name_plural = _("Health Policy Rules")
...
# -----------------------------------------------------------------------------
class HealthSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Health."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_students_health_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Health Schedule Period")
        verbose_name_plural = _("Health Schedule Periods")
...
# -----------------------------------------------------------------------------
class HealthFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Health."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_students_health_review'
        ordering = ['-submitted_at']
        verbose_name = _("Health Review")
        verbose_name_plural = _("Health Reviews")
...
# -----------------------------------------------------------------------------
class HealthWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_students_health_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Health Workflow Transition")
        verbose_name_plural = _("Health Workflow Transitions")
...
# -----------------------------------------------------------------------------
class HealthAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_health_access'
        verbose_name = _("Health Access Rule")
        verbose_name_plural = _("Health Access Rules")
...
# -----------------------------------------------------------------------------
class HealthConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_students_health_config'
        verbose_name = _("Health Config Parameter")
        verbose_name_plural = _("Health Config Parameters")
...
# -----------------------------------------------------------------------------
class HealthDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_students_health_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Health Document Attachment")
        verbose_name_plural = _("Health Document Attachments")
...
class HealthAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Health."""
    master = models.ForeignKey(HealthMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_students_health_audit'
        ordering = ['-timestamp']
        verbose_name = _("Health Audit Entry")
        verbose_name_plural = _("Health Audit Entries")
