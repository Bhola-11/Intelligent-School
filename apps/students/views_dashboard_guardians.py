"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Guardians, Parents & Emergency Contacts (Guardians).
...
class GuardiansDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Guardians."""
    template_name = 'students/guardians_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = GuardiansAuditTrail.objects.all().order_by('-timestamp')[:10] if 'GuardiansAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Guardians, Parents & Emergency Contacts Operations Dashboard",
            'domain_name': 'Guardians',
            'app_title': 'Students',
...
class GuardiansAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'students/guardians_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Guardians, Parents & Emergency Contacts Analytical Insights",
