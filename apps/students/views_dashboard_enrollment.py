"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Student Enrollment & Academic History (Enrollment).
...
class EnrollmentDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Enrollment."""
    template_name = 'students/enrollment_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = EnrollmentAuditTrail.objects.all().order_by('-timestamp')[:10] if 'EnrollmentAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Student Enrollment & Academic History Operations Dashboard",
...
class EnrollmentAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'students/enrollment_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Student Enrollment & Academic History Analytical Insights",
