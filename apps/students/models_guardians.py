"""
Models for EduFlow Guardians, Parents & Emergency Contacts (Guardians) in apps.students.
...
# -----------------------------------------------------------------------------
class GuardiansStatusChoices(models.TextChoices):
...
class GuardiansPriorityChoices(models.TextChoices):
...
class GuardiansTierChoices(models.TextChoices):
...
class GuardiansScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class GuardiansMasterQuerySet(models.QuerySet):
    def active(self):
...
class GuardiansMasterManager(models.Manager):
    def get_queryset(self):
        return GuardiansMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class GuardiansMaster(models.Model):
    """
    Primary master configuration entity for Guardians, Parents & Emergency Contacts.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = GuardiansStatusChoices
    PriorityChoices = GuardiansPriorityChoices
    TierChoices = GuardiansTierChoices
    ScopeChoices = GuardiansScopeChoices
    ...
    objects = GuardiansMasterManager()

    class Meta:
        db_table = 'eduflow_students_guardians_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Guardians Master")
        verbose_name_plural = _("Guardians Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_guard_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_guard_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_guard_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_guard_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'students:guardians_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/students/guardians/{self.pk}/"
...
# -----------------------------------------------------------------------------
class GuardiansItem(models.Model):
    """Subordinate component or line transaction item for Guardians."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Guardians Item")
        verbose_name_plural = _("Guardians Items")
...
# -----------------------------------------------------------------------------
class GuardiansAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Guardians."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_allocation'
        ordering = ['-start_time']
        verbose_name = _("Guardians Allocation")
        verbose_name_plural = _("Guardians Allocations")
...
# -----------------------------------------------------------------------------
class GuardiansMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Guardians."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Guardians Metric Record")
        verbose_name_plural = _("Guardians Metric Records")
...
# -----------------------------------------------------------------------------
class GuardiansPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Guardians."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_rule'
        ordering = ['rule_code']
        verbose_name = _("Guardians Policy Rule")
        verbose_name_plural = _("Guardians Policy Rules")
...
# -----------------------------------------------------------------------------
class GuardiansSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Guardians."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Guardians Schedule Period")
        verbose_name_plural = _("Guardians Schedule Periods")
...
# -----------------------------------------------------------------------------
class GuardiansFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Guardians."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_review'
        ordering = ['-submitted_at']
        verbose_name = _("Guardians Review")
        verbose_name_plural = _("Guardians Reviews")
...
# -----------------------------------------------------------------------------
class GuardiansWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Guardians Workflow Transition")
        verbose_name_plural = _("Guardians Workflow Transitions")
...
# -----------------------------------------------------------------------------
class GuardiansAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_access'
        verbose_name = _("Guardians Access Rule")
        verbose_name_plural = _("Guardians Access Rules")
...
# -----------------------------------------------------------------------------
class GuardiansConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_config'
        verbose_name = _("Guardians Config Parameter")
        verbose_name_plural = _("Guardians Config Parameters")
...
# -----------------------------------------------------------------------------
class GuardiansDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Guardians Document Attachment")
        verbose_name_plural = _("Guardians Document Attachments")
...
# -----------------------------------------------------------------------------
class GuardiansAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Guardians."""
    master = models.ForeignKey(GuardiansMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_students_guardians_audit'
        ordering = ['-timestamp']
        verbose_name = _("Guardians Audit Entry")
        verbose_name_plural = _("Guardians Audit Entries")
