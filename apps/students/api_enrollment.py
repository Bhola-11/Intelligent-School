"""
Extended REST API Controllers for EduFlow Student Enrollment & Academic History (Enrollment).
...
class EnrollmentAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(EnrollmentMaster, pk=pk)
        data = master.to_dict()
...
class EnrollmentAPISearchView(LoginRequiredMixin, View):
...
class EnrollmentAPIBatchUpdateView(LoginRequiredMixin, View):
...
class EnrollmentAPITelemetryView(LoginRequiredMixin, View):
