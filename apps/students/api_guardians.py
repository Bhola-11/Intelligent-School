"""
Extended REST API Controllers for EduFlow Guardians, Parents & Emergency Contacts (Guardians).
...
class GuardiansAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(GuardiansMaster, pk=pk)
        data = master.to_dict()
...
class GuardiansAPISearchView(LoginRequiredMixin, View):
...
class GuardiansAPIBatchUpdateView(LoginRequiredMixin, View):
...
class GuardiansAPITelemetryView(LoginRequiredMixin, View):
