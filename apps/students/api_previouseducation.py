"""
Extended REST API Controllers for EduFlow Student Previous Education & Transfers (PreviousEducation).
...
class PreviousEducationAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(PreviousEducationMaster, pk=pk)
        data = master.to_dict()
...
class PreviousEducationAPISearchView(LoginRequiredMixin, View):
...
class PreviousEducationAPIBatchUpdateView(LoginRequiredMixin, View):
...
class PreviousEducationAPITelemetryView(LoginRequiredMixin, View):
