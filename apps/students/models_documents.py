"""
Models for EduFlow Student Document Vault & Verifications (Documents) in apps.students.
...
# -----------------------------------------------------------------------------
class DocumentsStatusChoices(models.TextChoices):
...
class DocumentsPriorityChoices(models.TextChoices):
...
class DocumentsTierChoices(models.TextChoices):
...
class DocumentsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class DocumentsMasterQuerySet(models.QuerySet):
    def active(self):
...
class DocumentsMasterManager(models.Manager):
    def get_queryset(self):
        return DocumentsMasterQuerySet(self.model, using=self._db)
...
class DocumentsMaster(models.Model):
    """
    Primary master configuration entity for Student Document Vault & Verifications.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = DocumentsStatusChoices
    PriorityChoices = DocumentsPriorityChoices
    TierChoices = DocumentsTierChoices
    ScopeChoices = DocumentsScopeChoices
    ...
    objects = DocumentsMasterManager()

    class Meta:
        db_table = 'eduflow_students_documents_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Documents Master")
        verbose_name_plural = _("Documents Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'students:documents_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/students/documents/{self.pk}/"
...
class DocumentsItem(models.Model):
    """Subordinate component or line transaction item for Documents."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Documents Item")
...
# -----------------------------------------------------------------------------
class DocumentsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Documents."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_allocation'
        ordering = ['-start_time']
        verbose_name = _("Documents Allocation")
        verbose_name_plural = _("Documents Allocations")
...
# -----------------------------------------------------------------------------
class DocumentsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Documents."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Documents Metric Record")
        verbose_name_plural = _("Documents Metric Records")
...
# -----------------------------------------------------------------------------
class DocumentsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Documents."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_rule'
        ordering = ['rule_code']
        verbose_name = _("Documents Policy Rule")
        verbose_name_plural = _("Documents Policy Rules")
...
# -----------------------------------------------------------------------------
class DocumentsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Documents."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Documents Schedule Period")
        verbose_name_plural = _("Documents Schedule Periods")
...
# -----------------------------------------------------------------------------
class DocumentsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Documents."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_review'
        ordering = ['-submitted_at']
        verbose_name = _("Documents Review")
        verbose_name_plural = _("Documents Reviews")
...
# -----------------------------------------------------------------------------
class DocumentsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Documents Workflow Transition")
        verbose_name_plural = _("Documents Workflow Transitions")
...
# -----------------------------------------------------------------------------
class DocumentsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_access'
        verbose_name = _("Documents Access Rule")
        verbose_name_plural = _("Documents Access Rules")
...
# -----------------------------------------------------------------------------
class DocumentsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_config'
        verbose_name = _("Documents Config Parameter")
        verbose_name_plural = _("Documents Config Parameters")
...
# -----------------------------------------------------------------------------
class DocumentsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Documents Document Attachment")
        verbose_name_plural = _("Documents Document Attachments")
...
# -----------------------------------------------------------------------------
class DocumentsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Documents."""
    master = models.ForeignKey(DocumentsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_students_documents_audit'
        ordering = ['-timestamp']
        verbose_name = _("Documents Audit Entry")
        verbose_name_plural = _("Documents Audit Entries")
