"""
Extended REST API Controllers for EduFlow Student Medical & Health Records (Health).
...
class HealthAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(HealthMaster, pk=pk)
        data = master.to_dict()
...
class HealthAPISearchView(LoginRequiredMixin, View):
...
class HealthAPIBatchUpdateView(LoginRequiredMixin, View):
...
class HealthAPITelemetryView(LoginRequiredMixin, View):
