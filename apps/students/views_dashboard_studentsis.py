"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Student Information System (SIS) Core (StudentSIS).
...
class StudentSISDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for StudentSIS."""
    template_name = 'students/studentsis_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = StudentSISAuditTrail.objects.all().order_by('-timestamp')[:10] if 'StudentSISAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Student Information System (SIS) Core Operations Dashboard",
            'domain_name': 'StudentSIS',
            'app_title': 'Students',
...
class StudentSISAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'students/studentsis_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Student Information System (SIS) Core Analytical Insights",
