"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Student Medical & Health Records (Health).
...
class HealthDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Health."""
    template_name = 'students/health_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = HealthAuditTrail.objects.all().order_by('-timestamp')[:10] if 'HealthAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Student Medical & Health Records Operations Dashboard",
            'domain_name': 'Health',
            'app_title': 'Students',
...
class HealthAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'students/health_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Student Medical & Health Records Analytical Insights",
