"""
Models for EduFlow Student Enrollment & Academic History (Enrollment) in apps.students.
...
class EnrollmentPriorityChoices(models.TextChoices):
...
class EnrollmentTierChoices(models.TextChoices):
...
class EnrollmentScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class EnrollmentMasterQuerySet(models.QuerySet):
    def active(self):
...
class EnrollmentMasterManager(models.Manager):
    def get_queryset(self):
        return EnrollmentMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class EnrollmentMaster(models.Model):
    """
    Primary master configuration entity for Student Enrollment & Academic History.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = EnrollmentStatusChoices
    PriorityChoices = EnrollmentPriorityChoices
    TierChoices = EnrollmentTierChoices
    ScopeChoices = EnrollmentScopeChoices
    ...
    objects = EnrollmentMasterManager()

    class Meta:
        db_table = 'eduflow_students_enrollment_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Enrollment Master")
        verbose_name_plural = _("Enrollment Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'students:enrollment_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/students/enrollment/{self.pk}/"
...
class EnrollmentItem(models.Model):
    """Subordinate component or line transaction item for Enrollment."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_item'
...
# -----------------------------------------------------------------------------
class EnrollmentAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Enrollment."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_allocation'
        ordering = ['-start_time']
        verbose_name = _("Enrollment Allocation")
        verbose_name_plural = _("Enrollment Allocations")
...
# -----------------------------------------------------------------------------
class EnrollmentMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Enrollment."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Enrollment Metric Record")
        verbose_name_plural = _("Enrollment Metric Records")
...
# -----------------------------------------------------------------------------
class EnrollmentPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Enrollment."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_rule'
        ordering = ['rule_code']
        verbose_name = _("Enrollment Policy Rule")
        verbose_name_plural = _("Enrollment Policy Rules")
...
# -----------------------------------------------------------------------------
class EnrollmentSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Enrollment."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Enrollment Schedule Period")
        verbose_name_plural = _("Enrollment Schedule Periods")
...
# -----------------------------------------------------------------------------
class EnrollmentFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Enrollment."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_review'
        ordering = ['-submitted_at']
        verbose_name = _("Enrollment Review")
        verbose_name_plural = _("Enrollment Reviews")
...
# -----------------------------------------------------------------------------
class EnrollmentWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Enrollment Workflow Transition")
        verbose_name_plural = _("Enrollment Workflow Transitions")
...
# -----------------------------------------------------------------------------
class EnrollmentAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_access'
        verbose_name = _("Enrollment Access Rule")
        verbose_name_plural = _("Enrollment Access Rules")
...
# -----------------------------------------------------------------------------
class EnrollmentConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_config'
        verbose_name = _("Enrollment Config Parameter")
        verbose_name_plural = _("Enrollment Config Parameters")
...
# -----------------------------------------------------------------------------
class EnrollmentDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Enrollment Document Attachment")
        verbose_name_plural = _("Enrollment Document Attachments")
...
# -----------------------------------------------------------------------------
class EnrollmentAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Enrollment."""
    master = models.ForeignKey(EnrollmentMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_students_enrollment_audit'
        ordering = ['-timestamp']
        verbose_name = _("Enrollment Audit Entry")
        verbose_name_plural = _("Enrollment Audit Entries")
