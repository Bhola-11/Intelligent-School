"""
Extended REST API Controllers for EduFlow Student Information System (SIS) Core (StudentSIS).
...
class StudentSISAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(StudentSISMaster, pk=pk)
        data = master.to_dict()
...
class StudentSISAPISearchView(LoginRequiredMixin, View):
...
class StudentSISAPIBatchUpdateView(LoginRequiredMixin, View):
...
class StudentSISAPITelemetryView(LoginRequiredMixin, View):
