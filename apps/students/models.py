"""
Models for EduFlow Student Information System (SIS) Core (StudentSIS) in apps.students.
...
# -----------------------------------------------------------------------------
class StudentSISStatusChoices(models.TextChoices):
...
class StudentSISPriorityChoices(models.TextChoices):
...
class StudentSISTierChoices(models.TextChoices):
...
class StudentSISScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class StudentSISMasterQuerySet(models.QuerySet):
    def active(self):
...
class StudentSISMasterManager(models.Manager):
    def get_queryset(self):
        return StudentSISMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class StudentSISMaster(models.Model):
    """
    Primary master configuration entity for Student Information System (SIS) Core.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = StudentSISStatusChoices
    PriorityChoices = StudentSISPriorityChoices
    TierChoices = StudentSISTierChoices
    ScopeChoices = StudentSISScopeChoices
    ...
    objects = StudentSISMasterManager()

    class Meta:
        db_table = 'eduflow_students_studentsis_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("StudentSIS Master")
        verbose_name_plural = _("StudentSIS Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'students:studentsis_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/students/studentsis/{self.pk}/"
...
# -----------------------------------------------------------------------------
class StudentSISItem(models.Model):
    """Subordinate component or line transaction item for StudentSIS."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("StudentSIS Item")
        verbose_name_plural = _("StudentSIS Items")
...
# -----------------------------------------------------------------------------
class StudentSISAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for StudentSIS."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_allocation'
        ordering = ['-start_time']
        verbose_name = _("StudentSIS Allocation")
        verbose_name_plural = _("StudentSIS Allocations")
...
# -----------------------------------------------------------------------------
class StudentSISMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for StudentSIS."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("StudentSIS Metric Record")
        verbose_name_plural = _("StudentSIS Metric Records")
...
# -----------------------------------------------------------------------------
class StudentSISPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for StudentSIS."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_rule'
        ordering = ['rule_code']
        verbose_name = _("StudentSIS Policy Rule")
        verbose_name_plural = _("StudentSIS Policy Rules")
...
# -----------------------------------------------------------------------------
class StudentSISSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for StudentSIS."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("StudentSIS Schedule Period")
        verbose_name_plural = _("StudentSIS Schedule Periods")
...
# -----------------------------------------------------------------------------
class StudentSISFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for StudentSIS."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_review'
        ordering = ['-submitted_at']
        verbose_name = _("StudentSIS Review")
        verbose_name_plural = _("StudentSIS Reviews")
...
# -----------------------------------------------------------------------------
class StudentSISWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("StudentSIS Workflow Transition")
        verbose_name_plural = _("StudentSIS Workflow Transitions")
...
# -----------------------------------------------------------------------------
class StudentSISAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_access'
        verbose_name = _("StudentSIS Access Rule")
        verbose_name_plural = _("StudentSIS Access Rules")
...
# -----------------------------------------------------------------------------
class StudentSISConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_config'
        verbose_name = _("StudentSIS Config Parameter")
        verbose_name_plural = _("StudentSIS Config Parameters")
...
# -----------------------------------------------------------------------------
class StudentSISDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("StudentSIS Document Attachment")
        verbose_name_plural = _("StudentSIS Document Attachments")
...
# -----------------------------------------------------------------------------
class StudentSISAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on StudentSIS."""
    master = models.ForeignKey(StudentSISMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_students_studentsis_audit'
        ordering = ['-timestamp']
        verbose_name = _("StudentSIS Audit Entry")
        verbose_name_plural = _("StudentSIS Audit Entries")
