"""
Extended REST API Controllers for EduFlow Student Document Vault & Verifications (Documents).
...
class DocumentsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(DocumentsMaster, pk=pk)
        data = master.to_dict()
...
class DocumentsAPISearchView(LoginRequiredMixin, View):
...
class DocumentsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class DocumentsAPITelemetryView(LoginRequiredMixin, View):
