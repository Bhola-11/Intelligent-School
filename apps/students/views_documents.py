"""
Views for EduFlow Student Document Vault & Verifications (Documents).
...
try:
    ...
    from .forms import (
        DocumentsMasterForm, DocumentsItemForm, DocumentsAllocationForm,
        DocumentsMetricRecordForm, DocumentsPolicyRuleForm, DocumentsSchedulePeriodForm,
        DocumentsFeedbackReviewForm, DocumentsWorkflowTransitionForm, DocumentsAccessRuleForm,
        DocumentsConfigurationParameterForm, DocumentsDocumentAttachmentForm, DocumentsFilterForm,
        DocumentsBulkActionForm
    )
except ImportError:
    try:
...
class DocumentsListView(LoginRequiredMixin, ListView):
    template_name = 'students/documents_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = DocumentsMaster.objects.all() if 'DocumentsMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = DocumentsFilterForm(self.request.GET) if 'DocumentsFilterForm' in globals() else None
        ctx['bulk_form'] = DocumentsBulkActionForm() if 'DocumentsBulkActionForm' in globals() else None
        ctx['total_count'] = DocumentsMaster.objects.count() if 'DocumentsMaster' in globals() else 0
        ctx['active_count'] = DocumentsMaster.objects.filter(status='ACTIVE').count() if 'DocumentsMaster' in globals() else 0
        ctx['pending_count'] = DocumentsMaster.objects.filter(status='PENDING_REVIEW').count() if 'DocumentsMaster' in globals() else 0
        ctx['page_title'] = "Student Document Vault & Verifications Directory"
        ctx['domain_name'] = "Documents"
        return ctx

class DocumentsDetailView(LoginRequiredMixin, DetailView):
    template_name = 'students/documents_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(DocumentsMaster, pk=self.kwargs.get('pk'))
...
class DocumentsCreateView(LoginRequiredMixin, CreateView):
    template_name = 'students/documents_form.html'
    form_class = DocumentsMasterForm
    success_url = reverse_lazy('students:documents_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Documents record '{form.instance.name}' created successfully.")
        return super().form_valid(form)

class DocumentsUpdateView(LoginRequiredMixin, UpdateView):
    template_name = 'students/documents_form.html'
    form_class = DocumentsMasterForm
    success_url = reverse_lazy('students:documents_list')
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Documents record updated successfully.")
        return super().form_valid(form)

class DocumentsDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'students/documents_confirm_delete.html'
    success_url = reverse_lazy('students:documents_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class DocumentsItemCreateView(LoginRequiredMixin, CreateView):
    form_class = DocumentsItemForm
    template_name = 'students/documents_form.html'

    def form_valid(self, form):
        master = get_object_or_404(DocumentsMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('students:documents_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class DocumentsItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = DocumentsItemForm
    template_name = 'students/documents_form.html'

    def get_object(self):
        return get_object_or_404(DocumentsItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('students:documents_detail', kwargs={'pk': self.object.master_id})

class DocumentsItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'students/documents_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class DocumentsAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = DocumentsAllocationForm
    template_name = 'students/documents_form.html'
...
class DocumentsAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(DocumentsAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('students:documents_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class DocumentsMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = DocumentsMetricRecordForm
    template_name = 'students/documents_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class DocumentsPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = DocumentsPolicyRuleForm
    template_name = 'students/documents_form.html'
...
# -----------------------------------------------------------------------------
class DocumentsSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = DocumentsSchedulePeriodForm
    template_name = 'students/documents_form.html'
...
# -----------------------------------------------------------------------------
class DocumentsFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = DocumentsFeedbackReviewForm
    template_name = 'students/documents_form.html'
...
# -----------------------------------------------------------------------------
class DocumentsWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = DocumentsWorkflowTransitionForm
    template_name = 'students/documents_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class DocumentsAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = DocumentsAccessRuleForm
    template_name = 'students/documents_form.html'
...
# -----------------------------------------------------------------------------
class DocumentsConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = DocumentsConfigurationParameterForm
    template_name = 'students/documents_form.html'
...
# -----------------------------------------------------------------------------
class DocumentsDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = DocumentsDocumentAttachmentForm
    template_name = 'students/documents_form.html'
...
# -----------------------------------------------------------------------------
class DocumentsBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = DocumentsMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('students:documents_list')

class DocumentsExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="documents_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        writer = csv.writer(response)
        writer.writerow(['Code', 'Name', 'Category', 'Tier', 'Priority', 'Status', 'Capacity', 'Occupancy', 'Budget ($)', 'Incurred ($)', 'Start Date', 'End Date'])
        records = DocumentsMaster.objects.all() if 'DocumentsMaster' in globals() else []
        for r in records:
...
class DocumentsExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = DocumentsMaster.objects.all() if 'DocumentsMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class DocumentsAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = DocumentsMaster.objects.all() if 'DocumentsMaster' in globals() else []
...
class DocumentsAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(DocumentsMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
