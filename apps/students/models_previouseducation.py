"""
Models for EduFlow Student Previous Education & Transfers (PreviousEducation) in apps.students.
...
# -----------------------------------------------------------------------------
class PreviousEducationStatusChoices(models.TextChoices):
...
class PreviousEducationPriorityChoices(models.TextChoices):
...
class PreviousEducationTierChoices(models.TextChoices):
...
class PreviousEducationScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class PreviousEducationMasterQuerySet(models.QuerySet):
    def active(self):
...
class PreviousEducationMasterManager(models.Manager):
    def get_queryset(self):
        return PreviousEducationMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class PreviousEducationMaster(models.Model):
    """
    Primary master configuration entity for Student Previous Education & Transfers.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = PreviousEducationStatusChoices
    PriorityChoices = PreviousEducationPriorityChoices
    TierChoices = PreviousEducationTierChoices
    ScopeChoices = PreviousEducationScopeChoices
    ...
    objects = PreviousEducationMasterManager()

    class Meta:
        db_table = 'eduflow_students_previouseducation_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("PreviousEducation Master")
        verbose_name_plural = _("PreviousEducation Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_previ_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_previ_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_previ_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_previ_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'students:previouseducation_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/students/previouseducation/{self.pk}/"
...
# -----------------------------------------------------------------------------
class PreviousEducationItem(models.Model):
    """Subordinate component or line transaction item for PreviousEducation."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("PreviousEducation Item")
        verbose_name_plural = _("PreviousEducation Items")
...
# -----------------------------------------------------------------------------
class PreviousEducationAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for PreviousEducation."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_allocation'
        ordering = ['-start_time']
        verbose_name = _("PreviousEducation Allocation")
        verbose_name_plural = _("PreviousEducation Allocations")
...
# -----------------------------------------------------------------------------
class PreviousEducationMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for PreviousEducation."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("PreviousEducation Metric Record")
        verbose_name_plural = _("PreviousEducation Metric Records")
...
# -----------------------------------------------------------------------------
class PreviousEducationPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for PreviousEducation."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_rule'
        ordering = ['rule_code']
        verbose_name = _("PreviousEducation Policy Rule")
        verbose_name_plural = _("PreviousEducation Policy Rules")
...
# -----------------------------------------------------------------------------
class PreviousEducationSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for PreviousEducation."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("PreviousEducation Schedule Period")
        verbose_name_plural = _("PreviousEducation Schedule Periods")
...
# -----------------------------------------------------------------------------
class PreviousEducationFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for PreviousEducation."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_review'
        ordering = ['-submitted_at']
        verbose_name = _("PreviousEducation Review")
        verbose_name_plural = _("PreviousEducation Reviews")
...
# -----------------------------------------------------------------------------
class PreviousEducationWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("PreviousEducation Workflow Transition")
        verbose_name_plural = _("PreviousEducation Workflow Transitions")
...
# -----------------------------------------------------------------------------
class PreviousEducationAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_access'
        verbose_name = _("PreviousEducation Access Rule")
        verbose_name_plural = _("PreviousEducation Access Rules")
...
# -----------------------------------------------------------------------------
class PreviousEducationConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_config'
        verbose_name = _("PreviousEducation Config Parameter")
        verbose_name_plural = _("PreviousEducation Config Parameters")
...
# -----------------------------------------------------------------------------
class PreviousEducationDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("PreviousEducation Document Attachment")
        verbose_name_plural = _("PreviousEducation Document Attachments")
...
# -----------------------------------------------------------------------------
class PreviousEducationAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on PreviousEducation."""
    master = models.ForeignKey(PreviousEducationMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_students_previouseducation_audit'
        ordering = ['-timestamp']
        verbose_name = _("PreviousEducation Audit Entry")
        verbose_name_plural = _("PreviousEducation Audit Entries")
