"""
Models for EduFlow Institutional Document Vault & Access Control (DocumentVault) in apps.documents.
...
# -----------------------------------------------------------------------------
class DocumentVaultStatusChoices(models.TextChoices):
...
class DocumentVaultPriorityChoices(models.TextChoices):
...
class DocumentVaultTierChoices(models.TextChoices):
...
class DocumentVaultScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class DocumentVaultMasterQuerySet(models.QuerySet):
    def active(self):
...
class DocumentVaultMasterManager(models.Manager):
    def get_queryset(self):
        return DocumentVaultMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class DocumentVaultMaster(models.Model):
    """
    Primary master configuration entity for Institutional Document Vault & Access Control.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = DocumentVaultStatusChoices
    PriorityChoices = DocumentVaultPriorityChoices
    TierChoices = DocumentVaultTierChoices
    ScopeChoices = DocumentVaultScopeChoices
    ...
    objects = DocumentVaultMasterManager()

    class Meta:
        db_table = 'eduflow_documents_documentvault_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("DocumentVault Master")
        verbose_name_plural = _("DocumentVault Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_docum_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_docum_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_docum_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_docum_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'documents:documentvault_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/documents/documentvault/{self.pk}/"
...
class DocumentVaultItem(models.Model):
    """Subordinate component or line transaction item for DocumentVault."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("DocumentVault Item")
        verbose_name_plural = _("DocumentVault Items")
...
# -----------------------------------------------------------------------------
class DocumentVaultAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for DocumentVault."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_allocation'
        ordering = ['-start_time']
        verbose_name = _("DocumentVault Allocation")
        verbose_name_plural = _("DocumentVault Allocations")
...
# -----------------------------------------------------------------------------
class DocumentVaultMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for DocumentVault."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("DocumentVault Metric Record")
        verbose_name_plural = _("DocumentVault Metric Records")
...
# -----------------------------------------------------------------------------
class DocumentVaultPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for DocumentVault."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_rule'
        ordering = ['rule_code']
        verbose_name = _("DocumentVault Policy Rule")
        verbose_name_plural = _("DocumentVault Policy Rules")
...
# -----------------------------------------------------------------------------
class DocumentVaultSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for DocumentVault."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("DocumentVault Schedule Period")
        verbose_name_plural = _("DocumentVault Schedule Periods")
...
# -----------------------------------------------------------------------------
class DocumentVaultFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for DocumentVault."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_review'
        ordering = ['-submitted_at']
        verbose_name = _("DocumentVault Review")
        verbose_name_plural = _("DocumentVault Reviews")
...
# -----------------------------------------------------------------------------
class DocumentVaultWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("DocumentVault Workflow Transition")
        verbose_name_plural = _("DocumentVault Workflow Transitions")
...
# -----------------------------------------------------------------------------
class DocumentVaultAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_access'
        verbose_name = _("DocumentVault Access Rule")
        verbose_name_plural = _("DocumentVault Access Rules")
...
# -----------------------------------------------------------------------------
class DocumentVaultConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_config'
        verbose_name = _("DocumentVault Config Parameter")
        verbose_name_plural = _("DocumentVault Config Parameters")
...
# -----------------------------------------------------------------------------
class DocumentVaultDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("DocumentVault Document Attachment")
        verbose_name_plural = _("DocumentVault Document Attachments")
...
# -----------------------------------------------------------------------------
class DocumentVaultAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on DocumentVault."""
    master = models.ForeignKey(DocumentVaultMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_documents_documentvault_audit'
        ordering = ['-timestamp']
        verbose_name = _("DocumentVault Audit Entry")
        verbose_name_plural = _("DocumentVault Audit Entries")
