"""
Extended REST API Controllers for EduFlow Institutional Document Vault & Access Control (DocumentVault).
...
class DocumentVaultAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(DocumentVaultMaster, pk=pk)
        data = master.to_dict()
...
class DocumentVaultAPISearchView(LoginRequiredMixin, View):
...
class DocumentVaultAPIBatchUpdateView(LoginRequiredMixin, View):
...
class DocumentVaultAPITelemetryView(LoginRequiredMixin, View):
