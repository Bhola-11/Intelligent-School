"""
Models for EduFlow Academic Departments & Faculties (Departments) in apps.institution.
...
# -----------------------------------------------------------------------------
class DepartmentsStatusChoices(models.TextChoices):
...
class DepartmentsPriorityChoices(models.TextChoices):
...
class DepartmentsTierChoices(models.TextChoices):
...
class DepartmentsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class DepartmentsMasterQuerySet(models.QuerySet):
    def active(self):
...
class DepartmentsMasterManager(models.Manager):
    def get_queryset(self):
        return DepartmentsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class DepartmentsMaster(models.Model):
    """
    Primary master configuration entity for Academic Departments & Faculties.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = DepartmentsStatusChoices
    PriorityChoices = DepartmentsPriorityChoices
    TierChoices = DepartmentsTierChoices
    ScopeChoices = DepartmentsScopeChoices
    ...
    objects = DepartmentsMasterManager()

    class Meta:
        db_table = 'eduflow_institution_departments_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Departments Master")
        verbose_name_plural = _("Departments Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_depar_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_depar_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_depar_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_depar_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'institution:departments_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/institution/departments/{self.pk}/"
...
# -----------------------------------------------------------------------------
class DepartmentsItem(models.Model):
    """Subordinate component or line transaction item for Departments."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Departments Item")
        verbose_name_plural = _("Departments Items")
...
# -----------------------------------------------------------------------------
class DepartmentsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Departments."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_allocation'
        ordering = ['-start_time']
        verbose_name = _("Departments Allocation")
        verbose_name_plural = _("Departments Allocations")
...
# -----------------------------------------------------------------------------
class DepartmentsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Departments."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Departments Metric Record")
        verbose_name_plural = _("Departments Metric Records")
...
# -----------------------------------------------------------------------------
class DepartmentsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Departments."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_rule'
        ordering = ['rule_code']
        verbose_name = _("Departments Policy Rule")
        verbose_name_plural = _("Departments Policy Rules")
...
# -----------------------------------------------------------------------------
class DepartmentsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Departments."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Departments Schedule Period")
        verbose_name_plural = _("Departments Schedule Periods")
...
# -----------------------------------------------------------------------------
class DepartmentsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Departments."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_review'
        ordering = ['-submitted_at']
        verbose_name = _("Departments Review")
        verbose_name_plural = _("Departments Reviews")
...
# -----------------------------------------------------------------------------
class DepartmentsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Departments Workflow Transition")
        verbose_name_plural = _("Departments Workflow Transitions")
...
# -----------------------------------------------------------------------------
class DepartmentsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_access'
        verbose_name = _("Departments Access Rule")
        verbose_name_plural = _("Departments Access Rules")
...
# -----------------------------------------------------------------------------
class DepartmentsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_config'
        verbose_name = _("Departments Config Parameter")
        verbose_name_plural = _("Departments Config Parameters")
...
# -----------------------------------------------------------------------------
class DepartmentsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Departments Document Attachment")
        verbose_name_plural = _("Departments Document Attachments")
...
# -----------------------------------------------------------------------------
class DepartmentsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Departments."""
    master = models.ForeignKey(DepartmentsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_institution_departments_audit'
        ordering = ['-timestamp']
        verbose_name = _("Departments Audit Entry")
        verbose_name_plural = _("Departments Audit Entries")
