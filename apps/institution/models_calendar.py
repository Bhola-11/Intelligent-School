"""
Models for EduFlow Academic Calendar, Terms & Semesters (Calendar) in apps.institution.
...
# -----------------------------------------------------------------------------
class CalendarStatusChoices(models.TextChoices):
...
class CalendarPriorityChoices(models.TextChoices):
...
class CalendarTierChoices(models.TextChoices):
...
class CalendarScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class CalendarMasterQuerySet(models.QuerySet):
    def active(self):
...
class CalendarMasterManager(models.Manager):
    def get_queryset(self):
        return CalendarMasterQuerySet(self.model, using=self._db)
...
class CalendarMaster(models.Model):
    """
    Primary master configuration entity for Academic Calendar, Terms & Semesters.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = CalendarStatusChoices
    PriorityChoices = CalendarPriorityChoices
    TierChoices = CalendarTierChoices
    ScopeChoices = CalendarScopeChoices
    ...
    objects = CalendarMasterManager()

    class Meta:
        db_table = 'eduflow_institution_calendar_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Calendar Master")
        verbose_name_plural = _("Calendar Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_calen_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_calen_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_calen_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_calen_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'institution:calendar_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/institution/calendar/{self.pk}/"
...
class CalendarItem(models.Model):
    """Subordinate component or line transaction item for Calendar."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_item'
...
# -----------------------------------------------------------------------------
class CalendarAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Calendar."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_allocation'
        ordering = ['-start_time']
        verbose_name = _("Calendar Allocation")
        verbose_name_plural = _("Calendar Allocations")
...
# -----------------------------------------------------------------------------
class CalendarMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Calendar."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Calendar Metric Record")
        verbose_name_plural = _("Calendar Metric Records")
...
# -----------------------------------------------------------------------------
class CalendarPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Calendar."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_rule'
        ordering = ['rule_code']
        verbose_name = _("Calendar Policy Rule")
        verbose_name_plural = _("Calendar Policy Rules")
...
# -----------------------------------------------------------------------------
class CalendarSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Calendar."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Calendar Schedule Period")
        verbose_name_plural = _("Calendar Schedule Periods")
...
# -----------------------------------------------------------------------------
class CalendarFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Calendar."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_review'
        ordering = ['-submitted_at']
        verbose_name = _("Calendar Review")
        verbose_name_plural = _("Calendar Reviews")
...
# -----------------------------------------------------------------------------
class CalendarWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Calendar Workflow Transition")
        verbose_name_plural = _("Calendar Workflow Transitions")
...
# -----------------------------------------------------------------------------
class CalendarAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_access'
        verbose_name = _("Calendar Access Rule")
        verbose_name_plural = _("Calendar Access Rules")
...
# -----------------------------------------------------------------------------
class CalendarConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_config'
        verbose_name = _("Calendar Config Parameter")
        verbose_name_plural = _("Calendar Config Parameters")
...
# -----------------------------------------------------------------------------
class CalendarDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Calendar Document Attachment")
        verbose_name_plural = _("Calendar Document Attachments")
...
# -----------------------------------------------------------------------------
class CalendarAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Calendar."""
    master = models.ForeignKey(CalendarMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_institution_calendar_audit'
        ordering = ['-timestamp']
        verbose_name = _("Calendar Audit Entry")
        verbose_name_plural = _("Calendar Audit Entries")
