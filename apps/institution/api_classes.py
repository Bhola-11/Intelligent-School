"""
Extended REST API Controllers for EduFlow Class Levels, Sections & Batches (Classes).
...
class ClassesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ClassesMaster, pk=pk)
        data = master.to_dict()
...
class ClassesAPISearchView(LoginRequiredMixin, View):
...
class ClassesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ClassesAPITelemetryView(LoginRequiredMixin, View):
