"""
Views for EduFlow Academic Calendar, Terms & Semesters (Calendar).
...
try:
    ...
    from .forms import (
        CalendarMasterForm, CalendarItemForm, CalendarAllocationForm,
        CalendarMetricRecordForm, CalendarPolicyRuleForm, CalendarSchedulePeriodForm,
        CalendarFeedbackReviewForm, CalendarWorkflowTransitionForm, CalendarAccessRuleForm,
        CalendarConfigurationParameterForm, CalendarDocumentAttachmentForm, CalendarFilterForm,
        CalendarBulkActionForm
    )
except ImportError:
    try:
...
class CalendarListView(LoginRequiredMixin, ListView):
    template_name = 'institution/calendar_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = CalendarMaster.objects.all() if 'CalendarMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = CalendarFilterForm(self.request.GET) if 'CalendarFilterForm' in globals() else None
        ctx['bulk_form'] = CalendarBulkActionForm() if 'CalendarBulkActionForm' in globals() else None
        ctx['total_count'] = CalendarMaster.objects.count() if 'CalendarMaster' in globals() else 0
        ctx['active_count'] = CalendarMaster.objects.filter(status='ACTIVE').count() if 'CalendarMaster' in globals() else 0
        ctx['pending_count'] = CalendarMaster.objects.filter(status='PENDING_REVIEW').count() if 'CalendarMaster' in globals() else 0
        ctx['page_title'] = "Academic Calendar, Terms & Semesters Directory"
        ctx['domain_name'] = "Calendar"
        return ctx

class CalendarDetailView(LoginRequiredMixin, DetailView):
    template_name = 'institution/calendar_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(CalendarMaster, pk=self.kwargs.get('pk'))
...
class CalendarCreateView(LoginRequiredMixin, CreateView):
    template_name = 'institution/calendar_form.html'
    form_class = CalendarMasterForm
    success_url = reverse_lazy('institution:calendar_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Calendar record '{form.instance.name}' created successfully.")
...
class CalendarUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Calendar record updated successfully.")
        return super().form_valid(form)

class CalendarDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'institution/calendar_confirm_delete.html'
    success_url = reverse_lazy('institution:calendar_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class CalendarItemCreateView(LoginRequiredMixin, CreateView):
    form_class = CalendarItemForm
    template_name = 'institution/calendar_form.html'

    def form_valid(self, form):
        master = get_object_or_404(CalendarMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('institution:calendar_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class CalendarItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = CalendarItemForm
    template_name = 'institution/calendar_form.html'

    def get_object(self):
        return get_object_or_404(CalendarItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('institution:calendar_detail', kwargs={'pk': self.object.master_id})

class CalendarItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'institution/calendar_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class CalendarAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = CalendarAllocationForm
    template_name = 'institution/calendar_form.html'
...
class CalendarAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(CalendarAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('institution:calendar_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class CalendarMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = CalendarMetricRecordForm
    template_name = 'institution/calendar_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class CalendarPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = CalendarPolicyRuleForm
    template_name = 'institution/calendar_form.html'
...
# -----------------------------------------------------------------------------
class CalendarSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = CalendarSchedulePeriodForm
    template_name = 'institution/calendar_form.html'
...
# -----------------------------------------------------------------------------
class CalendarFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = CalendarFeedbackReviewForm
    template_name = 'institution/calendar_form.html'
...
# -----------------------------------------------------------------------------
class CalendarWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = CalendarWorkflowTransitionForm
    template_name = 'institution/calendar_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class CalendarAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = CalendarAccessRuleForm
    template_name = 'institution/calendar_form.html'
...
# -----------------------------------------------------------------------------
class CalendarConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = CalendarConfigurationParameterForm
    template_name = 'institution/calendar_form.html'
...
# -----------------------------------------------------------------------------
class CalendarDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = CalendarDocumentAttachmentForm
    template_name = 'institution/calendar_form.html'
...
# -----------------------------------------------------------------------------
class CalendarBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = CalendarMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('institution:calendar_list')

class CalendarExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="calendar_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
...
class CalendarExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = CalendarMaster.objects.all() if 'CalendarMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class CalendarAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = CalendarMaster.objects.all() if 'CalendarMaster' in globals() else []
...
class CalendarAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(CalendarMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
