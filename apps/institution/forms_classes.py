"""
Forms for EduFlow Class Levels, Sections & Batches (Classes).
...
class ClassesMasterForm(forms.ModelForm):
    """Primary form for creating and updating Classes master records."""
    class Meta:
        try:
            model = ClassesMaster
            fields = [
            ...
            widgets = {
                'code': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. CLAS-001'}),
...
class ClassesItemForm(forms.ModelForm):
...
class ClassesAllocationForm(forms.ModelForm):
    """Form for resource and faculty allocations."""
    class Meta:
        try:
            model = ClassesAllocation
...
class ClassesMetricRecordForm(forms.ModelForm):
    """Form for KPI metric observations."""
    class Meta:
        try:
            model = ClassesMetricRecord
...
class ClassesPolicyRuleForm(forms.ModelForm):
    """Form for institutional governance rules."""
    class Meta:
        try:
            model = ClassesPolicyRule
...
class ClassesSchedulePeriodForm(forms.ModelForm):
...
class ClassesFeedbackReviewForm(forms.ModelForm):
    """Form for qualitative feedback and evaluation."""
    class Meta:
        try:
            model = ClassesFeedbackReview
...
class ClassesWorkflowTransitionForm(forms.ModelForm):
    """Form for submitting workflow approval transitions."""
    class Meta:
        try:
            model = ClassesWorkflowTransition
...
class ClassesAccessRuleForm(forms.ModelForm):
    """Form for object-level permission rules."""
    class Meta:
        try:
            model = ClassesAccessRule
...
class ClassesConfigurationParameterForm(forms.ModelForm):
    """Form for key-value configuration parameters."""
    class Meta:
        try:
            model = ClassesConfigurationParameter
...
class ClassesDocumentAttachmentForm(forms.ModelForm):
    """Form for registering official document attachments."""
    class Meta:
        try:
            model = ClassesDocumentAttachment
...
class ClassesBulkActionForm(forms.Form):
