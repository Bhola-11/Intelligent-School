"""
Extended REST API Controllers for EduFlow Buildings, Floors, Rooms & Facilities (Facilities).
...
class FacilitiesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(FacilitiesMaster, pk=pk)
        data = master.to_dict()
...
class FacilitiesAPISearchView(LoginRequiredMixin, View):
...
class FacilitiesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class FacilitiesAPITelemetryView(LoginRequiredMixin, View):
