"""
Extended REST API Controllers for EduFlow Degree Programs & Course Catalog (Programs).
...
class ProgramsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ProgramsMaster, pk=pk)
        data = master.to_dict()
...
class ProgramsAPISearchView(LoginRequiredMixin, View):
...
class ProgramsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ProgramsAPITelemetryView(LoginRequiredMixin, View):
