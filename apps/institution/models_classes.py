"""
Models for EduFlow Class Levels, Sections & Batches (Classes) in apps.institution.
...
# -----------------------------------------------------------------------------
class ClassesStatusChoices(models.TextChoices):
...
class ClassesPriorityChoices(models.TextChoices):
...
class ClassesTierChoices(models.TextChoices):
...
class ClassesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ClassesMasterQuerySet(models.QuerySet):
    def active(self):
...
class ClassesMasterManager(models.Manager):
    def get_queryset(self):
        return ClassesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ClassesMaster(models.Model):
    """
    Primary master configuration entity for Class Levels, Sections & Batches.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ClassesStatusChoices
    PriorityChoices = ClassesPriorityChoices
    TierChoices = ClassesTierChoices
    ScopeChoices = ClassesScopeChoices
    ...
    objects = ClassesMasterManager()

    class Meta:
        db_table = 'eduflow_institution_classes_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Classes Master")
        verbose_name_plural = _("Classes Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_class_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_class_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_class_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_class_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'institution:classes_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/institution/classes/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ClassesItem(models.Model):
    """Subordinate component or line transaction item for Classes."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Classes Item")
        verbose_name_plural = _("Classes Items")
...
# -----------------------------------------------------------------------------
class ClassesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Classes."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_allocation'
        ordering = ['-start_time']
        verbose_name = _("Classes Allocation")
        verbose_name_plural = _("Classes Allocations")
...
# -----------------------------------------------------------------------------
class ClassesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Classes."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Classes Metric Record")
        verbose_name_plural = _("Classes Metric Records")
...
# -----------------------------------------------------------------------------
class ClassesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Classes."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_rule'
        ordering = ['rule_code']
        verbose_name = _("Classes Policy Rule")
        verbose_name_plural = _("Classes Policy Rules")
...
# -----------------------------------------------------------------------------
class ClassesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Classes."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Classes Schedule Period")
        verbose_name_plural = _("Classes Schedule Periods")
...
# -----------------------------------------------------------------------------
class ClassesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Classes."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_review'
        ordering = ['-submitted_at']
        verbose_name = _("Classes Review")
        verbose_name_plural = _("Classes Reviews")
...
# -----------------------------------------------------------------------------
class ClassesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Classes Workflow Transition")
        verbose_name_plural = _("Classes Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ClassesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_access'
        verbose_name = _("Classes Access Rule")
        verbose_name_plural = _("Classes Access Rules")
...
# -----------------------------------------------------------------------------
class ClassesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_config'
        verbose_name = _("Classes Config Parameter")
        verbose_name_plural = _("Classes Config Parameters")
...
# -----------------------------------------------------------------------------
class ClassesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Classes Document Attachment")
        verbose_name_plural = _("Classes Document Attachments")
...
# -----------------------------------------------------------------------------
class ClassesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Classes."""
    master = models.ForeignKey(ClassesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_institution_classes_audit'
        ordering = ['-timestamp']
        verbose_name = _("Classes Audit Entry")
        verbose_name_plural = _("Classes Audit Entries")
