"""
Extended REST API Controllers for EduFlow Institution Profile & Multi-Campus Architecture (Institution).
...
class InstitutionAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(InstitutionMaster, pk=pk)
        data = master.to_dict()
...
class InstitutionAPISearchView(LoginRequiredMixin, View):
...
class InstitutionAPIBatchUpdateView(LoginRequiredMixin, View):
...
class InstitutionAPITelemetryView(LoginRequiredMixin, View):
