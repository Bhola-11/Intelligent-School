"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Subjects, Syllabi & Credit Hours (Subjects).
...
class SubjectsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Subjects."""
    template_name = 'institution/subjects_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = SubjectsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'SubjectsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Subjects, Syllabi & Credit Hours Operations Dashboard",
            'domain_name': 'Subjects',
...
class SubjectsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'institution/subjects_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Subjects, Syllabi & Credit Hours Analytical Insights",
