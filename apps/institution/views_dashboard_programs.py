"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Degree Programs & Course Catalog (Programs).
...
class ProgramsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Programs."""
    template_name = 'institution/programs_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = ProgramsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'ProgramsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Degree Programs & Course Catalog Operations Dashboard",
            'domain_name': 'Programs',
...
class ProgramsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'institution/programs_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Degree Programs & Course Catalog Analytical Insights",
