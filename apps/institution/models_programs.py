"""
Models for EduFlow Degree Programs & Course Catalog (Programs) in apps.institution.
...
# -----------------------------------------------------------------------------
class ProgramsStatusChoices(models.TextChoices):
...
class ProgramsPriorityChoices(models.TextChoices):
...
class ProgramsTierChoices(models.TextChoices):
...
class ProgramsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ProgramsMasterQuerySet(models.QuerySet):
    def active(self):
...
class ProgramsMasterManager(models.Manager):
    def get_queryset(self):
        return ProgramsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ProgramsMaster(models.Model):
    """
    Primary master configuration entity for Degree Programs & Course Catalog.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ProgramsStatusChoices
    PriorityChoices = ProgramsPriorityChoices
    TierChoices = ProgramsTierChoices
    ScopeChoices = ProgramsScopeChoices
    ...
    objects = ProgramsMasterManager()

    class Meta:
        db_table = 'eduflow_institution_programs_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Programs Master")
        verbose_name_plural = _("Programs Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_progr_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_progr_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_progr_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_progr_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'institution:programs_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/institution/programs/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ProgramsItem(models.Model):
    """Subordinate component or line transaction item for Programs."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Programs Item")
        verbose_name_plural = _("Programs Items")
...
# -----------------------------------------------------------------------------
class ProgramsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Programs."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_allocation'
        ordering = ['-start_time']
        verbose_name = _("Programs Allocation")
        verbose_name_plural = _("Programs Allocations")
...
# -----------------------------------------------------------------------------
class ProgramsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Programs."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Programs Metric Record")
        verbose_name_plural = _("Programs Metric Records")
...
# -----------------------------------------------------------------------------
class ProgramsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Programs."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_rule'
        ordering = ['rule_code']
        verbose_name = _("Programs Policy Rule")
        verbose_name_plural = _("Programs Policy Rules")
...
# -----------------------------------------------------------------------------
class ProgramsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Programs."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Programs Schedule Period")
        verbose_name_plural = _("Programs Schedule Periods")
...
# -----------------------------------------------------------------------------
class ProgramsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Programs."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_review'
        ordering = ['-submitted_at']
        verbose_name = _("Programs Review")
        verbose_name_plural = _("Programs Reviews")
...
# -----------------------------------------------------------------------------
class ProgramsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Programs Workflow Transition")
        verbose_name_plural = _("Programs Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ProgramsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_access'
        verbose_name = _("Programs Access Rule")
        verbose_name_plural = _("Programs Access Rules")
...
# -----------------------------------------------------------------------------
class ProgramsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_config'
        verbose_name = _("Programs Config Parameter")
        verbose_name_plural = _("Programs Config Parameters")
...
# -----------------------------------------------------------------------------
class ProgramsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Programs Document Attachment")
        verbose_name_plural = _("Programs Document Attachments")
...
# -----------------------------------------------------------------------------
class ProgramsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Programs."""
    master = models.ForeignKey(ProgramsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_institution_programs_audit'
        ordering = ['-timestamp']
        verbose_name = _("Programs Audit Entry")
        verbose_name_plural = _("Programs Audit Entries")
