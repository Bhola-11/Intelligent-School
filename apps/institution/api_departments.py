"""
Extended REST API Controllers for EduFlow Academic Departments & Faculties (Departments).
...
class DepartmentsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(DepartmentsMaster, pk=pk)
        data = master.to_dict()
...
class DepartmentsAPISearchView(LoginRequiredMixin, View):
...
class DepartmentsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class DepartmentsAPITelemetryView(LoginRequiredMixin, View):
