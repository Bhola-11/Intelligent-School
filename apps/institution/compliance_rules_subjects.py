"""
Regulatory Compliance & Multi-Tenant Governance Rules for EduFlow Subjects, Syllabi & Credit Hours (Subjects).
...
class SubjectsComplianceEvaluator:
    """Enterprise multi-tenant compliance evaluator and security guard for Subjects."""
    ...
    def register_violation(self, rule_id, severity, description, entity_id=None):
        ...
        logger.warning(f"[Subjects Compliance Violation] {rule_id} [{severity}]: {description}")
        return violation
    ...
    def generate_privacy_impact_assessment(self, data_flow_categories):
        """Generates comprehensive Privacy Impact Assessment (PIA) for Subjects operations."""
        risk_score = 0
        findings = []
