"""
Extended REST API Controllers for EduFlow Academic Calendar, Terms & Semesters (Calendar).
...
class CalendarAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(CalendarMaster, pk=pk)
        data = master.to_dict()
...
class CalendarAPISearchView(LoginRequiredMixin, View):
...
class CalendarAPIBatchUpdateView(LoginRequiredMixin, View):
...
class CalendarAPITelemetryView(LoginRequiredMixin, View):
