"""
Extended REST API Controllers for EduFlow Subjects, Syllabi & Credit Hours (Subjects).
...
class SubjectsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(SubjectsMaster, pk=pk)
        data = master.to_dict()
...
class SubjectsAPISearchView(LoginRequiredMixin, View):
...
class SubjectsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class SubjectsAPITelemetryView(LoginRequiredMixin, View):
