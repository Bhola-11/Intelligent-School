"""
Models for EduFlow Buildings, Floors, Rooms & Facilities (Facilities) in apps.institution.
...
# -----------------------------------------------------------------------------
class FacilitiesStatusChoices(models.TextChoices):
...
class FacilitiesPriorityChoices(models.TextChoices):
...
class FacilitiesTierChoices(models.TextChoices):
...
class FacilitiesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class FacilitiesMasterQuerySet(models.QuerySet):
    def active(self):
...
class FacilitiesMasterManager(models.Manager):
    def get_queryset(self):
        return FacilitiesMasterQuerySet(self.model, using=self._db)
...
class FacilitiesMaster(models.Model):
    """
    Primary master configuration entity for Buildings, Floors, Rooms & Facilities.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = FacilitiesStatusChoices
    PriorityChoices = FacilitiesPriorityChoices
    TierChoices = FacilitiesTierChoices
    ScopeChoices = FacilitiesScopeChoices
    ...
    objects = FacilitiesMasterManager()

    class Meta:
        db_table = 'eduflow_institution_facilities_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Facilities Master")
        verbose_name_plural = _("Facilities Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_facil_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_facil_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_facil_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_facil_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'institution:facilities_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/institution/facilities/{self.pk}/"
...
# -----------------------------------------------------------------------------
class FacilitiesItem(models.Model):
    """Subordinate component or line transaction item for Facilities."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Facilities Item")
        verbose_name_plural = _("Facilities Items")
...
# -----------------------------------------------------------------------------
class FacilitiesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Facilities."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_allocation'
        ordering = ['-start_time']
        verbose_name = _("Facilities Allocation")
        verbose_name_plural = _("Facilities Allocations")
...
# -----------------------------------------------------------------------------
class FacilitiesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Facilities."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Facilities Metric Record")
        verbose_name_plural = _("Facilities Metric Records")
...
# -----------------------------------------------------------------------------
class FacilitiesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Facilities."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_rule'
        ordering = ['rule_code']
        verbose_name = _("Facilities Policy Rule")
        verbose_name_plural = _("Facilities Policy Rules")
...
# -----------------------------------------------------------------------------
class FacilitiesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Facilities."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Facilities Schedule Period")
        verbose_name_plural = _("Facilities Schedule Periods")
...
# -----------------------------------------------------------------------------
class FacilitiesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Facilities."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_review'
        ordering = ['-submitted_at']
        verbose_name = _("Facilities Review")
        verbose_name_plural = _("Facilities Reviews")
...
# -----------------------------------------------------------------------------
class FacilitiesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Facilities Workflow Transition")
        verbose_name_plural = _("Facilities Workflow Transitions")
...
# -----------------------------------------------------------------------------
class FacilitiesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_access'
        verbose_name = _("Facilities Access Rule")
        verbose_name_plural = _("Facilities Access Rules")
...
# -----------------------------------------------------------------------------
class FacilitiesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_config'
        verbose_name = _("Facilities Config Parameter")
        verbose_name_plural = _("Facilities Config Parameters")
...
# -----------------------------------------------------------------------------
class FacilitiesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Facilities Document Attachment")
        verbose_name_plural = _("Facilities Document Attachments")
...
# -----------------------------------------------------------------------------
class FacilitiesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Facilities."""
    master = models.ForeignKey(FacilitiesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_institution_facilities_audit'
        ordering = ['-timestamp']
        verbose_name = _("Facilities Audit Entry")
        verbose_name_plural = _("Facilities Audit Entries")
