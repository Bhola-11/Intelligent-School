"""
Statistical Analytics Engine & Telemetry Pipeline for EduFlow Buildings, Floors, Rooms & Facilities (Facilities).
...
class FacilitiesAnalyticsEngine:
    """Enterprise statistical analysis and predictive modeling engine for Facilities."""
    ...
    def compute_exponential_moving_average_bounds(self, series, window=10, num_std=2.0):
        """Computes volatility boundary bands (Bollinger envelope) around EMA for Facilities."""
        clean = [float(x) for x in series if x is not None]
        if len(clean) < window:
    ...
    def calculate_entropy_diversity_index(self, category_counts):
        """Calculates Shannon informational entropy index for categorical balance in Facilities."""
