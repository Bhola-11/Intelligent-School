"""
Models for EduFlow Subjects, Syllabi & Credit Hours (Subjects) in apps.institution.
...
# -----------------------------------------------------------------------------
class SubjectsStatusChoices(models.TextChoices):
...
class SubjectsPriorityChoices(models.TextChoices):
...
class SubjectsTierChoices(models.TextChoices):
...
class SubjectsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class SubjectsMasterQuerySet(models.QuerySet):
    def active(self):
...
class SubjectsMasterManager(models.Manager):
    def get_queryset(self):
        return SubjectsMasterQuerySet(self.model, using=self._db)
...
class SubjectsMaster(models.Model):
    """
    Primary master configuration entity for Subjects, Syllabi & Credit Hours.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = SubjectsStatusChoices
    PriorityChoices = SubjectsPriorityChoices
    TierChoices = SubjectsTierChoices
    ScopeChoices = SubjectsScopeChoices
    ...
    objects = SubjectsMasterManager()

    class Meta:
        db_table = 'eduflow_institution_subjects_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Subjects Master")
        verbose_name_plural = _("Subjects Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'institution:subjects_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/institution/subjects/{self.pk}/"
...
# -----------------------------------------------------------------------------
class SubjectsItem(models.Model):
    """Subordinate component or line transaction item for Subjects."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Subjects Item")
        verbose_name_plural = _("Subjects Items")
...
# -----------------------------------------------------------------------------
class SubjectsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Subjects."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_allocation'
        ordering = ['-start_time']
        verbose_name = _("Subjects Allocation")
        verbose_name_plural = _("Subjects Allocations")
...
# -----------------------------------------------------------------------------
class SubjectsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Subjects."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Subjects Metric Record")
        verbose_name_plural = _("Subjects Metric Records")
...
# -----------------------------------------------------------------------------
class SubjectsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Subjects."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_rule'
        ordering = ['rule_code']
        verbose_name = _("Subjects Policy Rule")
        verbose_name_plural = _("Subjects Policy Rules")
...
# -----------------------------------------------------------------------------
class SubjectsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Subjects."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Subjects Schedule Period")
        verbose_name_plural = _("Subjects Schedule Periods")
...
# -----------------------------------------------------------------------------
class SubjectsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Subjects."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_review'
        ordering = ['-submitted_at']
        verbose_name = _("Subjects Review")
        verbose_name_plural = _("Subjects Reviews")
...
# -----------------------------------------------------------------------------
class SubjectsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Subjects Workflow Transition")
        verbose_name_plural = _("Subjects Workflow Transitions")
...
# -----------------------------------------------------------------------------
class SubjectsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_access'
        verbose_name = _("Subjects Access Rule")
        verbose_name_plural = _("Subjects Access Rules")
...
# -----------------------------------------------------------------------------
class SubjectsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_config'
        verbose_name = _("Subjects Config Parameter")
        verbose_name_plural = _("Subjects Config Parameters")
...
# -----------------------------------------------------------------------------
class SubjectsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Subjects Document Attachment")
        verbose_name_plural = _("Subjects Document Attachments")
...
# -----------------------------------------------------------------------------
class SubjectsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Subjects."""
    master = models.ForeignKey(SubjectsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_institution_subjects_audit'
        ordering = ['-timestamp']
        verbose_name = _("Subjects Audit Entry")
        verbose_name_plural = _("Subjects Audit Entries")
