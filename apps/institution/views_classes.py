"""
Views for EduFlow Class Levels, Sections & Batches (Classes).
...
try:
    ...
    from .forms import (
        ClassesMasterForm, ClassesItemForm, ClassesAllocationForm,
        ClassesMetricRecordForm, ClassesPolicyRuleForm, ClassesSchedulePeriodForm,
        ClassesFeedbackReviewForm, ClassesWorkflowTransitionForm, ClassesAccessRuleForm,
        ClassesConfigurationParameterForm, ClassesDocumentAttachmentForm, ClassesFilterForm,
        ClassesBulkActionForm
    )
except ImportError:
    try:
...
class ClassesListView(LoginRequiredMixin, ListView):
    template_name = 'institution/classes_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = ClassesMaster.objects.all() if 'ClassesMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = ClassesFilterForm(self.request.GET) if 'ClassesFilterForm' in globals() else None
        ctx['bulk_form'] = ClassesBulkActionForm() if 'ClassesBulkActionForm' in globals() else None
        ctx['total_count'] = ClassesMaster.objects.count() if 'ClassesMaster' in globals() else 0
        ctx['active_count'] = ClassesMaster.objects.filter(status='ACTIVE').count() if 'ClassesMaster' in globals() else 0
        ctx['pending_count'] = ClassesMaster.objects.filter(status='PENDING_REVIEW').count() if 'ClassesMaster' in globals() else 0
        ctx['page_title'] = "Class Levels, Sections & Batches Directory"
        ctx['domain_name'] = "Classes"
        return ctx

class ClassesDetailView(LoginRequiredMixin, DetailView):
    template_name = 'institution/classes_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(ClassesMaster, pk=self.kwargs.get('pk'))
...
class ClassesCreateView(LoginRequiredMixin, CreateView):
    template_name = 'institution/classes_form.html'
    form_class = ClassesMasterForm
    success_url = reverse_lazy('institution:classes_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Classes record '{form.instance.name}' created successfully.")
...
class ClassesUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Classes record updated successfully.")
        return super().form_valid(form)

class ClassesDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'institution/classes_confirm_delete.html'
    success_url = reverse_lazy('institution:classes_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class ClassesItemCreateView(LoginRequiredMixin, CreateView):
    form_class = ClassesItemForm
    template_name = 'institution/classes_form.html'

    def form_valid(self, form):
        master = get_object_or_404(ClassesMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('institution:classes_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class ClassesItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = ClassesItemForm
    template_name = 'institution/classes_form.html'

    def get_object(self):
        return get_object_or_404(ClassesItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('institution:classes_detail', kwargs={'pk': self.object.master_id})

class ClassesItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'institution/classes_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class ClassesAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = ClassesAllocationForm
    template_name = 'institution/classes_form.html'
...
class ClassesAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(ClassesAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('institution:classes_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class ClassesMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = ClassesMetricRecordForm
    template_name = 'institution/classes_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class ClassesPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = ClassesPolicyRuleForm
    template_name = 'institution/classes_form.html'
...
# -----------------------------------------------------------------------------
class ClassesSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = ClassesSchedulePeriodForm
    template_name = 'institution/classes_form.html'
...
# -----------------------------------------------------------------------------
class ClassesFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = ClassesFeedbackReviewForm
    template_name = 'institution/classes_form.html'
...
# -----------------------------------------------------------------------------
class ClassesWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = ClassesWorkflowTransitionForm
    template_name = 'institution/classes_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class ClassesAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = ClassesAccessRuleForm
    template_name = 'institution/classes_form.html'
...
# -----------------------------------------------------------------------------
class ClassesConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = ClassesConfigurationParameterForm
    template_name = 'institution/classes_form.html'
...
# -----------------------------------------------------------------------------
class ClassesDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = ClassesDocumentAttachmentForm
    template_name = 'institution/classes_form.html'
...
# -----------------------------------------------------------------------------
class ClassesBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = ClassesMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('institution:classes_list')

class ClassesExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="classes_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        writer = csv.writer(response)
        writer.writerow(['Code', 'Name', 'Category', 'Tier', 'Priority', 'Status', 'Capacity', 'Occupancy', 'Budget ($)', 'Incurred ($)', 'Start Date', 'End Date'])
        records = ClassesMaster.objects.all() if 'ClassesMaster' in globals() else []
        for r in records:
...
class ClassesExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = ClassesMaster.objects.all() if 'ClassesMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class ClassesAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = ClassesMaster.objects.all() if 'ClassesMaster' in globals() else []
...
class ClassesAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(ClassesMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
