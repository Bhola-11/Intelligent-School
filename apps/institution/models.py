"""
Models for EduFlow Institution Profile & Multi-Campus Architecture (Institution) in apps.institution.
...
# -----------------------------------------------------------------------------
class InstitutionStatusChoices(models.TextChoices):
...
class InstitutionPriorityChoices(models.TextChoices):
...
class InstitutionTierChoices(models.TextChoices):
...
class InstitutionScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class InstitutionMasterQuerySet(models.QuerySet):
    def active(self):
...
class InstitutionMasterManager(models.Manager):
    def get_queryset(self):
        return InstitutionMasterQuerySet(self.model, using=self._db)
...
class InstitutionMaster(models.Model):
    """
    Primary master configuration entity for Institution Profile & Multi-Campus Architecture.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = InstitutionStatusChoices
    PriorityChoices = InstitutionPriorityChoices
    TierChoices = InstitutionTierChoices
    ScopeChoices = InstitutionScopeChoices
    ...
    objects = InstitutionMasterManager()

    class Meta:
        db_table = 'eduflow_institution_institution_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Institution Master")
        verbose_name_plural = _("Institution Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'institution:institution_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/institution/institution/{self.pk}/"
...
class InstitutionItem(models.Model):
    """Subordinate component or line transaction item for Institution."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Institution Item")
        verbose_name_plural = _("Institution Items")
...
# -----------------------------------------------------------------------------
class InstitutionAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Institution."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_allocation'
        ordering = ['-start_time']
        verbose_name = _("Institution Allocation")
        verbose_name_plural = _("Institution Allocations")
...
# -----------------------------------------------------------------------------
class InstitutionMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Institution."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Institution Metric Record")
        verbose_name_plural = _("Institution Metric Records")
...
# -----------------------------------------------------------------------------
class InstitutionPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Institution."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_rule'
        ordering = ['rule_code']
        verbose_name = _("Institution Policy Rule")
        verbose_name_plural = _("Institution Policy Rules")
...
# -----------------------------------------------------------------------------
class InstitutionSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Institution."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Institution Schedule Period")
        verbose_name_plural = _("Institution Schedule Periods")
...
# -----------------------------------------------------------------------------
class InstitutionFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Institution."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_review'
        ordering = ['-submitted_at']
        verbose_name = _("Institution Review")
        verbose_name_plural = _("Institution Reviews")
...
# -----------------------------------------------------------------------------
class InstitutionWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Institution Workflow Transition")
        verbose_name_plural = _("Institution Workflow Transitions")
...
# -----------------------------------------------------------------------------
class InstitutionAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_access'
        verbose_name = _("Institution Access Rule")
        verbose_name_plural = _("Institution Access Rules")
...
# -----------------------------------------------------------------------------
class InstitutionConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_config'
        verbose_name = _("Institution Config Parameter")
        verbose_name_plural = _("Institution Config Parameters")
...
# -----------------------------------------------------------------------------
class InstitutionDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Institution Document Attachment")
        verbose_name_plural = _("Institution Document Attachments")
...
# -----------------------------------------------------------------------------
class InstitutionAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Institution."""
    master = models.ForeignKey(InstitutionMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_institution_institution_audit'
        ordering = ['-timestamp']
        verbose_name = _("Institution Audit Entry")
        verbose_name_plural = _("Institution Audit Entries")
