"""
Models for EduFlow Report Cards Generation & Teacher Remarks (ReportCardGen) in apps.report_cards.
...
# -----------------------------------------------------------------------------
class ReportCardGenStatusChoices(models.TextChoices):
...
class ReportCardGenPriorityChoices(models.TextChoices):
...
class ReportCardGenTierChoices(models.TextChoices):
...
class ReportCardGenScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ReportCardGenMasterQuerySet(models.QuerySet):
    def active(self):
...
class ReportCardGenMasterManager(models.Manager):
    def get_queryset(self):
        return ReportCardGenMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ReportCardGenMaster(models.Model):
    """
    Primary master configuration entity for Report Cards Generation & Teacher Remarks.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ReportCardGenStatusChoices
    PriorityChoices = ReportCardGenPriorityChoices
    TierChoices = ReportCardGenTierChoices
    ScopeChoices = ReportCardGenScopeChoices
    ...
    objects = ReportCardGenMasterManager()

    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("ReportCardGen Master")
        verbose_name_plural = _("ReportCardGen Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_repor_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_repor_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_repor_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_repor_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'report_cards:reportcardgen_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/report_cards/reportcardgen/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ReportCardGenItem(models.Model):
    """Subordinate component or line transaction item for ReportCardGen."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("ReportCardGen Item")
        verbose_name_plural = _("ReportCardGen Items")
...
# -----------------------------------------------------------------------------
class ReportCardGenAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for ReportCardGen."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_allocation'
        ordering = ['-start_time']
        verbose_name = _("ReportCardGen Allocation")
        verbose_name_plural = _("ReportCardGen Allocations")
...
# -----------------------------------------------------------------------------
class ReportCardGenMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for ReportCardGen."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("ReportCardGen Metric Record")
        verbose_name_plural = _("ReportCardGen Metric Records")
...
# -----------------------------------------------------------------------------
class ReportCardGenPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for ReportCardGen."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_rule'
        ordering = ['rule_code']
        verbose_name = _("ReportCardGen Policy Rule")
        verbose_name_plural = _("ReportCardGen Policy Rules")
...
# -----------------------------------------------------------------------------
class ReportCardGenSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for ReportCardGen."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("ReportCardGen Schedule Period")
        verbose_name_plural = _("ReportCardGen Schedule Periods")
...
# -----------------------------------------------------------------------------
class ReportCardGenFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for ReportCardGen."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_review'
        ordering = ['-submitted_at']
        verbose_name = _("ReportCardGen Review")
        verbose_name_plural = _("ReportCardGen Reviews")
...
# -----------------------------------------------------------------------------
class ReportCardGenWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("ReportCardGen Workflow Transition")
        verbose_name_plural = _("ReportCardGen Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ReportCardGenAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_access'
        verbose_name = _("ReportCardGen Access Rule")
        verbose_name_plural = _("ReportCardGen Access Rules")
...
# -----------------------------------------------------------------------------
class ReportCardGenConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_config'
        verbose_name = _("ReportCardGen Config Parameter")
        verbose_name_plural = _("ReportCardGen Config Parameters")
...
# -----------------------------------------------------------------------------
class ReportCardGenDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("ReportCardGen Document Attachment")
        verbose_name_plural = _("ReportCardGen Document Attachments")
...
# -----------------------------------------------------------------------------
class ReportCardGenAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on ReportCardGen."""
    master = models.ForeignKey(ReportCardGenMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_reportcardgen_audit'
        ordering = ['-timestamp']
        verbose_name = _("ReportCardGen Audit Entry")
        verbose_name_plural = _("ReportCardGen Audit Entries")
