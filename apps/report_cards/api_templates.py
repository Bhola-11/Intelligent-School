"""
Extended REST API Controllers for EduFlow Report Card Custom Templates & Layouts (Templates).
...
class TemplatesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(TemplatesMaster, pk=pk)
        data = master.to_dict()
...
class TemplatesAPISearchView(LoginRequiredMixin, View):
...
class TemplatesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class TemplatesAPITelemetryView(LoginRequiredMixin, View):
