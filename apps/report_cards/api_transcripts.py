"""
Extended REST API Controllers for EduFlow Multi-Year Transcripts & Academic Records (Transcripts).
...
class TranscriptsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(TranscriptsMaster, pk=pk)
        data = master.to_dict()
...
class TranscriptsAPISearchView(LoginRequiredMixin, View):
...
class TranscriptsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class TranscriptsAPITelemetryView(LoginRequiredMixin, View):
