"""
Models for EduFlow Multi-Year Transcripts & Academic Records (Transcripts) in apps.report_cards.
...
# -----------------------------------------------------------------------------
class TranscriptsStatusChoices(models.TextChoices):
...
class TranscriptsPriorityChoices(models.TextChoices):
...
class TranscriptsTierChoices(models.TextChoices):
...
class TranscriptsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class TranscriptsMasterQuerySet(models.QuerySet):
    def active(self):
...
class TranscriptsMasterManager(models.Manager):
    def get_queryset(self):
        return TranscriptsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class TranscriptsMaster(models.Model):
    """
    Primary master configuration entity for Multi-Year Transcripts & Academic Records.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = TranscriptsStatusChoices
    PriorityChoices = TranscriptsPriorityChoices
    TierChoices = TranscriptsTierChoices
    ScopeChoices = TranscriptsScopeChoices
    ...
    objects = TranscriptsMasterManager()

    class Meta:
        db_table = 'eduflow_report_cards_transcripts_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Transcripts Master")
        verbose_name_plural = _("Transcripts Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'report_cards:transcripts_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/report_cards/transcripts/{self.pk}/"
...
# -----------------------------------------------------------------------------
class TranscriptsItem(models.Model):
    """Subordinate component or line transaction item for Transcripts."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Transcripts Item")
        verbose_name_plural = _("Transcripts Items")
...
# -----------------------------------------------------------------------------
class TranscriptsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Transcripts."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_allocation'
        ordering = ['-start_time']
        verbose_name = _("Transcripts Allocation")
        verbose_name_plural = _("Transcripts Allocations")
...
# -----------------------------------------------------------------------------
class TranscriptsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Transcripts."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Transcripts Metric Record")
        verbose_name_plural = _("Transcripts Metric Records")
...
# -----------------------------------------------------------------------------
class TranscriptsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Transcripts."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_rule'
        ordering = ['rule_code']
        verbose_name = _("Transcripts Policy Rule")
        verbose_name_plural = _("Transcripts Policy Rules")
...
# -----------------------------------------------------------------------------
class TranscriptsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Transcripts."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Transcripts Schedule Period")
        verbose_name_plural = _("Transcripts Schedule Periods")
...
# -----------------------------------------------------------------------------
class TranscriptsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Transcripts."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_review'
        ordering = ['-submitted_at']
        verbose_name = _("Transcripts Review")
        verbose_name_plural = _("Transcripts Reviews")
...
# -----------------------------------------------------------------------------
class TranscriptsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Transcripts Workflow Transition")
        verbose_name_plural = _("Transcripts Workflow Transitions")
...
# -----------------------------------------------------------------------------
class TranscriptsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_access'
        verbose_name = _("Transcripts Access Rule")
        verbose_name_plural = _("Transcripts Access Rules")
...
# -----------------------------------------------------------------------------
class TranscriptsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_config'
        verbose_name = _("Transcripts Config Parameter")
        verbose_name_plural = _("Transcripts Config Parameters")
...
# -----------------------------------------------------------------------------
class TranscriptsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Transcripts Document Attachment")
        verbose_name_plural = _("Transcripts Document Attachments")
...
# -----------------------------------------------------------------------------
class TranscriptsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Transcripts."""
    master = models.ForeignKey(TranscriptsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_transcripts_audit'
        ordering = ['-timestamp']
        verbose_name = _("Transcripts Audit Entry")
        verbose_name_plural = _("Transcripts Audit Entries")
