"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Multi-Year Transcripts & Academic Records (Transcripts).
...
class TranscriptsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Transcripts."""
    template_name = 'report_cards/transcripts_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = TranscriptsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'TranscriptsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Multi-Year Transcripts & Academic Records Operations Dashboard",
            'domain_name': 'Transcripts',
            'app_title': 'Report Cards',
...
class TranscriptsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'report_cards/transcripts_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Multi-Year Transcripts & Academic Records Analytical Insights",
