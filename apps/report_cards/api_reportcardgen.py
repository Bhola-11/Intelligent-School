"""
Extended REST API Controllers for EduFlow Report Cards Generation & Teacher Remarks (ReportCardGen).
...
class ReportCardGenAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ReportCardGenMaster, pk=pk)
        data = master.to_dict()
...
class ReportCardGenAPISearchView(LoginRequiredMixin, View):
...
class ReportCardGenAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ReportCardGenAPITelemetryView(LoginRequiredMixin, View):
