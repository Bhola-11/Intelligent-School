"""
Models for EduFlow Report Card Custom Templates & Layouts (Templates) in apps.report_cards.
...
# -----------------------------------------------------------------------------
class TemplatesStatusChoices(models.TextChoices):
...
class TemplatesPriorityChoices(models.TextChoices):
...
class TemplatesTierChoices(models.TextChoices):
...
class TemplatesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class TemplatesMasterQuerySet(models.QuerySet):
    def active(self):
...
class TemplatesMasterManager(models.Manager):
    def get_queryset(self):
        return TemplatesMasterQuerySet(self.model, using=self._db)
...
class TemplatesMaster(models.Model):
    """
    Primary master configuration entity for Report Card Custom Templates & Layouts.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = TemplatesStatusChoices
    PriorityChoices = TemplatesPriorityChoices
    TierChoices = TemplatesTierChoices
    ScopeChoices = TemplatesScopeChoices
    ...
    objects = TemplatesMasterManager()

    class Meta:
        db_table = 'eduflow_report_cards_templates_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Templates Master")
        verbose_name_plural = _("Templates Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_templ_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_templ_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_templ_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_templ_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'report_cards:templates_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/report_cards/templates/{self.pk}/"
...
# -----------------------------------------------------------------------------
class TemplatesItem(models.Model):
    """Subordinate component or line transaction item for Templates."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Templates Item")
        verbose_name_plural = _("Templates Items")
...
# -----------------------------------------------------------------------------
class TemplatesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Templates."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_allocation'
        ordering = ['-start_time']
        verbose_name = _("Templates Allocation")
        verbose_name_plural = _("Templates Allocations")
...
# -----------------------------------------------------------------------------
class TemplatesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Templates."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Templates Metric Record")
        verbose_name_plural = _("Templates Metric Records")
...
# -----------------------------------------------------------------------------
class TemplatesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Templates."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_rule'
        ordering = ['rule_code']
        verbose_name = _("Templates Policy Rule")
        verbose_name_plural = _("Templates Policy Rules")
...
# -----------------------------------------------------------------------------
class TemplatesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Templates."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Templates Schedule Period")
        verbose_name_plural = _("Templates Schedule Periods")
...
# -----------------------------------------------------------------------------
class TemplatesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Templates."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_review'
        ordering = ['-submitted_at']
        verbose_name = _("Templates Review")
        verbose_name_plural = _("Templates Reviews")
...
# -----------------------------------------------------------------------------
class TemplatesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Templates Workflow Transition")
        verbose_name_plural = _("Templates Workflow Transitions")
...
# -----------------------------------------------------------------------------
class TemplatesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_access'
        verbose_name = _("Templates Access Rule")
        verbose_name_plural = _("Templates Access Rules")
...
# -----------------------------------------------------------------------------
class TemplatesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_config'
        verbose_name = _("Templates Config Parameter")
        verbose_name_plural = _("Templates Config Parameters")
...
# -----------------------------------------------------------------------------
class TemplatesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Templates Document Attachment")
        verbose_name_plural = _("Templates Document Attachments")
...
# -----------------------------------------------------------------------------
class TemplatesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Templates."""
    master = models.ForeignKey(TemplatesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_report_cards_templates_audit'
        ordering = ['-timestamp']
        verbose_name = _("Templates Audit Entry")
        verbose_name_plural = _("Templates Audit Entries")
