"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Report Cards Generation & Teacher Remarks (ReportCardGen).
...
class ReportCardGenDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for ReportCardGen."""
    template_name = 'report_cards/reportcardgen_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = ReportCardGenAuditTrail.objects.all().order_by('-timestamp')[:10] if 'ReportCardGenAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Report Cards Generation & Teacher Remarks Operations Dashboard",
            'domain_name': 'ReportCardGen',
...
class ReportCardGenAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'report_cards/reportcardgen_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Report Cards Generation & Teacher Remarks Analytical Insights",
