"""
Models for EduFlow Technician Allocations & Maintenance Costs (WorkOrders) in apps.maintenance.
...
# -----------------------------------------------------------------------------
class WorkOrdersStatusChoices(models.TextChoices):
...
class WorkOrdersPriorityChoices(models.TextChoices):
...
class WorkOrdersTierChoices(models.TextChoices):
...
class WorkOrdersScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class WorkOrdersMasterQuerySet(models.QuerySet):
    def active(self):
...
class WorkOrdersMasterManager(models.Manager):
    def get_queryset(self):
        return WorkOrdersMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class WorkOrdersMaster(models.Model):
    """
    Primary master configuration entity for Technician Allocations & Maintenance Costs.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = WorkOrdersStatusChoices
    PriorityChoices = WorkOrdersPriorityChoices
    TierChoices = WorkOrdersTierChoices
    ScopeChoices = WorkOrdersScopeChoices
    ...
    objects = WorkOrdersMasterManager()

    class Meta:
        db_table = 'eduflow_maintenance_workorders_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("WorkOrders Master")
        verbose_name_plural = _("WorkOrders Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_worko_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_worko_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_worko_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_worko_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'maintenance:workorders_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/maintenance/workorders/{self.pk}/"
...
# -----------------------------------------------------------------------------
class WorkOrdersItem(models.Model):
    """Subordinate component or line transaction item for WorkOrders."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("WorkOrders Item")
        verbose_name_plural = _("WorkOrders Items")
...
# -----------------------------------------------------------------------------
class WorkOrdersAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for WorkOrders."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_allocation'
        ordering = ['-start_time']
        verbose_name = _("WorkOrders Allocation")
        verbose_name_plural = _("WorkOrders Allocations")
...
# -----------------------------------------------------------------------------
class WorkOrdersMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for WorkOrders."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("WorkOrders Metric Record")
        verbose_name_plural = _("WorkOrders Metric Records")
...
# -----------------------------------------------------------------------------
class WorkOrdersPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for WorkOrders."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_rule'
        ordering = ['rule_code']
        verbose_name = _("WorkOrders Policy Rule")
        verbose_name_plural = _("WorkOrders Policy Rules")
...
# -----------------------------------------------------------------------------
class WorkOrdersSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for WorkOrders."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("WorkOrders Schedule Period")
        verbose_name_plural = _("WorkOrders Schedule Periods")
...
# -----------------------------------------------------------------------------
class WorkOrdersFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for WorkOrders."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_review'
        ordering = ['-submitted_at']
        verbose_name = _("WorkOrders Review")
        verbose_name_plural = _("WorkOrders Reviews")
...
# -----------------------------------------------------------------------------
class WorkOrdersWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("WorkOrders Workflow Transition")
        verbose_name_plural = _("WorkOrders Workflow Transitions")
...
# -----------------------------------------------------------------------------
class WorkOrdersAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_access'
        verbose_name = _("WorkOrders Access Rule")
        verbose_name_plural = _("WorkOrders Access Rules")
...
# -----------------------------------------------------------------------------
class WorkOrdersConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_config'
        verbose_name = _("WorkOrders Config Parameter")
        verbose_name_plural = _("WorkOrders Config Parameters")
...
# -----------------------------------------------------------------------------
class WorkOrdersDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("WorkOrders Document Attachment")
        verbose_name_plural = _("WorkOrders Document Attachments")
...
# -----------------------------------------------------------------------------
class WorkOrdersAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on WorkOrders."""
    master = models.ForeignKey(WorkOrdersMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_workorders_audit'
        ordering = ['-timestamp']
        verbose_name = _("WorkOrders Audit Entry")
        verbose_name_plural = _("WorkOrders Audit Entries")
