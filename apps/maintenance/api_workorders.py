"""
Extended REST API Controllers for EduFlow Technician Allocations & Maintenance Costs (WorkOrders).
...
class WorkOrdersAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(WorkOrdersMaster, pk=pk)
        data = master.to_dict()
...
class WorkOrdersAPISearchView(LoginRequiredMixin, View):
...
class WorkOrdersAPIBatchUpdateView(LoginRequiredMixin, View):
...
class WorkOrdersAPITelemetryView(LoginRequiredMixin, View):
