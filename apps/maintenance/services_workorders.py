"""
Domain Services for EduFlow Technician Allocations & Maintenance Costs (WorkOrders).
...
class WorkOrdersService:
    """Enterprise domain business service for WorkOrders."""
    ...
    def create_record(cls, code, name, user_username="system", **kwargs):
        ...
        logger.info(f"Created WorkOrders record {record.code} by {user_username}")
        return record
    ...
    def update_record(cls, record_id, user_username="system", **kwargs):
        ...
        record = WorkOrdersMaster.objects.select_for_update().get(pk=record_id)
    ...
    def calculate_utilization_metrics(cls, record_id):
        ...
        record = WorkOrdersMaster.objects.get(pk=record_id)
    ...
    def allocate_resource(cls, record_id, assignee_id, assignee_name, role="Lead", start_time=None, end_time=None):
        ...
        allocation = WorkOrdersAllocation.objects.create(
            master=record,
    ...
    def release_allocation(cls, allocation_id):
        ...
        alloc = WorkOrdersAllocation.objects.get(pk=allocation_id)
    ...
    def set_configuration_parameter(cls, record_id, key, val, data_type="STRING", desc=""):
        ...
        record = WorkOrdersMaster.objects.get(pk=record_id)
        param, _ = WorkOrdersConfigurationParameter.objects.update_or_create(
            master=record,
            param_key=key,
    ...
    def _log_action(cls, record, action_type, user_username, old_state, new_state, summary):
        if 'WorkOrdersAuditTrail' in globals():
            try:
                WorkOrdersAuditTrail.objects.create(
                    master=record,
                ...
                logger.warning(f"Failed to record audit log for WorkOrders: {e}")
