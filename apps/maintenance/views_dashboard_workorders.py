"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Technician Allocations & Maintenance Costs (WorkOrders).
...
class WorkOrdersDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for WorkOrders."""
    template_name = 'maintenance/workorders_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = WorkOrdersAuditTrail.objects.all().order_by('-timestamp')[:10] if 'WorkOrdersAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Technician Allocations & Maintenance Costs Operations Dashboard",
            'domain_name': 'WorkOrders',
...
class WorkOrdersAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'maintenance/workorders_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Technician Allocations & Maintenance Costs Analytical Insights",
