"""
Extended REST API Controllers for EduFlow Maintenance Tickets & Work Orders (MaintenanceTickets).
...
class MaintenanceTicketsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(MaintenanceTicketsMaster, pk=pk)
        data = master.to_dict()
...
class MaintenanceTicketsAPISearchView(LoginRequiredMixin, View):
...
class MaintenanceTicketsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class MaintenanceTicketsAPITelemetryView(LoginRequiredMixin, View):
