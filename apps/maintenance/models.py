"""
Models for EduFlow Maintenance Tickets & Work Orders (MaintenanceTickets) in apps.maintenance.
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsStatusChoices(models.TextChoices):
...
class MaintenanceTicketsPriorityChoices(models.TextChoices):
...
class MaintenanceTicketsTierChoices(models.TextChoices):
...
class MaintenanceTicketsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsMasterQuerySet(models.QuerySet):
    def active(self):
...
class MaintenanceTicketsMasterManager(models.Manager):
    def get_queryset(self):
        return MaintenanceTicketsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsMaster(models.Model):
    """
    Primary master configuration entity for Maintenance Tickets & Work Orders.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = MaintenanceTicketsStatusChoices
    PriorityChoices = MaintenanceTicketsPriorityChoices
    TierChoices = MaintenanceTicketsTierChoices
    ScopeChoices = MaintenanceTicketsScopeChoices
    ...
    objects = MaintenanceTicketsMasterManager()

    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("MaintenanceTickets Master")
        verbose_name_plural = _("MaintenanceTickets Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_maint_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_maint_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_maint_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_maint_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'maintenance:maintenancetickets_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/maintenance/maintenancetickets/{self.pk}/"
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsItem(models.Model):
    """Subordinate component or line transaction item for MaintenanceTickets."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("MaintenanceTickets Item")
        verbose_name_plural = _("MaintenanceTickets Items")
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for MaintenanceTickets."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_allocation'
        ordering = ['-start_time']
        verbose_name = _("MaintenanceTickets Allocation")
        verbose_name_plural = _("MaintenanceTickets Allocations")
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for MaintenanceTickets."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("MaintenanceTickets Metric Record")
        verbose_name_plural = _("MaintenanceTickets Metric Records")
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for MaintenanceTickets."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_rule'
        ordering = ['rule_code']
        verbose_name = _("MaintenanceTickets Policy Rule")
        verbose_name_plural = _("MaintenanceTickets Policy Rules")
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for MaintenanceTickets."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("MaintenanceTickets Schedule Period")
        verbose_name_plural = _("MaintenanceTickets Schedule Periods")
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for MaintenanceTickets."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_review'
        ordering = ['-submitted_at']
        verbose_name = _("MaintenanceTickets Review")
        verbose_name_plural = _("MaintenanceTickets Reviews")
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("MaintenanceTickets Workflow Transition")
        verbose_name_plural = _("MaintenanceTickets Workflow Transitions")
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_access'
        verbose_name = _("MaintenanceTickets Access Rule")
        verbose_name_plural = _("MaintenanceTickets Access Rules")
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_config'
        verbose_name = _("MaintenanceTickets Config Parameter")
        verbose_name_plural = _("MaintenanceTickets Config Parameters")
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("MaintenanceTickets Document Attachment")
        verbose_name_plural = _("MaintenanceTickets Document Attachments")
...
# -----------------------------------------------------------------------------
class MaintenanceTicketsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on MaintenanceTickets."""
    master = models.ForeignKey(MaintenanceTicketsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_maintenance_maintenancetickets_audit'
        ordering = ['-timestamp']
        verbose_name = _("MaintenanceTickets Audit Entry")
        verbose_name_plural = _("MaintenanceTickets Audit Entries")
