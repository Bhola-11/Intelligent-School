"""
Models for EduFlow Event Participants & Competitions Tracker (Competitions) in apps.events.
...
# -----------------------------------------------------------------------------
class CompetitionsStatusChoices(models.TextChoices):
...
class CompetitionsPriorityChoices(models.TextChoices):
...
class CompetitionsTierChoices(models.TextChoices):
...
class CompetitionsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class CompetitionsMasterQuerySet(models.QuerySet):
    def active(self):
...
class CompetitionsMasterManager(models.Manager):
    def get_queryset(self):
        return CompetitionsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class CompetitionsMaster(models.Model):
    """
    Primary master configuration entity for Event Participants & Competitions Tracker.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = CompetitionsStatusChoices
    PriorityChoices = CompetitionsPriorityChoices
    TierChoices = CompetitionsTierChoices
    ScopeChoices = CompetitionsScopeChoices
    ...
    objects = CompetitionsMasterManager()

    class Meta:
        db_table = 'eduflow_events_competitions_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Competitions Master")
        verbose_name_plural = _("Competitions Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_compe_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_compe_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_compe_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_compe_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'events:competitions_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/events/competitions/{self.pk}/"
...
# -----------------------------------------------------------------------------
class CompetitionsItem(models.Model):
    """Subordinate component or line transaction item for Competitions."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Competitions Item")
        verbose_name_plural = _("Competitions Items")
...
# -----------------------------------------------------------------------------
class CompetitionsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Competitions."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_allocation'
        ordering = ['-start_time']
        verbose_name = _("Competitions Allocation")
        verbose_name_plural = _("Competitions Allocations")
...
# -----------------------------------------------------------------------------
class CompetitionsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Competitions."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Competitions Metric Record")
        verbose_name_plural = _("Competitions Metric Records")
...
# -----------------------------------------------------------------------------
class CompetitionsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Competitions."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_rule'
        ordering = ['rule_code']
        verbose_name = _("Competitions Policy Rule")
        verbose_name_plural = _("Competitions Policy Rules")
...
# -----------------------------------------------------------------------------
class CompetitionsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Competitions."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Competitions Schedule Period")
        verbose_name_plural = _("Competitions Schedule Periods")
...
# -----------------------------------------------------------------------------
class CompetitionsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Competitions."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_review'
        ordering = ['-submitted_at']
        verbose_name = _("Competitions Review")
        verbose_name_plural = _("Competitions Reviews")
...
# -----------------------------------------------------------------------------
class CompetitionsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Competitions Workflow Transition")
        verbose_name_plural = _("Competitions Workflow Transitions")
...
# -----------------------------------------------------------------------------
class CompetitionsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_access'
        verbose_name = _("Competitions Access Rule")
        verbose_name_plural = _("Competitions Access Rules")
...
# -----------------------------------------------------------------------------
class CompetitionsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_config'
        verbose_name = _("Competitions Config Parameter")
        verbose_name_plural = _("Competitions Config Parameters")
...
# -----------------------------------------------------------------------------
class CompetitionsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Competitions Document Attachment")
        verbose_name_plural = _("Competitions Document Attachments")
...
# -----------------------------------------------------------------------------
class CompetitionsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Competitions."""
    master = models.ForeignKey(CompetitionsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_events_competitions_audit'
        ordering = ['-timestamp']
        verbose_name = _("Competitions Audit Entry")
        verbose_name_plural = _("Competitions Audit Entries")
