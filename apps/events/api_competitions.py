"""
Extended REST API Controllers for EduFlow Event Participants & Competitions Tracker (Competitions).
...
class CompetitionsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(CompetitionsMaster, pk=pk)
        data = master.to_dict()
...
class CompetitionsAPISearchView(LoginRequiredMixin, View):
...
class CompetitionsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class CompetitionsAPITelemetryView(LoginRequiredMixin, View):
