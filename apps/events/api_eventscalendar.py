"""
Extended REST API Controllers for EduFlow Institutional Events & Activity Calendar (EventsCalendar).
...
class EventsCalendarAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(EventsCalendarMaster, pk=pk)
        data = master.to_dict()
...
class EventsCalendarAPISearchView(LoginRequiredMixin, View):
...
class EventsCalendarAPIBatchUpdateView(LoginRequiredMixin, View):
...
class EventsCalendarAPITelemetryView(LoginRequiredMixin, View):
