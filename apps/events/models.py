"""
Models for EduFlow Institutional Events & Activity Calendar (EventsCalendar) in apps.events.
...
# -----------------------------------------------------------------------------
class EventsCalendarStatusChoices(models.TextChoices):
...
class EventsCalendarPriorityChoices(models.TextChoices):
...
class EventsCalendarTierChoices(models.TextChoices):
...
class EventsCalendarScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class EventsCalendarMasterQuerySet(models.QuerySet):
    def active(self):
...
class EventsCalendarMasterManager(models.Manager):
    def get_queryset(self):
        return EventsCalendarMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class EventsCalendarMaster(models.Model):
    """
    Primary master configuration entity for Institutional Events & Activity Calendar.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = EventsCalendarStatusChoices
    PriorityChoices = EventsCalendarPriorityChoices
    TierChoices = EventsCalendarTierChoices
    ScopeChoices = EventsCalendarScopeChoices
    ...
    objects = EventsCalendarMasterManager()

    class Meta:
        db_table = 'eduflow_events_eventscalendar_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("EventsCalendar Master")
        verbose_name_plural = _("EventsCalendar Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_event_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_event_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_event_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_event_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'events:eventscalendar_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/events/eventscalendar/{self.pk}/"
...
# -----------------------------------------------------------------------------
class EventsCalendarItem(models.Model):
    """Subordinate component or line transaction item for EventsCalendar."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("EventsCalendar Item")
        verbose_name_plural = _("EventsCalendar Items")
...
# -----------------------------------------------------------------------------
class EventsCalendarAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for EventsCalendar."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_allocation'
        ordering = ['-start_time']
        verbose_name = _("EventsCalendar Allocation")
        verbose_name_plural = _("EventsCalendar Allocations")
...
# -----------------------------------------------------------------------------
class EventsCalendarMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for EventsCalendar."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("EventsCalendar Metric Record")
        verbose_name_plural = _("EventsCalendar Metric Records")
...
# -----------------------------------------------------------------------------
class EventsCalendarPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for EventsCalendar."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_rule'
        ordering = ['rule_code']
        verbose_name = _("EventsCalendar Policy Rule")
        verbose_name_plural = _("EventsCalendar Policy Rules")
...
# -----------------------------------------------------------------------------
class EventsCalendarSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for EventsCalendar."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("EventsCalendar Schedule Period")
        verbose_name_plural = _("EventsCalendar Schedule Periods")
...
# -----------------------------------------------------------------------------
class EventsCalendarFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for EventsCalendar."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_review'
        ordering = ['-submitted_at']
        verbose_name = _("EventsCalendar Review")
        verbose_name_plural = _("EventsCalendar Reviews")
...
# -----------------------------------------------------------------------------
class EventsCalendarWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("EventsCalendar Workflow Transition")
        verbose_name_plural = _("EventsCalendar Workflow Transitions")
...
# -----------------------------------------------------------------------------
class EventsCalendarAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_access'
        verbose_name = _("EventsCalendar Access Rule")
        verbose_name_plural = _("EventsCalendar Access Rules")
...
# -----------------------------------------------------------------------------
class EventsCalendarConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_config'
        verbose_name = _("EventsCalendar Config Parameter")
        verbose_name_plural = _("EventsCalendar Config Parameters")
...
# -----------------------------------------------------------------------------
class EventsCalendarDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("EventsCalendar Document Attachment")
        verbose_name_plural = _("EventsCalendar Document Attachments")
...
# -----------------------------------------------------------------------------
class EventsCalendarAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on EventsCalendar."""
    master = models.ForeignKey(EventsCalendarMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_events_eventscalendar_audit'
        ordering = ['-timestamp']
        verbose_name = _("EventsCalendar Audit Entry")
        verbose_name_plural = _("EventsCalendar Audit Entries")
