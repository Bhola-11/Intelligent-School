"""
Models for EduFlow Hostel Gate Passes & Resident Discipline (GatePasses) in apps.hostel.
...
# -----------------------------------------------------------------------------
class GatePassesStatusChoices(models.TextChoices):
...
class GatePassesPriorityChoices(models.TextChoices):
...
class GatePassesTierChoices(models.TextChoices):
...
class GatePassesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class GatePassesMasterQuerySet(models.QuerySet):
    def active(self):
...
class GatePassesMasterManager(models.Manager):
    def get_queryset(self):
        return GatePassesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class GatePassesMaster(models.Model):
    """
    Primary master configuration entity for Hostel Gate Passes & Resident Discipline.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = GatePassesStatusChoices
    PriorityChoices = GatePassesPriorityChoices
    TierChoices = GatePassesTierChoices
    ScopeChoices = GatePassesScopeChoices
    ...
    objects = GatePassesMasterManager()

    class Meta:
        db_table = 'eduflow_hostel_gatepasses_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("GatePasses Master")
        verbose_name_plural = _("GatePasses Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_gatep_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_gatep_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_gatep_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_gatep_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'hostel:gatepasses_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/hostel/gatepasses/{self.pk}/"
...
# -----------------------------------------------------------------------------
class GatePassesItem(models.Model):
    """Subordinate component or line transaction item for GatePasses."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("GatePasses Item")
        verbose_name_plural = _("GatePasses Items")
...
# -----------------------------------------------------------------------------
class GatePassesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for GatePasses."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_allocation'
        ordering = ['-start_time']
        verbose_name = _("GatePasses Allocation")
        verbose_name_plural = _("GatePasses Allocations")
...
# -----------------------------------------------------------------------------
class GatePassesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for GatePasses."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("GatePasses Metric Record")
        verbose_name_plural = _("GatePasses Metric Records")
...
# -----------------------------------------------------------------------------
class GatePassesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for GatePasses."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_rule'
        ordering = ['rule_code']
        verbose_name = _("GatePasses Policy Rule")
        verbose_name_plural = _("GatePasses Policy Rules")
...
# -----------------------------------------------------------------------------
class GatePassesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for GatePasses."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("GatePasses Schedule Period")
        verbose_name_plural = _("GatePasses Schedule Periods")
...
# -----------------------------------------------------------------------------
class GatePassesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for GatePasses."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_review'
        ordering = ['-submitted_at']
        verbose_name = _("GatePasses Review")
        verbose_name_plural = _("GatePasses Reviews")
...
# -----------------------------------------------------------------------------
class GatePassesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("GatePasses Workflow Transition")
        verbose_name_plural = _("GatePasses Workflow Transitions")
...
# -----------------------------------------------------------------------------
class GatePassesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_access'
        verbose_name = _("GatePasses Access Rule")
        verbose_name_plural = _("GatePasses Access Rules")
...
# -----------------------------------------------------------------------------
class GatePassesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_config'
        verbose_name = _("GatePasses Config Parameter")
        verbose_name_plural = _("GatePasses Config Parameters")
...
# -----------------------------------------------------------------------------
class GatePassesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("GatePasses Document Attachment")
        verbose_name_plural = _("GatePasses Document Attachments")
...
# -----------------------------------------------------------------------------
class GatePassesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on GatePasses."""
    master = models.ForeignKey(GatePassesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_hostel_gatepasses_audit'
        ordering = ['-timestamp']
        verbose_name = _("GatePasses Audit Entry")
        verbose_name_plural = _("GatePasses Audit Entries")
