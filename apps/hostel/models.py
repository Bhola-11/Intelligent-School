"""
Models for EduFlow Hostel Buildings, Blocks & Rooms (HostelFacilities) in apps.hostel.
...
# -----------------------------------------------------------------------------
class HostelFacilitiesStatusChoices(models.TextChoices):
...
class HostelFacilitiesPriorityChoices(models.TextChoices):
...
class HostelFacilitiesTierChoices(models.TextChoices):
...
class HostelFacilitiesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class HostelFacilitiesMasterQuerySet(models.QuerySet):
    def active(self):
...
class HostelFacilitiesMasterManager(models.Manager):
    def get_queryset(self):
        return HostelFacilitiesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class HostelFacilitiesMaster(models.Model):
    """
    Primary master configuration entity for Hostel Buildings, Blocks & Rooms.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = HostelFacilitiesStatusChoices
    PriorityChoices = HostelFacilitiesPriorityChoices
    TierChoices = HostelFacilitiesTierChoices
    ScopeChoices = HostelFacilitiesScopeChoices
    ...
    objects = HostelFacilitiesMasterManager()

    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("HostelFacilities Master")
        verbose_name_plural = _("HostelFacilities Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_hoste_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_hoste_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_hoste_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_hoste_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'hostel:hostelfacilities_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/hostel/hostelfacilities/{self.pk}/"
...
# -----------------------------------------------------------------------------
class HostelFacilitiesItem(models.Model):
    """Subordinate component or line transaction item for HostelFacilities."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("HostelFacilities Item")
        verbose_name_plural = _("HostelFacilities Items")
...
# -----------------------------------------------------------------------------
class HostelFacilitiesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for HostelFacilities."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_allocation'
        ordering = ['-start_time']
        verbose_name = _("HostelFacilities Allocation")
        verbose_name_plural = _("HostelFacilities Allocations")
...
# -----------------------------------------------------------------------------
class HostelFacilitiesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for HostelFacilities."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("HostelFacilities Metric Record")
        verbose_name_plural = _("HostelFacilities Metric Records")
...
# -----------------------------------------------------------------------------
class HostelFacilitiesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for HostelFacilities."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_rule'
        ordering = ['rule_code']
        verbose_name = _("HostelFacilities Policy Rule")
        verbose_name_plural = _("HostelFacilities Policy Rules")
...
# -----------------------------------------------------------------------------
class HostelFacilitiesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for HostelFacilities."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("HostelFacilities Schedule Period")
        verbose_name_plural = _("HostelFacilities Schedule Periods")
...
# -----------------------------------------------------------------------------
class HostelFacilitiesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for HostelFacilities."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_review'
        ordering = ['-submitted_at']
        verbose_name = _("HostelFacilities Review")
        verbose_name_plural = _("HostelFacilities Reviews")
...
# -----------------------------------------------------------------------------
class HostelFacilitiesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("HostelFacilities Workflow Transition")
        verbose_name_plural = _("HostelFacilities Workflow Transitions")
...
# -----------------------------------------------------------------------------
class HostelFacilitiesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_access'
        verbose_name = _("HostelFacilities Access Rule")
        verbose_name_plural = _("HostelFacilities Access Rules")
...
# -----------------------------------------------------------------------------
class HostelFacilitiesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_config'
        verbose_name = _("HostelFacilities Config Parameter")
        verbose_name_plural = _("HostelFacilities Config Parameters")
...
# -----------------------------------------------------------------------------
class HostelFacilitiesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("HostelFacilities Document Attachment")
        verbose_name_plural = _("HostelFacilities Document Attachments")
...
# -----------------------------------------------------------------------------
class HostelFacilitiesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on HostelFacilities."""
    master = models.ForeignKey(HostelFacilitiesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_hostel_hostelfacilities_audit'
        ordering = ['-timestamp']
        verbose_name = _("HostelFacilities Audit Entry")
        verbose_name_plural = _("HostelFacilities Audit Entries")
