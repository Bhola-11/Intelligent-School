"""
Extended REST API Controllers for EduFlow Hostel Bed Allocation & Check-In (BedAllocations).
...
class BedAllocationsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(BedAllocationsMaster, pk=pk)
        data = master.to_dict()
...
class BedAllocationsAPISearchView(LoginRequiredMixin, View):
...
class BedAllocationsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class BedAllocationsAPITelemetryView(LoginRequiredMixin, View):
