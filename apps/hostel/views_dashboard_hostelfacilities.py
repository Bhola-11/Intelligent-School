"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Hostel Buildings, Blocks & Rooms (HostelFacilities).
...
class HostelFacilitiesDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for HostelFacilities."""
    template_name = 'hostel/hostelfacilities_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = HostelFacilitiesAuditTrail.objects.all().order_by('-timestamp')[:10] if 'HostelFacilitiesAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Hostel Buildings, Blocks & Rooms Operations Dashboard",
            'domain_name': 'HostelFacilities',
            'app_title': 'Hostel',
...
class HostelFacilitiesAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'hostel/hostelfacilities_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Hostel Buildings, Blocks & Rooms Analytical Insights",
