"""
Extended REST API Controllers for EduFlow Hostel Gate Passes & Resident Discipline (GatePasses).
...
class GatePassesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(GatePassesMaster, pk=pk)
        data = master.to_dict()
...
class GatePassesAPISearchView(LoginRequiredMixin, View):
...
class GatePassesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class GatePassesAPITelemetryView(LoginRequiredMixin, View):
