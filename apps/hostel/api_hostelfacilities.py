"""
Extended REST API Controllers for EduFlow Hostel Buildings, Blocks & Rooms (HostelFacilities).
...
class HostelFacilitiesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(HostelFacilitiesMaster, pk=pk)
        data = master.to_dict()
...
class HostelFacilitiesAPISearchView(LoginRequiredMixin, View):
...
class HostelFacilitiesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class HostelFacilitiesAPITelemetryView(LoginRequiredMixin, View):
