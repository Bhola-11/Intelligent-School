"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Hostel Bed Allocation & Check-In (BedAllocations).
...
class BedAllocationsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for BedAllocations."""
    template_name = 'hostel/bedallocations_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = BedAllocationsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'BedAllocationsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Hostel Bed Allocation & Check-In Operations Dashboard",
            'domain_name': 'BedAllocations',
            'app_title': 'Hostel',
...
class BedAllocationsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'hostel/bedallocations_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Hostel Bed Allocation & Check-In Analytical Insights",
