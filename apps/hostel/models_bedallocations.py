"""
Models for EduFlow Hostel Bed Allocation & Check-In (BedAllocations) in apps.hostel.
...
# -----------------------------------------------------------------------------
class BedAllocationsStatusChoices(models.TextChoices):
...
class BedAllocationsPriorityChoices(models.TextChoices):
...
class BedAllocationsTierChoices(models.TextChoices):
...
class BedAllocationsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class BedAllocationsMasterQuerySet(models.QuerySet):
    def active(self):
...
class BedAllocationsMasterManager(models.Manager):
    def get_queryset(self):
        return BedAllocationsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class BedAllocationsMaster(models.Model):
    """
    Primary master configuration entity for Hostel Bed Allocation & Check-In.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = BedAllocationsStatusChoices
    PriorityChoices = BedAllocationsPriorityChoices
    TierChoices = BedAllocationsTierChoices
    ScopeChoices = BedAllocationsScopeChoices
    ...
    objects = BedAllocationsMasterManager()

    class Meta:
        db_table = 'eduflow_hostel_bedallocations_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("BedAllocations Master")
        verbose_name_plural = _("BedAllocations Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_bedal_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_bedal_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_bedal_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_bedal_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'hostel:bedallocations_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/hostel/bedallocations/{self.pk}/"
...
# -----------------------------------------------------------------------------
class BedAllocationsItem(models.Model):
    """Subordinate component or line transaction item for BedAllocations."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("BedAllocations Item")
        verbose_name_plural = _("BedAllocations Items")
...
# -----------------------------------------------------------------------------
class BedAllocationsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for BedAllocations."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_allocation'
        ordering = ['-start_time']
        verbose_name = _("BedAllocations Allocation")
        verbose_name_plural = _("BedAllocations Allocations")
...
# -----------------------------------------------------------------------------
class BedAllocationsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for BedAllocations."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("BedAllocations Metric Record")
        verbose_name_plural = _("BedAllocations Metric Records")
...
# -----------------------------------------------------------------------------
class BedAllocationsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for BedAllocations."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_rule'
        ordering = ['rule_code']
        verbose_name = _("BedAllocations Policy Rule")
        verbose_name_plural = _("BedAllocations Policy Rules")
...
# -----------------------------------------------------------------------------
class BedAllocationsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for BedAllocations."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("BedAllocations Schedule Period")
        verbose_name_plural = _("BedAllocations Schedule Periods")
...
# -----------------------------------------------------------------------------
class BedAllocationsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for BedAllocations."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_review'
        ordering = ['-submitted_at']
        verbose_name = _("BedAllocations Review")
        verbose_name_plural = _("BedAllocations Reviews")
...
# -----------------------------------------------------------------------------
class BedAllocationsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("BedAllocations Workflow Transition")
        verbose_name_plural = _("BedAllocations Workflow Transitions")
...
# -----------------------------------------------------------------------------
class BedAllocationsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_access'
        verbose_name = _("BedAllocations Access Rule")
        verbose_name_plural = _("BedAllocations Access Rules")
...
# -----------------------------------------------------------------------------
class BedAllocationsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_config'
        verbose_name = _("BedAllocations Config Parameter")
        verbose_name_plural = _("BedAllocations Config Parameters")
...
# -----------------------------------------------------------------------------
class BedAllocationsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("BedAllocations Document Attachment")
        verbose_name_plural = _("BedAllocations Document Attachments")
...
# -----------------------------------------------------------------------------
class BedAllocationsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on BedAllocations."""
    master = models.ForeignKey(BedAllocationsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_hostel_bedallocations_audit'
        ordering = ['-timestamp']
        verbose_name = _("BedAllocations Audit Entry")
        verbose_name_plural = _("BedAllocations Audit Entries")
