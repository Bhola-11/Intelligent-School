"""
Extended REST API Controllers for EduFlow Notice Board & Announcements (NoticeBoard).
...
class NoticeBoardAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(NoticeBoardMaster, pk=pk)
        data = master.to_dict()
...
class NoticeBoardAPISearchView(LoginRequiredMixin, View):
...
class NoticeBoardAPIBatchUpdateView(LoginRequiredMixin, View):
...
class NoticeBoardAPITelemetryView(LoginRequiredMixin, View):
