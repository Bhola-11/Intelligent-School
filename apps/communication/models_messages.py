"""
Models for EduFlow Internal Messaging & Communication Threads (Messages) in apps.communication.
...
# -----------------------------------------------------------------------------
class MessagesStatusChoices(models.TextChoices):
...
class MessagesPriorityChoices(models.TextChoices):
...
class MessagesTierChoices(models.TextChoices):
...
class MessagesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class MessagesMasterQuerySet(models.QuerySet):
    def active(self):
...
class MessagesMasterManager(models.Manager):
    def get_queryset(self):
        return MessagesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class MessagesMaster(models.Model):
    """
    Primary master configuration entity for Internal Messaging & Communication Threads.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = MessagesStatusChoices
    PriorityChoices = MessagesPriorityChoices
    TierChoices = MessagesTierChoices
    ScopeChoices = MessagesScopeChoices
    ...
    objects = MessagesMasterManager()

    class Meta:
        db_table = 'eduflow_communication_messages_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Messages Master")
        verbose_name_plural = _("Messages Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_messa_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_messa_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_messa_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_messa_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'communication:messages_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/communication/messages/{self.pk}/"
...
# -----------------------------------------------------------------------------
class MessagesItem(models.Model):
    """Subordinate component or line transaction item for Messages."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Messages Item")
        verbose_name_plural = _("Messages Items")
...
# -----------------------------------------------------------------------------
class MessagesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Messages."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_allocation'
        ordering = ['-start_time']
        verbose_name = _("Messages Allocation")
        verbose_name_plural = _("Messages Allocations")
...
# -----------------------------------------------------------------------------
class MessagesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Messages."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Messages Metric Record")
        verbose_name_plural = _("Messages Metric Records")
...
# -----------------------------------------------------------------------------
class MessagesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Messages."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_rule'
        ordering = ['rule_code']
        verbose_name = _("Messages Policy Rule")
        verbose_name_plural = _("Messages Policy Rules")
...
# -----------------------------------------------------------------------------
class MessagesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Messages."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Messages Schedule Period")
        verbose_name_plural = _("Messages Schedule Periods")
...
# -----------------------------------------------------------------------------
class MessagesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Messages."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_review'
        ordering = ['-submitted_at']
        verbose_name = _("Messages Review")
        verbose_name_plural = _("Messages Reviews")
...
# -----------------------------------------------------------------------------
class MessagesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Messages Workflow Transition")
        verbose_name_plural = _("Messages Workflow Transitions")
...
# -----------------------------------------------------------------------------
class MessagesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_access'
        verbose_name = _("Messages Access Rule")
        verbose_name_plural = _("Messages Access Rules")
...
# -----------------------------------------------------------------------------
class MessagesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_config'
        verbose_name = _("Messages Config Parameter")
        verbose_name_plural = _("Messages Config Parameters")
...
# -----------------------------------------------------------------------------
class MessagesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Messages Document Attachment")
        verbose_name_plural = _("Messages Document Attachments")
...
# -----------------------------------------------------------------------------
class MessagesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Messages."""
    master = models.ForeignKey(MessagesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_communication_messages_audit'
        ordering = ['-timestamp']
        verbose_name = _("Messages Audit Entry")
        verbose_name_plural = _("Messages Audit Entries")
