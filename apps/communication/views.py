"""
Views for EduFlow Notice Board & Announcements (NoticeBoard).
...
try:
    ...
    from .forms import (
        NoticeBoardMasterForm, NoticeBoardItemForm, NoticeBoardAllocationForm,
        NoticeBoardMetricRecordForm, NoticeBoardPolicyRuleForm, NoticeBoardSchedulePeriodForm,
        NoticeBoardFeedbackReviewForm, NoticeBoardWorkflowTransitionForm, NoticeBoardAccessRuleForm,
        NoticeBoardConfigurationParameterForm, NoticeBoardDocumentAttachmentForm, NoticeBoardFilterForm,
        NoticeBoardBulkActionForm
    )
except ImportError:
    try:
        from .models import (
...
class NoticeBoardListView(LoginRequiredMixin, ListView):
    template_name = 'communication/noticeboard_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = NoticeBoardMaster.objects.all() if 'NoticeBoardMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = NoticeBoardFilterForm(self.request.GET) if 'NoticeBoardFilterForm' in globals() else None
        ctx['bulk_form'] = NoticeBoardBulkActionForm() if 'NoticeBoardBulkActionForm' in globals() else None
        ctx['total_count'] = NoticeBoardMaster.objects.count() if 'NoticeBoardMaster' in globals() else 0
        ctx['active_count'] = NoticeBoardMaster.objects.filter(status='ACTIVE').count() if 'NoticeBoardMaster' in globals() else 0
        ctx['pending_count'] = NoticeBoardMaster.objects.filter(status='PENDING_REVIEW').count() if 'NoticeBoardMaster' in globals() else 0
        ctx['page_title'] = "Notice Board & Announcements Directory"
        ctx['domain_name'] = "NoticeBoard"
        return ctx

class NoticeBoardDetailView(LoginRequiredMixin, DetailView):
    template_name = 'communication/noticeboard_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(NoticeBoardMaster, pk=self.kwargs.get('pk'))
...
class NoticeBoardCreateView(LoginRequiredMixin, CreateView):
    template_name = 'communication/noticeboard_form.html'
    form_class = NoticeBoardMasterForm
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f"NoticeBoard record '{form.instance.name}' created successfully.")
...
class NoticeBoardUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"NoticeBoard record updated successfully.")
        return super().form_valid(form)

class NoticeBoardDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'communication/noticeboard_confirm_delete.html'
    success_url = reverse_lazy('communication:noticeboard_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class NoticeBoardItemCreateView(LoginRequiredMixin, CreateView):
    form_class = NoticeBoardItemForm
    template_name = 'communication/noticeboard_form.html'

    def form_valid(self, form):
        master = get_object_or_404(NoticeBoardMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('communication:noticeboard_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class NoticeBoardItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = NoticeBoardItemForm
    template_name = 'communication/noticeboard_form.html'

    def get_object(self):
        return get_object_or_404(NoticeBoardItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('communication:noticeboard_detail', kwargs={'pk': self.object.master_id})

class NoticeBoardItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'communication/noticeboard_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class NoticeBoardAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = NoticeBoardAllocationForm
    template_name = 'communication/noticeboard_form.html'
...
class NoticeBoardAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(NoticeBoardAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('communication:noticeboard_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class NoticeBoardMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = NoticeBoardMetricRecordForm
    template_name = 'communication/noticeboard_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class NoticeBoardPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = NoticeBoardPolicyRuleForm
    template_name = 'communication/noticeboard_form.html'
...
# -----------------------------------------------------------------------------
class NoticeBoardSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = NoticeBoardSchedulePeriodForm
    template_name = 'communication/noticeboard_form.html'
...
# -----------------------------------------------------------------------------
class NoticeBoardFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = NoticeBoardFeedbackReviewForm
    template_name = 'communication/noticeboard_form.html'
...
# -----------------------------------------------------------------------------
class NoticeBoardWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = NoticeBoardWorkflowTransitionForm
    template_name = 'communication/noticeboard_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class NoticeBoardAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = NoticeBoardAccessRuleForm
    template_name = 'communication/noticeboard_form.html'
...
# -----------------------------------------------------------------------------
class NoticeBoardConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = NoticeBoardConfigurationParameterForm
    template_name = 'communication/noticeboard_form.html'
...
# -----------------------------------------------------------------------------
class NoticeBoardDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = NoticeBoardDocumentAttachmentForm
    template_name = 'communication/noticeboard_form.html'
...
# -----------------------------------------------------------------------------
class NoticeBoardBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = NoticeBoardMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('communication:noticeboard_list')

class NoticeBoardExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="noticeboard_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        writer = csv.writer(response)
        writer.writerow(['Code', 'Name', 'Category', 'Tier', 'Priority', 'Status', 'Capacity', 'Occupancy', 'Budget ($)', 'Incurred ($)', 'Start Date', 'End Date'])
        records = NoticeBoardMaster.objects.all() if 'NoticeBoardMaster' in globals() else []
        for r in records:
...
class NoticeBoardExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = NoticeBoardMaster.objects.all() if 'NoticeBoardMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class NoticeBoardAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = NoticeBoardMaster.objects.all() if 'NoticeBoardMaster' in globals() else []
...
class NoticeBoardAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(NoticeBoardMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
