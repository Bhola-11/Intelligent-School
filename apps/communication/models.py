"""
Models for EduFlow Notice Board & Announcements (NoticeBoard) in apps.communication.
...
# -----------------------------------------------------------------------------
class NoticeBoardStatusChoices(models.TextChoices):
...
class NoticeBoardPriorityChoices(models.TextChoices):
...
class NoticeBoardTierChoices(models.TextChoices):
...
class NoticeBoardScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class NoticeBoardMasterQuerySet(models.QuerySet):
    def active(self):
...
class NoticeBoardMasterManager(models.Manager):
    def get_queryset(self):
        return NoticeBoardMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class NoticeBoardMaster(models.Model):
    """
    Primary master configuration entity for Notice Board & Announcements.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = NoticeBoardStatusChoices
    PriorityChoices = NoticeBoardPriorityChoices
    TierChoices = NoticeBoardTierChoices
    ScopeChoices = NoticeBoardScopeChoices
    ...
    objects = NoticeBoardMasterManager()

    class Meta:
        db_table = 'eduflow_communication_noticeboard_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("NoticeBoard Master")
        verbose_name_plural = _("NoticeBoard Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_notic_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_notic_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_notic_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_notic_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'communication:noticeboard_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/communication/noticeboard/{self.pk}/"
...
class NoticeBoardItem(models.Model):
    """Subordinate component or line transaction item for NoticeBoard."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("NoticeBoard Item")
        verbose_name_plural = _("NoticeBoard Items")
...
# -----------------------------------------------------------------------------
class NoticeBoardAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for NoticeBoard."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_allocation'
        ordering = ['-start_time']
        verbose_name = _("NoticeBoard Allocation")
        verbose_name_plural = _("NoticeBoard Allocations")
...
# -----------------------------------------------------------------------------
class NoticeBoardMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for NoticeBoard."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("NoticeBoard Metric Record")
        verbose_name_plural = _("NoticeBoard Metric Records")
...
# -----------------------------------------------------------------------------
class NoticeBoardPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for NoticeBoard."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_rule'
        ordering = ['rule_code']
        verbose_name = _("NoticeBoard Policy Rule")
        verbose_name_plural = _("NoticeBoard Policy Rules")
...
# -----------------------------------------------------------------------------
class NoticeBoardSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for NoticeBoard."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("NoticeBoard Schedule Period")
        verbose_name_plural = _("NoticeBoard Schedule Periods")
...
# -----------------------------------------------------------------------------
class NoticeBoardFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for NoticeBoard."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_review'
        ordering = ['-submitted_at']
        verbose_name = _("NoticeBoard Review")
        verbose_name_plural = _("NoticeBoard Reviews")
...
# -----------------------------------------------------------------------------
class NoticeBoardWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("NoticeBoard Workflow Transition")
        verbose_name_plural = _("NoticeBoard Workflow Transitions")
...
# -----------------------------------------------------------------------------
class NoticeBoardAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_access'
        verbose_name = _("NoticeBoard Access Rule")
        verbose_name_plural = _("NoticeBoard Access Rules")
...
# -----------------------------------------------------------------------------
class NoticeBoardConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_config'
        verbose_name = _("NoticeBoard Config Parameter")
        verbose_name_plural = _("NoticeBoard Config Parameters")
...
# -----------------------------------------------------------------------------
class NoticeBoardDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("NoticeBoard Document Attachment")
        verbose_name_plural = _("NoticeBoard Document Attachments")
...
# -----------------------------------------------------------------------------
class NoticeBoardAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on NoticeBoard."""
    master = models.ForeignKey(NoticeBoardMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_communication_noticeboard_audit'
        ordering = ['-timestamp']
        verbose_name = _("NoticeBoard Audit Entry")
        verbose_name_plural = _("NoticeBoard Audit Entries")
