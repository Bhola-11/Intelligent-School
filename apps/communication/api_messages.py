"""
Extended REST API Controllers for EduFlow Internal Messaging & Communication Threads (Messages).
...
class MessagesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(MessagesMaster, pk=pk)
        data = master.to_dict()
...
class MessagesAPISearchView(LoginRequiredMixin, View):
...
class MessagesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class MessagesAPITelemetryView(LoginRequiredMixin, View):
