"""
Extended REST API Controllers for EduFlow Institutional Circulars & Acknowledgments (Circulars).
...
class CircularsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(CircularsMaster, pk=pk)
        data = master.to_dict()
...
class CircularsAPISearchView(LoginRequiredMixin, View):
...
class CircularsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class CircularsAPITelemetryView(LoginRequiredMixin, View):
