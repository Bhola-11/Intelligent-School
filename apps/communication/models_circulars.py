"""
Models for EduFlow Institutional Circulars & Acknowledgments (Circulars) in apps.communication.
...
# -----------------------------------------------------------------------------
class CircularsStatusChoices(models.TextChoices):
...
class CircularsPriorityChoices(models.TextChoices):
...
class CircularsTierChoices(models.TextChoices):
...
class CircularsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class CircularsMasterQuerySet(models.QuerySet):
    def active(self):
...
class CircularsMasterManager(models.Manager):
    def get_queryset(self):
        return CircularsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class CircularsMaster(models.Model):
    """
    Primary master configuration entity for Institutional Circulars & Acknowledgments.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = CircularsStatusChoices
    PriorityChoices = CircularsPriorityChoices
    TierChoices = CircularsTierChoices
    ScopeChoices = CircularsScopeChoices
    ...
    objects = CircularsMasterManager()

    class Meta:
        db_table = 'eduflow_communication_circulars_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Circulars Master")
        verbose_name_plural = _("Circulars Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'communication:circulars_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/communication/circulars/{self.pk}/"
...
# -----------------------------------------------------------------------------
class CircularsItem(models.Model):
    """Subordinate component or line transaction item for Circulars."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Circulars Item")
        verbose_name_plural = _("Circulars Items")
...
# -----------------------------------------------------------------------------
class CircularsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Circulars."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_allocation'
        ordering = ['-start_time']
        verbose_name = _("Circulars Allocation")
        verbose_name_plural = _("Circulars Allocations")
...
# -----------------------------------------------------------------------------
class CircularsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Circulars."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Circulars Metric Record")
        verbose_name_plural = _("Circulars Metric Records")
...
# -----------------------------------------------------------------------------
class CircularsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Circulars."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_rule'
        ordering = ['rule_code']
        verbose_name = _("Circulars Policy Rule")
        verbose_name_plural = _("Circulars Policy Rules")
...
# -----------------------------------------------------------------------------
class CircularsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Circulars."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Circulars Schedule Period")
        verbose_name_plural = _("Circulars Schedule Periods")
...
# -----------------------------------------------------------------------------
class CircularsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Circulars."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_review'
        ordering = ['-submitted_at']
        verbose_name = _("Circulars Review")
        verbose_name_plural = _("Circulars Reviews")
...
# -----------------------------------------------------------------------------
class CircularsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Circulars Workflow Transition")
        verbose_name_plural = _("Circulars Workflow Transitions")
...
# -----------------------------------------------------------------------------
class CircularsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_access'
        verbose_name = _("Circulars Access Rule")
        verbose_name_plural = _("Circulars Access Rules")
...
# -----------------------------------------------------------------------------
class CircularsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_config'
        verbose_name = _("Circulars Config Parameter")
        verbose_name_plural = _("Circulars Config Parameters")
...
# -----------------------------------------------------------------------------
class CircularsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Circulars Document Attachment")
        verbose_name_plural = _("Circulars Document Attachments")
...
# -----------------------------------------------------------------------------
class CircularsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Circulars."""
    master = models.ForeignKey(CircularsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_communication_circulars_audit'
        ordering = ['-timestamp']
        verbose_name = _("Circulars Audit Entry")
        verbose_name_plural = _("Circulars Audit Entries")
