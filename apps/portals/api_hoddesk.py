"""
Extended REST API Controllers for EduFlow Department Head (HOD) Overview (HODDesk).
...
class HODDeskAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(HODDeskMaster, pk=pk)
        data = master.to_dict()
...
class HODDeskAPISearchView(LoginRequiredMixin, View):
...
class HODDeskAPIBatchUpdateView(LoginRequiredMixin, View):
...
class HODDeskAPITelemetryView(LoginRequiredMixin, View):
