"""
Views for EduFlow Principal & Director Portal (Principal).
...
try:
    ...
    from .forms import (
        PrincipalMasterForm, PrincipalItemForm, PrincipalAllocationForm,
        PrincipalMetricRecordForm, PrincipalPolicyRuleForm, PrincipalSchedulePeriodForm,
        PrincipalFeedbackReviewForm, PrincipalWorkflowTransitionForm, PrincipalAccessRuleForm,
        PrincipalConfigurationParameterForm, PrincipalDocumentAttachmentForm, PrincipalFilterForm,
        PrincipalBulkActionForm
    )
except ImportError:
    try:
...
class PrincipalListView(LoginRequiredMixin, ListView):
    template_name = 'portals/principal_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = PrincipalMaster.objects.all() if 'PrincipalMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = PrincipalFilterForm(self.request.GET) if 'PrincipalFilterForm' in globals() else None
        ctx['bulk_form'] = PrincipalBulkActionForm() if 'PrincipalBulkActionForm' in globals() else None
        ctx['total_count'] = PrincipalMaster.objects.count() if 'PrincipalMaster' in globals() else 0
        ctx['active_count'] = PrincipalMaster.objects.filter(status='ACTIVE').count() if 'PrincipalMaster' in globals() else 0
        ctx['pending_count'] = PrincipalMaster.objects.filter(status='PENDING_REVIEW').count() if 'PrincipalMaster' in globals() else 0
        ctx['page_title'] = "Principal & Director Portal Directory"
        ctx['domain_name'] = "Principal"
        return ctx

class PrincipalDetailView(LoginRequiredMixin, DetailView):
    template_name = 'portals/principal_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(PrincipalMaster, pk=self.kwargs.get('pk'))
...
class PrincipalCreateView(LoginRequiredMixin, CreateView):
    template_name = 'portals/principal_form.html'
    form_class = PrincipalMasterForm
    success_url = reverse_lazy('portals:principal_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Principal record '{form.instance.name}' created successfully.")
...
class PrincipalUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Principal record updated successfully.")
        return super().form_valid(form)

class PrincipalDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'portals/principal_confirm_delete.html'
    success_url = reverse_lazy('portals:principal_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class PrincipalItemCreateView(LoginRequiredMixin, CreateView):
    form_class = PrincipalItemForm
    template_name = 'portals/principal_form.html'

    def form_valid(self, form):
        master = get_object_or_404(PrincipalMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('portals:principal_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class PrincipalItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = PrincipalItemForm
    template_name = 'portals/principal_form.html'

    def get_object(self):
        return get_object_or_404(PrincipalItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('portals:principal_detail', kwargs={'pk': self.object.master_id})

class PrincipalItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'portals/principal_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class PrincipalAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = PrincipalAllocationForm
    template_name = 'portals/principal_form.html'
...
class PrincipalAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(PrincipalAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('portals:principal_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class PrincipalMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = PrincipalMetricRecordForm
    template_name = 'portals/principal_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class PrincipalPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = PrincipalPolicyRuleForm
    template_name = 'portals/principal_form.html'
...
# -----------------------------------------------------------------------------
class PrincipalSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = PrincipalSchedulePeriodForm
    template_name = 'portals/principal_form.html'
...
# -----------------------------------------------------------------------------
class PrincipalFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = PrincipalFeedbackReviewForm
    template_name = 'portals/principal_form.html'
...
# -----------------------------------------------------------------------------
class PrincipalWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = PrincipalWorkflowTransitionForm
    template_name = 'portals/principal_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class PrincipalAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = PrincipalAccessRuleForm
    template_name = 'portals/principal_form.html'
...
# -----------------------------------------------------------------------------
class PrincipalConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = PrincipalConfigurationParameterForm
    template_name = 'portals/principal_form.html'
...
# -----------------------------------------------------------------------------
class PrincipalDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = PrincipalDocumentAttachmentForm
    template_name = 'portals/principal_form.html'
...
# -----------------------------------------------------------------------------
class PrincipalBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = PrincipalMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('portals:principal_list')

class PrincipalExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="principal_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
...
class PrincipalExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = PrincipalMaster.objects.all() if 'PrincipalMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class PrincipalAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = PrincipalMaster.objects.all() if 'PrincipalMaster' in globals() else []
...
class PrincipalAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(PrincipalMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
