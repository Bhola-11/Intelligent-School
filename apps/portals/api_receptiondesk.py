"""
Extended REST API Controllers for EduFlow Reception & Front Office Desk (ReceptionDesk).
...
class ReceptionDeskAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ReceptionDeskMaster, pk=pk)
        data = master.to_dict()
...
class ReceptionDeskAPISearchView(LoginRequiredMixin, View):
...
class ReceptionDeskAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ReceptionDeskAPITelemetryView(LoginRequiredMixin, View):
