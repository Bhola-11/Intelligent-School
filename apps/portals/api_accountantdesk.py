"""
Extended REST API Controllers for EduFlow Accountant Financial Operations Desk (AccountantDesk).
...
class AccountantDeskAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(AccountantDeskMaster, pk=pk)
        data = master.to_dict()
...
class AccountantDeskAPISearchView(LoginRequiredMixin, View):
...
class AccountantDeskAPIBatchUpdateView(LoginRequiredMixin, View):
...
class AccountantDeskAPITelemetryView(LoginRequiredMixin, View):
