"""
Domain Services for EduFlow Principal & Director Portal (Principal).
...
class PrincipalService:
    """Enterprise domain business service for Principal."""
    ...
    def create_record(cls, code, name, user_username="system", **kwargs):
        ...
        logger.info(f"Created Principal record {record.code} by {user_username}")
        return record
    ...
    def update_record(cls, record_id, user_username="system", **kwargs):
        ...
        record = PrincipalMaster.objects.select_for_update().get(pk=record_id)
    ...
    def allocate_resource(cls, record_id, assignee_id, assignee_name, role="Lead", start_time=None, end_time=None):
        ...
        allocation = PrincipalAllocation.objects.create(
            master=record,
    ...
    def release_allocation(cls, allocation_id):
        ...
        alloc = PrincipalAllocation.objects.get(pk=allocation_id)
    ...
    def set_configuration_parameter(cls, record_id, key, val, data_type="STRING", desc=""):
        ...
        record = PrincipalMaster.objects.get(pk=record_id)
        param, _ = PrincipalConfigurationParameter.objects.update_or_create(
            master=record,
            param_key=key,
    ...
    def _log_action(cls, record, action_type, user_username, old_state, new_state, summary):
        if 'PrincipalAuditTrail' in globals():
            try:
                ...
                logger.warning(f"Failed to record audit log for Principal: {e}")
