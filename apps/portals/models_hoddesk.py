"""
Models for EduFlow Department Head (HOD) Overview (HODDesk) in apps.portals.
...
# -----------------------------------------------------------------------------
class HODDeskStatusChoices(models.TextChoices):
...
class HODDeskPriorityChoices(models.TextChoices):
...
class HODDeskTierChoices(models.TextChoices):
...
class HODDeskScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class HODDeskMasterQuerySet(models.QuerySet):
    def active(self):
...
class HODDeskMasterManager(models.Manager):
    def get_queryset(self):
        return HODDeskMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class HODDeskMaster(models.Model):
    """
    Primary master configuration entity for Department Head (HOD) Overview.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = HODDeskStatusChoices
    PriorityChoices = HODDeskPriorityChoices
    TierChoices = HODDeskTierChoices
    ScopeChoices = HODDeskScopeChoices
    ...
    objects = HODDeskMasterManager()

    class Meta:
        db_table = 'eduflow_portals_hoddesk_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("HODDesk Master")
        verbose_name_plural = _("HODDesk Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_hodde_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_hodde_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_hodde_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_hodde_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:hoddesk_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/hoddesk/{self.pk}/"
...
# -----------------------------------------------------------------------------
class HODDeskItem(models.Model):
    """Subordinate component or line transaction item for HODDesk."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("HODDesk Item")
        verbose_name_plural = _("HODDesk Items")
...
# -----------------------------------------------------------------------------
class HODDeskAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for HODDesk."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_allocation'
        ordering = ['-start_time']
        verbose_name = _("HODDesk Allocation")
        verbose_name_plural = _("HODDesk Allocations")
...
# -----------------------------------------------------------------------------
class HODDeskMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for HODDesk."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("HODDesk Metric Record")
        verbose_name_plural = _("HODDesk Metric Records")
...
# -----------------------------------------------------------------------------
class HODDeskPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for HODDesk."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_rule'
        ordering = ['rule_code']
        verbose_name = _("HODDesk Policy Rule")
        verbose_name_plural = _("HODDesk Policy Rules")
...
# -----------------------------------------------------------------------------
class HODDeskSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for HODDesk."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("HODDesk Schedule Period")
        verbose_name_plural = _("HODDesk Schedule Periods")
...
# -----------------------------------------------------------------------------
class HODDeskFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for HODDesk."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_review'
        ordering = ['-submitted_at']
        verbose_name = _("HODDesk Review")
        verbose_name_plural = _("HODDesk Reviews")
...
# -----------------------------------------------------------------------------
class HODDeskWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("HODDesk Workflow Transition")
        verbose_name_plural = _("HODDesk Workflow Transitions")
...
# -----------------------------------------------------------------------------
class HODDeskAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_access'
        verbose_name = _("HODDesk Access Rule")
        verbose_name_plural = _("HODDesk Access Rules")
...
# -----------------------------------------------------------------------------
class HODDeskConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_config'
        verbose_name = _("HODDesk Config Parameter")
        verbose_name_plural = _("HODDesk Config Parameters")
...
# -----------------------------------------------------------------------------
class HODDeskDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("HODDesk Document Attachment")
        verbose_name_plural = _("HODDesk Document Attachments")
...
# -----------------------------------------------------------------------------
class HODDeskAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on HODDesk."""
    master = models.ForeignKey(HODDeskMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_hoddesk_audit'
        ordering = ['-timestamp']
        verbose_name = _("HODDesk Audit Entry")
        verbose_name_plural = _("HODDesk Audit Entries")
