"""
Models for EduFlow Teacher Workspace & Daily Register (TeacherWorkspace) in apps.portals.
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceStatusChoices(models.TextChoices):
...
class TeacherWorkspacePriorityChoices(models.TextChoices):
...
class TeacherWorkspaceTierChoices(models.TextChoices):
...
class TeacherWorkspaceScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceMasterQuerySet(models.QuerySet):
    def active(self):
...
class TeacherWorkspaceMasterManager(models.Manager):
    def get_queryset(self):
        return TeacherWorkspaceMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceMaster(models.Model):
    """
    Primary master configuration entity for Teacher Workspace & Daily Register.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = TeacherWorkspaceStatusChoices
    PriorityChoices = TeacherWorkspacePriorityChoices
    TierChoices = TeacherWorkspaceTierChoices
    ScopeChoices = TeacherWorkspaceScopeChoices
    ...
    objects = TeacherWorkspaceMasterManager()

    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("TeacherWorkspace Master")
        verbose_name_plural = _("TeacherWorkspace Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_teach_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_teach_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_teach_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_teach_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:teacherworkspace_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/teacherworkspace/{self.pk}/"
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceItem(models.Model):
    """Subordinate component or line transaction item for TeacherWorkspace."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("TeacherWorkspace Item")
        verbose_name_plural = _("TeacherWorkspace Items")
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for TeacherWorkspace."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_allocation'
        ordering = ['-start_time']
        verbose_name = _("TeacherWorkspace Allocation")
        verbose_name_plural = _("TeacherWorkspace Allocations")
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for TeacherWorkspace."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("TeacherWorkspace Metric Record")
        verbose_name_plural = _("TeacherWorkspace Metric Records")
...
# -----------------------------------------------------------------------------
class TeacherWorkspacePolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for TeacherWorkspace."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_rule'
        ordering = ['rule_code']
        verbose_name = _("TeacherWorkspace Policy Rule")
        verbose_name_plural = _("TeacherWorkspace Policy Rules")
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for TeacherWorkspace."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("TeacherWorkspace Schedule Period")
        verbose_name_plural = _("TeacherWorkspace Schedule Periods")
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for TeacherWorkspace."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_review'
        ordering = ['-submitted_at']
        verbose_name = _("TeacherWorkspace Review")
        verbose_name_plural = _("TeacherWorkspace Reviews")
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("TeacherWorkspace Workflow Transition")
        verbose_name_plural = _("TeacherWorkspace Workflow Transitions")
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_access'
        verbose_name = _("TeacherWorkspace Access Rule")
        verbose_name_plural = _("TeacherWorkspace Access Rules")
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_config'
        verbose_name = _("TeacherWorkspace Config Parameter")
        verbose_name_plural = _("TeacherWorkspace Config Parameters")
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("TeacherWorkspace Document Attachment")
        verbose_name_plural = _("TeacherWorkspace Document Attachments")
...
# -----------------------------------------------------------------------------
class TeacherWorkspaceAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on TeacherWorkspace."""
    master = models.ForeignKey(TeacherWorkspaceMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_teacherworkspace_audit'
        ordering = ['-timestamp']
        verbose_name = _("TeacherWorkspace Audit Entry")
        verbose_name_plural = _("TeacherWorkspace Audit Entries")
