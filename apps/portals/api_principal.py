"""
Extended REST API Controllers for EduFlow Principal & Director Portal (Principal).
...
class PrincipalAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(PrincipalMaster, pk=pk)
        data = master.to_dict()
...
class PrincipalAPISearchView(LoginRequiredMixin, View):
...
class PrincipalAPIBatchUpdateView(LoginRequiredMixin, View):
...
class PrincipalAPITelemetryView(LoginRequiredMixin, View):
