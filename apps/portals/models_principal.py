"""
Models for EduFlow Principal & Director Portal (Principal) in apps.portals.
...
# -----------------------------------------------------------------------------
class PrincipalStatusChoices(models.TextChoices):
...
class PrincipalPriorityChoices(models.TextChoices):
...
class PrincipalTierChoices(models.TextChoices):
...
class PrincipalScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class PrincipalMasterQuerySet(models.QuerySet):
    def active(self):
...
class PrincipalMasterManager(models.Manager):
    def get_queryset(self):
        return PrincipalMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class PrincipalMaster(models.Model):
    """
    Primary master configuration entity for Principal & Director Portal.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = PrincipalStatusChoices
    PriorityChoices = PrincipalPriorityChoices
    TierChoices = PrincipalTierChoices
    ScopeChoices = PrincipalScopeChoices
    ...
    objects = PrincipalMasterManager()

    class Meta:
        db_table = 'eduflow_portals_principal_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Principal Master")
        verbose_name_plural = _("Principal Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_princ_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_princ_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_princ_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_princ_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:principal_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/principal/{self.pk}/"
...
class PrincipalItem(models.Model):
    """Subordinate component or line transaction item for Principal."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Principal Item")
        verbose_name_plural = _("Principal Items")
...
# -----------------------------------------------------------------------------
class PrincipalAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Principal."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_allocation'
        ordering = ['-start_time']
        verbose_name = _("Principal Allocation")
        verbose_name_plural = _("Principal Allocations")
...
# -----------------------------------------------------------------------------
class PrincipalMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Principal."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Principal Metric Record")
        verbose_name_plural = _("Principal Metric Records")
...
# -----------------------------------------------------------------------------
class PrincipalPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Principal."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_rule'
        ordering = ['rule_code']
        verbose_name = _("Principal Policy Rule")
        verbose_name_plural = _("Principal Policy Rules")
...
# -----------------------------------------------------------------------------
class PrincipalSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Principal."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Principal Schedule Period")
        verbose_name_plural = _("Principal Schedule Periods")
...
# -----------------------------------------------------------------------------
class PrincipalFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Principal."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_review'
        ordering = ['-submitted_at']
        verbose_name = _("Principal Review")
        verbose_name_plural = _("Principal Reviews")
...
# -----------------------------------------------------------------------------
class PrincipalWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Principal Workflow Transition")
        verbose_name_plural = _("Principal Workflow Transitions")
...
# -----------------------------------------------------------------------------
class PrincipalAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_access'
        verbose_name = _("Principal Access Rule")
        verbose_name_plural = _("Principal Access Rules")
...
# -----------------------------------------------------------------------------
class PrincipalConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_config'
        verbose_name = _("Principal Config Parameter")
        verbose_name_plural = _("Principal Config Parameters")
...
# -----------------------------------------------------------------------------
class PrincipalDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Principal Document Attachment")
        verbose_name_plural = _("Principal Document Attachments")
...
# -----------------------------------------------------------------------------
class PrincipalAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Principal."""
    master = models.ForeignKey(PrincipalMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_principal_audit'
        ordering = ['-timestamp']
        verbose_name = _("Principal Audit Entry")
        verbose_name_plural = _("Principal Audit Entries")
