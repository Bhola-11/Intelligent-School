"""
Models for EduFlow Super Administrator Executive Portal (SuperAdmin) in apps.portals.
...
# -----------------------------------------------------------------------------
class SuperAdminStatusChoices(models.TextChoices):
...
class SuperAdminPriorityChoices(models.TextChoices):
...
class SuperAdminTierChoices(models.TextChoices):
...
class SuperAdminScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class SuperAdminMasterQuerySet(models.QuerySet):
    def active(self):
...
class SuperAdminMasterManager(models.Manager):
    def get_queryset(self):
        return SuperAdminMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class SuperAdminMaster(models.Model):
    """
    Primary master configuration entity for Super Administrator Executive Portal.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = SuperAdminStatusChoices
    PriorityChoices = SuperAdminPriorityChoices
    TierChoices = SuperAdminTierChoices
    ScopeChoices = SuperAdminScopeChoices
    ...
    objects = SuperAdminMasterManager()

    class Meta:
        db_table = 'eduflow_portals_superadmin_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("SuperAdmin Master")
        verbose_name_plural = _("SuperAdmin Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_super_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_super_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_super_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_super_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:superadmin_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/superadmin/{self.pk}/"
...
# -----------------------------------------------------------------------------
class SuperAdminItem(models.Model):
    """Subordinate component or line transaction item for SuperAdmin."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("SuperAdmin Item")
        verbose_name_plural = _("SuperAdmin Items")
...
# -----------------------------------------------------------------------------
class SuperAdminAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for SuperAdmin."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_allocation'
        ordering = ['-start_time']
        verbose_name = _("SuperAdmin Allocation")
        verbose_name_plural = _("SuperAdmin Allocations")
...
# -----------------------------------------------------------------------------
class SuperAdminMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for SuperAdmin."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("SuperAdmin Metric Record")
        verbose_name_plural = _("SuperAdmin Metric Records")
...
# -----------------------------------------------------------------------------
class SuperAdminPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for SuperAdmin."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_rule'
        ordering = ['rule_code']
        verbose_name = _("SuperAdmin Policy Rule")
        verbose_name_plural = _("SuperAdmin Policy Rules")
...
# -----------------------------------------------------------------------------
class SuperAdminSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for SuperAdmin."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("SuperAdmin Schedule Period")
        verbose_name_plural = _("SuperAdmin Schedule Periods")
...
# -----------------------------------------------------------------------------
class SuperAdminFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for SuperAdmin."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_review'
        ordering = ['-submitted_at']
        verbose_name = _("SuperAdmin Review")
        verbose_name_plural = _("SuperAdmin Reviews")
...
# -----------------------------------------------------------------------------
class SuperAdminWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("SuperAdmin Workflow Transition")
        verbose_name_plural = _("SuperAdmin Workflow Transitions")
...
# -----------------------------------------------------------------------------
class SuperAdminAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_access'
        verbose_name = _("SuperAdmin Access Rule")
        verbose_name_plural = _("SuperAdmin Access Rules")
...
# -----------------------------------------------------------------------------
class SuperAdminConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_config'
        verbose_name = _("SuperAdmin Config Parameter")
        verbose_name_plural = _("SuperAdmin Config Parameters")
...
# -----------------------------------------------------------------------------
class SuperAdminDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("SuperAdmin Document Attachment")
        verbose_name_plural = _("SuperAdmin Document Attachments")
...
# -----------------------------------------------------------------------------
class SuperAdminAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on SuperAdmin."""
    master = models.ForeignKey(SuperAdminMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_superadmin_audit'
        ordering = ['-timestamp']
        verbose_name = _("SuperAdmin Audit Entry")
        verbose_name_plural = _("SuperAdmin Audit Entries")
