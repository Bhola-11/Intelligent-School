"""
Extended REST API Controllers for EduFlow Examination Coordinator Suite (ExamDesk).
...
class ExamDeskAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ExamDeskMaster, pk=pk)
        data = master.to_dict()
...
class ExamDeskAPISearchView(LoginRequiredMixin, View):
...
class ExamDeskAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ExamDeskAPITelemetryView(LoginRequiredMixin, View):
