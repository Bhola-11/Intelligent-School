"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Reception & Front Office Desk (ReceptionDesk).
...
class ReceptionDeskDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for ReceptionDesk."""
    template_name = 'portals/receptiondesk_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = ReceptionDeskAuditTrail.objects.all().order_by('-timestamp')[:10] if 'ReceptionDeskAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Reception & Front Office Desk Operations Dashboard",
            'domain_name': 'ReceptionDesk',
            'app_title': 'Portals',
...
class ReceptionDeskAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'portals/receptiondesk_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Reception & Front Office Desk Analytical Insights",
