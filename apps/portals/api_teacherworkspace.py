"""
Extended REST API Controllers for EduFlow Teacher Workspace & Daily Register (TeacherWorkspace).
...
class TeacherWorkspaceAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(TeacherWorkspaceMaster, pk=pk)
        data = master.to_dict()
...
class TeacherWorkspaceAPISearchView(LoginRequiredMixin, View):
...
class TeacherWorkspaceAPIBatchUpdateView(LoginRequiredMixin, View):
...
class TeacherWorkspaceAPITelemetryView(LoginRequiredMixin, View):
