"""
Models for EduFlow Accountant Financial Operations Desk (AccountantDesk) in apps.portals.
...
# -----------------------------------------------------------------------------
class AccountantDeskStatusChoices(models.TextChoices):
...
class AccountantDeskPriorityChoices(models.TextChoices):
...
class AccountantDeskTierChoices(models.TextChoices):
...
class AccountantDeskScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class AccountantDeskMasterQuerySet(models.QuerySet):
    def active(self):
...
class AccountantDeskMasterManager(models.Manager):
    def get_queryset(self):
        return AccountantDeskMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class AccountantDeskMaster(models.Model):
    """
    Primary master configuration entity for Accountant Financial Operations Desk.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = AccountantDeskStatusChoices
    PriorityChoices = AccountantDeskPriorityChoices
    TierChoices = AccountantDeskTierChoices
    ScopeChoices = AccountantDeskScopeChoices
    ...
    objects = AccountantDeskMasterManager()

    class Meta:
        db_table = 'eduflow_portals_accountantdesk_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("AccountantDesk Master")
        verbose_name_plural = _("AccountantDesk Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_accou_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_accou_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_accou_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_accou_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:accountantdesk_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/accountantdesk/{self.pk}/"
...
# -----------------------------------------------------------------------------
class AccountantDeskItem(models.Model):
    """Subordinate component or line transaction item for AccountantDesk."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("AccountantDesk Item")
        verbose_name_plural = _("AccountantDesk Items")
...
# -----------------------------------------------------------------------------
class AccountantDeskAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for AccountantDesk."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_allocation'
        ordering = ['-start_time']
        verbose_name = _("AccountantDesk Allocation")
        verbose_name_plural = _("AccountantDesk Allocations")
...
# -----------------------------------------------------------------------------
class AccountantDeskMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for AccountantDesk."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("AccountantDesk Metric Record")
        verbose_name_plural = _("AccountantDesk Metric Records")
...
# -----------------------------------------------------------------------------
class AccountantDeskPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for AccountantDesk."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_rule'
        ordering = ['rule_code']
        verbose_name = _("AccountantDesk Policy Rule")
        verbose_name_plural = _("AccountantDesk Policy Rules")
...
# -----------------------------------------------------------------------------
class AccountantDeskSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for AccountantDesk."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("AccountantDesk Schedule Period")
        verbose_name_plural = _("AccountantDesk Schedule Periods")
...
# -----------------------------------------------------------------------------
class AccountantDeskFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for AccountantDesk."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_review'
        ordering = ['-submitted_at']
        verbose_name = _("AccountantDesk Review")
        verbose_name_plural = _("AccountantDesk Reviews")
...
# -----------------------------------------------------------------------------
class AccountantDeskWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("AccountantDesk Workflow Transition")
        verbose_name_plural = _("AccountantDesk Workflow Transitions")
...
# -----------------------------------------------------------------------------
class AccountantDeskAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_access'
        verbose_name = _("AccountantDesk Access Rule")
        verbose_name_plural = _("AccountantDesk Access Rules")
...
# -----------------------------------------------------------------------------
class AccountantDeskConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_config'
        verbose_name = _("AccountantDesk Config Parameter")
        verbose_name_plural = _("AccountantDesk Config Parameters")
...
# -----------------------------------------------------------------------------
class AccountantDeskDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("AccountantDesk Document Attachment")
        verbose_name_plural = _("AccountantDesk Document Attachments")
...
# -----------------------------------------------------------------------------
class AccountantDeskAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on AccountantDesk."""
    master = models.ForeignKey(AccountantDeskMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_accountantdesk_audit'
        ordering = ['-timestamp']
        verbose_name = _("AccountantDesk Audit Entry")
        verbose_name_plural = _("AccountantDesk Audit Entries")
