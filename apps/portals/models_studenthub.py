"""
Models for EduFlow Student Learning Hub & Portal (StudentHub) in apps.portals.
...
# -----------------------------------------------------------------------------
class StudentHubStatusChoices(models.TextChoices):
...
class StudentHubPriorityChoices(models.TextChoices):
...
class StudentHubTierChoices(models.TextChoices):
...
class StudentHubScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class StudentHubMasterQuerySet(models.QuerySet):
    def active(self):
...
class StudentHubMasterManager(models.Manager):
    def get_queryset(self):
        return StudentHubMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class StudentHubMaster(models.Model):
    """
    Primary master configuration entity for Student Learning Hub & Portal.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = StudentHubStatusChoices
    PriorityChoices = StudentHubPriorityChoices
    TierChoices = StudentHubTierChoices
    ScopeChoices = StudentHubScopeChoices
    ...
    objects = StudentHubMasterManager()

    class Meta:
        db_table = 'eduflow_portals_studenthub_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("StudentHub Master")
        verbose_name_plural = _("StudentHub Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:studenthub_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/studenthub/{self.pk}/"
...
# -----------------------------------------------------------------------------
class StudentHubItem(models.Model):
    """Subordinate component or line transaction item for StudentHub."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("StudentHub Item")
        verbose_name_plural = _("StudentHub Items")
...
# -----------------------------------------------------------------------------
class StudentHubAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for StudentHub."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_allocation'
        ordering = ['-start_time']
        verbose_name = _("StudentHub Allocation")
        verbose_name_plural = _("StudentHub Allocations")
...
# -----------------------------------------------------------------------------
class StudentHubMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for StudentHub."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("StudentHub Metric Record")
        verbose_name_plural = _("StudentHub Metric Records")
...
# -----------------------------------------------------------------------------
class StudentHubPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for StudentHub."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_rule'
        ordering = ['rule_code']
        verbose_name = _("StudentHub Policy Rule")
        verbose_name_plural = _("StudentHub Policy Rules")
...
# -----------------------------------------------------------------------------
class StudentHubSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for StudentHub."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("StudentHub Schedule Period")
        verbose_name_plural = _("StudentHub Schedule Periods")
...
# -----------------------------------------------------------------------------
class StudentHubFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for StudentHub."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_review'
        ordering = ['-submitted_at']
        verbose_name = _("StudentHub Review")
        verbose_name_plural = _("StudentHub Reviews")
...
# -----------------------------------------------------------------------------
class StudentHubWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("StudentHub Workflow Transition")
        verbose_name_plural = _("StudentHub Workflow Transitions")
...
# -----------------------------------------------------------------------------
class StudentHubAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_access'
        verbose_name = _("StudentHub Access Rule")
        verbose_name_plural = _("StudentHub Access Rules")
...
# -----------------------------------------------------------------------------
class StudentHubConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_config'
        verbose_name = _("StudentHub Config Parameter")
        verbose_name_plural = _("StudentHub Config Parameters")
...
# -----------------------------------------------------------------------------
class StudentHubDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("StudentHub Document Attachment")
        verbose_name_plural = _("StudentHub Document Attachments")
...
# -----------------------------------------------------------------------------
class StudentHubAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on StudentHub."""
    master = models.ForeignKey(StudentHubMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_studenthub_audit'
        ordering = ['-timestamp']
        verbose_name = _("StudentHub Audit Entry")
        verbose_name_plural = _("StudentHub Audit Entries")
