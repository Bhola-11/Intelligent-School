"""
Extended REST API Controllers for EduFlow Librarian Operations Center (LibrarianDesk).
...
class LibrarianDeskAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(LibrarianDeskMaster, pk=pk)
        data = master.to_dict()
...
class LibrarianDeskAPISearchView(LoginRequiredMixin, View):
...
class LibrarianDeskAPIBatchUpdateView(LoginRequiredMixin, View):
...
class LibrarianDeskAPITelemetryView(LoginRequiredMixin, View):
