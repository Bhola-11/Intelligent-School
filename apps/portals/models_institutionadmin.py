"""
Models for EduFlow Institution Administrator Portal (InstitutionAdmin) in apps.portals.
...
# -----------------------------------------------------------------------------
class InstitutionAdminStatusChoices(models.TextChoices):
...
class InstitutionAdminPriorityChoices(models.TextChoices):
...
class InstitutionAdminTierChoices(models.TextChoices):
...
class InstitutionAdminScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class InstitutionAdminMasterQuerySet(models.QuerySet):
    def active(self):
...
class InstitutionAdminMasterManager(models.Manager):
    def get_queryset(self):
        return InstitutionAdminMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class InstitutionAdminMaster(models.Model):
    """
    Primary master configuration entity for Institution Administrator Portal.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = InstitutionAdminStatusChoices
    PriorityChoices = InstitutionAdminPriorityChoices
    TierChoices = InstitutionAdminTierChoices
    ScopeChoices = InstitutionAdminScopeChoices
    ...
    objects = InstitutionAdminMasterManager()

    class Meta:
        db_table = 'eduflow_portals_institutionadmin_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("InstitutionAdmin Master")
        verbose_name_plural = _("InstitutionAdmin Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_insti_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_insti_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_insti_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_insti_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:institutionadmin_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/institutionadmin/{self.pk}/"
...
# -----------------------------------------------------------------------------
class InstitutionAdminItem(models.Model):
    """Subordinate component or line transaction item for InstitutionAdmin."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("InstitutionAdmin Item")
        verbose_name_plural = _("InstitutionAdmin Items")
...
# -----------------------------------------------------------------------------
class InstitutionAdminAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for InstitutionAdmin."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_allocation'
        ordering = ['-start_time']
        verbose_name = _("InstitutionAdmin Allocation")
        verbose_name_plural = _("InstitutionAdmin Allocations")
...
# -----------------------------------------------------------------------------
class InstitutionAdminMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for InstitutionAdmin."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("InstitutionAdmin Metric Record")
        verbose_name_plural = _("InstitutionAdmin Metric Records")
...
# -----------------------------------------------------------------------------
class InstitutionAdminPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for InstitutionAdmin."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_rule'
        ordering = ['rule_code']
        verbose_name = _("InstitutionAdmin Policy Rule")
        verbose_name_plural = _("InstitutionAdmin Policy Rules")
...
# -----------------------------------------------------------------------------
class InstitutionAdminSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for InstitutionAdmin."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("InstitutionAdmin Schedule Period")
        verbose_name_plural = _("InstitutionAdmin Schedule Periods")
...
# -----------------------------------------------------------------------------
class InstitutionAdminFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for InstitutionAdmin."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_review'
        ordering = ['-submitted_at']
        verbose_name = _("InstitutionAdmin Review")
        verbose_name_plural = _("InstitutionAdmin Reviews")
...
# -----------------------------------------------------------------------------
class InstitutionAdminWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("InstitutionAdmin Workflow Transition")
        verbose_name_plural = _("InstitutionAdmin Workflow Transitions")
...
# -----------------------------------------------------------------------------
class InstitutionAdminAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_access'
        verbose_name = _("InstitutionAdmin Access Rule")
        verbose_name_plural = _("InstitutionAdmin Access Rules")
...
# -----------------------------------------------------------------------------
class InstitutionAdminConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_config'
        verbose_name = _("InstitutionAdmin Config Parameter")
        verbose_name_plural = _("InstitutionAdmin Config Parameters")
...
# -----------------------------------------------------------------------------
class InstitutionAdminDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("InstitutionAdmin Document Attachment")
        verbose_name_plural = _("InstitutionAdmin Document Attachments")
...
# -----------------------------------------------------------------------------
class InstitutionAdminAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on InstitutionAdmin."""
    master = models.ForeignKey(InstitutionAdminMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_institutionadmin_audit'
        ordering = ['-timestamp']
        verbose_name = _("InstitutionAdmin Audit Entry")
        verbose_name_plural = _("InstitutionAdmin Audit Entries")
