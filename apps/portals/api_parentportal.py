"""
Extended REST API Controllers for EduFlow Parent & Guardian Multi-Child Portal (ParentPortal).
...
class ParentPortalAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ParentPortalMaster, pk=pk)
        data = master.to_dict()
...
class ParentPortalAPISearchView(LoginRequiredMixin, View):
...
class ParentPortalAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ParentPortalAPITelemetryView(LoginRequiredMixin, View):
