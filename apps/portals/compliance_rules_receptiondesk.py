"""
Regulatory Compliance & Multi-Tenant Governance Rules for EduFlow Reception & Front Office Desk (ReceptionDesk).
Module: apps.portals
...
class ReceptionDeskComplianceEvaluator:
    """Enterprise multi-tenant compliance evaluator and security guard for ReceptionDesk."""
    ...
    def register_violation(self, rule_id, severity, description, entity_id=None):
        ...
        logger.warning(f"[ReceptionDesk Compliance Violation] {rule_id} [{severity}]: {description}")
        return violation
    ...
    def generate_privacy_impact_assessment(self, data_flow_categories):
        """Generates comprehensive Privacy Impact Assessment (PIA) for ReceptionDesk operations."""
        risk_score = 0
        findings = []
