"""
Models for EduFlow Examination Coordinator Suite (ExamDesk) in apps.portals.
...
# -----------------------------------------------------------------------------
class ExamDeskStatusChoices(models.TextChoices):
...
class ExamDeskPriorityChoices(models.TextChoices):
...
class ExamDeskTierChoices(models.TextChoices):
...
class ExamDeskScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ExamDeskMasterQuerySet(models.QuerySet):
    def active(self):
...
class ExamDeskMasterManager(models.Manager):
    def get_queryset(self):
        return ExamDeskMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ExamDeskMaster(models.Model):
    """
    Primary master configuration entity for Examination Coordinator Suite.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ExamDeskStatusChoices
    PriorityChoices = ExamDeskPriorityChoices
    TierChoices = ExamDeskTierChoices
    ScopeChoices = ExamDeskScopeChoices
    ...
    objects = ExamDeskMasterManager()

    class Meta:
        db_table = 'eduflow_portals_examdesk_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("ExamDesk Master")
        verbose_name_plural = _("ExamDesk Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_examd_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_examd_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_examd_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_examd_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:examdesk_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/examdesk/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ExamDeskItem(models.Model):
    """Subordinate component or line transaction item for ExamDesk."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("ExamDesk Item")
        verbose_name_plural = _("ExamDesk Items")
...
# -----------------------------------------------------------------------------
class ExamDeskAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for ExamDesk."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_allocation'
        ordering = ['-start_time']
        verbose_name = _("ExamDesk Allocation")
        verbose_name_plural = _("ExamDesk Allocations")
...
# -----------------------------------------------------------------------------
class ExamDeskMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for ExamDesk."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("ExamDesk Metric Record")
        verbose_name_plural = _("ExamDesk Metric Records")
...
# -----------------------------------------------------------------------------
class ExamDeskPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for ExamDesk."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_rule'
        ordering = ['rule_code']
        verbose_name = _("ExamDesk Policy Rule")
        verbose_name_plural = _("ExamDesk Policy Rules")
...
# -----------------------------------------------------------------------------
class ExamDeskSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for ExamDesk."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("ExamDesk Schedule Period")
        verbose_name_plural = _("ExamDesk Schedule Periods")
...
# -----------------------------------------------------------------------------
class ExamDeskFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for ExamDesk."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_review'
        ordering = ['-submitted_at']
        verbose_name = _("ExamDesk Review")
        verbose_name_plural = _("ExamDesk Reviews")
...
# -----------------------------------------------------------------------------
class ExamDeskWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("ExamDesk Workflow Transition")
        verbose_name_plural = _("ExamDesk Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ExamDeskAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_access'
        verbose_name = _("ExamDesk Access Rule")
        verbose_name_plural = _("ExamDesk Access Rules")
...
# -----------------------------------------------------------------------------
class ExamDeskConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_config'
        verbose_name = _("ExamDesk Config Parameter")
        verbose_name_plural = _("ExamDesk Config Parameters")
...
# -----------------------------------------------------------------------------
class ExamDeskDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("ExamDesk Document Attachment")
        verbose_name_plural = _("ExamDesk Document Attachments")
...
# -----------------------------------------------------------------------------
class ExamDeskAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on ExamDesk."""
    master = models.ForeignKey(ExamDeskMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_examdesk_audit'
        ordering = ['-timestamp']
        verbose_name = _("ExamDesk Audit Entry")
        verbose_name_plural = _("ExamDesk Audit Entries")
