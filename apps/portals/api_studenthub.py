"""
Extended REST API Controllers for EduFlow Student Learning Hub & Portal (StudentHub).
...
class StudentHubAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(StudentHubMaster, pk=pk)
        data = master.to_dict()
...
class StudentHubAPISearchView(LoginRequiredMixin, View):
...
class StudentHubAPIBatchUpdateView(LoginRequiredMixin, View):
...
class StudentHubAPITelemetryView(LoginRequiredMixin, View):
