"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Institution Administrator Portal (InstitutionAdmin).
...
class InstitutionAdminDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for InstitutionAdmin."""
    template_name = 'portals/institutionadmin_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = InstitutionAdminAuditTrail.objects.all().order_by('-timestamp')[:10] if 'InstitutionAdminAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Institution Administrator Portal Operations Dashboard",
            'domain_name': 'InstitutionAdmin',
            'app_title': 'Portals',
...
class InstitutionAdminAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'portals/institutionadmin_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Institution Administrator Portal Analytical Insights",
