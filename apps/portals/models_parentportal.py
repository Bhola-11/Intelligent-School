"""
Models for EduFlow Parent & Guardian Multi-Child Portal (ParentPortal) in apps.portals.
...
# -----------------------------------------------------------------------------
class ParentPortalStatusChoices(models.TextChoices):
...
class ParentPortalPriorityChoices(models.TextChoices):
...
class ParentPortalTierChoices(models.TextChoices):
...
class ParentPortalScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ParentPortalMasterQuerySet(models.QuerySet):
    def active(self):
...
class ParentPortalMasterManager(models.Manager):
    def get_queryset(self):
        return ParentPortalMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ParentPortalMaster(models.Model):
    """
    Primary master configuration entity for Parent & Guardian Multi-Child Portal.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ParentPortalStatusChoices
    PriorityChoices = ParentPortalPriorityChoices
    TierChoices = ParentPortalTierChoices
    ScopeChoices = ParentPortalScopeChoices
    ...
    objects = ParentPortalMasterManager()

    class Meta:
        db_table = 'eduflow_portals_parentportal_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("ParentPortal Master")
        verbose_name_plural = _("ParentPortal Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_paren_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_paren_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_paren_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_paren_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:parentportal_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/parentportal/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ParentPortalItem(models.Model):
    """Subordinate component or line transaction item for ParentPortal."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("ParentPortal Item")
        verbose_name_plural = _("ParentPortal Items")
...
# -----------------------------------------------------------------------------
class ParentPortalAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for ParentPortal."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_allocation'
        ordering = ['-start_time']
        verbose_name = _("ParentPortal Allocation")
        verbose_name_plural = _("ParentPortal Allocations")
...
# -----------------------------------------------------------------------------
class ParentPortalMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for ParentPortal."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("ParentPortal Metric Record")
        verbose_name_plural = _("ParentPortal Metric Records")
...
# -----------------------------------------------------------------------------
class ParentPortalPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for ParentPortal."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_rule'
        ordering = ['rule_code']
        verbose_name = _("ParentPortal Policy Rule")
        verbose_name_plural = _("ParentPortal Policy Rules")
...
# -----------------------------------------------------------------------------
class ParentPortalSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for ParentPortal."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("ParentPortal Schedule Period")
        verbose_name_plural = _("ParentPortal Schedule Periods")
...
# -----------------------------------------------------------------------------
class ParentPortalFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for ParentPortal."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_review'
        ordering = ['-submitted_at']
        verbose_name = _("ParentPortal Review")
        verbose_name_plural = _("ParentPortal Reviews")
...
# -----------------------------------------------------------------------------
class ParentPortalWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("ParentPortal Workflow Transition")
        verbose_name_plural = _("ParentPortal Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ParentPortalAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_access'
        verbose_name = _("ParentPortal Access Rule")
        verbose_name_plural = _("ParentPortal Access Rules")
...
# -----------------------------------------------------------------------------
class ParentPortalConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_config'
        verbose_name = _("ParentPortal Config Parameter")
        verbose_name_plural = _("ParentPortal Config Parameters")
...
# -----------------------------------------------------------------------------
class ParentPortalDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("ParentPortal Document Attachment")
        verbose_name_plural = _("ParentPortal Document Attachments")
...
# -----------------------------------------------------------------------------
class ParentPortalAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on ParentPortal."""
    master = models.ForeignKey(ParentPortalMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_parentportal_audit'
        ordering = ['-timestamp']
        verbose_name = _("ParentPortal Audit Entry")
        verbose_name_plural = _("ParentPortal Audit Entries")
