"""
Models for EduFlow Reception & Front Office Desk (ReceptionDesk) in apps.portals.
...
# -----------------------------------------------------------------------------
class ReceptionDeskStatusChoices(models.TextChoices):
...
class ReceptionDeskPriorityChoices(models.TextChoices):
...
class ReceptionDeskTierChoices(models.TextChoices):
...
class ReceptionDeskScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ReceptionDeskMasterQuerySet(models.QuerySet):
    def active(self):
...
class ReceptionDeskMasterManager(models.Manager):
    def get_queryset(self):
        return ReceptionDeskMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ReceptionDeskMaster(models.Model):
    """
    Primary master configuration entity for Reception & Front Office Desk.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ReceptionDeskStatusChoices
    PriorityChoices = ReceptionDeskPriorityChoices
    TierChoices = ReceptionDeskTierChoices
    ScopeChoices = ReceptionDeskScopeChoices
    ...
    objects = ReceptionDeskMasterManager()

    class Meta:
        db_table = 'eduflow_portals_receptiondesk_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("ReceptionDesk Master")
        verbose_name_plural = _("ReceptionDesk Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_recep_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_recep_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_recep_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_recep_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:receptiondesk_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/receptiondesk/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ReceptionDeskItem(models.Model):
    """Subordinate component or line transaction item for ReceptionDesk."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("ReceptionDesk Item")
        verbose_name_plural = _("ReceptionDesk Items")
...
# -----------------------------------------------------------------------------
class ReceptionDeskAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for ReceptionDesk."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_allocation'
        ordering = ['-start_time']
        verbose_name = _("ReceptionDesk Allocation")
        verbose_name_plural = _("ReceptionDesk Allocations")
...
# -----------------------------------------------------------------------------
class ReceptionDeskMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for ReceptionDesk."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("ReceptionDesk Metric Record")
        verbose_name_plural = _("ReceptionDesk Metric Records")
...
# -----------------------------------------------------------------------------
class ReceptionDeskPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for ReceptionDesk."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_rule'
        ordering = ['rule_code']
        verbose_name = _("ReceptionDesk Policy Rule")
        verbose_name_plural = _("ReceptionDesk Policy Rules")
...
# -----------------------------------------------------------------------------
class ReceptionDeskSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for ReceptionDesk."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("ReceptionDesk Schedule Period")
        verbose_name_plural = _("ReceptionDesk Schedule Periods")
...
# -----------------------------------------------------------------------------
class ReceptionDeskFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for ReceptionDesk."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_review'
        ordering = ['-submitted_at']
        verbose_name = _("ReceptionDesk Review")
        verbose_name_plural = _("ReceptionDesk Reviews")
...
# -----------------------------------------------------------------------------
class ReceptionDeskWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("ReceptionDesk Workflow Transition")
        verbose_name_plural = _("ReceptionDesk Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ReceptionDeskAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_access'
        verbose_name = _("ReceptionDesk Access Rule")
        verbose_name_plural = _("ReceptionDesk Access Rules")
...
# -----------------------------------------------------------------------------
class ReceptionDeskConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_config'
        verbose_name = _("ReceptionDesk Config Parameter")
        verbose_name_plural = _("ReceptionDesk Config Parameters")
...
# -----------------------------------------------------------------------------
class ReceptionDeskDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("ReceptionDesk Document Attachment")
        verbose_name_plural = _("ReceptionDesk Document Attachments")
...
# -----------------------------------------------------------------------------
class ReceptionDeskAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on ReceptionDesk."""
    master = models.ForeignKey(ReceptionDeskMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_receptiondesk_audit'
        ordering = ['-timestamp']
        verbose_name = _("ReceptionDesk Audit Entry")
        verbose_name_plural = _("ReceptionDesk Audit Entries")
