"""
Extended REST API Controllers for EduFlow Institution Administrator Portal (InstitutionAdmin).
...
class InstitutionAdminAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(InstitutionAdminMaster, pk=pk)
        data = master.to_dict()
...
class InstitutionAdminAPISearchView(LoginRequiredMixin, View):
...
class InstitutionAdminAPIBatchUpdateView(LoginRequiredMixin, View):
...
class InstitutionAdminAPITelemetryView(LoginRequiredMixin, View):
