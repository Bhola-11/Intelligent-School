"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Librarian Operations Center (LibrarianDesk).
...
class LibrarianDeskDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for LibrarianDesk."""
    template_name = 'portals/librariandesk_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = LibrarianDeskAuditTrail.objects.all().order_by('-timestamp')[:10] if 'LibrarianDeskAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Librarian Operations Center Operations Dashboard",
            'domain_name': 'LibrarianDesk',
            'app_title': 'Portals',
...
class LibrarianDeskAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'portals/librariandesk_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Librarian Operations Center Analytical Insights",
