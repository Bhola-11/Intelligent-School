"""
Models for EduFlow Librarian Operations Center (LibrarianDesk) in apps.portals.
...
# -----------------------------------------------------------------------------
class LibrarianDeskStatusChoices(models.TextChoices):
...
class LibrarianDeskPriorityChoices(models.TextChoices):
...
class LibrarianDeskTierChoices(models.TextChoices):
...
class LibrarianDeskScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class LibrarianDeskMasterQuerySet(models.QuerySet):
    def active(self):
...
class LibrarianDeskMasterManager(models.Manager):
    def get_queryset(self):
        return LibrarianDeskMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class LibrarianDeskMaster(models.Model):
    """
    Primary master configuration entity for Librarian Operations Center.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = LibrarianDeskStatusChoices
    PriorityChoices = LibrarianDeskPriorityChoices
    TierChoices = LibrarianDeskTierChoices
    ScopeChoices = LibrarianDeskScopeChoices
    ...
    objects = LibrarianDeskMasterManager()

    class Meta:
        db_table = 'eduflow_portals_librariandesk_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("LibrarianDesk Master")
        verbose_name_plural = _("LibrarianDesk Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_libra_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_libra_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_libra_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_libra_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'portals:librariandesk_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/portals/librariandesk/{self.pk}/"
...
# -----------------------------------------------------------------------------
class LibrarianDeskItem(models.Model):
    """Subordinate component or line transaction item for LibrarianDesk."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("LibrarianDesk Item")
        verbose_name_plural = _("LibrarianDesk Items")
...
# -----------------------------------------------------------------------------
class LibrarianDeskAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for LibrarianDesk."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_allocation'
        ordering = ['-start_time']
        verbose_name = _("LibrarianDesk Allocation")
        verbose_name_plural = _("LibrarianDesk Allocations")
...
# -----------------------------------------------------------------------------
class LibrarianDeskMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for LibrarianDesk."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("LibrarianDesk Metric Record")
        verbose_name_plural = _("LibrarianDesk Metric Records")
...
# -----------------------------------------------------------------------------
class LibrarianDeskPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for LibrarianDesk."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_rule'
        ordering = ['rule_code']
        verbose_name = _("LibrarianDesk Policy Rule")
        verbose_name_plural = _("LibrarianDesk Policy Rules")
...
# -----------------------------------------------------------------------------
class LibrarianDeskSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for LibrarianDesk."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("LibrarianDesk Schedule Period")
        verbose_name_plural = _("LibrarianDesk Schedule Periods")
...
# -----------------------------------------------------------------------------
class LibrarianDeskFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for LibrarianDesk."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_review'
        ordering = ['-submitted_at']
        verbose_name = _("LibrarianDesk Review")
        verbose_name_plural = _("LibrarianDesk Reviews")
...
# -----------------------------------------------------------------------------
class LibrarianDeskWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("LibrarianDesk Workflow Transition")
        verbose_name_plural = _("LibrarianDesk Workflow Transitions")
...
# -----------------------------------------------------------------------------
class LibrarianDeskAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_access'
        verbose_name = _("LibrarianDesk Access Rule")
        verbose_name_plural = _("LibrarianDesk Access Rules")
...
# -----------------------------------------------------------------------------
class LibrarianDeskConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_config'
        verbose_name = _("LibrarianDesk Config Parameter")
        verbose_name_plural = _("LibrarianDesk Config Parameters")
...
# -----------------------------------------------------------------------------
class LibrarianDeskDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("LibrarianDesk Document Attachment")
        verbose_name_plural = _("LibrarianDesk Document Attachments")
...
# -----------------------------------------------------------------------------
class LibrarianDeskAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on LibrarianDesk."""
    master = models.ForeignKey(LibrarianDeskMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_portals_librariandesk_audit'
        ordering = ['-timestamp']
        verbose_name = _("LibrarianDesk Audit Entry")
        verbose_name_plural = _("LibrarianDesk Audit Entries")
