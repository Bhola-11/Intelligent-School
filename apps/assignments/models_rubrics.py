"""
Models for EduFlow Assignment Attachments & Rubric Matrix (Rubrics) in apps.assignments.
...
# -----------------------------------------------------------------------------
class RubricsStatusChoices(models.TextChoices):
...
class RubricsPriorityChoices(models.TextChoices):
...
class RubricsTierChoices(models.TextChoices):
...
class RubricsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class RubricsMasterQuerySet(models.QuerySet):
    def active(self):
...
class RubricsMasterManager(models.Manager):
    def get_queryset(self):
        return RubricsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class RubricsMaster(models.Model):
    """
    Primary master configuration entity for Assignment Attachments & Rubric Matrix.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = RubricsStatusChoices
    PriorityChoices = RubricsPriorityChoices
    TierChoices = RubricsTierChoices
    ScopeChoices = RubricsScopeChoices
    ...
    objects = RubricsMasterManager()

    class Meta:
        db_table = 'eduflow_assignments_rubrics_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Rubrics Master")
        verbose_name_plural = _("Rubrics Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_rubri_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_rubri_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_rubri_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_rubri_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'assignments:rubrics_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/assignments/rubrics/{self.pk}/"
...
class RubricsItem(models.Model):
    """Subordinate component or line transaction item for Rubrics."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Rubrics Item")
        verbose_name_plural = _("Rubrics Items")
...
# -----------------------------------------------------------------------------
class RubricsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Rubrics."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_allocation'
        ordering = ['-start_time']
        verbose_name = _("Rubrics Allocation")
        verbose_name_plural = _("Rubrics Allocations")
...
# -----------------------------------------------------------------------------
class RubricsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Rubrics."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Rubrics Metric Record")
        verbose_name_plural = _("Rubrics Metric Records")
...
# -----------------------------------------------------------------------------
class RubricsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Rubrics."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_rule'
        ordering = ['rule_code']
        verbose_name = _("Rubrics Policy Rule")
        verbose_name_plural = _("Rubrics Policy Rules")
...
# -----------------------------------------------------------------------------
class RubricsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Rubrics."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Rubrics Schedule Period")
        verbose_name_plural = _("Rubrics Schedule Periods")
...
# -----------------------------------------------------------------------------
class RubricsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Rubrics."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_review'
        ordering = ['-submitted_at']
        verbose_name = _("Rubrics Review")
        verbose_name_plural = _("Rubrics Reviews")
...
# -----------------------------------------------------------------------------
class RubricsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Rubrics Workflow Transition")
        verbose_name_plural = _("Rubrics Workflow Transitions")
...
# -----------------------------------------------------------------------------
class RubricsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_access'
        verbose_name = _("Rubrics Access Rule")
        verbose_name_plural = _("Rubrics Access Rules")
...
# -----------------------------------------------------------------------------
class RubricsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_config'
        verbose_name = _("Rubrics Config Parameter")
        verbose_name_plural = _("Rubrics Config Parameters")
...
# -----------------------------------------------------------------------------
class RubricsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Rubrics Document Attachment")
        verbose_name_plural = _("Rubrics Document Attachments")
...
# -----------------------------------------------------------------------------
class RubricsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Rubrics."""
    master = models.ForeignKey(RubricsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_assignments_rubrics_audit'
        ordering = ['-timestamp']
        verbose_name = _("Rubrics Audit Entry")
        verbose_name_plural = _("Rubrics Audit Entries")
