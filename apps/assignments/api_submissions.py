"""
Extended REST API Controllers for EduFlow Student Assignment Submissions Desk (Submissions).
...
class SubmissionsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(SubmissionsMaster, pk=pk)
        data = master.to_dict()
...
class SubmissionsAPISearchView(LoginRequiredMixin, View):
...
class SubmissionsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class SubmissionsAPITelemetryView(LoginRequiredMixin, View):
