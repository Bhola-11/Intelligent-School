"""
Extended REST API Controllers for EduFlow Assignment Attachments & Rubric Matrix (Rubrics).
...
class RubricsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(RubricsMaster, pk=pk)
        data = master.to_dict()
...
class RubricsAPISearchView(LoginRequiredMixin, View):
...
class RubricsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class RubricsAPITelemetryView(LoginRequiredMixin, View):
