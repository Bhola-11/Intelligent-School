"""
Extended REST API Controllers for EduFlow Assignment Grading, Rubrics & Feedback (GradingFeedback).
...
class GradingFeedbackAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(GradingFeedbackMaster, pk=pk)
        data = master.to_dict()
...
class GradingFeedbackAPISearchView(LoginRequiredMixin, View):
...
class GradingFeedbackAPIBatchUpdateView(LoginRequiredMixin, View):
...
class GradingFeedbackAPITelemetryView(LoginRequiredMixin, View):
