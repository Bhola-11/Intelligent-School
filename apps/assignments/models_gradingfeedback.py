"""
Models for EduFlow Assignment Grading, Rubrics & Feedback (GradingFeedback) in apps.assignments.
...
# -----------------------------------------------------------------------------
class GradingFeedbackStatusChoices(models.TextChoices):
...
class GradingFeedbackPriorityChoices(models.TextChoices):
...
class GradingFeedbackTierChoices(models.TextChoices):
...
class GradingFeedbackScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class GradingFeedbackMasterQuerySet(models.QuerySet):
    def active(self):
...
class GradingFeedbackMasterManager(models.Manager):
    def get_queryset(self):
        return GradingFeedbackMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class GradingFeedbackMaster(models.Model):
    """
    Primary master configuration entity for Assignment Grading, Rubrics & Feedback.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = GradingFeedbackStatusChoices
    PriorityChoices = GradingFeedbackPriorityChoices
    TierChoices = GradingFeedbackTierChoices
    ScopeChoices = GradingFeedbackScopeChoices
    ...
    objects = GradingFeedbackMasterManager()

    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("GradingFeedback Master")
        verbose_name_plural = _("GradingFeedback Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_gradi_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_gradi_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_gradi_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_gradi_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'assignments:gradingfeedback_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/assignments/gradingfeedback/{self.pk}/"
...
# -----------------------------------------------------------------------------
class GradingFeedbackItem(models.Model):
    """Subordinate component or line transaction item for GradingFeedback."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("GradingFeedback Item")
        verbose_name_plural = _("GradingFeedback Items")
...
# -----------------------------------------------------------------------------
class GradingFeedbackAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for GradingFeedback."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_allocation'
        ordering = ['-start_time']
        verbose_name = _("GradingFeedback Allocation")
        verbose_name_plural = _("GradingFeedback Allocations")
...
# -----------------------------------------------------------------------------
class GradingFeedbackMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for GradingFeedback."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("GradingFeedback Metric Record")
        verbose_name_plural = _("GradingFeedback Metric Records")
...
# -----------------------------------------------------------------------------
class GradingFeedbackPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for GradingFeedback."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_rule'
        ordering = ['rule_code']
        verbose_name = _("GradingFeedback Policy Rule")
        verbose_name_plural = _("GradingFeedback Policy Rules")
...
# -----------------------------------------------------------------------------
class GradingFeedbackSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for GradingFeedback."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("GradingFeedback Schedule Period")
        verbose_name_plural = _("GradingFeedback Schedule Periods")
...
# -----------------------------------------------------------------------------
class GradingFeedbackFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for GradingFeedback."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_review'
        ordering = ['-submitted_at']
        verbose_name = _("GradingFeedback Review")
        verbose_name_plural = _("GradingFeedback Reviews")
...
# -----------------------------------------------------------------------------
class GradingFeedbackWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("GradingFeedback Workflow Transition")
        verbose_name_plural = _("GradingFeedback Workflow Transitions")
...
# -----------------------------------------------------------------------------
class GradingFeedbackAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_access'
        verbose_name = _("GradingFeedback Access Rule")
        verbose_name_plural = _("GradingFeedback Access Rules")
...
# -----------------------------------------------------------------------------
class GradingFeedbackConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_config'
        verbose_name = _("GradingFeedback Config Parameter")
        verbose_name_plural = _("GradingFeedback Config Parameters")
...
# -----------------------------------------------------------------------------
class GradingFeedbackDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("GradingFeedback Document Attachment")
        verbose_name_plural = _("GradingFeedback Document Attachments")
...
# -----------------------------------------------------------------------------
class GradingFeedbackAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on GradingFeedback."""
    master = models.ForeignKey(GradingFeedbackMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_assignments_gradingfeedback_audit'
        ordering = ['-timestamp']
        verbose_name = _("GradingFeedback Audit Entry")
        verbose_name_plural = _("GradingFeedback Audit Entries")
