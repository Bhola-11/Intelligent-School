"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Student Transport Subscriptions & Fees (TransportSubs).
...
class TransportSubsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for TransportSubs."""
    template_name = 'transport/transportsubs_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = TransportSubsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'TransportSubsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Student Transport Subscriptions & Fees Operations Dashboard",
            'domain_name': 'TransportSubs',
            'app_title': 'Transport',
...
class TransportSubsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'transport/transportsubs_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Student Transport Subscriptions & Fees Analytical Insights",
