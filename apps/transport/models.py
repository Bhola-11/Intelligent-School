"""
Models for EduFlow Transport Routes, Stops & Timings (Routes) in apps.transport.
...
# -----------------------------------------------------------------------------
class RoutesStatusChoices(models.TextChoices):
...
class RoutesPriorityChoices(models.TextChoices):
...
class RoutesTierChoices(models.TextChoices):
...
class RoutesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class RoutesMasterQuerySet(models.QuerySet):
    def active(self):
...
class RoutesMasterManager(models.Manager):
    def get_queryset(self):
        return RoutesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class RoutesMaster(models.Model):
    """
    Primary master configuration entity for Transport Routes, Stops & Timings.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = RoutesStatusChoices
    PriorityChoices = RoutesPriorityChoices
    TierChoices = RoutesTierChoices
    ScopeChoices = RoutesScopeChoices
    ...
    objects = RoutesMasterManager()

    class Meta:
        db_table = 'eduflow_transport_routes_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Routes Master")
        verbose_name_plural = _("Routes Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_route_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_route_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_route_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_route_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'transport:routes_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/transport/routes/{self.pk}/"
...
# -----------------------------------------------------------------------------
class RoutesItem(models.Model):
    """Subordinate component or line transaction item for Routes."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Routes Item")
        verbose_name_plural = _("Routes Items")
...
# -----------------------------------------------------------------------------
class RoutesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Routes."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_allocation'
        ordering = ['-start_time']
        verbose_name = _("Routes Allocation")
        verbose_name_plural = _("Routes Allocations")
...
# -----------------------------------------------------------------------------
class RoutesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Routes."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Routes Metric Record")
        verbose_name_plural = _("Routes Metric Records")
...
# -----------------------------------------------------------------------------
class RoutesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Routes."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_rule'
        ordering = ['rule_code']
        verbose_name = _("Routes Policy Rule")
        verbose_name_plural = _("Routes Policy Rules")
...
# -----------------------------------------------------------------------------
class RoutesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Routes."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Routes Schedule Period")
        verbose_name_plural = _("Routes Schedule Periods")
...
# -----------------------------------------------------------------------------
class RoutesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Routes."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_review'
        ordering = ['-submitted_at']
        verbose_name = _("Routes Review")
        verbose_name_plural = _("Routes Reviews")
...
# -----------------------------------------------------------------------------
class RoutesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Routes Workflow Transition")
        verbose_name_plural = _("Routes Workflow Transitions")
...
# -----------------------------------------------------------------------------
class RoutesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_access'
        verbose_name = _("Routes Access Rule")
        verbose_name_plural = _("Routes Access Rules")
...
# -----------------------------------------------------------------------------
class RoutesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_config'
        verbose_name = _("Routes Config Parameter")
        verbose_name_plural = _("Routes Config Parameters")
...
# -----------------------------------------------------------------------------
class RoutesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Routes Document Attachment")
        verbose_name_plural = _("Routes Document Attachments")
...
# -----------------------------------------------------------------------------
class RoutesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Routes."""
    master = models.ForeignKey(RoutesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_transport_routes_audit'
        ordering = ['-timestamp']
        verbose_name = _("Routes Audit Entry")
        verbose_name_plural = _("Routes Audit Entries")
