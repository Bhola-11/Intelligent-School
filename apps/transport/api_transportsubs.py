"""
Extended REST API Controllers for EduFlow Student Transport Subscriptions & Fees (TransportSubs).
...
class TransportSubsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(TransportSubsMaster, pk=pk)
        data = master.to_dict()
...
class TransportSubsAPISearchView(LoginRequiredMixin, View):
...
class TransportSubsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class TransportSubsAPITelemetryView(LoginRequiredMixin, View):
