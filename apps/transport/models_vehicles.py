"""
Models for EduFlow Vehicles, Drivers & Fleet Maintenance (Vehicles) in apps.transport.
...
# -----------------------------------------------------------------------------
class VehiclesStatusChoices(models.TextChoices):
...
class VehiclesPriorityChoices(models.TextChoices):
...
class VehiclesTierChoices(models.TextChoices):
...
class VehiclesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class VehiclesMasterQuerySet(models.QuerySet):
    def active(self):
...
class VehiclesMasterManager(models.Manager):
    def get_queryset(self):
        return VehiclesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class VehiclesMaster(models.Model):
    """
    Primary master configuration entity for Vehicles, Drivers & Fleet Maintenance.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = VehiclesStatusChoices
    PriorityChoices = VehiclesPriorityChoices
    TierChoices = VehiclesTierChoices
    ScopeChoices = VehiclesScopeChoices
    ...
    objects = VehiclesMasterManager()

    class Meta:
        db_table = 'eduflow_transport_vehicles_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Vehicles Master")
        verbose_name_plural = _("Vehicles Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_vehic_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_vehic_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_vehic_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_vehic_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'transport:vehicles_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/transport/vehicles/{self.pk}/"
...
# -----------------------------------------------------------------------------
class VehiclesItem(models.Model):
    """Subordinate component or line transaction item for Vehicles."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Vehicles Item")
        verbose_name_plural = _("Vehicles Items")
...
# -----------------------------------------------------------------------------
class VehiclesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Vehicles."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_allocation'
        ordering = ['-start_time']
        verbose_name = _("Vehicles Allocation")
        verbose_name_plural = _("Vehicles Allocations")
...
# -----------------------------------------------------------------------------
class VehiclesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Vehicles."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Vehicles Metric Record")
        verbose_name_plural = _("Vehicles Metric Records")
...
# -----------------------------------------------------------------------------
class VehiclesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Vehicles."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_rule'
        ordering = ['rule_code']
        verbose_name = _("Vehicles Policy Rule")
        verbose_name_plural = _("Vehicles Policy Rules")
...
# -----------------------------------------------------------------------------
class VehiclesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Vehicles."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Vehicles Schedule Period")
        verbose_name_plural = _("Vehicles Schedule Periods")
...
# -----------------------------------------------------------------------------
class VehiclesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Vehicles."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_review'
        ordering = ['-submitted_at']
        verbose_name = _("Vehicles Review")
        verbose_name_plural = _("Vehicles Reviews")
...
# -----------------------------------------------------------------------------
class VehiclesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Vehicles Workflow Transition")
        verbose_name_plural = _("Vehicles Workflow Transitions")
...
# -----------------------------------------------------------------------------
class VehiclesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_access'
        verbose_name = _("Vehicles Access Rule")
        verbose_name_plural = _("Vehicles Access Rules")
...
# -----------------------------------------------------------------------------
class VehiclesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_config'
        verbose_name = _("Vehicles Config Parameter")
        verbose_name_plural = _("Vehicles Config Parameters")
...
# -----------------------------------------------------------------------------
class VehiclesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Vehicles Document Attachment")
        verbose_name_plural = _("Vehicles Document Attachments")
...
# -----------------------------------------------------------------------------
class VehiclesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Vehicles."""
    master = models.ForeignKey(VehiclesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_transport_vehicles_audit'
        ordering = ['-timestamp']
        verbose_name = _("Vehicles Audit Entry")
        verbose_name_plural = _("Vehicles Audit Entries")
