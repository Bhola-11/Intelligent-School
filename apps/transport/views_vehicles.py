"""
Views for EduFlow Vehicles, Drivers & Fleet Maintenance (Vehicles).
...
try:
    ...
    from .forms import (
        VehiclesMasterForm, VehiclesItemForm, VehiclesAllocationForm,
        VehiclesMetricRecordForm, VehiclesPolicyRuleForm, VehiclesSchedulePeriodForm,
        VehiclesFeedbackReviewForm, VehiclesWorkflowTransitionForm, VehiclesAccessRuleForm,
        VehiclesConfigurationParameterForm, VehiclesDocumentAttachmentForm, VehiclesFilterForm,
        VehiclesBulkActionForm
    )
except ImportError:
    try:
...
class VehiclesListView(LoginRequiredMixin, ListView):
    template_name = 'transport/vehicles_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = VehiclesMaster.objects.all() if 'VehiclesMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = VehiclesFilterForm(self.request.GET) if 'VehiclesFilterForm' in globals() else None
        ctx['bulk_form'] = VehiclesBulkActionForm() if 'VehiclesBulkActionForm' in globals() else None
        ctx['total_count'] = VehiclesMaster.objects.count() if 'VehiclesMaster' in globals() else 0
        ctx['active_count'] = VehiclesMaster.objects.filter(status='ACTIVE').count() if 'VehiclesMaster' in globals() else 0
        ctx['pending_count'] = VehiclesMaster.objects.filter(status='PENDING_REVIEW').count() if 'VehiclesMaster' in globals() else 0
        ctx['page_title'] = "Vehicles, Drivers & Fleet Maintenance Directory"
        ctx['domain_name'] = "Vehicles"
        return ctx

class VehiclesDetailView(LoginRequiredMixin, DetailView):
    template_name = 'transport/vehicles_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(VehiclesMaster, pk=self.kwargs.get('pk'))
...
class VehiclesCreateView(LoginRequiredMixin, CreateView):
    template_name = 'transport/vehicles_form.html'
    form_class = VehiclesMasterForm
    success_url = reverse_lazy('transport:vehicles_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Vehicles record '{form.instance.name}' created successfully.")
...
class VehiclesUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Vehicles record updated successfully.")
        return super().form_valid(form)

class VehiclesDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'transport/vehicles_confirm_delete.html'
    success_url = reverse_lazy('transport:vehicles_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class VehiclesItemCreateView(LoginRequiredMixin, CreateView):
    form_class = VehiclesItemForm
    template_name = 'transport/vehicles_form.html'

    def form_valid(self, form):
        master = get_object_or_404(VehiclesMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('transport:vehicles_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class VehiclesItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = VehiclesItemForm
    template_name = 'transport/vehicles_form.html'

    def get_object(self):
        return get_object_or_404(VehiclesItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('transport:vehicles_detail', kwargs={'pk': self.object.master_id})

class VehiclesItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'transport/vehicles_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class VehiclesAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = VehiclesAllocationForm
    template_name = 'transport/vehicles_form.html'
...
class VehiclesAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(VehiclesAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('transport:vehicles_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class VehiclesMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = VehiclesMetricRecordForm
    template_name = 'transport/vehicles_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class VehiclesPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = VehiclesPolicyRuleForm
    template_name = 'transport/vehicles_form.html'
...
# -----------------------------------------------------------------------------
class VehiclesSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = VehiclesSchedulePeriodForm
    template_name = 'transport/vehicles_form.html'
...
# -----------------------------------------------------------------------------
class VehiclesFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = VehiclesFeedbackReviewForm
    template_name = 'transport/vehicles_form.html'
...
# -----------------------------------------------------------------------------
class VehiclesWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = VehiclesWorkflowTransitionForm
    template_name = 'transport/vehicles_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class VehiclesAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = VehiclesAccessRuleForm
    template_name = 'transport/vehicles_form.html'
...
# -----------------------------------------------------------------------------
class VehiclesConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = VehiclesConfigurationParameterForm
    template_name = 'transport/vehicles_form.html'
...
# -----------------------------------------------------------------------------
class VehiclesDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = VehiclesDocumentAttachmentForm
    template_name = 'transport/vehicles_form.html'
...
# -----------------------------------------------------------------------------
class VehiclesBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = VehiclesMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('transport:vehicles_list')

class VehiclesExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="vehicles_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
...
class VehiclesExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = VehiclesMaster.objects.all() if 'VehiclesMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class VehiclesAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = VehiclesMaster.objects.all() if 'VehiclesMaster' in globals() else []
...
class VehiclesAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(VehiclesMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
