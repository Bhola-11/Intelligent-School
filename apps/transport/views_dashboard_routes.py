"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Transport Routes, Stops & Timings (Routes).
...
class RoutesDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Routes."""
    template_name = 'transport/routes_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = RoutesAuditTrail.objects.all().order_by('-timestamp')[:10] if 'RoutesAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Transport Routes, Stops & Timings Operations Dashboard",
            'domain_name': 'Routes',
            'app_title': 'Transport',
...
class RoutesAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'transport/routes_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Transport Routes, Stops & Timings Analytical Insights",
