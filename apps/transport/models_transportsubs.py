"""
Models for EduFlow Student Transport Subscriptions & Fees (TransportSubs) in apps.transport.
...
# -----------------------------------------------------------------------------
class TransportSubsStatusChoices(models.TextChoices):
...
class TransportSubsPriorityChoices(models.TextChoices):
...
class TransportSubsTierChoices(models.TextChoices):
...
class TransportSubsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class TransportSubsMasterQuerySet(models.QuerySet):
    def active(self):
...
class TransportSubsMasterManager(models.Manager):
    def get_queryset(self):
        return TransportSubsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class TransportSubsMaster(models.Model):
    """
    Primary master configuration entity for Student Transport Subscriptions & Fees.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = TransportSubsStatusChoices
    PriorityChoices = TransportSubsPriorityChoices
    TierChoices = TransportSubsTierChoices
    ScopeChoices = TransportSubsScopeChoices
    ...
    objects = TransportSubsMasterManager()

    class Meta:
        db_table = 'eduflow_transport_transportsubs_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("TransportSubs Master")
        verbose_name_plural = _("TransportSubs Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_trans_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_trans_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_trans_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_trans_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'transport:transportsubs_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/transport/transportsubs/{self.pk}/"
...
# -----------------------------------------------------------------------------
class TransportSubsItem(models.Model):
    """Subordinate component or line transaction item for TransportSubs."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("TransportSubs Item")
        verbose_name_plural = _("TransportSubs Items")
...
# -----------------------------------------------------------------------------
class TransportSubsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for TransportSubs."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_allocation'
        ordering = ['-start_time']
        verbose_name = _("TransportSubs Allocation")
        verbose_name_plural = _("TransportSubs Allocations")
...
# -----------------------------------------------------------------------------
class TransportSubsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for TransportSubs."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("TransportSubs Metric Record")
        verbose_name_plural = _("TransportSubs Metric Records")
...
# -----------------------------------------------------------------------------
class TransportSubsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for TransportSubs."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_rule'
        ordering = ['rule_code']
        verbose_name = _("TransportSubs Policy Rule")
        verbose_name_plural = _("TransportSubs Policy Rules")
...
# -----------------------------------------------------------------------------
class TransportSubsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for TransportSubs."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("TransportSubs Schedule Period")
        verbose_name_plural = _("TransportSubs Schedule Periods")
...
# -----------------------------------------------------------------------------
class TransportSubsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for TransportSubs."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_review'
        ordering = ['-submitted_at']
        verbose_name = _("TransportSubs Review")
        verbose_name_plural = _("TransportSubs Reviews")
...
# -----------------------------------------------------------------------------
class TransportSubsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("TransportSubs Workflow Transition")
        verbose_name_plural = _("TransportSubs Workflow Transitions")
...
# -----------------------------------------------------------------------------
class TransportSubsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_access'
        verbose_name = _("TransportSubs Access Rule")
        verbose_name_plural = _("TransportSubs Access Rules")
...
# -----------------------------------------------------------------------------
class TransportSubsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_config'
        verbose_name = _("TransportSubs Config Parameter")
        verbose_name_plural = _("TransportSubs Config Parameters")
...
# -----------------------------------------------------------------------------
class TransportSubsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("TransportSubs Document Attachment")
        verbose_name_plural = _("TransportSubs Document Attachments")
...
# -----------------------------------------------------------------------------
class TransportSubsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on TransportSubs."""
    master = models.ForeignKey(TransportSubsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_transport_transportsubs_audit'
        ordering = ['-timestamp']
        verbose_name = _("TransportSubs Audit Entry")
        verbose_name_plural = _("TransportSubs Audit Entries")
