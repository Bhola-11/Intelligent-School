"""
Forms for EduFlow Student Transport Subscriptions & Fees (TransportSubs).
...
class TransportSubsMasterForm(forms.ModelForm):
    """Primary form for creating and updating TransportSubs master records."""
    class Meta:
        try:
            model = TransportSubsMaster
            fields = [
            ...
            widgets = {
                'code': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. TRAN-001'}),
...
class TransportSubsItemForm(forms.ModelForm):
    """Form for sub-items and component line-items."""
    class Meta:
        try:
            model = TransportSubsItem
...
class TransportSubsAllocationForm(forms.ModelForm):
    """Form for resource and faculty allocations."""
    class Meta:
        try:
            model = TransportSubsAllocation
...
class TransportSubsMetricRecordForm(forms.ModelForm):
    """Form for KPI metric observations."""
    class Meta:
        try:
            model = TransportSubsMetricRecord
...
class TransportSubsPolicyRuleForm(forms.ModelForm):
    """Form for institutional governance rules."""
    class Meta:
        try:
            model = TransportSubsPolicyRule
...
class TransportSubsSchedulePeriodForm(forms.ModelForm):
    """Form for timetable and schedule periods."""
    class Meta:
        try:
            model = TransportSubsSchedulePeriod
...
class TransportSubsFeedbackReviewForm(forms.ModelForm):
    """Form for qualitative feedback and evaluation."""
    class Meta:
        try:
            model = TransportSubsFeedbackReview
...
class TransportSubsWorkflowTransitionForm(forms.ModelForm):
    """Form for submitting workflow approval transitions."""
    class Meta:
        try:
            model = TransportSubsWorkflowTransition
...
class TransportSubsAccessRuleForm(forms.ModelForm):
    """Form for object-level permission rules."""
    class Meta:
        try:
            model = TransportSubsAccessRule
...
class TransportSubsConfigurationParameterForm(forms.ModelForm):
    """Form for key-value configuration parameters."""
    class Meta:
        try:
            model = TransportSubsConfigurationParameter
...
class TransportSubsDocumentAttachmentForm(forms.ModelForm):
    """Form for registering official document attachments."""
    class Meta:
        try:
            model = TransportSubsDocumentAttachment
...
class TransportSubsFilterForm(forms.Form):
...
class TransportSubsBulkActionForm(forms.Form):
