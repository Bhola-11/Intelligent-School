"""
Extended REST API Controllers for EduFlow Transport Routes, Stops & Timings (Routes).
...
class RoutesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(RoutesMaster, pk=pk)
        data = master.to_dict()
...
class RoutesAPISearchView(LoginRequiredMixin, View):
...
class RoutesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class RoutesAPITelemetryView(LoginRequiredMixin, View):
