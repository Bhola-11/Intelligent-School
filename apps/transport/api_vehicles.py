"""
Extended REST API Controllers for EduFlow Vehicles, Drivers & Fleet Maintenance (Vehicles).
...
class VehiclesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(VehiclesMaster, pk=pk)
        data = master.to_dict()
...
class VehiclesAPISearchView(LoginRequiredMixin, View):
...
class VehiclesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class VehiclesAPITelemetryView(LoginRequiredMixin, View):
