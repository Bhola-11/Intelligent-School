"""
Extended REST API Controllers for EduFlow Leave Policies & Entitlement Setup (LeavePolicies).
...
class LeavePoliciesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(LeavePoliciesMaster, pk=pk)
        data = master.to_dict()
...
class LeavePoliciesAPISearchView(LoginRequiredMixin, View):
...
class LeavePoliciesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class LeavePoliciesAPITelemetryView(LoginRequiredMixin, View):
