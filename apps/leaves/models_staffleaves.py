"""
Models for EduFlow Staff Leave Applications & Approval Workflows (StaffLeaves) in apps.leaves.
...
# -----------------------------------------------------------------------------
class StaffLeavesStatusChoices(models.TextChoices):
...
class StaffLeavesPriorityChoices(models.TextChoices):
...
class StaffLeavesTierChoices(models.TextChoices):
...
class StaffLeavesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class StaffLeavesMasterQuerySet(models.QuerySet):
    def active(self):
...
class StaffLeavesMasterManager(models.Manager):
    def get_queryset(self):
        return StaffLeavesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class StaffLeavesMaster(models.Model):
    """
    Primary master configuration entity for Staff Leave Applications & Approval Workflows.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = StaffLeavesStatusChoices
    PriorityChoices = StaffLeavesPriorityChoices
    TierChoices = StaffLeavesTierChoices
    ScopeChoices = StaffLeavesScopeChoices
    ...
    objects = StaffLeavesMasterManager()

    class Meta:
        db_table = 'eduflow_leaves_staffleaves_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("StaffLeaves Master")
        verbose_name_plural = _("StaffLeaves Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'leaves:staffleaves_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/leaves/staffleaves/{self.pk}/"
...
# -----------------------------------------------------------------------------
class StaffLeavesItem(models.Model):
    """Subordinate component or line transaction item for StaffLeaves."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("StaffLeaves Item")
        verbose_name_plural = _("StaffLeaves Items")
...
# -----------------------------------------------------------------------------
class StaffLeavesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for StaffLeaves."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_allocation'
        ordering = ['-start_time']
        verbose_name = _("StaffLeaves Allocation")
        verbose_name_plural = _("StaffLeaves Allocations")
...
# -----------------------------------------------------------------------------
class StaffLeavesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for StaffLeaves."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("StaffLeaves Metric Record")
        verbose_name_plural = _("StaffLeaves Metric Records")
...
# -----------------------------------------------------------------------------
class StaffLeavesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for StaffLeaves."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_rule'
        ordering = ['rule_code']
        verbose_name = _("StaffLeaves Policy Rule")
        verbose_name_plural = _("StaffLeaves Policy Rules")
...
# -----------------------------------------------------------------------------
class StaffLeavesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for StaffLeaves."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("StaffLeaves Schedule Period")
        verbose_name_plural = _("StaffLeaves Schedule Periods")
...
# -----------------------------------------------------------------------------
class StaffLeavesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for StaffLeaves."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_review'
        ordering = ['-submitted_at']
        verbose_name = _("StaffLeaves Review")
        verbose_name_plural = _("StaffLeaves Reviews")
...
# -----------------------------------------------------------------------------
class StaffLeavesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("StaffLeaves Workflow Transition")
        verbose_name_plural = _("StaffLeaves Workflow Transitions")
...
# -----------------------------------------------------------------------------
class StaffLeavesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_access'
        verbose_name = _("StaffLeaves Access Rule")
        verbose_name_plural = _("StaffLeaves Access Rules")
...
# -----------------------------------------------------------------------------
class StaffLeavesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_config'
        verbose_name = _("StaffLeaves Config Parameter")
        verbose_name_plural = _("StaffLeaves Config Parameters")
...
# -----------------------------------------------------------------------------
class StaffLeavesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("StaffLeaves Document Attachment")
        verbose_name_plural = _("StaffLeaves Document Attachments")
...
# -----------------------------------------------------------------------------
class StaffLeavesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on StaffLeaves."""
    master = models.ForeignKey(StaffLeavesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_leaves_staffleaves_audit'
        ordering = ['-timestamp']
        verbose_name = _("StaffLeaves Audit Entry")
        verbose_name_plural = _("StaffLeaves Audit Entries")
