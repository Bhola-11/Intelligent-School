"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Leave Policies & Entitlement Setup (LeavePolicies).
...
class LeavePoliciesDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for LeavePolicies."""
    template_name = 'leaves/leavepolicies_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = LeavePoliciesAuditTrail.objects.all().order_by('-timestamp')[:10] if 'LeavePoliciesAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Leave Policies & Entitlement Setup Operations Dashboard",
            'domain_name': 'LeavePolicies',
            'app_title': 'Leaves',
...
class LeavePoliciesAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'leaves/leavepolicies_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Leave Policies & Entitlement Setup Analytical Insights",
