"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Staff Leave Applications & Approval Workflows (StaffLeaves).
...
class StaffLeavesDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for StaffLeaves."""
    template_name = 'leaves/staffleaves_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = StaffLeavesAuditTrail.objects.all().order_by('-timestamp')[:10] if 'StaffLeavesAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Staff Leave Applications & Approval Workflows Operations Dashboard",
            'domain_name': 'StaffLeaves',
            'app_title': 'Leaves',
...
class StaffLeavesAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'leaves/staffleaves_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Staff Leave Applications & Approval Workflows Analytical Insights",
