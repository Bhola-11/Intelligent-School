"""
Extended REST API Controllers for EduFlow Student Leave Requests & Parent Consents (StudentLeaves).
...
class StudentLeavesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(StudentLeavesMaster, pk=pk)
        data = master.to_dict()
...
class StudentLeavesAPISearchView(LoginRequiredMixin, View):
...
class StudentLeavesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class StudentLeavesAPITelemetryView(LoginRequiredMixin, View):
