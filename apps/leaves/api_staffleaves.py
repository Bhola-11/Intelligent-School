"""
Extended REST API Controllers for EduFlow Staff Leave Applications & Approval Workflows (StaffLeaves).
...
class StaffLeavesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(StaffLeavesMaster, pk=pk)
        data = master.to_dict()
...
class StaffLeavesAPISearchView(LoginRequiredMixin, View):
...
class StaffLeavesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class StaffLeavesAPITelemetryView(LoginRequiredMixin, View):
