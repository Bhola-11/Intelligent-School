"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Student Leave Requests & Parent Consents (StudentLeaves).
...
class StudentLeavesDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for StudentLeaves."""
    template_name = 'leaves/studentleaves_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = StudentLeavesAuditTrail.objects.all().order_by('-timestamp')[:10] if 'StudentLeavesAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Student Leave Requests & Parent Consents Operations Dashboard",
            'domain_name': 'StudentLeaves',
            'app_title': 'Leaves',
...
class StudentLeavesAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'leaves/studentleaves_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Student Leave Requests & Parent Consents Analytical Insights",
