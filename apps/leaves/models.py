"""
Models for EduFlow Leave Policies & Entitlement Setup (LeavePolicies) in apps.leaves.
...
# -----------------------------------------------------------------------------
class LeavePoliciesStatusChoices(models.TextChoices):
...
class LeavePoliciesPriorityChoices(models.TextChoices):
...
class LeavePoliciesTierChoices(models.TextChoices):
...
class LeavePoliciesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class LeavePoliciesMasterQuerySet(models.QuerySet):
    def active(self):
...
class LeavePoliciesMasterManager(models.Manager):
    def get_queryset(self):
        return LeavePoliciesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class LeavePoliciesMaster(models.Model):
    """
    Primary master configuration entity for Leave Policies & Entitlement Setup.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = LeavePoliciesStatusChoices
    PriorityChoices = LeavePoliciesPriorityChoices
    TierChoices = LeavePoliciesTierChoices
    ScopeChoices = LeavePoliciesScopeChoices
    ...
    objects = LeavePoliciesMasterManager()

    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("LeavePolicies Master")
        verbose_name_plural = _("LeavePolicies Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_leave_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_leave_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_leave_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_leave_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'leaves:leavepolicies_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/leaves/leavepolicies/{self.pk}/"
...
# -----------------------------------------------------------------------------
class LeavePoliciesItem(models.Model):
    """Subordinate component or line transaction item for LeavePolicies."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("LeavePolicies Item")
        verbose_name_plural = _("LeavePolicies Items")
...
# -----------------------------------------------------------------------------
class LeavePoliciesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for LeavePolicies."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_allocation'
        ordering = ['-start_time']
        verbose_name = _("LeavePolicies Allocation")
        verbose_name_plural = _("LeavePolicies Allocations")
...
# -----------------------------------------------------------------------------
class LeavePoliciesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for LeavePolicies."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("LeavePolicies Metric Record")
        verbose_name_plural = _("LeavePolicies Metric Records")
...
# -----------------------------------------------------------------------------
class LeavePoliciesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for LeavePolicies."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_rule'
        ordering = ['rule_code']
        verbose_name = _("LeavePolicies Policy Rule")
        verbose_name_plural = _("LeavePolicies Policy Rules")
...
# -----------------------------------------------------------------------------
class LeavePoliciesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for LeavePolicies."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("LeavePolicies Schedule Period")
        verbose_name_plural = _("LeavePolicies Schedule Periods")
...
# -----------------------------------------------------------------------------
class LeavePoliciesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for LeavePolicies."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_review'
        ordering = ['-submitted_at']
        verbose_name = _("LeavePolicies Review")
        verbose_name_plural = _("LeavePolicies Reviews")
...
# -----------------------------------------------------------------------------
class LeavePoliciesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("LeavePolicies Workflow Transition")
        verbose_name_plural = _("LeavePolicies Workflow Transitions")
...
# -----------------------------------------------------------------------------
class LeavePoliciesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_access'
        verbose_name = _("LeavePolicies Access Rule")
        verbose_name_plural = _("LeavePolicies Access Rules")
...
# -----------------------------------------------------------------------------
class LeavePoliciesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_config'
        verbose_name = _("LeavePolicies Config Parameter")
        verbose_name_plural = _("LeavePolicies Config Parameters")
...
# -----------------------------------------------------------------------------
class LeavePoliciesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("LeavePolicies Document Attachment")
        verbose_name_plural = _("LeavePolicies Document Attachments")
...
# -----------------------------------------------------------------------------
class LeavePoliciesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on LeavePolicies."""
    master = models.ForeignKey(LeavePoliciesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_leaves_leavepolicies_audit'
        ordering = ['-timestamp']
        verbose_name = _("LeavePolicies Audit Entry")
        verbose_name_plural = _("LeavePolicies Audit Entries")
