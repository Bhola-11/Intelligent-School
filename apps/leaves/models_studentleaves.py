"""
Models for EduFlow Student Leave Requests & Parent Consents (StudentLeaves) in apps.leaves.
...
# -----------------------------------------------------------------------------
class StudentLeavesStatusChoices(models.TextChoices):
...
class StudentLeavesPriorityChoices(models.TextChoices):
...
class StudentLeavesTierChoices(models.TextChoices):
...
class StudentLeavesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class StudentLeavesMasterQuerySet(models.QuerySet):
    def active(self):
...
class StudentLeavesMasterManager(models.Manager):
    def get_queryset(self):
        return StudentLeavesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class StudentLeavesMaster(models.Model):
    """
    Primary master configuration entity for Student Leave Requests & Parent Consents.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = StudentLeavesStatusChoices
    PriorityChoices = StudentLeavesPriorityChoices
    TierChoices = StudentLeavesTierChoices
    ScopeChoices = StudentLeavesScopeChoices
    ...
    objects = StudentLeavesMasterManager()

    class Meta:
        db_table = 'eduflow_leaves_studentleaves_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("StudentLeaves Master")
        verbose_name_plural = _("StudentLeaves Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_stude_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_stude_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_stude_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_stude_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'leaves:studentleaves_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/leaves/studentleaves/{self.pk}/"
...
# -----------------------------------------------------------------------------
class StudentLeavesItem(models.Model):
    """Subordinate component or line transaction item for StudentLeaves."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("StudentLeaves Item")
        verbose_name_plural = _("StudentLeaves Items")
...
# -----------------------------------------------------------------------------
class StudentLeavesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for StudentLeaves."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_allocation'
        ordering = ['-start_time']
        verbose_name = _("StudentLeaves Allocation")
        verbose_name_plural = _("StudentLeaves Allocations")
...
# -----------------------------------------------------------------------------
class StudentLeavesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for StudentLeaves."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("StudentLeaves Metric Record")
        verbose_name_plural = _("StudentLeaves Metric Records")
...
# -----------------------------------------------------------------------------
class StudentLeavesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for StudentLeaves."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_rule'
        ordering = ['rule_code']
        verbose_name = _("StudentLeaves Policy Rule")
        verbose_name_plural = _("StudentLeaves Policy Rules")
...
# -----------------------------------------------------------------------------
class StudentLeavesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for StudentLeaves."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("StudentLeaves Schedule Period")
        verbose_name_plural = _("StudentLeaves Schedule Periods")
...
# -----------------------------------------------------------------------------
class StudentLeavesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for StudentLeaves."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_review'
        ordering = ['-submitted_at']
        verbose_name = _("StudentLeaves Review")
        verbose_name_plural = _("StudentLeaves Reviews")
...
# -----------------------------------------------------------------------------
class StudentLeavesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("StudentLeaves Workflow Transition")
        verbose_name_plural = _("StudentLeaves Workflow Transitions")
...
# -----------------------------------------------------------------------------
class StudentLeavesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_access'
        verbose_name = _("StudentLeaves Access Rule")
        verbose_name_plural = _("StudentLeaves Access Rules")
...
# -----------------------------------------------------------------------------
class StudentLeavesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_config'
        verbose_name = _("StudentLeaves Config Parameter")
        verbose_name_plural = _("StudentLeaves Config Parameters")
...
# -----------------------------------------------------------------------------
class StudentLeavesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("StudentLeaves Document Attachment")
        verbose_name_plural = _("StudentLeaves Document Attachments")
...
# -----------------------------------------------------------------------------
class StudentLeavesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on StudentLeaves."""
    master = models.ForeignKey(StudentLeavesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_leaves_studentleaves_audit'
        ordering = ['-timestamp']
        verbose_name = _("StudentLeaves Audit Entry")
        verbose_name_plural = _("StudentLeaves Audit Entries")
