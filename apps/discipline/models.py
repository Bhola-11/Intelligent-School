"""
Models for EduFlow Disciplinary Incidents & Case Management (DisciplineCases) in apps.discipline.
...
# -----------------------------------------------------------------------------
class DisciplineCasesStatusChoices(models.TextChoices):
...
class DisciplineCasesPriorityChoices(models.TextChoices):
...
class DisciplineCasesTierChoices(models.TextChoices):
...
class DisciplineCasesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class DisciplineCasesMasterQuerySet(models.QuerySet):
    def active(self):
...
class DisciplineCasesMasterManager(models.Manager):
    def get_queryset(self):
        return DisciplineCasesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class DisciplineCasesMaster(models.Model):
    """
    Primary master configuration entity for Disciplinary Incidents & Case Management.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = DisciplineCasesStatusChoices
    PriorityChoices = DisciplineCasesPriorityChoices
    TierChoices = DisciplineCasesTierChoices
    ScopeChoices = DisciplineCasesScopeChoices
    ...
    objects = DisciplineCasesMasterManager()

    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("DisciplineCases Master")
        verbose_name_plural = _("DisciplineCases Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_disci_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_disci_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_disci_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_disci_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'discipline:disciplinecases_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/discipline/disciplinecases/{self.pk}/"
...
# -----------------------------------------------------------------------------
class DisciplineCasesItem(models.Model):
    """Subordinate component or line transaction item for DisciplineCases."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("DisciplineCases Item")
        verbose_name_plural = _("DisciplineCases Items")
...
# -----------------------------------------------------------------------------
class DisciplineCasesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for DisciplineCases."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_allocation'
        ordering = ['-start_time']
        verbose_name = _("DisciplineCases Allocation")
        verbose_name_plural = _("DisciplineCases Allocations")
...
# -----------------------------------------------------------------------------
class DisciplineCasesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for DisciplineCases."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("DisciplineCases Metric Record")
        verbose_name_plural = _("DisciplineCases Metric Records")
...
# -----------------------------------------------------------------------------
class DisciplineCasesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for DisciplineCases."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_rule'
        ordering = ['rule_code']
        verbose_name = _("DisciplineCases Policy Rule")
        verbose_name_plural = _("DisciplineCases Policy Rules")
...
# -----------------------------------------------------------------------------
class DisciplineCasesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for DisciplineCases."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("DisciplineCases Schedule Period")
        verbose_name_plural = _("DisciplineCases Schedule Periods")
...
# -----------------------------------------------------------------------------
class DisciplineCasesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for DisciplineCases."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_review'
        ordering = ['-submitted_at']
        verbose_name = _("DisciplineCases Review")
        verbose_name_plural = _("DisciplineCases Reviews")
...
# -----------------------------------------------------------------------------
class DisciplineCasesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("DisciplineCases Workflow Transition")
        verbose_name_plural = _("DisciplineCases Workflow Transitions")
...
# -----------------------------------------------------------------------------
class DisciplineCasesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_access'
        verbose_name = _("DisciplineCases Access Rule")
        verbose_name_plural = _("DisciplineCases Access Rules")
...
# -----------------------------------------------------------------------------
class DisciplineCasesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_config'
        verbose_name = _("DisciplineCases Config Parameter")
        verbose_name_plural = _("DisciplineCases Config Parameters")
...
# -----------------------------------------------------------------------------
class DisciplineCasesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("DisciplineCases Document Attachment")
        verbose_name_plural = _("DisciplineCases Document Attachments")
...
# -----------------------------------------------------------------------------
class DisciplineCasesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on DisciplineCases."""
    master = models.ForeignKey(DisciplineCasesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_discipline_disciplinecases_audit'
        ordering = ['-timestamp']
        verbose_name = _("DisciplineCases Audit Entry")
        verbose_name_plural = _("DisciplineCases Audit Entries")
