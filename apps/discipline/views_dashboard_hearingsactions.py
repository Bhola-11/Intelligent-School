"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Disciplinary Hearings, Sanctions & Appeals (HearingsActions).
...
class HearingsActionsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for HearingsActions."""
    template_name = 'discipline/hearingsactions_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = HearingsActionsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'HearingsActionsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Disciplinary Hearings, Sanctions & Appeals Operations Dashboard",
            'domain_name': 'HearingsActions',
...
class HearingsActionsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'discipline/hearingsactions_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Disciplinary Hearings, Sanctions & Appeals Analytical Insights",
