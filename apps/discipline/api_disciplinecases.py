"""
Extended REST API Controllers for EduFlow Disciplinary Incidents & Case Management (DisciplineCases).
...
class DisciplineCasesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(DisciplineCasesMaster, pk=pk)
        data = master.to_dict()
...
class DisciplineCasesAPISearchView(LoginRequiredMixin, View):
...
class DisciplineCasesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class DisciplineCasesAPITelemetryView(LoginRequiredMixin, View):
