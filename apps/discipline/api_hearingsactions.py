"""
Extended REST API Controllers for EduFlow Disciplinary Hearings, Sanctions & Appeals (HearingsActions).
...
class HearingsActionsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(HearingsActionsMaster, pk=pk)
        data = master.to_dict()
...
class HearingsActionsAPISearchView(LoginRequiredMixin, View):
...
class HearingsActionsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class HearingsActionsAPITelemetryView(LoginRequiredMixin, View):
