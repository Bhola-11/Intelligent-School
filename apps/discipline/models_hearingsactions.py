"""
Models for EduFlow Disciplinary Hearings, Sanctions & Appeals (HearingsActions) in apps.discipline.
...
# -----------------------------------------------------------------------------
class HearingsActionsStatusChoices(models.TextChoices):
...
class HearingsActionsPriorityChoices(models.TextChoices):
...
class HearingsActionsTierChoices(models.TextChoices):
...
class HearingsActionsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class HearingsActionsMasterQuerySet(models.QuerySet):
    def active(self):
...
class HearingsActionsMasterManager(models.Manager):
    def get_queryset(self):
        return HearingsActionsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class HearingsActionsMaster(models.Model):
    """
    Primary master configuration entity for Disciplinary Hearings, Sanctions & Appeals.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = HearingsActionsStatusChoices
    PriorityChoices = HearingsActionsPriorityChoices
    TierChoices = HearingsActionsTierChoices
    ScopeChoices = HearingsActionsScopeChoices
    ...
    objects = HearingsActionsMasterManager()

    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("HearingsActions Master")
        verbose_name_plural = _("HearingsActions Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_heari_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_heari_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_heari_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_heari_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'discipline:hearingsactions_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/discipline/hearingsactions/{self.pk}/"
...
# -----------------------------------------------------------------------------
class HearingsActionsItem(models.Model):
    """Subordinate component or line transaction item for HearingsActions."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("HearingsActions Item")
        verbose_name_plural = _("HearingsActions Items")
...
# -----------------------------------------------------------------------------
class HearingsActionsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for HearingsActions."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_allocation'
        ordering = ['-start_time']
        verbose_name = _("HearingsActions Allocation")
        verbose_name_plural = _("HearingsActions Allocations")
...
# -----------------------------------------------------------------------------
class HearingsActionsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for HearingsActions."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("HearingsActions Metric Record")
        verbose_name_plural = _("HearingsActions Metric Records")
...
# -----------------------------------------------------------------------------
class HearingsActionsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for HearingsActions."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_rule'
        ordering = ['rule_code']
        verbose_name = _("HearingsActions Policy Rule")
        verbose_name_plural = _("HearingsActions Policy Rules")
...
# -----------------------------------------------------------------------------
class HearingsActionsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for HearingsActions."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("HearingsActions Schedule Period")
        verbose_name_plural = _("HearingsActions Schedule Periods")
...
# -----------------------------------------------------------------------------
class HearingsActionsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for HearingsActions."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_review'
        ordering = ['-submitted_at']
        verbose_name = _("HearingsActions Review")
        verbose_name_plural = _("HearingsActions Reviews")
...
# -----------------------------------------------------------------------------
class HearingsActionsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("HearingsActions Workflow Transition")
        verbose_name_plural = _("HearingsActions Workflow Transitions")
...
# -----------------------------------------------------------------------------
class HearingsActionsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_access'
        verbose_name = _("HearingsActions Access Rule")
        verbose_name_plural = _("HearingsActions Access Rules")
...
# -----------------------------------------------------------------------------
class HearingsActionsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_config'
        verbose_name = _("HearingsActions Config Parameter")
        verbose_name_plural = _("HearingsActions Config Parameters")
...
# -----------------------------------------------------------------------------
class HearingsActionsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("HearingsActions Document Attachment")
        verbose_name_plural = _("HearingsActions Document Attachments")
...
# -----------------------------------------------------------------------------
class HearingsActionsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on HearingsActions."""
    master = models.ForeignKey(HearingsActionsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_discipline_hearingsactions_audit'
        ordering = ['-timestamp']
        verbose_name = _("HearingsActions Audit Entry")
        verbose_name_plural = _("HearingsActions Audit Entries")
