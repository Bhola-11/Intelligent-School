"""
Models for EduFlow Project Scaffolding & Core Architecture (Architecture) in apps.core.
...
# -----------------------------------------------------------------------------
class ArchitectureStatusChoices(models.TextChoices):
...
class ArchitecturePriorityChoices(models.TextChoices):
...
class ArchitectureTierChoices(models.TextChoices):
...
class ArchitectureScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ArchitectureMasterQuerySet(models.QuerySet):
    def active(self):
...
class ArchitectureMasterManager(models.Manager):
    def get_queryset(self):
        return ArchitectureMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ArchitectureMaster(models.Model):
    """
    Primary master configuration entity for Project Scaffolding & Core Architecture.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ArchitectureStatusChoices
    PriorityChoices = ArchitecturePriorityChoices
    TierChoices = ArchitectureTierChoices
    ScopeChoices = ArchitectureScopeChoices
    ...
    objects = ArchitectureMasterManager()

    class Meta:
        db_table = 'eduflow_core_architecture_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Architecture Master")
        verbose_name_plural = _("Architecture Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_archi_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_archi_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_archi_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_archi_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'core:architecture_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/core/architecture/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ArchitectureItem(models.Model):
    """Subordinate component or line transaction item for Architecture."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Architecture Item")
        verbose_name_plural = _("Architecture Items")
...
# -----------------------------------------------------------------------------
class ArchitectureAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Architecture."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_allocation'
        ordering = ['-start_time']
        verbose_name = _("Architecture Allocation")
        verbose_name_plural = _("Architecture Allocations")
...
# -----------------------------------------------------------------------------
class ArchitectureMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Architecture."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Architecture Metric Record")
        verbose_name_plural = _("Architecture Metric Records")
...
# -----------------------------------------------------------------------------
class ArchitecturePolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Architecture."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_rule'
        ordering = ['rule_code']
        verbose_name = _("Architecture Policy Rule")
        verbose_name_plural = _("Architecture Policy Rules")
...
# -----------------------------------------------------------------------------
class ArchitectureSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Architecture."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Architecture Schedule Period")
        verbose_name_plural = _("Architecture Schedule Periods")
...
# -----------------------------------------------------------------------------
class ArchitectureFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Architecture."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_review'
        ordering = ['-submitted_at']
        verbose_name = _("Architecture Review")
        verbose_name_plural = _("Architecture Reviews")
...
# -----------------------------------------------------------------------------
class ArchitectureWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Architecture Workflow Transition")
        verbose_name_plural = _("Architecture Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ArchitectureAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_access'
        verbose_name = _("Architecture Access Rule")
        verbose_name_plural = _("Architecture Access Rules")
...
# -----------------------------------------------------------------------------
class ArchitectureConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_config'
        verbose_name = _("Architecture Config Parameter")
        verbose_name_plural = _("Architecture Config Parameters")
...
# -----------------------------------------------------------------------------
class ArchitectureDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Architecture Document Attachment")
        verbose_name_plural = _("Architecture Document Attachments")
...
# -----------------------------------------------------------------------------
class ArchitectureAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Architecture."""
    master = models.ForeignKey(ArchitectureMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_core_architecture_audit'
        ordering = ['-timestamp']
        verbose_name = _("Architecture Audit Entry")
        verbose_name_plural = _("Architecture Audit Entries")
