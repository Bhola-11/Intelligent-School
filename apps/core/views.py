"""
Views for EduFlow Project Scaffolding & Core Architecture (Architecture).
...
try:
    ...
    from .forms import (
        ArchitectureMasterForm, ArchitectureItemForm, ArchitectureAllocationForm,
        ArchitectureMetricRecordForm, ArchitecturePolicyRuleForm, ArchitectureSchedulePeriodForm,
        ArchitectureFeedbackReviewForm, ArchitectureWorkflowTransitionForm, ArchitectureAccessRuleForm,
        ArchitectureConfigurationParameterForm, ArchitectureDocumentAttachmentForm, ArchitectureFilterForm,
        ArchitectureBulkActionForm
    )
except ImportError:
    try:
        from .models import (
...
# -----------------------------------------------------------------------------
class ArchitectureListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = ArchitectureFilterForm(self.request.GET) if 'ArchitectureFilterForm' in globals() else None
        ctx['bulk_form'] = ArchitectureBulkActionForm() if 'ArchitectureBulkActionForm' in globals() else None
        ctx['total_count'] = ArchitectureMaster.objects.count() if 'ArchitectureMaster' in globals() else 0
        ctx['active_count'] = ArchitectureMaster.objects.filter(status='ACTIVE').count() if 'ArchitectureMaster' in globals() else 0
        ctx['pending_count'] = ArchitectureMaster.objects.filter(status='PENDING_REVIEW').count() if 'ArchitectureMaster' in globals() else 0
        ctx['page_title'] = "Project Scaffolding & Core Architecture Directory"
        ctx['domain_name'] = "Architecture"
        return ctx

class ArchitectureDetailView(LoginRequiredMixin, DetailView):
    template_name = 'core/architecture_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(ArchitectureMaster, pk=self.kwargs.get('pk'))
...
class ArchitectureCreateView(LoginRequiredMixin, CreateView):
    template_name = 'core/architecture_form.html'
    form_class = ArchitectureMasterForm
    success_url = reverse_lazy('core:architecture_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Architecture record '{form.instance.name}' created successfully.")
        return super().form_valid(form)

class ArchitectureUpdateView(LoginRequiredMixin, UpdateView):
    template_name = 'core/architecture_form.html'
    form_class = ArchitectureMasterForm
    success_url = reverse_lazy('core:architecture_list')
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Architecture record updated successfully.")
        return super().form_valid(form)

class ArchitectureDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'core/architecture_confirm_delete.html'
    success_url = reverse_lazy('core:architecture_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class ArchitectureItemCreateView(LoginRequiredMixin, CreateView):
    form_class = ArchitectureItemForm
    template_name = 'core/architecture_form.html'

    def form_valid(self, form):
        master = get_object_or_404(ArchitectureMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('core:architecture_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class ArchitectureItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = ArchitectureItemForm
    template_name = 'core/architecture_form.html'

    def get_object(self):
        return get_object_or_404(ArchitectureItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('core:architecture_detail', kwargs={'pk': self.object.master_id})

class ArchitectureItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'core/architecture_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class ArchitectureAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = ArchitectureAllocationForm
    template_name = 'core/architecture_form.html'
...
class ArchitectureAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(ArchitectureAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('core:architecture_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class ArchitectureMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = ArchitectureMetricRecordForm
    template_name = 'core/architecture_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class ArchitecturePolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = ArchitecturePolicyRuleForm
    template_name = 'core/architecture_form.html'
...
# -----------------------------------------------------------------------------
class ArchitectureSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = ArchitectureSchedulePeriodForm
    template_name = 'core/architecture_form.html'
...
# -----------------------------------------------------------------------------
class ArchitectureFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = ArchitectureFeedbackReviewForm
    template_name = 'core/architecture_form.html'
...
# -----------------------------------------------------------------------------
class ArchitectureWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = ArchitectureWorkflowTransitionForm
    template_name = 'core/architecture_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class ArchitectureAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = ArchitectureAccessRuleForm
    template_name = 'core/architecture_form.html'
...
# -----------------------------------------------------------------------------
class ArchitectureConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = ArchitectureConfigurationParameterForm
    template_name = 'core/architecture_form.html'
...
# -----------------------------------------------------------------------------
class ArchitectureDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = ArchitectureDocumentAttachmentForm
    template_name = 'core/architecture_form.html'
...
# -----------------------------------------------------------------------------
class ArchitectureBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        return redirect('core:architecture_list')

class ArchitectureExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="architecture_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
...
class ArchitectureExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = ArchitectureMaster.objects.all() if 'ArchitectureMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class ArchitectureAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = ArchitectureMaster.objects.all() if 'ArchitectureMaster' in globals() else []
...
class ArchitectureAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(ArchitectureMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
