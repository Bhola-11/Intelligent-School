"""
Models for EduFlow Core Base Models & Model Mixins (BaseModels) in apps.core.
...
# -----------------------------------------------------------------------------
class BaseModelsStatusChoices(models.TextChoices):
...
class BaseModelsPriorityChoices(models.TextChoices):
...
class BaseModelsTierChoices(models.TextChoices):
...
class BaseModelsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class BaseModelsMasterQuerySet(models.QuerySet):
    def active(self):
...
class BaseModelsMasterManager(models.Manager):
    def get_queryset(self):
        return BaseModelsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class BaseModelsMaster(models.Model):
    """
    Primary master configuration entity for Core Base Models & Model Mixins.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = BaseModelsStatusChoices
    PriorityChoices = BaseModelsPriorityChoices
    TierChoices = BaseModelsTierChoices
    ScopeChoices = BaseModelsScopeChoices
    ...
    objects = BaseModelsMasterManager()

    class Meta:
        db_table = 'eduflow_core_basemodels_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("BaseModels Master")
        verbose_name_plural = _("BaseModels Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_basem_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_basem_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_basem_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_basem_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'core:basemodels_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/core/basemodels/{self.pk}/"
...
# -----------------------------------------------------------------------------
class BaseModelsItem(models.Model):
    """Subordinate component or line transaction item for BaseModels."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("BaseModels Item")
        verbose_name_plural = _("BaseModels Items")
...
# -----------------------------------------------------------------------------
class BaseModelsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for BaseModels."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_allocation'
        ordering = ['-start_time']
        verbose_name = _("BaseModels Allocation")
        verbose_name_plural = _("BaseModels Allocations")
...
# -----------------------------------------------------------------------------
class BaseModelsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for BaseModels."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("BaseModels Metric Record")
        verbose_name_plural = _("BaseModels Metric Records")
...
# -----------------------------------------------------------------------------
class BaseModelsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for BaseModels."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_rule'
        ordering = ['rule_code']
        verbose_name = _("BaseModels Policy Rule")
        verbose_name_plural = _("BaseModels Policy Rules")
...
# -----------------------------------------------------------------------------
class BaseModelsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for BaseModels."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("BaseModels Schedule Period")
        verbose_name_plural = _("BaseModels Schedule Periods")
...
# -----------------------------------------------------------------------------
class BaseModelsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for BaseModels."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_review'
        ordering = ['-submitted_at']
        verbose_name = _("BaseModels Review")
        verbose_name_plural = _("BaseModels Reviews")
...
# -----------------------------------------------------------------------------
class BaseModelsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("BaseModels Workflow Transition")
        verbose_name_plural = _("BaseModels Workflow Transitions")
...
# -----------------------------------------------------------------------------
class BaseModelsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_access'
        verbose_name = _("BaseModels Access Rule")
        verbose_name_plural = _("BaseModels Access Rules")
...
# -----------------------------------------------------------------------------
class BaseModelsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_config'
        verbose_name = _("BaseModels Config Parameter")
        verbose_name_plural = _("BaseModels Config Parameters")
...
# -----------------------------------------------------------------------------
class BaseModelsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("BaseModels Document Attachment")
        verbose_name_plural = _("BaseModels Document Attachments")
...
# -----------------------------------------------------------------------------
class BaseModelsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on BaseModels."""
    master = models.ForeignKey(BaseModelsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_core_basemodels_audit'
        ordering = ['-timestamp']
        verbose_name = _("BaseModels Audit Entry")
        verbose_name_plural = _("BaseModels Audit Entries")
