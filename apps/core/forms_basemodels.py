"""
Forms for EduFlow Core Base Models & Model Mixins (BaseModels).
...
class BaseModelsMasterForm(forms.ModelForm):
    """Primary form for creating and updating BaseModels master records."""
    class Meta:
        try:
            model = BaseModelsMaster
            fields = [
            ...
            widgets = {
                'code': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'e.g. BASE-001'}),
...
class BaseModelsItemForm(forms.ModelForm):
    """Form for sub-items and component line-items."""
    class Meta:
        try:
            model = BaseModelsItem
...
class BaseModelsAllocationForm(forms.ModelForm):
    """Form for resource and faculty allocations."""
    class Meta:
        try:
            model = BaseModelsAllocation
...
class BaseModelsMetricRecordForm(forms.ModelForm):
    """Form for KPI metric observations."""
    class Meta:
        try:
            model = BaseModelsMetricRecord
...
class BaseModelsPolicyRuleForm(forms.ModelForm):
    """Form for institutional governance rules."""
    class Meta:
        try:
            model = BaseModelsPolicyRule
...
class BaseModelsSchedulePeriodForm(forms.ModelForm):
    """Form for timetable and schedule periods."""
    class Meta:
        try:
            model = BaseModelsSchedulePeriod
...
class BaseModelsFeedbackReviewForm(forms.ModelForm):
    """Form for qualitative feedback and evaluation."""
    class Meta:
        try:
            model = BaseModelsFeedbackReview
...
class BaseModelsWorkflowTransitionForm(forms.ModelForm):
    """Form for submitting workflow approval transitions."""
    class Meta:
        try:
            model = BaseModelsWorkflowTransition
...
class BaseModelsAccessRuleForm(forms.ModelForm):
    """Form for object-level permission rules."""
    class Meta:
        try:
            model = BaseModelsAccessRule
...
class BaseModelsConfigurationParameterForm(forms.ModelForm):
    """Form for key-value configuration parameters."""
    class Meta:
        try:
            model = BaseModelsConfigurationParameter
...
class BaseModelsDocumentAttachmentForm(forms.ModelForm):
    """Form for registering official document attachments."""
    class Meta:
        try:
            model = BaseModelsDocumentAttachment
...
class BaseModelsFilterForm(forms.Form):
...
class BaseModelsBulkActionForm(forms.Form):
