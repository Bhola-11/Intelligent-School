"""
Extended REST API Controllers for EduFlow Core Base Models & Model Mixins (BaseModels).
...
class BaseModelsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(BaseModelsMaster, pk=pk)
        data = master.to_dict()
...
class BaseModelsAPISearchView(LoginRequiredMixin, View):
...
class BaseModelsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class BaseModelsAPITelemetryView(LoginRequiredMixin, View):
