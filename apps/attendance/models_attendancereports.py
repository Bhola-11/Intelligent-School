"""
Models for EduFlow Attendance Policies, Alerts & Analytics (AttendanceReports) in apps.attendance.
...
# -----------------------------------------------------------------------------
class AttendanceReportsStatusChoices(models.TextChoices):
...
class AttendanceReportsPriorityChoices(models.TextChoices):
...
class AttendanceReportsTierChoices(models.TextChoices):
...
class AttendanceReportsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class AttendanceReportsMasterQuerySet(models.QuerySet):
    def active(self):
...
class AttendanceReportsMasterManager(models.Manager):
    def get_queryset(self):
        return AttendanceReportsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class AttendanceReportsMaster(models.Model):
    """
    Primary master configuration entity for Attendance Policies, Alerts & Analytics.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = AttendanceReportsStatusChoices
    PriorityChoices = AttendanceReportsPriorityChoices
    TierChoices = AttendanceReportsTierChoices
    ScopeChoices = AttendanceReportsScopeChoices
    ...
    objects = AttendanceReportsMasterManager()

    class Meta:
        db_table = 'eduflow_attendance_attendancereports_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("AttendanceReports Master")
        verbose_name_plural = _("AttendanceReports Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_atten_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_atten_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_atten_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_atten_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'attendance:attendancereports_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/attendance/attendancereports/{self.pk}/"
...
# -----------------------------------------------------------------------------
class AttendanceReportsItem(models.Model):
    """Subordinate component or line transaction item for AttendanceReports."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("AttendanceReports Item")
        verbose_name_plural = _("AttendanceReports Items")
...
# -----------------------------------------------------------------------------
class AttendanceReportsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for AttendanceReports."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_allocation'
        ordering = ['-start_time']
        verbose_name = _("AttendanceReports Allocation")
        verbose_name_plural = _("AttendanceReports Allocations")
...
# -----------------------------------------------------------------------------
class AttendanceReportsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for AttendanceReports."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("AttendanceReports Metric Record")
        verbose_name_plural = _("AttendanceReports Metric Records")
...
# -----------------------------------------------------------------------------
class AttendanceReportsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for AttendanceReports."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_rule'
        ordering = ['rule_code']
        verbose_name = _("AttendanceReports Policy Rule")
        verbose_name_plural = _("AttendanceReports Policy Rules")
...
# -----------------------------------------------------------------------------
class AttendanceReportsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for AttendanceReports."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("AttendanceReports Schedule Period")
        verbose_name_plural = _("AttendanceReports Schedule Periods")
...
# -----------------------------------------------------------------------------
class AttendanceReportsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for AttendanceReports."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_review'
        ordering = ['-submitted_at']
        verbose_name = _("AttendanceReports Review")
        verbose_name_plural = _("AttendanceReports Reviews")
...
# -----------------------------------------------------------------------------
class AttendanceReportsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("AttendanceReports Workflow Transition")
        verbose_name_plural = _("AttendanceReports Workflow Transitions")
...
# -----------------------------------------------------------------------------
class AttendanceReportsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_access'
        verbose_name = _("AttendanceReports Access Rule")
        verbose_name_plural = _("AttendanceReports Access Rules")
...
# -----------------------------------------------------------------------------
class AttendanceReportsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_config'
        verbose_name = _("AttendanceReports Config Parameter")
        verbose_name_plural = _("AttendanceReports Config Parameters")
...
# -----------------------------------------------------------------------------
class AttendanceReportsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("AttendanceReports Document Attachment")
        verbose_name_plural = _("AttendanceReports Document Attachments")
...
# -----------------------------------------------------------------------------
class AttendanceReportsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on AttendanceReports."""
    master = models.ForeignKey(AttendanceReportsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_attendance_attendancereports_audit'
        ordering = ['-timestamp']
        verbose_name = _("AttendanceReports Audit Entry")
        verbose_name_plural = _("AttendanceReports Audit Entries")
