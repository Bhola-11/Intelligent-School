"""
Models for EduFlow Teacher & Staff Attendance Management (StaffAttendance) in apps.attendance.
...
# -----------------------------------------------------------------------------
class StaffAttendanceStatusChoices(models.TextChoices):
...
class StaffAttendancePriorityChoices(models.TextChoices):
...
class StaffAttendanceTierChoices(models.TextChoices):
...
class StaffAttendanceScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class StaffAttendanceMasterQuerySet(models.QuerySet):
    def active(self):
...
class StaffAttendanceMasterManager(models.Manager):
    def get_queryset(self):
        return StaffAttendanceMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class StaffAttendanceMaster(models.Model):
    """
    Primary master configuration entity for Teacher & Staff Attendance Management.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = StaffAttendanceStatusChoices
    PriorityChoices = StaffAttendancePriorityChoices
    TierChoices = StaffAttendanceTierChoices
    ScopeChoices = StaffAttendanceScopeChoices
    ...
    objects = StaffAttendanceMasterManager()

    class Meta:
        db_table = 'eduflow_attendance_staffattendance_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("StaffAttendance Master")
        verbose_name_plural = _("StaffAttendance Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_staff_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_staff_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_staff_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_staff_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'attendance:staffattendance_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/attendance/staffattendance/{self.pk}/"
...
# -----------------------------------------------------------------------------
class StaffAttendanceItem(models.Model):
    """Subordinate component or line transaction item for StaffAttendance."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("StaffAttendance Item")
        verbose_name_plural = _("StaffAttendance Items")
...
# -----------------------------------------------------------------------------
class StaffAttendanceAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for StaffAttendance."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_allocation'
        ordering = ['-start_time']
        verbose_name = _("StaffAttendance Allocation")
        verbose_name_plural = _("StaffAttendance Allocations")
...
# -----------------------------------------------------------------------------
class StaffAttendanceMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for StaffAttendance."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("StaffAttendance Metric Record")
        verbose_name_plural = _("StaffAttendance Metric Records")
...
# -----------------------------------------------------------------------------
class StaffAttendancePolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for StaffAttendance."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_rule'
        ordering = ['rule_code']
        verbose_name = _("StaffAttendance Policy Rule")
        verbose_name_plural = _("StaffAttendance Policy Rules")
...
# -----------------------------------------------------------------------------
class StaffAttendanceSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for StaffAttendance."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("StaffAttendance Schedule Period")
        verbose_name_plural = _("StaffAttendance Schedule Periods")
...
# -----------------------------------------------------------------------------
class StaffAttendanceFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for StaffAttendance."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_review'
        ordering = ['-submitted_at']
        verbose_name = _("StaffAttendance Review")
        verbose_name_plural = _("StaffAttendance Reviews")
...
# -----------------------------------------------------------------------------
class StaffAttendanceWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("StaffAttendance Workflow Transition")
        verbose_name_plural = _("StaffAttendance Workflow Transitions")
...
# -----------------------------------------------------------------------------
class StaffAttendanceAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_access'
        verbose_name = _("StaffAttendance Access Rule")
        verbose_name_plural = _("StaffAttendance Access Rules")
...
# -----------------------------------------------------------------------------
class StaffAttendanceConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_config'
        verbose_name = _("StaffAttendance Config Parameter")
        verbose_name_plural = _("StaffAttendance Config Parameters")
...
# -----------------------------------------------------------------------------
class StaffAttendanceDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("StaffAttendance Document Attachment")
        verbose_name_plural = _("StaffAttendance Document Attachments")
...
# -----------------------------------------------------------------------------
class StaffAttendanceAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on StaffAttendance."""
    master = models.ForeignKey(StaffAttendanceMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_attendance_staffattendance_audit'
        ordering = ['-timestamp']
        verbose_name = _("StaffAttendance Audit Entry")
        verbose_name_plural = _("StaffAttendance Audit Entries")
