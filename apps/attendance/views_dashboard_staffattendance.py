"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Teacher & Staff Attendance Management (StaffAttendance).
...
class StaffAttendanceDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for StaffAttendance."""
    template_name = 'attendance/staffattendance_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = StaffAttendanceAuditTrail.objects.all().order_by('-timestamp')[:10] if 'StaffAttendanceAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Teacher & Staff Attendance Management Operations Dashboard",
            'domain_name': 'StaffAttendance',
...
class StaffAttendanceAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'attendance/staffattendance_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Teacher & Staff Attendance Management Analytical Insights",
