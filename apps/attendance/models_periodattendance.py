"""
Models for EduFlow Period & Subject-Wise Attendance Tracking (PeriodAttendance) in apps.attendance.
...
# -----------------------------------------------------------------------------
class PeriodAttendanceStatusChoices(models.TextChoices):
...
class PeriodAttendancePriorityChoices(models.TextChoices):
...
class PeriodAttendanceTierChoices(models.TextChoices):
...
class PeriodAttendanceScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class PeriodAttendanceMasterQuerySet(models.QuerySet):
    def active(self):
...
class PeriodAttendanceMasterManager(models.Manager):
    def get_queryset(self):
        return PeriodAttendanceMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class PeriodAttendanceMaster(models.Model):
    """
    Primary master configuration entity for Period & Subject-Wise Attendance Tracking.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = PeriodAttendanceStatusChoices
    PriorityChoices = PeriodAttendancePriorityChoices
    TierChoices = PeriodAttendanceTierChoices
    ScopeChoices = PeriodAttendanceScopeChoices
    ...
    objects = PeriodAttendanceMasterManager()

    class Meta:
        db_table = 'eduflow_attendance_periodattendance_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("PeriodAttendance Master")
        verbose_name_plural = _("PeriodAttendance Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_perio_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_perio_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_perio_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_perio_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'attendance:periodattendance_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/attendance/periodattendance/{self.pk}/"
...
# -----------------------------------------------------------------------------
class PeriodAttendanceItem(models.Model):
    """Subordinate component or line transaction item for PeriodAttendance."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("PeriodAttendance Item")
        verbose_name_plural = _("PeriodAttendance Items")
...
# -----------------------------------------------------------------------------
class PeriodAttendanceAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for PeriodAttendance."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_allocation'
        ordering = ['-start_time']
        verbose_name = _("PeriodAttendance Allocation")
        verbose_name_plural = _("PeriodAttendance Allocations")
...
# -----------------------------------------------------------------------------
class PeriodAttendanceMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for PeriodAttendance."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("PeriodAttendance Metric Record")
        verbose_name_plural = _("PeriodAttendance Metric Records")
...
# -----------------------------------------------------------------------------
class PeriodAttendancePolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for PeriodAttendance."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_rule'
        ordering = ['rule_code']
        verbose_name = _("PeriodAttendance Policy Rule")
        verbose_name_plural = _("PeriodAttendance Policy Rules")
...
# -----------------------------------------------------------------------------
class PeriodAttendanceSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for PeriodAttendance."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("PeriodAttendance Schedule Period")
        verbose_name_plural = _("PeriodAttendance Schedule Periods")
...
# -----------------------------------------------------------------------------
class PeriodAttendanceFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for PeriodAttendance."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_review'
        ordering = ['-submitted_at']
        verbose_name = _("PeriodAttendance Review")
        verbose_name_plural = _("PeriodAttendance Reviews")
...
# -----------------------------------------------------------------------------
class PeriodAttendanceWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("PeriodAttendance Workflow Transition")
        verbose_name_plural = _("PeriodAttendance Workflow Transitions")
...
# -----------------------------------------------------------------------------
class PeriodAttendanceAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_access'
        verbose_name = _("PeriodAttendance Access Rule")
        verbose_name_plural = _("PeriodAttendance Access Rules")
...
# -----------------------------------------------------------------------------
class PeriodAttendanceConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_config'
        verbose_name = _("PeriodAttendance Config Parameter")
        verbose_name_plural = _("PeriodAttendance Config Parameters")
...
# -----------------------------------------------------------------------------
class PeriodAttendanceDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("PeriodAttendance Document Attachment")
        verbose_name_plural = _("PeriodAttendance Document Attachments")
...
# -----------------------------------------------------------------------------
class PeriodAttendanceAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on PeriodAttendance."""
    master = models.ForeignKey(PeriodAttendanceMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_attendance_periodattendance_audit'
        ordering = ['-timestamp']
        verbose_name = _("PeriodAttendance Audit Entry")
        verbose_name_plural = _("PeriodAttendance Audit Entries")
