"""
Extended REST API Controllers for EduFlow Teacher & Staff Attendance Management (StaffAttendance).
...
class StaffAttendanceAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(StaffAttendanceMaster, pk=pk)
        data = master.to_dict()
...
class StaffAttendanceAPISearchView(LoginRequiredMixin, View):
...
class StaffAttendanceAPIBatchUpdateView(LoginRequiredMixin, View):
...
class StaffAttendanceAPITelemetryView(LoginRequiredMixin, View):
