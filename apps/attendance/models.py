"""
Models for EduFlow Student Daily Attendance Register (DailyAttendance) in apps.attendance.
...
# -----------------------------------------------------------------------------
class DailyAttendanceStatusChoices(models.TextChoices):
...
class DailyAttendancePriorityChoices(models.TextChoices):
...
class DailyAttendanceTierChoices(models.TextChoices):
...
class DailyAttendanceScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class DailyAttendanceMasterQuerySet(models.QuerySet):
    def active(self):
...
class DailyAttendanceMasterManager(models.Manager):
    def get_queryset(self):
        return DailyAttendanceMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class DailyAttendanceMaster(models.Model):
    """
    Primary master configuration entity for Student Daily Attendance Register.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = DailyAttendanceStatusChoices
    PriorityChoices = DailyAttendancePriorityChoices
    TierChoices = DailyAttendanceTierChoices
    ScopeChoices = DailyAttendanceScopeChoices
    ...
    objects = DailyAttendanceMasterManager()

    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("DailyAttendance Master")
        verbose_name_plural = _("DailyAttendance Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_daily_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_daily_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_daily_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_daily_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'attendance:dailyattendance_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/attendance/dailyattendance/{self.pk}/"
...
# -----------------------------------------------------------------------------
class DailyAttendanceItem(models.Model):
    """Subordinate component or line transaction item for DailyAttendance."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("DailyAttendance Item")
        verbose_name_plural = _("DailyAttendance Items")
...
# -----------------------------------------------------------------------------
class DailyAttendanceAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for DailyAttendance."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_allocation'
        ordering = ['-start_time']
        verbose_name = _("DailyAttendance Allocation")
        verbose_name_plural = _("DailyAttendance Allocations")
...
# -----------------------------------------------------------------------------
class DailyAttendanceMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for DailyAttendance."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("DailyAttendance Metric Record")
        verbose_name_plural = _("DailyAttendance Metric Records")
...
# -----------------------------------------------------------------------------
class DailyAttendancePolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for DailyAttendance."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_rule'
        ordering = ['rule_code']
        verbose_name = _("DailyAttendance Policy Rule")
        verbose_name_plural = _("DailyAttendance Policy Rules")
...
# -----------------------------------------------------------------------------
class DailyAttendanceSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for DailyAttendance."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("DailyAttendance Schedule Period")
        verbose_name_plural = _("DailyAttendance Schedule Periods")
...
# -----------------------------------------------------------------------------
class DailyAttendanceFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for DailyAttendance."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_review'
        ordering = ['-submitted_at']
        verbose_name = _("DailyAttendance Review")
        verbose_name_plural = _("DailyAttendance Reviews")
...
# -----------------------------------------------------------------------------
class DailyAttendanceWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("DailyAttendance Workflow Transition")
        verbose_name_plural = _("DailyAttendance Workflow Transitions")
...
# -----------------------------------------------------------------------------
class DailyAttendanceAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_access'
        verbose_name = _("DailyAttendance Access Rule")
        verbose_name_plural = _("DailyAttendance Access Rules")
...
# -----------------------------------------------------------------------------
class DailyAttendanceConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_config'
        verbose_name = _("DailyAttendance Config Parameter")
        verbose_name_plural = _("DailyAttendance Config Parameters")
...
# -----------------------------------------------------------------------------
class DailyAttendanceDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("DailyAttendance Document Attachment")
        verbose_name_plural = _("DailyAttendance Document Attachments")
...
# -----------------------------------------------------------------------------
class DailyAttendanceAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on DailyAttendance."""
    master = models.ForeignKey(DailyAttendanceMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_attendance_dailyattendance_audit'
        ordering = ['-timestamp']
        verbose_name = _("DailyAttendance Audit Entry")
        verbose_name_plural = _("DailyAttendance Audit Entries")
