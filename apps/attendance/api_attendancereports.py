"""
Extended REST API Controllers for EduFlow Attendance Policies, Alerts & Analytics (AttendanceReports).
...
class AttendanceReportsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(AttendanceReportsMaster, pk=pk)
        data = master.to_dict()
...
class AttendanceReportsAPISearchView(LoginRequiredMixin, View):
...
class AttendanceReportsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class AttendanceReportsAPITelemetryView(LoginRequiredMixin, View):
