"""
Extended REST API Controllers for EduFlow Student Daily Attendance Register (DailyAttendance).
...
class DailyAttendanceAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(DailyAttendanceMaster, pk=pk)
        data = master.to_dict()
...
class DailyAttendanceAPISearchView(LoginRequiredMixin, View):
...
class DailyAttendanceAPIBatchUpdateView(LoginRequiredMixin, View):
...
class DailyAttendanceAPITelemetryView(LoginRequiredMixin, View):
