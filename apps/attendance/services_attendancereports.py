"""
Domain Services for EduFlow Attendance Policies, Alerts & Analytics (AttendanceReports).
...
class AttendanceReportsService:
    """Enterprise domain business service for AttendanceReports."""
    ...
    def create_record(cls, code, name, user_username="system", **kwargs):
        ...
        logger.info(f"Created AttendanceReports record {record.code} by {user_username}")
        return record
    ...
    def update_record(cls, record_id, user_username="system", **kwargs):
        ...
        record = AttendanceReportsMaster.objects.select_for_update().get(pk=record_id)
    ...
    def calculate_utilization_metrics(cls, record_id):
        ...
        record = AttendanceReportsMaster.objects.get(pk=record_id)
    ...
    def allocate_resource(cls, record_id, assignee_id, assignee_name, role="Lead", start_time=None, end_time=None):
        ...
        allocation = AttendanceReportsAllocation.objects.create(
            master=record,
    ...
    def release_allocation(cls, allocation_id):
        ...
        alloc = AttendanceReportsAllocation.objects.get(pk=allocation_id)
    ...
    def set_configuration_parameter(cls, record_id, key, val, data_type="STRING", desc=""):
        ...
        record = AttendanceReportsMaster.objects.get(pk=record_id)
        param, _ = AttendanceReportsConfigurationParameter.objects.update_or_create(
            master=record,
            param_key=key,
    ...
    def _log_action(cls, record, action_type, user_username, old_state, new_state, summary):
        if 'AttendanceReportsAuditTrail' in globals():
            try:
                AttendanceReportsAuditTrail.objects.create(
                    master=record,
                ...
                logger.warning(f"Failed to record audit log for AttendanceReports: {e}")
