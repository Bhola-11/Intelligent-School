"""
Extended REST API Controllers for EduFlow Period & Subject-Wise Attendance Tracking (PeriodAttendance).
...
class PeriodAttendanceAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(PeriodAttendanceMaster, pk=pk)
        data = master.to_dict()
...
class PeriodAttendanceAPISearchView(LoginRequiredMixin, View):
    """Fast indexed autocomplete search endpoint."""
    def get(self, request, *args, **kwargs):
        query = request.GET.get('q', '').strip()
        status_filter = request.GET.get('status')
        qs = PeriodAttendanceMaster.objects.all() if 'PeriodAttendanceMaster' in globals() else []
        if query:
...
class PeriodAttendanceAPIBatchUpdateView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        qs = PeriodAttendanceMaster.objects.filter(pk__in=ids)
        updated_count = 0
        for rec in qs:
            if action == 'ACTIVATE':
...
class PeriodAttendanceAPITelemetryView(LoginRequiredMixin, View):
