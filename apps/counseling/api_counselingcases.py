"""
Extended REST API Controllers for EduFlow Counseling Cases & Student Support Plans (CounselingCases).
...
class CounselingCasesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(CounselingCasesMaster, pk=pk)
        data = master.to_dict()
...
class CounselingCasesAPISearchView(LoginRequiredMixin, View):
...
class CounselingCasesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class CounselingCasesAPITelemetryView(LoginRequiredMixin, View):
