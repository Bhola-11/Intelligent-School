"""
Models for EduFlow Counseling Cases & Student Support Plans (CounselingCases) in apps.counseling.
...
# -----------------------------------------------------------------------------
class CounselingCasesStatusChoices(models.TextChoices):
...
class CounselingCasesPriorityChoices(models.TextChoices):
...
class CounselingCasesTierChoices(models.TextChoices):
...
class CounselingCasesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class CounselingCasesMasterQuerySet(models.QuerySet):
    def active(self):
...
class CounselingCasesMasterManager(models.Manager):
    def get_queryset(self):
        return CounselingCasesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class CounselingCasesMaster(models.Model):
    """
    Primary master configuration entity for Counseling Cases & Student Support Plans.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = CounselingCasesStatusChoices
    PriorityChoices = CounselingCasesPriorityChoices
    TierChoices = CounselingCasesTierChoices
    ScopeChoices = CounselingCasesScopeChoices
    ...
    objects = CounselingCasesMasterManager()

    class Meta:
        db_table = 'eduflow_counseling_counselingcases_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("CounselingCases Master")
        verbose_name_plural = _("CounselingCases Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_couns_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_couns_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_couns_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_couns_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'counseling:counselingcases_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/counseling/counselingcases/{self.pk}/"
...
# -----------------------------------------------------------------------------
class CounselingCasesItem(models.Model):
    """Subordinate component or line transaction item for CounselingCases."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("CounselingCases Item")
        verbose_name_plural = _("CounselingCases Items")
...
# -----------------------------------------------------------------------------
class CounselingCasesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for CounselingCases."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_allocation'
        ordering = ['-start_time']
        verbose_name = _("CounselingCases Allocation")
        verbose_name_plural = _("CounselingCases Allocations")
...
# -----------------------------------------------------------------------------
class CounselingCasesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for CounselingCases."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("CounselingCases Metric Record")
        verbose_name_plural = _("CounselingCases Metric Records")
...
# -----------------------------------------------------------------------------
class CounselingCasesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for CounselingCases."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_rule'
        ordering = ['rule_code']
        verbose_name = _("CounselingCases Policy Rule")
        verbose_name_plural = _("CounselingCases Policy Rules")
...
# -----------------------------------------------------------------------------
class CounselingCasesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for CounselingCases."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("CounselingCases Schedule Period")
        verbose_name_plural = _("CounselingCases Schedule Periods")
...
# -----------------------------------------------------------------------------
class CounselingCasesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for CounselingCases."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_review'
        ordering = ['-submitted_at']
        verbose_name = _("CounselingCases Review")
        verbose_name_plural = _("CounselingCases Reviews")
...
# -----------------------------------------------------------------------------
class CounselingCasesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("CounselingCases Workflow Transition")
        verbose_name_plural = _("CounselingCases Workflow Transitions")
...
# -----------------------------------------------------------------------------
class CounselingCasesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_access'
        verbose_name = _("CounselingCases Access Rule")
        verbose_name_plural = _("CounselingCases Access Rules")
...
# -----------------------------------------------------------------------------
class CounselingCasesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_config'
        verbose_name = _("CounselingCases Config Parameter")
        verbose_name_plural = _("CounselingCases Config Parameters")
...
# -----------------------------------------------------------------------------
class CounselingCasesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("CounselingCases Document Attachment")
        verbose_name_plural = _("CounselingCases Document Attachments")
...
# -----------------------------------------------------------------------------
class CounselingCasesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on CounselingCases."""
    master = models.ForeignKey(CounselingCasesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_counseling_counselingcases_audit'
        ordering = ['-timestamp']
        verbose_name = _("CounselingCases Audit Entry")
        verbose_name_plural = _("CounselingCases Audit Entries")
