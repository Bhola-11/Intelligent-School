"""
Models for EduFlow Grading Schemes & Grade Scale Boundaries (GradeScales) in apps.grading.
...
# -----------------------------------------------------------------------------
class GradeScalesStatusChoices(models.TextChoices):
...
class GradeScalesPriorityChoices(models.TextChoices):
...
class GradeScalesTierChoices(models.TextChoices):
...
class GradeScalesScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class GradeScalesMasterQuerySet(models.QuerySet):
    def active(self):
...
class GradeScalesMasterManager(models.Manager):
    def get_queryset(self):
        return GradeScalesMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class GradeScalesMaster(models.Model):
    """
    Primary master configuration entity for Grading Schemes & Grade Scale Boundaries.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = GradeScalesStatusChoices
    PriorityChoices = GradeScalesPriorityChoices
    TierChoices = GradeScalesTierChoices
    ScopeChoices = GradeScalesScopeChoices
    ...
    objects = GradeScalesMasterManager()

    class Meta:
        db_table = 'eduflow_grading_gradescales_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("GradeScales Master")
        verbose_name_plural = _("GradeScales Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_grade_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_grade_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_grade_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_grade_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'grading:gradescales_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/grading/gradescales/{self.pk}/"
...
# -----------------------------------------------------------------------------
class GradeScalesItem(models.Model):
    """Subordinate component or line transaction item for GradeScales."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("GradeScales Item")
        verbose_name_plural = _("GradeScales Items")
...
# -----------------------------------------------------------------------------
class GradeScalesAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for GradeScales."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_allocation'
        ordering = ['-start_time']
        verbose_name = _("GradeScales Allocation")
        verbose_name_plural = _("GradeScales Allocations")
...
# -----------------------------------------------------------------------------
class GradeScalesMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for GradeScales."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("GradeScales Metric Record")
        verbose_name_plural = _("GradeScales Metric Records")
...
# -----------------------------------------------------------------------------
class GradeScalesPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for GradeScales."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_rule'
        ordering = ['rule_code']
        verbose_name = _("GradeScales Policy Rule")
        verbose_name_plural = _("GradeScales Policy Rules")
...
# -----------------------------------------------------------------------------
class GradeScalesSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for GradeScales."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("GradeScales Schedule Period")
        verbose_name_plural = _("GradeScales Schedule Periods")
...
# -----------------------------------------------------------------------------
class GradeScalesFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for GradeScales."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_review'
        ordering = ['-submitted_at']
        verbose_name = _("GradeScales Review")
        verbose_name_plural = _("GradeScales Reviews")
...
# -----------------------------------------------------------------------------
class GradeScalesWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("GradeScales Workflow Transition")
        verbose_name_plural = _("GradeScales Workflow Transitions")
...
# -----------------------------------------------------------------------------
class GradeScalesAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_access'
        verbose_name = _("GradeScales Access Rule")
        verbose_name_plural = _("GradeScales Access Rules")
...
# -----------------------------------------------------------------------------
class GradeScalesConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_config'
        verbose_name = _("GradeScales Config Parameter")
        verbose_name_plural = _("GradeScales Config Parameters")
...
# -----------------------------------------------------------------------------
class GradeScalesDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("GradeScales Document Attachment")
        verbose_name_plural = _("GradeScales Document Attachments")
...
# -----------------------------------------------------------------------------
class GradeScalesAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on GradeScales."""
    master = models.ForeignKey(GradeScalesMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_grading_gradescales_audit'
        ordering = ['-timestamp']
        verbose_name = _("GradeScales Audit Entry")
        verbose_name_plural = _("GradeScales Audit Entries")
