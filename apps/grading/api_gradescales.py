"""
Extended REST API Controllers for EduFlow Grading Schemes & Grade Scale Boundaries (GradeScales).
...
class GradeScalesAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(GradeScalesMaster, pk=pk)
        data = master.to_dict()
...
class GradeScalesAPISearchView(LoginRequiredMixin, View):
...
class GradeScalesAPIBatchUpdateView(LoginRequiredMixin, View):
...
class GradeScalesAPITelemetryView(LoginRequiredMixin, View):
