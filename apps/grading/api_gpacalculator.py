"""
Extended REST API Controllers for EduFlow GPA, CGPA, Term Average & Rank Engine (GPACalculator).
...
class GPACalculatorAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(GPACalculatorMaster, pk=pk)
        data = master.to_dict()
...
class GPACalculatorAPISearchView(LoginRequiredMixin, View):
...
class GPACalculatorAPIBatchUpdateView(LoginRequiredMixin, View):
...
class GPACalculatorAPITelemetryView(LoginRequiredMixin, View):
