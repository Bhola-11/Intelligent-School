"""
Extended REST API Controllers for EduFlow Subject Marks & Component Entry Register (MarksEntry).
...
class MarksEntryAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(MarksEntryMaster, pk=pk)
        data = master.to_dict()
...
class MarksEntryAPISearchView(LoginRequiredMixin, View):
...
class MarksEntryAPIBatchUpdateView(LoginRequiredMixin, View):
...
class MarksEntryAPITelemetryView(LoginRequiredMixin, View):
