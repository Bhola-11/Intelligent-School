"""
Models for EduFlow Subject Marks & Component Entry Register (MarksEntry) in apps.grading.
...
# -----------------------------------------------------------------------------
class MarksEntryStatusChoices(models.TextChoices):
...
class MarksEntryPriorityChoices(models.TextChoices):
...
class MarksEntryTierChoices(models.TextChoices):
...
class MarksEntryScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class MarksEntryMasterQuerySet(models.QuerySet):
    def active(self):
...
class MarksEntryMasterManager(models.Manager):
    def get_queryset(self):
        return MarksEntryMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class MarksEntryMaster(models.Model):
    """
    Primary master configuration entity for Subject Marks & Component Entry Register.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = MarksEntryStatusChoices
    PriorityChoices = MarksEntryPriorityChoices
    TierChoices = MarksEntryTierChoices
    ScopeChoices = MarksEntryScopeChoices
    ...
    objects = MarksEntryMasterManager()

    class Meta:
        db_table = 'eduflow_grading_marksentry_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("MarksEntry Master")
        verbose_name_plural = _("MarksEntry Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_marks_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_marks_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_marks_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_marks_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'grading:marksentry_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/grading/marksentry/{self.pk}/"
...
# -----------------------------------------------------------------------------
class MarksEntryItem(models.Model):
    """Subordinate component or line transaction item for MarksEntry."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("MarksEntry Item")
        verbose_name_plural = _("MarksEntry Items")
...
# -----------------------------------------------------------------------------
class MarksEntryAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for MarksEntry."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_allocation'
        ordering = ['-start_time']
        verbose_name = _("MarksEntry Allocation")
        verbose_name_plural = _("MarksEntry Allocations")
...
# -----------------------------------------------------------------------------
class MarksEntryMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for MarksEntry."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("MarksEntry Metric Record")
        verbose_name_plural = _("MarksEntry Metric Records")
...
# -----------------------------------------------------------------------------
class MarksEntryPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for MarksEntry."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_rule'
        ordering = ['rule_code']
        verbose_name = _("MarksEntry Policy Rule")
        verbose_name_plural = _("MarksEntry Policy Rules")
...
# -----------------------------------------------------------------------------
class MarksEntrySchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for MarksEntry."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("MarksEntry Schedule Period")
        verbose_name_plural = _("MarksEntry Schedule Periods")
...
# -----------------------------------------------------------------------------
class MarksEntryFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for MarksEntry."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_review'
        ordering = ['-submitted_at']
        verbose_name = _("MarksEntry Review")
        verbose_name_plural = _("MarksEntry Reviews")
...
# -----------------------------------------------------------------------------
class MarksEntryWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("MarksEntry Workflow Transition")
        verbose_name_plural = _("MarksEntry Workflow Transitions")
...
# -----------------------------------------------------------------------------
class MarksEntryAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_access'
        verbose_name = _("MarksEntry Access Rule")
        verbose_name_plural = _("MarksEntry Access Rules")
...
# -----------------------------------------------------------------------------
class MarksEntryConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_config'
        verbose_name = _("MarksEntry Config Parameter")
        verbose_name_plural = _("MarksEntry Config Parameters")
...
# -----------------------------------------------------------------------------
class MarksEntryDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("MarksEntry Document Attachment")
        verbose_name_plural = _("MarksEntry Document Attachments")
...
# -----------------------------------------------------------------------------
class MarksEntryAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on MarksEntry."""
    master = models.ForeignKey(MarksEntryMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_grading_marksentry_audit'
        ordering = ['-timestamp']
        verbose_name = _("MarksEntry Audit Entry")
        verbose_name_plural = _("MarksEntry Audit Entries")
