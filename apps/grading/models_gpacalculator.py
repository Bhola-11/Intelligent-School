"""
Models for EduFlow GPA, CGPA, Term Average & Rank Engine (GPACalculator) in apps.grading.
...
# -----------------------------------------------------------------------------
class GPACalculatorStatusChoices(models.TextChoices):
...
class GPACalculatorPriorityChoices(models.TextChoices):
...
class GPACalculatorTierChoices(models.TextChoices):
...
class GPACalculatorScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class GPACalculatorMasterQuerySet(models.QuerySet):
    def active(self):
...
class GPACalculatorMasterManager(models.Manager):
    def get_queryset(self):
        return GPACalculatorMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class GPACalculatorMaster(models.Model):
    """
    Primary master configuration entity for GPA, CGPA, Term Average & Rank Engine.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = GPACalculatorStatusChoices
    PriorityChoices = GPACalculatorPriorityChoices
    TierChoices = GPACalculatorTierChoices
    ScopeChoices = GPACalculatorScopeChoices
    ...
    objects = GPACalculatorMasterManager()

    class Meta:
        db_table = 'eduflow_grading_gpacalculator_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("GPACalculator Master")
        verbose_name_plural = _("GPACalculator Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_gpaca_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_gpaca_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_gpaca_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_gpaca_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'grading:gpacalculator_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/grading/gpacalculator/{self.pk}/"
...
# -----------------------------------------------------------------------------
class GPACalculatorItem(models.Model):
    """Subordinate component or line transaction item for GPACalculator."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("GPACalculator Item")
        verbose_name_plural = _("GPACalculator Items")
...
# -----------------------------------------------------------------------------
class GPACalculatorAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for GPACalculator."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_allocation'
        ordering = ['-start_time']
        verbose_name = _("GPACalculator Allocation")
        verbose_name_plural = _("GPACalculator Allocations")
...
# -----------------------------------------------------------------------------
class GPACalculatorMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for GPACalculator."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("GPACalculator Metric Record")
        verbose_name_plural = _("GPACalculator Metric Records")
...
# -----------------------------------------------------------------------------
class GPACalculatorPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for GPACalculator."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_rule'
        ordering = ['rule_code']
        verbose_name = _("GPACalculator Policy Rule")
        verbose_name_plural = _("GPACalculator Policy Rules")
...
# -----------------------------------------------------------------------------
class GPACalculatorSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for GPACalculator."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("GPACalculator Schedule Period")
        verbose_name_plural = _("GPACalculator Schedule Periods")
...
# -----------------------------------------------------------------------------
class GPACalculatorFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for GPACalculator."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_review'
        ordering = ['-submitted_at']
        verbose_name = _("GPACalculator Review")
        verbose_name_plural = _("GPACalculator Reviews")
...
# -----------------------------------------------------------------------------
class GPACalculatorWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("GPACalculator Workflow Transition")
        verbose_name_plural = _("GPACalculator Workflow Transitions")
...
# -----------------------------------------------------------------------------
class GPACalculatorAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_access'
        verbose_name = _("GPACalculator Access Rule")
        verbose_name_plural = _("GPACalculator Access Rules")
...
# -----------------------------------------------------------------------------
class GPACalculatorConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_config'
        verbose_name = _("GPACalculator Config Parameter")
        verbose_name_plural = _("GPACalculator Config Parameters")
...
# -----------------------------------------------------------------------------
class GPACalculatorDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("GPACalculator Document Attachment")
        verbose_name_plural = _("GPACalculator Document Attachments")
...
# -----------------------------------------------------------------------------
class GPACalculatorAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on GPACalculator."""
    master = models.ForeignKey(GPACalculatorMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_grading_gpacalculator_audit'
        ordering = ['-timestamp']
        verbose_name = _("GPACalculator Audit Entry")
        verbose_name_plural = _("GPACalculator Audit Entries")
