"""
Domain Services for EduFlow Subject Marks & Component Entry Register (MarksEntry).
...
class MarksEntryService:
    """Enterprise domain business service for MarksEntry."""
    ...
    def create_record(cls, code, name, user_username="system", **kwargs):
        ...
        logger.info(f"Created MarksEntry record {record.code} by {user_username}")
        return record
    ...
    def update_record(cls, record_id, user_username="system", **kwargs):
        ...
        record = MarksEntryMaster.objects.select_for_update().get(pk=record_id)
    ...
    def calculate_utilization_metrics(cls, record_id):
        ...
        record = MarksEntryMaster.objects.get(pk=record_id)
    ...
    def allocate_resource(cls, record_id, assignee_id, assignee_name, role="Lead", start_time=None, end_time=None):
        ...
        allocation = MarksEntryAllocation.objects.create(
            master=record,
    ...
    def release_allocation(cls, allocation_id):
        ...
        alloc = MarksEntryAllocation.objects.get(pk=allocation_id)
    ...
    def set_configuration_parameter(cls, record_id, key, val, data_type="STRING", desc=""):
        ...
        record = MarksEntryMaster.objects.get(pk=record_id)
        param, _ = MarksEntryConfigurationParameter.objects.update_or_create(
            master=record,
            param_key=key,
    ...
    def _log_action(cls, record, action_type, user_username, old_state, new_state, summary):
        if 'MarksEntryAuditTrail' in globals():
            try:
                MarksEntryAuditTrail.objects.create(
                    master=record,
                ...
                logger.warning(f"Failed to record audit log for MarksEntry: {e}")
