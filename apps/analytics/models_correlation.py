"""
Models for EduFlow Attendance Correlation & At-Risk Predictor (Correlation) in apps.analytics.
...
# -----------------------------------------------------------------------------
class CorrelationStatusChoices(models.TextChoices):
...
class CorrelationPriorityChoices(models.TextChoices):
...
class CorrelationTierChoices(models.TextChoices):
...
class CorrelationScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class CorrelationMasterQuerySet(models.QuerySet):
    def active(self):
...
class CorrelationMasterManager(models.Manager):
    def get_queryset(self):
        return CorrelationMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class CorrelationMaster(models.Model):
    """
    Primary master configuration entity for Attendance Correlation & At-Risk Predictor.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = CorrelationStatusChoices
    PriorityChoices = CorrelationPriorityChoices
    TierChoices = CorrelationTierChoices
    ScopeChoices = CorrelationScopeChoices
    ...
    objects = CorrelationMasterManager()

    class Meta:
        db_table = 'eduflow_analytics_correlation_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Correlation Master")
        verbose_name_plural = _("Correlation Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_corre_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_corre_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_corre_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_corre_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'analytics:correlation_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/analytics/correlation/{self.pk}/"
...
# -----------------------------------------------------------------------------
class CorrelationItem(models.Model):
    """Subordinate component or line transaction item for Correlation."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Correlation Item")
        verbose_name_plural = _("Correlation Items")
...
# -----------------------------------------------------------------------------
class CorrelationAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Correlation."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_allocation'
        ordering = ['-start_time']
        verbose_name = _("Correlation Allocation")
        verbose_name_plural = _("Correlation Allocations")
...
# -----------------------------------------------------------------------------
class CorrelationMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Correlation."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Correlation Metric Record")
        verbose_name_plural = _("Correlation Metric Records")
...
# -----------------------------------------------------------------------------
class CorrelationPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Correlation."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_rule'
        ordering = ['rule_code']
        verbose_name = _("Correlation Policy Rule")
        verbose_name_plural = _("Correlation Policy Rules")
...
# -----------------------------------------------------------------------------
class CorrelationSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Correlation."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Correlation Schedule Period")
        verbose_name_plural = _("Correlation Schedule Periods")
...
# -----------------------------------------------------------------------------
class CorrelationFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Correlation."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_review'
        ordering = ['-submitted_at']
        verbose_name = _("Correlation Review")
        verbose_name_plural = _("Correlation Reviews")
...
# -----------------------------------------------------------------------------
class CorrelationWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Correlation Workflow Transition")
        verbose_name_plural = _("Correlation Workflow Transitions")
...
# -----------------------------------------------------------------------------
class CorrelationAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_access'
        verbose_name = _("Correlation Access Rule")
        verbose_name_plural = _("Correlation Access Rules")
...
# -----------------------------------------------------------------------------
class CorrelationConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_config'
        verbose_name = _("Correlation Config Parameter")
        verbose_name_plural = _("Correlation Config Parameters")
...
# -----------------------------------------------------------------------------
class CorrelationDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Correlation Document Attachment")
        verbose_name_plural = _("Correlation Document Attachments")
...
# -----------------------------------------------------------------------------
class CorrelationAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Correlation."""
    master = models.ForeignKey(CorrelationMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_analytics_correlation_audit'
        ordering = ['-timestamp']
        verbose_name = _("Correlation Audit Entry")
        verbose_name_plural = _("Correlation Audit Entries")
