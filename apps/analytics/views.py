"""
Views for EduFlow Student Academic Performance Trends (Performance).
...
try:
    ...
    from .forms import (
        PerformanceMasterForm, PerformanceItemForm, PerformanceAllocationForm,
        PerformanceMetricRecordForm, PerformancePolicyRuleForm, PerformanceSchedulePeriodForm,
        PerformanceFeedbackReviewForm, PerformanceWorkflowTransitionForm, PerformanceAccessRuleForm,
        PerformanceConfigurationParameterForm, PerformanceDocumentAttachmentForm, PerformanceFilterForm,
        PerformanceBulkActionForm
    )
except ImportError:
    try:
        from .models import (
...
class PerformanceListView(LoginRequiredMixin, ListView):
    template_name = 'analytics/performance_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = PerformanceMaster.objects.all() if 'PerformanceMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = PerformanceFilterForm(self.request.GET) if 'PerformanceFilterForm' in globals() else None
        ctx['bulk_form'] = PerformanceBulkActionForm() if 'PerformanceBulkActionForm' in globals() else None
        ctx['total_count'] = PerformanceMaster.objects.count() if 'PerformanceMaster' in globals() else 0
        ctx['active_count'] = PerformanceMaster.objects.filter(status='ACTIVE').count() if 'PerformanceMaster' in globals() else 0
        ctx['pending_count'] = PerformanceMaster.objects.filter(status='PENDING_REVIEW').count() if 'PerformanceMaster' in globals() else 0
        ctx['page_title'] = "Student Academic Performance Trends Directory"
        ctx['domain_name'] = "Performance"
        return ctx

class PerformanceDetailView(LoginRequiredMixin, DetailView):
    template_name = 'analytics/performance_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(PerformanceMaster, pk=self.kwargs.get('pk'))
...
class PerformanceCreateView(LoginRequiredMixin, CreateView):
    template_name = 'analytics/performance_form.html'
    form_class = PerformanceMasterForm
    success_url = reverse_lazy('analytics:performance_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Performance record '{form.instance.name}' created successfully.")
...
class PerformanceUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Performance record updated successfully.")
        return super().form_valid(form)

class PerformanceDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'analytics/performance_confirm_delete.html'
    success_url = reverse_lazy('analytics:performance_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class PerformanceItemCreateView(LoginRequiredMixin, CreateView):
    form_class = PerformanceItemForm
    template_name = 'analytics/performance_form.html'

    def form_valid(self, form):
        master = get_object_or_404(PerformanceMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('analytics:performance_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class PerformanceItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = PerformanceItemForm
    template_name = 'analytics/performance_form.html'

    def get_object(self):
        return get_object_or_404(PerformanceItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('analytics:performance_detail', kwargs={'pk': self.object.master_id})

class PerformanceItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'analytics/performance_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class PerformanceAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = PerformanceAllocationForm
    template_name = 'analytics/performance_form.html'
...
class PerformanceAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(PerformanceAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('analytics:performance_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class PerformanceMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = PerformanceMetricRecordForm
    template_name = 'analytics/performance_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class PerformancePolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = PerformancePolicyRuleForm
    template_name = 'analytics/performance_form.html'
...
# -----------------------------------------------------------------------------
class PerformanceSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = PerformanceSchedulePeriodForm
    template_name = 'analytics/performance_form.html'
...
# -----------------------------------------------------------------------------
class PerformanceFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = PerformanceFeedbackReviewForm
    template_name = 'analytics/performance_form.html'
...
# -----------------------------------------------------------------------------
class PerformanceWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = PerformanceWorkflowTransitionForm
    template_name = 'analytics/performance_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class PerformanceAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = PerformanceAccessRuleForm
    template_name = 'analytics/performance_form.html'
...
# -----------------------------------------------------------------------------
class PerformanceConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = PerformanceConfigurationParameterForm
    template_name = 'analytics/performance_form.html'
...
# -----------------------------------------------------------------------------
class PerformanceDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = PerformanceDocumentAttachmentForm
    template_name = 'analytics/performance_form.html'
...
# -----------------------------------------------------------------------------
class PerformanceBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = PerformanceMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('analytics:performance_list')

class PerformanceExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="performance_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        writer = csv.writer(response)
        writer.writerow(['Code', 'Name', 'Category', 'Tier', 'Priority', 'Status', 'Capacity', 'Occupancy', 'Budget ($)', 'Incurred ($)', 'Start Date', 'End Date'])
        records = PerformanceMaster.objects.all() if 'PerformanceMaster' in globals() else []
        for r in records:
...
class PerformanceExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = PerformanceMaster.objects.all() if 'PerformanceMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class PerformanceAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = PerformanceMaster.objects.all() if 'PerformanceMaster' in globals() else []
...
class PerformanceAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(PerformanceMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
