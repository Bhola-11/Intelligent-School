"""
Extended REST API Controllers for EduFlow Student Academic Performance Trends (Performance).
...
class PerformanceAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(PerformanceMaster, pk=pk)
        data = master.to_dict()
...
class PerformanceAPISearchView(LoginRequiredMixin, View):
...
class PerformanceAPIBatchUpdateView(LoginRequiredMixin, View):
...
class PerformanceAPITelemetryView(LoginRequiredMixin, View):
