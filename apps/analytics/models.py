"""
Models for EduFlow Student Academic Performance Trends (Performance) in apps.analytics.
...
class PerformancePriorityChoices(models.TextChoices):
...
class PerformanceTierChoices(models.TextChoices):
...
class PerformanceScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class PerformanceMasterQuerySet(models.QuerySet):
    def active(self):
...
class PerformanceMasterManager(models.Manager):
    def get_queryset(self):
        return PerformanceMasterQuerySet(self.model, using=self._db)
...
class PerformanceMaster(models.Model):
    """
    Primary master configuration entity for Student Academic Performance Trends.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = PerformanceStatusChoices
    PriorityChoices = PerformancePriorityChoices
    TierChoices = PerformanceTierChoices
    ScopeChoices = PerformanceScopeChoices
    ...
    objects = PerformanceMasterManager()

    class Meta:
        db_table = 'eduflow_analytics_performance_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Performance Master")
        verbose_name_plural = _("Performance Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_perfo_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_perfo_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_perfo_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_perfo_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'analytics:performance_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/analytics/performance/{self.pk}/"
...
class PerformanceItem(models.Model):
    """Subordinate component or line transaction item for Performance."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Performance Item")
        verbose_name_plural = _("Performance Items")
...
# -----------------------------------------------------------------------------
class PerformanceAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Performance."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_allocation'
        ordering = ['-start_time']
        verbose_name = _("Performance Allocation")
        verbose_name_plural = _("Performance Allocations")
...
# -----------------------------------------------------------------------------
class PerformanceMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Performance."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Performance Metric Record")
        verbose_name_plural = _("Performance Metric Records")
...
# -----------------------------------------------------------------------------
class PerformancePolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Performance."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_rule'
        ordering = ['rule_code']
        verbose_name = _("Performance Policy Rule")
        verbose_name_plural = _("Performance Policy Rules")
...
# -----------------------------------------------------------------------------
class PerformanceSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Performance."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Performance Schedule Period")
        verbose_name_plural = _("Performance Schedule Periods")
...
# -----------------------------------------------------------------------------
class PerformanceFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Performance."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_review'
...
# -----------------------------------------------------------------------------
class PerformanceWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Performance Workflow Transition")
        verbose_name_plural = _("Performance Workflow Transitions")
...
# -----------------------------------------------------------------------------
class PerformanceAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_access'
        verbose_name = _("Performance Access Rule")
        verbose_name_plural = _("Performance Access Rules")
...
# -----------------------------------------------------------------------------
class PerformanceConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_config'
        verbose_name = _("Performance Config Parameter")
        verbose_name_plural = _("Performance Config Parameters")
...
# -----------------------------------------------------------------------------
class PerformanceDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Performance Document Attachment")
        verbose_name_plural = _("Performance Document Attachments")
...
# -----------------------------------------------------------------------------
class PerformanceAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Performance."""
    master = models.ForeignKey(PerformanceMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_analytics_performance_audit'
        ordering = ['-timestamp']
        verbose_name = _("Performance Audit Entry")
        verbose_name_plural = _("Performance Audit Entries")
