"""Django Admin configuration for EduFlow Student Academic Performance Trends (Performance)."""
...
if 'PerformanceMaster' in globals():
    @admin.register(PerformanceMaster)
    class PerformanceMasterAdmin(admin.ModelAdmin):
...
if 'PerformanceAllocation' in globals():
    @admin.register(PerformanceAllocation)
    class PerformanceAllocationAdmin(admin.ModelAdmin):
...
if 'PerformanceMetricRecord' in globals():
    @admin.register(PerformanceMetricRecord)
    class PerformanceMetricRecordAdmin(admin.ModelAdmin):
        list_display = ['master', 'metric_name', 'target_value', 'actual_value', 'score_percentage', 'is_passing']
        list_filter = ['is_passing']

if 'PerformancePolicyRule' in globals():
    @admin.register(PerformancePolicyRule)
    class PerformancePolicyRuleAdmin(admin.ModelAdmin):
        list_display = ['master', 'rule_code', 'rule_name', 'threshold_value', 'is_enforced']
        list_filter = ['is_enforced']

if 'PerformanceSchedulePeriod' in globals():
    @admin.register(PerformanceSchedulePeriod)
    class PerformanceSchedulePeriodAdmin(admin.ModelAdmin):
        list_display = ['master', 'period_title', 'day_of_week', 'start_time', 'end_time', 'room_number']
        list_filter = ['day_of_week', 'is_break_period']

if 'PerformanceFeedbackReview' in globals():
    @admin.register(PerformanceFeedbackReview)
    class PerformanceFeedbackReviewAdmin(admin.ModelAdmin):
        list_display = ['master', 'reviewer_name', 'reviewer_role', 'rating', 'is_resolved', 'submitted_at']
        list_filter = ['rating', 'is_resolved']

if 'PerformanceWorkflowTransition' in globals():
    @admin.register(PerformanceWorkflowTransition)
    class PerformanceWorkflowTransitionAdmin(admin.ModelAdmin):
        list_display = ['master', 'from_stage', 'to_stage', 'actor_username', 'is_approved', 'transition_timestamp']
        list_filter = ['is_approved']

if 'PerformanceAccessRule' in globals():
    @admin.register(PerformanceAccessRule)
    class PerformanceAccessRuleAdmin(admin.ModelAdmin):
        list_display = ['master', 'role_allowed', 'can_read', 'can_write', 'can_delete', 'can_export']
        list_filter = ['role_allowed']

if 'PerformanceConfigurationParameter' in globals():
    @admin.register(PerformanceConfigurationParameter)
    class PerformanceConfigurationParameterAdmin(admin.ModelAdmin):
        list_display = ['master', 'param_key', 'param_value', 'data_type', 'is_editable']

if 'PerformanceDocumentAttachment' in globals():
    @admin.register(PerformanceDocumentAttachment)
    class PerformanceDocumentAttachmentAdmin(admin.ModelAdmin):
        list_display = ['master', 'title', 'mime_type', 'file_size_bytes', 'uploaded_by', 'uploaded_at']

if 'PerformanceAuditTrail' in globals():
    @admin.register(PerformanceAuditTrail)
    class PerformanceAuditTrailAdmin(admin.ModelAdmin):
