"""
Extended REST API Controllers for EduFlow Attendance Correlation & At-Risk Predictor (Correlation).
...
class CorrelationAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(CorrelationMaster, pk=pk)
        data = master.to_dict()
...
class CorrelationAPISearchView(LoginRequiredMixin, View):
...
class CorrelationAPIBatchUpdateView(LoginRequiredMixin, View):
...
class CorrelationAPITelemetryView(LoginRequiredMixin, View):
