"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Institutional KPI & Executive Metrics (InstitutionKPI).
...
class InstitutionKPIDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for InstitutionKPI."""
    template_name = 'analytics/institutionkpi_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = InstitutionKPIAuditTrail.objects.all().order_by('-timestamp')[:10] if 'InstitutionKPIAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Institutional KPI & Executive Metrics Operations Dashboard",
            'domain_name': 'InstitutionKPI',
            'app_title': 'Analytics',
...
class InstitutionKPIAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'analytics/institutionkpi_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Institutional KPI & Executive Metrics Analytical Insights",
