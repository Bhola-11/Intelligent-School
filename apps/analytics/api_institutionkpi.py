"""
Extended REST API Controllers for EduFlow Institutional KPI & Executive Metrics (InstitutionKPI).
...
class InstitutionKPIAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(InstitutionKPIMaster, pk=pk)
        data = master.to_dict()
...
class InstitutionKPIAPISearchView(LoginRequiredMixin, View):
...
class InstitutionKPIAPIBatchUpdateView(LoginRequiredMixin, View):
...
class InstitutionKPIAPITelemetryView(LoginRequiredMixin, View):
