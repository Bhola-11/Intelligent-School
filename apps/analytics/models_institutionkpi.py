"""
Models for EduFlow Institutional KPI & Executive Metrics (InstitutionKPI) in apps.analytics.
...
# -----------------------------------------------------------------------------
class InstitutionKPIStatusChoices(models.TextChoices):
...
class InstitutionKPIPriorityChoices(models.TextChoices):
...
class InstitutionKPITierChoices(models.TextChoices):
...
class InstitutionKPIScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class InstitutionKPIMasterQuerySet(models.QuerySet):
    def active(self):
...
class InstitutionKPIMasterManager(models.Manager):
    def get_queryset(self):
        return InstitutionKPIMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class InstitutionKPIMaster(models.Model):
    """
    Primary master configuration entity for Institutional KPI & Executive Metrics.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = InstitutionKPIStatusChoices
    PriorityChoices = InstitutionKPIPriorityChoices
    TierChoices = InstitutionKPITierChoices
    ScopeChoices = InstitutionKPIScopeChoices
    ...
    objects = InstitutionKPIMasterManager()

    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("InstitutionKPI Master")
        verbose_name_plural = _("InstitutionKPI Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'analytics:institutionkpi_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/analytics/institutionkpi/{self.pk}/"
...
# -----------------------------------------------------------------------------
class InstitutionKPIItem(models.Model):
    """Subordinate component or line transaction item for InstitutionKPI."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("InstitutionKPI Item")
        verbose_name_plural = _("InstitutionKPI Items")
...
# -----------------------------------------------------------------------------
class InstitutionKPIAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for InstitutionKPI."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_allocation'
        ordering = ['-start_time']
        verbose_name = _("InstitutionKPI Allocation")
        verbose_name_plural = _("InstitutionKPI Allocations")
...
# -----------------------------------------------------------------------------
class InstitutionKPIMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for InstitutionKPI."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("InstitutionKPI Metric Record")
        verbose_name_plural = _("InstitutionKPI Metric Records")
...
# -----------------------------------------------------------------------------
class InstitutionKPIPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for InstitutionKPI."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_rule'
        ordering = ['rule_code']
        verbose_name = _("InstitutionKPI Policy Rule")
        verbose_name_plural = _("InstitutionKPI Policy Rules")
...
# -----------------------------------------------------------------------------
class InstitutionKPISchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for InstitutionKPI."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("InstitutionKPI Schedule Period")
        verbose_name_plural = _("InstitutionKPI Schedule Periods")
...
# -----------------------------------------------------------------------------
class InstitutionKPIFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for InstitutionKPI."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_review'
        ordering = ['-submitted_at']
        verbose_name = _("InstitutionKPI Review")
        verbose_name_plural = _("InstitutionKPI Reviews")
...
# -----------------------------------------------------------------------------
class InstitutionKPIWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("InstitutionKPI Workflow Transition")
        verbose_name_plural = _("InstitutionKPI Workflow Transitions")
...
# -----------------------------------------------------------------------------
class InstitutionKPIAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_access'
        verbose_name = _("InstitutionKPI Access Rule")
        verbose_name_plural = _("InstitutionKPI Access Rules")
...
# -----------------------------------------------------------------------------
class InstitutionKPIConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_config'
        verbose_name = _("InstitutionKPI Config Parameter")
        verbose_name_plural = _("InstitutionKPI Config Parameters")
...
# -----------------------------------------------------------------------------
class InstitutionKPIDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("InstitutionKPI Document Attachment")
        verbose_name_plural = _("InstitutionKPI Document Attachments")
...
# -----------------------------------------------------------------------------
class InstitutionKPIAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on InstitutionKPI."""
    master = models.ForeignKey(InstitutionKPIMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_analytics_institutionkpi_audit'
        ordering = ['-timestamp']
        verbose_name = _("InstitutionKPI Audit Entry")
        verbose_name_plural = _("InstitutionKPI Audit Entries")
