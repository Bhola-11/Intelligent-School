"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Student Academic Performance Trends (Performance).
...
class PerformanceDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Performance."""
    template_name = 'analytics/performance_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = PerformanceAuditTrail.objects.all().order_by('-timestamp')[:10] if 'PerformanceAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Student Academic Performance Trends Operations Dashboard",
...
class PerformanceAnalyticsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Student Academic Performance Trends Analytical Insights",
