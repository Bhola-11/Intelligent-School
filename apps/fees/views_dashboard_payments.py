"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Fee Payments, Receipts & Gateways (Payments).
...
class PaymentsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Payments."""
    template_name = 'fees/payments_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = PaymentsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'PaymentsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Fee Payments, Receipts & Gateways Operations Dashboard",
...
class PaymentsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'fees/payments_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Fee Payments, Receipts & Gateways Analytical Insights",
