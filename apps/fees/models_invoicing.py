"""
Models for EduFlow Student Fee Invoicing & Billing Engine (Invoicing) in apps.fees.
...
# -----------------------------------------------------------------------------
class InvoicingStatusChoices(models.TextChoices):
...
class InvoicingPriorityChoices(models.TextChoices):
...
class InvoicingTierChoices(models.TextChoices):
...
class InvoicingScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class InvoicingMasterQuerySet(models.QuerySet):
    def active(self):
...
class InvoicingMasterManager(models.Manager):
    def get_queryset(self):
        return InvoicingMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class InvoicingMaster(models.Model):
    """
    Primary master configuration entity for Student Fee Invoicing & Billing Engine.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = InvoicingStatusChoices
    PriorityChoices = InvoicingPriorityChoices
    TierChoices = InvoicingTierChoices
    ScopeChoices = InvoicingScopeChoices
    ...
    objects = InvoicingMasterManager()

    class Meta:
        db_table = 'eduflow_fees_invoicing_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Invoicing Master")
        verbose_name_plural = _("Invoicing Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_invoi_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_invoi_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_invoi_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_invoi_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'fees:invoicing_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/fees/invoicing/{self.pk}/"
...
class InvoicingItem(models.Model):
    """Subordinate component or line transaction item for Invoicing."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Invoicing Item")
        verbose_name_plural = _("Invoicing Items")
...
# -----------------------------------------------------------------------------
class InvoicingAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Invoicing."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_allocation'
        ordering = ['-start_time']
        verbose_name = _("Invoicing Allocation")
        verbose_name_plural = _("Invoicing Allocations")
...
# -----------------------------------------------------------------------------
class InvoicingMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Invoicing."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Invoicing Metric Record")
        verbose_name_plural = _("Invoicing Metric Records")
...
# -----------------------------------------------------------------------------
class InvoicingPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Invoicing."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_rule'
        ordering = ['rule_code']
        verbose_name = _("Invoicing Policy Rule")
        verbose_name_plural = _("Invoicing Policy Rules")
...
# -----------------------------------------------------------------------------
class InvoicingSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Invoicing."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Invoicing Schedule Period")
        verbose_name_plural = _("Invoicing Schedule Periods")
...
# -----------------------------------------------------------------------------
class InvoicingFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Invoicing."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_review'
        ordering = ['-submitted_at']
        verbose_name = _("Invoicing Review")
        verbose_name_plural = _("Invoicing Reviews")
...
# -----------------------------------------------------------------------------
class InvoicingWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Invoicing Workflow Transition")
        verbose_name_plural = _("Invoicing Workflow Transitions")
...
# -----------------------------------------------------------------------------
class InvoicingAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_access'
        verbose_name = _("Invoicing Access Rule")
        verbose_name_plural = _("Invoicing Access Rules")
...
# -----------------------------------------------------------------------------
class InvoicingConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_config'
        verbose_name = _("Invoicing Config Parameter")
        verbose_name_plural = _("Invoicing Config Parameters")
...
# -----------------------------------------------------------------------------
class InvoicingDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Invoicing Document Attachment")
        verbose_name_plural = _("Invoicing Document Attachments")
...
# -----------------------------------------------------------------------------
class InvoicingAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Invoicing."""
    master = models.ForeignKey(InvoicingMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_fees_invoicing_audit'
        ordering = ['-timestamp']
        verbose_name = _("Invoicing Audit Entry")
        verbose_name_plural = _("Invoicing Audit Entries")
