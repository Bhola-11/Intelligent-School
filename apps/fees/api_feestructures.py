"""
Extended REST API Controllers for EduFlow Fee Categories & Structure Configuration (FeeStructures).
...
class FeeStructuresAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(FeeStructuresMaster, pk=pk)
        data = master.to_dict()
...
class FeeStructuresAPISearchView(LoginRequiredMixin, View):
...
class FeeStructuresAPIBatchUpdateView(LoginRequiredMixin, View):
...
class FeeStructuresAPITelemetryView(LoginRequiredMixin, View):
