"""
Extended REST API Controllers for EduFlow Fee Payments, Receipts & Gateways (Payments).
...
class PaymentsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(PaymentsMaster, pk=pk)
        data = master.to_dict()
...
class PaymentsAPISearchView(LoginRequiredMixin, View):
...
class PaymentsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class PaymentsAPITelemetryView(LoginRequiredMixin, View):
