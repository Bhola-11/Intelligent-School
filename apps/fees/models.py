"""
Models for EduFlow Fee Categories & Structure Configuration (FeeStructures) in apps.fees.
...
# -----------------------------------------------------------------------------
class FeeStructuresStatusChoices(models.TextChoices):
...
class FeeStructuresPriorityChoices(models.TextChoices):
...
class FeeStructuresTierChoices(models.TextChoices):
...
class FeeStructuresScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class FeeStructuresMasterQuerySet(models.QuerySet):
    def active(self):
...
class FeeStructuresMasterManager(models.Manager):
    def get_queryset(self):
        return FeeStructuresMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class FeeStructuresMaster(models.Model):
    """
    Primary master configuration entity for Fee Categories & Structure Configuration.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = FeeStructuresStatusChoices
    PriorityChoices = FeeStructuresPriorityChoices
    TierChoices = FeeStructuresTierChoices
    ScopeChoices = FeeStructuresScopeChoices
    ...
    objects = FeeStructuresMasterManager()

    class Meta:
        db_table = 'eduflow_fees_feestructures_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("FeeStructures Master")
        verbose_name_plural = _("FeeStructures Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_feest_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_feest_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_feest_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_feest_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'fees:feestructures_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/fees/feestructures/{self.pk}/"
...
# -----------------------------------------------------------------------------
class FeeStructuresItem(models.Model):
    """Subordinate component or line transaction item for FeeStructures."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("FeeStructures Item")
        verbose_name_plural = _("FeeStructures Items")
...
# -----------------------------------------------------------------------------
class FeeStructuresAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for FeeStructures."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_allocation'
        ordering = ['-start_time']
        verbose_name = _("FeeStructures Allocation")
        verbose_name_plural = _("FeeStructures Allocations")
...
# -----------------------------------------------------------------------------
class FeeStructuresMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for FeeStructures."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("FeeStructures Metric Record")
        verbose_name_plural = _("FeeStructures Metric Records")
...
# -----------------------------------------------------------------------------
class FeeStructuresPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for FeeStructures."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_rule'
        ordering = ['rule_code']
        verbose_name = _("FeeStructures Policy Rule")
        verbose_name_plural = _("FeeStructures Policy Rules")
...
# -----------------------------------------------------------------------------
class FeeStructuresSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for FeeStructures."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("FeeStructures Schedule Period")
        verbose_name_plural = _("FeeStructures Schedule Periods")
...
# -----------------------------------------------------------------------------
class FeeStructuresFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for FeeStructures."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_review'
        ordering = ['-submitted_at']
        verbose_name = _("FeeStructures Review")
        verbose_name_plural = _("FeeStructures Reviews")
...
# -----------------------------------------------------------------------------
class FeeStructuresWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("FeeStructures Workflow Transition")
        verbose_name_plural = _("FeeStructures Workflow Transitions")
...
# -----------------------------------------------------------------------------
class FeeStructuresAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_access'
        verbose_name = _("FeeStructures Access Rule")
        verbose_name_plural = _("FeeStructures Access Rules")
...
# -----------------------------------------------------------------------------
class FeeStructuresConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_config'
        verbose_name = _("FeeStructures Config Parameter")
        verbose_name_plural = _("FeeStructures Config Parameters")
...
# -----------------------------------------------------------------------------
class FeeStructuresDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("FeeStructures Document Attachment")
        verbose_name_plural = _("FeeStructures Document Attachments")
...
# -----------------------------------------------------------------------------
class FeeStructuresAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on FeeStructures."""
    master = models.ForeignKey(FeeStructuresMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_fees_feestructures_audit'
        ordering = ['-timestamp']
        verbose_name = _("FeeStructures Audit Entry")
        verbose_name_plural = _("FeeStructures Audit Entries")
