"""Django Admin configuration for EduFlow Scholarships, Discounts & Late Fines (Concessions)."""
...
if 'ConcessionsMaster' in globals():
    @admin.register(ConcessionsMaster)
    class ConcessionsMasterAdmin(admin.ModelAdmin):
...
if 'ConcessionsItem' in globals():
    @admin.register(ConcessionsItem)
    class ConcessionsItemAdmin(admin.ModelAdmin):
...
if 'ConcessionsAllocation' in globals():
    @admin.register(ConcessionsAllocation)
    class ConcessionsAllocationAdmin(admin.ModelAdmin):
...
if 'ConcessionsMetricRecord' in globals():
    @admin.register(ConcessionsMetricRecord)
    class ConcessionsMetricRecordAdmin(admin.ModelAdmin):
        list_display = ['master', 'metric_name', 'target_value', 'actual_value', 'score_percentage', 'is_passing']
        list_filter = ['is_passing']

if 'ConcessionsPolicyRule' in globals():
    @admin.register(ConcessionsPolicyRule)
    class ConcessionsPolicyRuleAdmin(admin.ModelAdmin):
        list_display = ['master', 'rule_code', 'rule_name', 'threshold_value', 'is_enforced']
        list_filter = ['is_enforced']

if 'ConcessionsSchedulePeriod' in globals():
    @admin.register(ConcessionsSchedulePeriod)
    class ConcessionsSchedulePeriodAdmin(admin.ModelAdmin):
        list_display = ['master', 'period_title', 'day_of_week', 'start_time', 'end_time', 'room_number']
        list_filter = ['day_of_week', 'is_break_period']

if 'ConcessionsFeedbackReview' in globals():
    @admin.register(ConcessionsFeedbackReview)
    class ConcessionsFeedbackReviewAdmin(admin.ModelAdmin):
        list_display = ['master', 'reviewer_name', 'reviewer_role', 'rating', 'is_resolved', 'submitted_at']
        list_filter = ['rating', 'is_resolved']

if 'ConcessionsWorkflowTransition' in globals():
    @admin.register(ConcessionsWorkflowTransition)
    class ConcessionsWorkflowTransitionAdmin(admin.ModelAdmin):
        list_display = ['master', 'from_stage', 'to_stage', 'actor_username', 'is_approved', 'transition_timestamp']
        list_filter = ['is_approved']

if 'ConcessionsAccessRule' in globals():
    @admin.register(ConcessionsAccessRule)
    class ConcessionsAccessRuleAdmin(admin.ModelAdmin):
        list_display = ['master', 'role_allowed', 'can_read', 'can_write', 'can_delete', 'can_export']
        list_filter = ['role_allowed']

if 'ConcessionsConfigurationParameter' in globals():
    @admin.register(ConcessionsConfigurationParameter)
    class ConcessionsConfigurationParameterAdmin(admin.ModelAdmin):
        list_display = ['master', 'param_key', 'param_value', 'data_type', 'is_editable']

if 'ConcessionsDocumentAttachment' in globals():
    @admin.register(ConcessionsDocumentAttachment)
    class ConcessionsDocumentAttachmentAdmin(admin.ModelAdmin):
        list_display = ['master', 'title', 'mime_type', 'file_size_bytes', 'uploaded_by', 'uploaded_at']

if 'ConcessionsAuditTrail' in globals():
    @admin.register(ConcessionsAuditTrail)
    class ConcessionsAuditTrailAdmin(admin.ModelAdmin):
