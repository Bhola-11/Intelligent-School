"""
Extended REST API Controllers for EduFlow Student Fee Invoicing & Billing Engine (Invoicing).
...
class InvoicingAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(InvoicingMaster, pk=pk)
        data = master.to_dict()
...
class InvoicingAPISearchView(LoginRequiredMixin, View):
...
class InvoicingAPIBatchUpdateView(LoginRequiredMixin, View):
...
class InvoicingAPITelemetryView(LoginRequiredMixin, View):
