"""
Models for EduFlow Fee Payments, Receipts & Gateways (Payments) in apps.fees.
...
# -----------------------------------------------------------------------------
class PaymentsStatusChoices(models.TextChoices):
...
class PaymentsPriorityChoices(models.TextChoices):
...
class PaymentsTierChoices(models.TextChoices):
...
class PaymentsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class PaymentsMasterQuerySet(models.QuerySet):
    def active(self):
...
class PaymentsMasterManager(models.Manager):
    def get_queryset(self):
        return PaymentsMasterQuerySet(self.model, using=self._db)
...
class PaymentsMaster(models.Model):
    """
    Primary master configuration entity for Fee Payments, Receipts & Gateways.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = PaymentsStatusChoices
    PriorityChoices = PaymentsPriorityChoices
    TierChoices = PaymentsTierChoices
    ScopeChoices = PaymentsScopeChoices
    ...
    objects = PaymentsMasterManager()

    class Meta:
        db_table = 'eduflow_fees_payments_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Payments Master")
        verbose_name_plural = _("Payments Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_payme_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_payme_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_payme_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_payme_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'fees:payments_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/fees/payments/{self.pk}/"
...
class PaymentsItem(models.Model):
    """Subordinate component or line transaction item for Payments."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Payments Item")
        verbose_name_plural = _("Payments Items")
...
# -----------------------------------------------------------------------------
class PaymentsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Payments."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_allocation'
        ordering = ['-start_time']
        verbose_name = _("Payments Allocation")
        verbose_name_plural = _("Payments Allocations")
...
# -----------------------------------------------------------------------------
class PaymentsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Payments."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Payments Metric Record")
        verbose_name_plural = _("Payments Metric Records")
...
# -----------------------------------------------------------------------------
class PaymentsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Payments."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_rule'
        ordering = ['rule_code']
        verbose_name = _("Payments Policy Rule")
        verbose_name_plural = _("Payments Policy Rules")
...
# -----------------------------------------------------------------------------
class PaymentsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Payments."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Payments Schedule Period")
        verbose_name_plural = _("Payments Schedule Periods")
...
# -----------------------------------------------------------------------------
class PaymentsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Payments."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_review'
        ordering = ['-submitted_at']
        verbose_name = _("Payments Review")
        verbose_name_plural = _("Payments Reviews")
...
# -----------------------------------------------------------------------------
class PaymentsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Payments Workflow Transition")
        verbose_name_plural = _("Payments Workflow Transitions")
...
# -----------------------------------------------------------------------------
class PaymentsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_access'
        verbose_name = _("Payments Access Rule")
        verbose_name_plural = _("Payments Access Rules")
...
# -----------------------------------------------------------------------------
class PaymentsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_config'
        verbose_name = _("Payments Config Parameter")
        verbose_name_plural = _("Payments Config Parameters")
...
# -----------------------------------------------------------------------------
class PaymentsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Payments Document Attachment")
        verbose_name_plural = _("Payments Document Attachments")
...
# -----------------------------------------------------------------------------
class PaymentsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Payments."""
    master = models.ForeignKey(PaymentsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_fees_payments_audit'
        ordering = ['-timestamp']
        verbose_name = _("Payments Audit Entry")
        verbose_name_plural = _("Payments Audit Entries")
