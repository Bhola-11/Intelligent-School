"""
Models for EduFlow Scholarships, Discounts & Late Fines (Concessions) in apps.fees.
...
# -----------------------------------------------------------------------------
class ConcessionsStatusChoices(models.TextChoices):
...
class ConcessionsPriorityChoices(models.TextChoices):
...
class ConcessionsTierChoices(models.TextChoices):
...
class ConcessionsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ConcessionsMasterQuerySet(models.QuerySet):
    def active(self):
...
class ConcessionsMasterManager(models.Manager):
    def get_queryset(self):
        return ConcessionsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ConcessionsMaster(models.Model):
    """
    Primary master configuration entity for Scholarships, Discounts & Late Fines.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ConcessionsStatusChoices
    PriorityChoices = ConcessionsPriorityChoices
    TierChoices = ConcessionsTierChoices
    ScopeChoices = ConcessionsScopeChoices
    ...
    objects = ConcessionsMasterManager()

    class Meta:
        db_table = 'eduflow_fees_concessions_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Concessions Master")
        verbose_name_plural = _("Concessions Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_conce_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_conce_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_conce_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_conce_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'fees:concessions_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/fees/concessions/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ConcessionsItem(models.Model):
    """Subordinate component or line transaction item for Concessions."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Concessions Item")
        verbose_name_plural = _("Concessions Items")
...
# -----------------------------------------------------------------------------
class ConcessionsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Concessions."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_allocation'
        ordering = ['-start_time']
        verbose_name = _("Concessions Allocation")
        verbose_name_plural = _("Concessions Allocations")
...
# -----------------------------------------------------------------------------
class ConcessionsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Concessions."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Concessions Metric Record")
        verbose_name_plural = _("Concessions Metric Records")
...
# -----------------------------------------------------------------------------
class ConcessionsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Concessions."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_rule'
        ordering = ['rule_code']
        verbose_name = _("Concessions Policy Rule")
        verbose_name_plural = _("Concessions Policy Rules")
...
# -----------------------------------------------------------------------------
class ConcessionsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Concessions."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Concessions Schedule Period")
        verbose_name_plural = _("Concessions Schedule Periods")
...
# -----------------------------------------------------------------------------
class ConcessionsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Concessions."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_review'
        ordering = ['-submitted_at']
        verbose_name = _("Concessions Review")
        verbose_name_plural = _("Concessions Reviews")
...
# -----------------------------------------------------------------------------
class ConcessionsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Concessions Workflow Transition")
        verbose_name_plural = _("Concessions Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ConcessionsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_access'
        verbose_name = _("Concessions Access Rule")
        verbose_name_plural = _("Concessions Access Rules")
...
# -----------------------------------------------------------------------------
class ConcessionsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_config'
        verbose_name = _("Concessions Config Parameter")
        verbose_name_plural = _("Concessions Config Parameters")
...
# -----------------------------------------------------------------------------
class ConcessionsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Concessions Document Attachment")
        verbose_name_plural = _("Concessions Document Attachments")
...
# -----------------------------------------------------------------------------
class ConcessionsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Concessions."""
    master = models.ForeignKey(ConcessionsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_fees_concessions_audit'
        ordering = ['-timestamp']
        verbose_name = _("Concessions Audit Entry")
        verbose_name_plural = _("Concessions Audit Entries")
