"""
Extended REST API Controllers for EduFlow Scholarships, Discounts & Late Fines (Concessions).
...
class ConcessionsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ConcessionsMaster, pk=pk)
        data = master.to_dict()
...
class ConcessionsAPISearchView(LoginRequiredMixin, View):
...
class ConcessionsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ConcessionsAPITelemetryView(LoginRequiredMixin, View):
