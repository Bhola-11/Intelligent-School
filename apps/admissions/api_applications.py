"""
Extended REST API Controllers for EduFlow Online Admissions Application Pipeline (Applications).
...
class ApplicationsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ApplicationsMaster, pk=pk)
        data = master.to_dict()
...
class ApplicationsAPISearchView(LoginRequiredMixin, View):
...
class ApplicationsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ApplicationsAPITelemetryView(LoginRequiredMixin, View):
