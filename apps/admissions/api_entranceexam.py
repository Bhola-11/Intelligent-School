"""
Extended REST API Controllers for EduFlow Admissions Entrance Exams & Interviews (EntranceExam).
...
class EntranceExamAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(EntranceExamMaster, pk=pk)
        data = master.to_dict()
...
class EntranceExamAPISearchView(LoginRequiredMixin, View):
...
class EntranceExamAPIBatchUpdateView(LoginRequiredMixin, View):
...
class EntranceExamAPITelemetryView(LoginRequiredMixin, View):
