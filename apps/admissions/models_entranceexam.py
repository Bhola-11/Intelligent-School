"""
Models for EduFlow Admissions Entrance Exams & Interviews (EntranceExam) in apps.admissions.
...
# -----------------------------------------------------------------------------
class EntranceExamStatusChoices(models.TextChoices):
...
class EntranceExamPriorityChoices(models.TextChoices):
...
class EntranceExamTierChoices(models.TextChoices):
...
class EntranceExamScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class EntranceExamMasterQuerySet(models.QuerySet):
    def active(self):
...
class EntranceExamMasterManager(models.Manager):
    def get_queryset(self):
        return EntranceExamMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class EntranceExamMaster(models.Model):
    """
    Primary master configuration entity for Admissions Entrance Exams & Interviews.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = EntranceExamStatusChoices
    PriorityChoices = EntranceExamPriorityChoices
    TierChoices = EntranceExamTierChoices
    ScopeChoices = EntranceExamScopeChoices
    ...
    objects = EntranceExamMasterManager()

    class Meta:
        db_table = 'eduflow_admissions_entranceexam_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("EntranceExam Master")
        verbose_name_plural = _("EntranceExam Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_entra_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_entra_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_entra_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_entra_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'admissions:entranceexam_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/admissions/entranceexam/{self.pk}/"
...
# -----------------------------------------------------------------------------
class EntranceExamItem(models.Model):
    """Subordinate component or line transaction item for EntranceExam."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("EntranceExam Item")
        verbose_name_plural = _("EntranceExam Items")
...
# -----------------------------------------------------------------------------
class EntranceExamAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for EntranceExam."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_allocation'
        ordering = ['-start_time']
        verbose_name = _("EntranceExam Allocation")
        verbose_name_plural = _("EntranceExam Allocations")
...
# -----------------------------------------------------------------------------
class EntranceExamMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for EntranceExam."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("EntranceExam Metric Record")
        verbose_name_plural = _("EntranceExam Metric Records")
...
# -----------------------------------------------------------------------------
class EntranceExamPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for EntranceExam."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_rule'
        ordering = ['rule_code']
        verbose_name = _("EntranceExam Policy Rule")
        verbose_name_plural = _("EntranceExam Policy Rules")
...
# -----------------------------------------------------------------------------
class EntranceExamSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for EntranceExam."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("EntranceExam Schedule Period")
        verbose_name_plural = _("EntranceExam Schedule Periods")
...
# -----------------------------------------------------------------------------
class EntranceExamFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for EntranceExam."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_review'
        ordering = ['-submitted_at']
        verbose_name = _("EntranceExam Review")
        verbose_name_plural = _("EntranceExam Reviews")
...
# -----------------------------------------------------------------------------
class EntranceExamWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("EntranceExam Workflow Transition")
        verbose_name_plural = _("EntranceExam Workflow Transitions")
...
# -----------------------------------------------------------------------------
class EntranceExamAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_access'
        verbose_name = _("EntranceExam Access Rule")
        verbose_name_plural = _("EntranceExam Access Rules")
...
# -----------------------------------------------------------------------------
class EntranceExamConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_config'
        verbose_name = _("EntranceExam Config Parameter")
        verbose_name_plural = _("EntranceExam Config Parameters")
...
# -----------------------------------------------------------------------------
class EntranceExamDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("EntranceExam Document Attachment")
        verbose_name_plural = _("EntranceExam Document Attachments")
...
# -----------------------------------------------------------------------------
class EntranceExamAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on EntranceExam."""
    master = models.ForeignKey(EntranceExamMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_admissions_entranceexam_audit'
        ordering = ['-timestamp']
        verbose_name = _("EntranceExam Audit Entry")
        verbose_name_plural = _("EntranceExam Audit Entries")
