"""
Views for EduFlow Admissions Document Verification Desk (Verification).
...
try:
    ...
    from .forms import (
        VerificationMasterForm, VerificationItemForm, VerificationAllocationForm,
        VerificationMetricRecordForm, VerificationPolicyRuleForm, VerificationSchedulePeriodForm,
        VerificationFeedbackReviewForm, VerificationWorkflowTransitionForm, VerificationAccessRuleForm,
        VerificationConfigurationParameterForm, VerificationDocumentAttachmentForm, VerificationFilterForm,
        VerificationBulkActionForm
    )
except ImportError:
    try:
...
class VerificationListView(LoginRequiredMixin, ListView):
    template_name = 'admissions/verification_list.html'
    context_object_name = 'records'
    paginate_by = 25

    def get_queryset(self):
        qs = VerificationMaster.objects.all() if 'VerificationMaster' in globals() else []
        if not qs:
            return []
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['filter_form'] = VerificationFilterForm(self.request.GET) if 'VerificationFilterForm' in globals() else None
        ctx['bulk_form'] = VerificationBulkActionForm() if 'VerificationBulkActionForm' in globals() else None
        ctx['total_count'] = VerificationMaster.objects.count() if 'VerificationMaster' in globals() else 0
        ctx['active_count'] = VerificationMaster.objects.filter(status='ACTIVE').count() if 'VerificationMaster' in globals() else 0
        ctx['pending_count'] = VerificationMaster.objects.filter(status='PENDING_REVIEW').count() if 'VerificationMaster' in globals() else 0
        ctx['page_title'] = "Admissions Document Verification Desk Directory"
        ctx['domain_name'] = "Verification"
        return ctx

class VerificationDetailView(LoginRequiredMixin, DetailView):
    template_name = 'admissions/verification_detail.html'
    context_object_name = 'record'

    def get_object(self):
        return get_object_or_404(VerificationMaster, pk=self.kwargs.get('pk'))
...
class VerificationCreateView(LoginRequiredMixin, CreateView):
    template_name = 'admissions/verification_form.html'
    form_class = VerificationMasterForm
    success_url = reverse_lazy('admissions:verification_list')

    def form_valid(self, form):
        form.instance.created_by_user = self.request.user.username
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Verification record '{form.instance.name}' created successfully.")
...
class VerificationUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        form.instance.updated_by_user = self.request.user.username
        messages.success(self.request, f"Verification record updated successfully.")
        return super().form_valid(form)

class VerificationDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'admissions/verification_confirm_delete.html'
    success_url = reverse_lazy('admissions:verification_list')
...
# 2. Line Item Views
# -----------------------------------------------------------------------------
class VerificationItemCreateView(LoginRequiredMixin, CreateView):
    form_class = VerificationItemForm
    template_name = 'admissions/verification_form.html'

    def form_valid(self, form):
        master = get_object_or_404(VerificationMaster, pk=self.kwargs.get('master_pk'))
        form.instance.master = master
        messages.success(self.request, f"Added item '{form.instance.title}' to {master.name}.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse('admissions:verification_detail', kwargs={'pk': self.kwargs.get('master_pk')})

class VerificationItemUpdateView(LoginRequiredMixin, UpdateView):
    form_class = VerificationItemForm
    template_name = 'admissions/verification_form.html'

    def get_object(self):
        return get_object_or_404(VerificationItem, pk=self.kwargs.get('pk'))

    def get_success_url(self):
        return reverse('admissions:verification_detail', kwargs={'pk': self.object.master_id})

class VerificationItemDeleteView(LoginRequiredMixin, DeleteView):
    template_name = 'admissions/verification_confirm_delete.html'
...
# 3. Allocation Views
# -----------------------------------------------------------------------------
class VerificationAllocationCreateView(LoginRequiredMixin, CreateView):
    form_class = VerificationAllocationForm
    template_name = 'admissions/verification_form.html'
...
class VerificationAllocationReleaseView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        alloc = get_object_or_404(VerificationAllocation, pk=self.kwargs.get('pk'))
        alloc.is_active = False
        alloc.end_time = timezone.now()
        alloc.save()
        messages.info(request, f"Allocation for {alloc.assignee_name} released.")
        return redirect('admissions:verification_detail', pk=alloc.master_id)

# -----------------------------------------------------------------------------
# 4. Metric Record Views
# -----------------------------------------------------------------------------
class VerificationMetricCreateView(LoginRequiredMixin, CreateView):
    form_class = VerificationMetricRecordForm
    template_name = 'admissions/verification_form.html'
...
# 5. Policy Rule Views
# -----------------------------------------------------------------------------
class VerificationPolicyRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = VerificationPolicyRuleForm
    template_name = 'admissions/verification_form.html'
...
# -----------------------------------------------------------------------------
class VerificationSchedulePeriodCreateView(LoginRequiredMixin, CreateView):
    form_class = VerificationSchedulePeriodForm
    template_name = 'admissions/verification_form.html'
...
# -----------------------------------------------------------------------------
class VerificationFeedbackReviewCreateView(LoginRequiredMixin, CreateView):
    form_class = VerificationFeedbackReviewForm
    template_name = 'admissions/verification_form.html'
...
# -----------------------------------------------------------------------------
class VerificationWorkflowTransitionCreateView(LoginRequiredMixin, CreateView):
    form_class = VerificationWorkflowTransitionForm
    template_name = 'admissions/verification_form.html'
...
# 9. Access Rule Views
# -----------------------------------------------------------------------------
class VerificationAccessRuleCreateView(LoginRequiredMixin, CreateView):
    form_class = VerificationAccessRuleForm
    template_name = 'admissions/verification_form.html'
...
# -----------------------------------------------------------------------------
class VerificationConfigurationParameterCreateView(LoginRequiredMixin, CreateView):
    form_class = VerificationConfigurationParameterForm
    template_name = 'admissions/verification_form.html'
...
# -----------------------------------------------------------------------------
class VerificationDocumentAttachmentCreateView(LoginRequiredMixin, CreateView):
    form_class = VerificationDocumentAttachmentForm
    template_name = 'admissions/verification_form.html'
...
# -----------------------------------------------------------------------------
class VerificationBulkStatusUpdateView(LoginRequiredMixin, View):
    def post(self, request, *args, **kwargs):
        selected_ids = request.POST.get('selected_ids', '').split(',')
        action = request.POST.get('action')
        ids = [int(i) for i in selected_ids if i.isdigit()]
        if ids:
            qs = VerificationMaster.objects.filter(pk__in=ids)
            if action == 'ACTIVATE':
        ...
        return redirect('admissions:verification_list')

class VerificationExportCSVView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="verification_export_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        writer = csv.writer(response)
        writer.writerow(['Code', 'Name', 'Category', 'Tier', 'Priority', 'Status', 'Capacity', 'Occupancy', 'Budget ($)', 'Incurred ($)', 'Start Date', 'End Date'])
        records = VerificationMaster.objects.all() if 'VerificationMaster' in globals() else []
        for r in records:
...
class VerificationExportJSONView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = VerificationMaster.objects.all() if 'VerificationMaster' in globals() else []
        data = [r.to_dict() for r in qs]
        return JsonResponse({'status': 'success', 'data': data}, safe=False)

class VerificationAPIListView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        qs = VerificationMaster.objects.all() if 'VerificationMaster' in globals() else []
...
class VerificationAPIMetricsView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        master = get_object_or_404(VerificationMaster, pk=self.kwargs.get('pk'))
        return JsonResponse({
            'code': master.code,
            'name': master.name,
