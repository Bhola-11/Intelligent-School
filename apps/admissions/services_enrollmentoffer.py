"""
Domain Services for EduFlow Admission Offers & Student Account Provisioning (EnrollmentOffer).
...
class EnrollmentOfferService:
    """Enterprise domain business service for EnrollmentOffer."""
    ...
    def create_record(cls, code, name, user_username="system", **kwargs):
        ...
        record = EnrollmentOfferMaster.objects.create(
        ...
        logger.info(f"Created EnrollmentOffer record {record.code} by {user_username}")
        return record
    ...
    def update_record(cls, record_id, user_username="system", **kwargs):
        ...
        record = EnrollmentOfferMaster.objects.select_for_update().get(pk=record_id)
    ...
    def calculate_utilization_metrics(cls, record_id):
        ...
        record = EnrollmentOfferMaster.objects.get(pk=record_id)
    ...
    def allocate_resource(cls, record_id, assignee_id, assignee_name, role="Lead", start_time=None, end_time=None):
        ...
        allocation = EnrollmentOfferAllocation.objects.create(
            master=record,
    ...
    def release_allocation(cls, allocation_id):
        ...
        alloc = EnrollmentOfferAllocation.objects.get(pk=allocation_id)
    ...
    def execute_workflow_transition(cls, record_id, from_stage, to_stage, actor, comments="", is_approved=True):
        ...
        record = EnrollmentOfferMaster.objects.get(pk=record_id)
        trans = EnrollmentOfferWorkflowTransition.objects.create(
            master=record,
    ...
    def set_configuration_parameter(cls, record_id, key, val, data_type="STRING", desc=""):
        ...
        record = EnrollmentOfferMaster.objects.get(pk=record_id)
        param, _ = EnrollmentOfferConfigurationParameter.objects.update_or_create(
            master=record,
            param_key=key,
    ...
    def _log_action(cls, record, action_type, user_username, old_state, new_state, summary):
        if 'EnrollmentOfferAuditTrail' in globals():
            try:
                EnrollmentOfferAuditTrail.objects.create(
                    master=record,
                ...
                logger.warning(f"Failed to record audit log for EnrollmentOffer: {e}")
