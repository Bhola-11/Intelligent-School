"""
Models for EduFlow Admissions Document Verification Desk (Verification) in apps.admissions.
...
class VerificationPriorityChoices(models.TextChoices):
...
class VerificationTierChoices(models.TextChoices):
...
class VerificationScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class VerificationMasterQuerySet(models.QuerySet):
    def active(self):
...
class VerificationMasterManager(models.Manager):
    def get_queryset(self):
        return VerificationMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class VerificationMaster(models.Model):
    """
    Primary master configuration entity for Admissions Document Verification Desk.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = VerificationStatusChoices
    PriorityChoices = VerificationPriorityChoices
    TierChoices = VerificationTierChoices
    ScopeChoices = VerificationScopeChoices
    ...
    objects = VerificationMasterManager()

    class Meta:
        db_table = 'eduflow_admissions_verification_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Verification Master")
        verbose_name_plural = _("Verification Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_verif_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_verif_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_verif_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_verif_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'admissions:verification_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/admissions/verification/{self.pk}/"
...
class VerificationItem(models.Model):
    """Subordinate component or line transaction item for Verification."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Verification Item")
        verbose_name_plural = _("Verification Items")
...
# -----------------------------------------------------------------------------
class VerificationAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Verification."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_allocation'
        ordering = ['-start_time']
        verbose_name = _("Verification Allocation")
        verbose_name_plural = _("Verification Allocations")
...
# -----------------------------------------------------------------------------
class VerificationMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Verification."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Verification Metric Record")
        verbose_name_plural = _("Verification Metric Records")
...
# -----------------------------------------------------------------------------
class VerificationPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Verification."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_rule'
        ordering = ['rule_code']
        verbose_name = _("Verification Policy Rule")
        verbose_name_plural = _("Verification Policy Rules")
...
# -----------------------------------------------------------------------------
class VerificationSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Verification."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Verification Schedule Period")
        verbose_name_plural = _("Verification Schedule Periods")
...
# -----------------------------------------------------------------------------
class VerificationFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Verification."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_review'
        ordering = ['-submitted_at']
        verbose_name = _("Verification Review")
        verbose_name_plural = _("Verification Reviews")
...
# -----------------------------------------------------------------------------
class VerificationWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Verification Workflow Transition")
        verbose_name_plural = _("Verification Workflow Transitions")
...
# -----------------------------------------------------------------------------
class VerificationAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_access'
        verbose_name = _("Verification Access Rule")
        verbose_name_plural = _("Verification Access Rules")
...
# -----------------------------------------------------------------------------
class VerificationConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_config'
        verbose_name = _("Verification Config Parameter")
        verbose_name_plural = _("Verification Config Parameters")
...
# -----------------------------------------------------------------------------
class VerificationDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Verification Document Attachment")
        verbose_name_plural = _("Verification Document Attachments")
...
# -----------------------------------------------------------------------------
class VerificationAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Verification."""
    master = models.ForeignKey(VerificationMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_admissions_verification_audit'
        ordering = ['-timestamp']
        verbose_name = _("Verification Audit Entry")
        verbose_name_plural = _("Verification Audit Entries")
