"""
Models for EduFlow Online Admissions Application Pipeline (Applications) in apps.admissions.
...
# -----------------------------------------------------------------------------
class ApplicationsStatusChoices(models.TextChoices):
...
class ApplicationsPriorityChoices(models.TextChoices):
...
class ApplicationsTierChoices(models.TextChoices):
...
class ApplicationsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ApplicationsMasterQuerySet(models.QuerySet):
    def active(self):
...
class ApplicationsMasterManager(models.Manager):
    def get_queryset(self):
        return ApplicationsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ApplicationsMaster(models.Model):
    """
    Primary master configuration entity for Online Admissions Application Pipeline.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ApplicationsStatusChoices
    PriorityChoices = ApplicationsPriorityChoices
    TierChoices = ApplicationsTierChoices
    ScopeChoices = ApplicationsScopeChoices
    ...
    objects = ApplicationsMasterManager()

    class Meta:
        db_table = 'eduflow_admissions_applications_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Applications Master")
        verbose_name_plural = _("Applications Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_appli_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_appli_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_appli_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_appli_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'admissions:applications_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/admissions/applications/{self.pk}/"
...
class ApplicationsItem(models.Model):
    """Subordinate component or line transaction item for Applications."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Applications Item")
        verbose_name_plural = _("Applications Items")
...
# -----------------------------------------------------------------------------
class ApplicationsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Applications."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_allocation'
        ordering = ['-start_time']
        verbose_name = _("Applications Allocation")
        verbose_name_plural = _("Applications Allocations")
...
# -----------------------------------------------------------------------------
class ApplicationsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Applications."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Applications Metric Record")
        verbose_name_plural = _("Applications Metric Records")
...
# -----------------------------------------------------------------------------
class ApplicationsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Applications."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_rule'
        ordering = ['rule_code']
        verbose_name = _("Applications Policy Rule")
        verbose_name_plural = _("Applications Policy Rules")
...
# -----------------------------------------------------------------------------
class ApplicationsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Applications."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Applications Schedule Period")
        verbose_name_plural = _("Applications Schedule Periods")
...
# -----------------------------------------------------------------------------
class ApplicationsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Applications."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_review'
        ordering = ['-submitted_at']
        verbose_name = _("Applications Review")
        verbose_name_plural = _("Applications Reviews")
...
# -----------------------------------------------------------------------------
class ApplicationsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Applications Workflow Transition")
        verbose_name_plural = _("Applications Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ApplicationsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_access'
        verbose_name = _("Applications Access Rule")
        verbose_name_plural = _("Applications Access Rules")
...
# -----------------------------------------------------------------------------
class ApplicationsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_config'
        verbose_name = _("Applications Config Parameter")
        verbose_name_plural = _("Applications Config Parameters")
...
# -----------------------------------------------------------------------------
class ApplicationsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Applications Document Attachment")
        verbose_name_plural = _("Applications Document Attachments")
...
# -----------------------------------------------------------------------------
class ApplicationsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Applications."""
    master = models.ForeignKey(ApplicationsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_admissions_applications_audit'
        ordering = ['-timestamp']
        verbose_name = _("Applications Audit Entry")
        verbose_name_plural = _("Applications Audit Entries")
