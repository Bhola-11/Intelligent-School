"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Admissions Entrance Exams & Interviews (EntranceExam).
...
class EntranceExamDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for EntranceExam."""
    template_name = 'admissions/entranceexam_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = EntranceExamAuditTrail.objects.all().order_by('-timestamp')[:10] if 'EntranceExamAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Admissions Entrance Exams & Interviews Operations Dashboard",
            'domain_name': 'EntranceExam',
...
class EntranceExamAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'admissions/entranceexam_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Admissions Entrance Exams & Interviews Analytical Insights",
