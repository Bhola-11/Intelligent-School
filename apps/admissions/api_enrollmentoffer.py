"""
Extended REST API Controllers for EduFlow Admission Offers & Student Account Provisioning (EnrollmentOffer).
...
class EnrollmentOfferAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(EnrollmentOfferMaster, pk=pk)
        data = master.to_dict()
...
class EnrollmentOfferAPISearchView(LoginRequiredMixin, View):
...
class EnrollmentOfferAPIBatchUpdateView(LoginRequiredMixin, View):
...
class EnrollmentOfferAPITelemetryView(LoginRequiredMixin, View):
