"""
Models for EduFlow Admission Offers & Student Account Provisioning (EnrollmentOffer) in apps.admissions.
...
# -----------------------------------------------------------------------------
class EnrollmentOfferStatusChoices(models.TextChoices):
...
class EnrollmentOfferPriorityChoices(models.TextChoices):
...
class EnrollmentOfferTierChoices(models.TextChoices):
...
class EnrollmentOfferScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class EnrollmentOfferMasterQuerySet(models.QuerySet):
    def active(self):
...
class EnrollmentOfferMasterManager(models.Manager):
    def get_queryset(self):
        return EnrollmentOfferMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class EnrollmentOfferMaster(models.Model):
    """
    Primary master configuration entity for Admission Offers & Student Account Provisioning.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = EnrollmentOfferStatusChoices
    PriorityChoices = EnrollmentOfferPriorityChoices
    TierChoices = EnrollmentOfferTierChoices
    ScopeChoices = EnrollmentOfferScopeChoices
    ...
    objects = EnrollmentOfferMasterManager()

    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("EnrollmentOffer Master")
        verbose_name_plural = _("EnrollmentOffer Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_enrol_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_enrol_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_enrol_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_enrol_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'admissions:enrollmentoffer_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/admissions/enrollmentoffer/{self.pk}/"
...
# -----------------------------------------------------------------------------
class EnrollmentOfferItem(models.Model):
    """Subordinate component or line transaction item for EnrollmentOffer."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("EnrollmentOffer Item")
        verbose_name_plural = _("EnrollmentOffer Items")
...
# -----------------------------------------------------------------------------
class EnrollmentOfferAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for EnrollmentOffer."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_allocation'
        ordering = ['-start_time']
        verbose_name = _("EnrollmentOffer Allocation")
        verbose_name_plural = _("EnrollmentOffer Allocations")
...
# -----------------------------------------------------------------------------
class EnrollmentOfferMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for EnrollmentOffer."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("EnrollmentOffer Metric Record")
        verbose_name_plural = _("EnrollmentOffer Metric Records")
...
# -----------------------------------------------------------------------------
class EnrollmentOfferPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for EnrollmentOffer."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_rule'
        ordering = ['rule_code']
        verbose_name = _("EnrollmentOffer Policy Rule")
        verbose_name_plural = _("EnrollmentOffer Policy Rules")
...
# -----------------------------------------------------------------------------
class EnrollmentOfferSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for EnrollmentOffer."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("EnrollmentOffer Schedule Period")
        verbose_name_plural = _("EnrollmentOffer Schedule Periods")
...
# -----------------------------------------------------------------------------
class EnrollmentOfferFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for EnrollmentOffer."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_review'
        ordering = ['-submitted_at']
        verbose_name = _("EnrollmentOffer Review")
        verbose_name_plural = _("EnrollmentOffer Reviews")
...
# -----------------------------------------------------------------------------
class EnrollmentOfferWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("EnrollmentOffer Workflow Transition")
        verbose_name_plural = _("EnrollmentOffer Workflow Transitions")
...
# -----------------------------------------------------------------------------
class EnrollmentOfferAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_access'
        verbose_name = _("EnrollmentOffer Access Rule")
        verbose_name_plural = _("EnrollmentOffer Access Rules")
...
# -----------------------------------------------------------------------------
class EnrollmentOfferConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_config'
        verbose_name = _("EnrollmentOffer Config Parameter")
        verbose_name_plural = _("EnrollmentOffer Config Parameters")
...
# -----------------------------------------------------------------------------
class EnrollmentOfferDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("EnrollmentOffer Document Attachment")
        verbose_name_plural = _("EnrollmentOffer Document Attachments")
...
# -----------------------------------------------------------------------------
class EnrollmentOfferAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on EnrollmentOffer."""
    master = models.ForeignKey(EnrollmentOfferMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_admissions_enrollmentoffer_audit'
        ordering = ['-timestamp']
        verbose_name = _("EnrollmentOffer Audit Entry")
        verbose_name_plural = _("EnrollmentOffer Audit Entries")
