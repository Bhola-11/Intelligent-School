"""
Extended REST API Controllers for EduFlow Admissions Document Verification Desk (Verification).
...
class VerificationAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(VerificationMaster, pk=pk)
        data = master.to_dict()
...
class VerificationAPISearchView(LoginRequiredMixin, View):
...
class VerificationAPIBatchUpdateView(LoginRequiredMixin, View):
...
class VerificationAPITelemetryView(LoginRequiredMixin, View):
