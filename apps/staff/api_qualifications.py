"""
Extended REST API Controllers for EduFlow Staff Qualifications, Degrees & Certifications (Qualifications).
...
class QualificationsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(QualificationsMaster, pk=pk)
        data = master.to_dict()
...
class QualificationsAPISearchView(LoginRequiredMixin, View):
...
class QualificationsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class QualificationsAPITelemetryView(LoginRequiredMixin, View):
