"""
Enterprise Integration Adapter & Data Interchange for EduFlow Staff Profiles & Employee Directory (StaffDirectory).
Module: apps.staff
...
class StaffDirectoryIntegrationAdapter:
    """Enterprise external integration pipeline and ETL payload transformer for StaffDirectory."""
    ...
    def build_external_lms_export_payload(self, record_code, record_attributes):
        """Transforms internal StaffDirectory record into standard SCORM/LTI exchange dictionary."""
        payload = {
            "schema_version": "LTI_v1.3",
            "resource_link_id": f"eduflow-{code_prefix}-{record_code}",
            "domain": "StaffDirectory",
            "institution_id": self.institution_id,
            "context_label": "Staff Profiles & Employee Directory",
            "custom_claims": {
                "code": record_code,
    ...
    def dispatch_webhook_event(self, event_name, event_data, target_endpoint, shared_secret):
        ...
        headers = {
            ...
            "X-EduFlow-Domain": "StaffDirectory",
        }
        ...
        logger.info(f"[StaffDirectory] Webhook queued for {target_endpoint}: {event_name}")
    ...
    def construct_graphql_projection_query(self, entity_name, fields_requested, filter_params=None):
        ...
        return "query GetStaffDirectory { " + str(entity_name) + str(filter_part) + " { " + str(fields_str) + " } }"
    ...
    def format_asynchronous_celery_task_envelope(self, task_name, task_args, queue_name="eduflow_operations"):
        ...
        return {
            "task": f"apps.staff.tasks.{task_name}",
