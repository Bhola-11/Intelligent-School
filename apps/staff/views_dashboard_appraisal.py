"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Staff Performance Appraisals & Reviews (Appraisal).
...
class AppraisalDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Appraisal."""
    template_name = 'staff/appraisal_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = AppraisalAuditTrail.objects.all().order_by('-timestamp')[:10] if 'AppraisalAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Staff Performance Appraisals & Reviews Operations Dashboard",
            'domain_name': 'Appraisal',
            'app_title': 'Staff',
...
class AppraisalAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'staff/appraisal_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Staff Performance Appraisals & Reviews Analytical Insights",
