"""
Extended REST API Controllers for EduFlow Teacher Workload & Teaching Allocations (Workload).
...
class WorkloadAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(WorkloadMaster, pk=pk)
        data = master.to_dict()
...
class WorkloadAPISearchView(LoginRequiredMixin, View):
...
class WorkloadAPIBatchUpdateView(LoginRequiredMixin, View):
...
class WorkloadAPITelemetryView(LoginRequiredMixin, View):
