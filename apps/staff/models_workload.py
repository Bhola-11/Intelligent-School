"""
Models for EduFlow Teacher Workload & Teaching Allocations (Workload) in apps.staff.
...
# -----------------------------------------------------------------------------
class WorkloadStatusChoices(models.TextChoices):
...
class WorkloadPriorityChoices(models.TextChoices):
...
class WorkloadTierChoices(models.TextChoices):
...
class WorkloadScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class WorkloadMasterQuerySet(models.QuerySet):
    def active(self):
...
class WorkloadMasterManager(models.Manager):
    def get_queryset(self):
        return WorkloadMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class WorkloadMaster(models.Model):
    """
    Primary master configuration entity for Teacher Workload & Teaching Allocations.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = WorkloadStatusChoices
    PriorityChoices = WorkloadPriorityChoices
    TierChoices = WorkloadTierChoices
    ScopeChoices = WorkloadScopeChoices
    ...
    objects = WorkloadMasterManager()

    class Meta:
        db_table = 'eduflow_staff_workload_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Workload Master")
        verbose_name_plural = _("Workload Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_workl_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_workl_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_workl_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_workl_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'staff:workload_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/staff/workload/{self.pk}/"
...
class WorkloadItem(models.Model):
    """Subordinate component or line transaction item for Workload."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Workload Item")
        verbose_name_plural = _("Workload Items")
...
class WorkloadAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Workload."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_allocation'
        ordering = ['-start_time']
        verbose_name = _("Workload Allocation")
        verbose_name_plural = _("Workload Allocations")
...
# -----------------------------------------------------------------------------
class WorkloadMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Workload."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Workload Metric Record")
        verbose_name_plural = _("Workload Metric Records")
...
# -----------------------------------------------------------------------------
class WorkloadPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Workload."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_rule'
        ordering = ['rule_code']
        verbose_name = _("Workload Policy Rule")
        verbose_name_plural = _("Workload Policy Rules")
...
# -----------------------------------------------------------------------------
class WorkloadSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Workload."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Workload Schedule Period")
        verbose_name_plural = _("Workload Schedule Periods")
...
# -----------------------------------------------------------------------------
class WorkloadFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Workload."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_review'
        ordering = ['-submitted_at']
        verbose_name = _("Workload Review")
        verbose_name_plural = _("Workload Reviews")
...
# -----------------------------------------------------------------------------
class WorkloadWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Workload Workflow Transition")
        verbose_name_plural = _("Workload Workflow Transitions")
...
# -----------------------------------------------------------------------------
class WorkloadAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_access'
        verbose_name = _("Workload Access Rule")
        verbose_name_plural = _("Workload Access Rules")
...
# -----------------------------------------------------------------------------
class WorkloadConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_config'
        verbose_name = _("Workload Config Parameter")
        verbose_name_plural = _("Workload Config Parameters")
...
# -----------------------------------------------------------------------------
class WorkloadDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Workload Document Attachment")
        verbose_name_plural = _("Workload Document Attachments")
...
# -----------------------------------------------------------------------------
class WorkloadAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Workload."""
    master = models.ForeignKey(WorkloadMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_staff_workload_audit'
        ordering = ['-timestamp']
        verbose_name = _("Workload Audit Entry")
        verbose_name_plural = _("Workload Audit Entries")
