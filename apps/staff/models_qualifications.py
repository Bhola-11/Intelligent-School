"""
Models for EduFlow Staff Qualifications, Degrees & Certifications (Qualifications) in apps.staff.
...
# -----------------------------------------------------------------------------
class QualificationsStatusChoices(models.TextChoices):
...
class QualificationsPriorityChoices(models.TextChoices):
...
class QualificationsTierChoices(models.TextChoices):
...
class QualificationsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class QualificationsMasterQuerySet(models.QuerySet):
    def active(self):
...
class QualificationsMasterManager(models.Manager):
    def get_queryset(self):
        return QualificationsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class QualificationsMaster(models.Model):
    """
    Primary master configuration entity for Staff Qualifications, Degrees & Certifications.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = QualificationsStatusChoices
    PriorityChoices = QualificationsPriorityChoices
    TierChoices = QualificationsTierChoices
    ScopeChoices = QualificationsScopeChoices
    ...
    objects = QualificationsMasterManager()

    class Meta:
        db_table = 'eduflow_staff_qualifications_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Qualifications Master")
        verbose_name_plural = _("Qualifications Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_quali_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_quali_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_quali_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_quali_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'staff:qualifications_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/staff/qualifications/{self.pk}/"
...
# -----------------------------------------------------------------------------
class QualificationsItem(models.Model):
    """Subordinate component or line transaction item for Qualifications."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Qualifications Item")
        verbose_name_plural = _("Qualifications Items")
...
# -----------------------------------------------------------------------------
class QualificationsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Qualifications."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_allocation'
        ordering = ['-start_time']
        verbose_name = _("Qualifications Allocation")
        verbose_name_plural = _("Qualifications Allocations")
...
# -----------------------------------------------------------------------------
class QualificationsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Qualifications."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Qualifications Metric Record")
        verbose_name_plural = _("Qualifications Metric Records")
...
# -----------------------------------------------------------------------------
class QualificationsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Qualifications."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_rule'
        ordering = ['rule_code']
        verbose_name = _("Qualifications Policy Rule")
        verbose_name_plural = _("Qualifications Policy Rules")
...
# -----------------------------------------------------------------------------
class QualificationsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Qualifications."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Qualifications Schedule Period")
        verbose_name_plural = _("Qualifications Schedule Periods")
...
# -----------------------------------------------------------------------------
class QualificationsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Qualifications."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_review'
        ordering = ['-submitted_at']
        verbose_name = _("Qualifications Review")
        verbose_name_plural = _("Qualifications Reviews")
...
# -----------------------------------------------------------------------------
class QualificationsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Qualifications Workflow Transition")
        verbose_name_plural = _("Qualifications Workflow Transitions")
...
# -----------------------------------------------------------------------------
class QualificationsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_access'
        verbose_name = _("Qualifications Access Rule")
        verbose_name_plural = _("Qualifications Access Rules")
...
# -----------------------------------------------------------------------------
class QualificationsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_config'
        verbose_name = _("Qualifications Config Parameter")
        verbose_name_plural = _("Qualifications Config Parameters")
...
# -----------------------------------------------------------------------------
class QualificationsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Qualifications Document Attachment")
        verbose_name_plural = _("Qualifications Document Attachments")
...
# -----------------------------------------------------------------------------
class QualificationsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Qualifications."""
    master = models.ForeignKey(QualificationsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_staff_qualifications_audit'
        ordering = ['-timestamp']
        verbose_name = _("Qualifications Audit Entry")
        verbose_name_plural = _("Qualifications Audit Entries")
