"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Teacher Workload & Teaching Allocations (Workload).
...
class WorkloadDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Workload."""
    template_name = 'staff/workload_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = WorkloadAuditTrail.objects.all().order_by('-timestamp')[:10] if 'WorkloadAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Teacher Workload & Teaching Allocations Operations Dashboard",
...
class WorkloadAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'staff/workload_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Teacher Workload & Teaching Allocations Analytical Insights",
