"""
Domain Services for EduFlow Staff Performance Appraisals & Reviews (Appraisal).
...
class AppraisalService:
    """Enterprise domain business service for Appraisal."""
    ...
    def create_record(cls, code, name, user_username="system", **kwargs):
        ...
        logger.info(f"Created Appraisal record {record.code} by {user_username}")
        return record
    ...
    def update_record(cls, record_id, user_username="system", **kwargs):
        ...
        record = AppraisalMaster.objects.select_for_update().get(pk=record_id)
    ...
    def allocate_resource(cls, record_id, assignee_id, assignee_name, role="Lead", start_time=None, end_time=None):
        ...
        allocation = AppraisalAllocation.objects.create(
            master=record,
    ...
    def release_allocation(cls, allocation_id):
        ...
        alloc = AppraisalAllocation.objects.get(pk=allocation_id)
    ...
    def set_configuration_parameter(cls, record_id, key, val, data_type="STRING", desc=""):
        ...
        record = AppraisalMaster.objects.get(pk=record_id)
        param, _ = AppraisalConfigurationParameter.objects.update_or_create(
            master=record,
            param_key=key,
    ...
    def _log_action(cls, record, action_type, user_username, old_state, new_state, summary):
        if 'AppraisalAuditTrail' in globals():
            try:
                ...
                logger.warning(f"Failed to record audit log for Appraisal: {e}")
