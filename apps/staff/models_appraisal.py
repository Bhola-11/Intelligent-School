"""
Models for EduFlow Staff Performance Appraisals & Reviews (Appraisal) in apps.staff.
...
# -----------------------------------------------------------------------------
class AppraisalStatusChoices(models.TextChoices):
...
class AppraisalPriorityChoices(models.TextChoices):
...
class AppraisalTierChoices(models.TextChoices):
...
class AppraisalScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class AppraisalMasterQuerySet(models.QuerySet):
    def active(self):
...
class AppraisalMasterManager(models.Manager):
    def get_queryset(self):
        return AppraisalMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class AppraisalMaster(models.Model):
    """
    Primary master configuration entity for Staff Performance Appraisals & Reviews.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = AppraisalStatusChoices
    PriorityChoices = AppraisalPriorityChoices
    TierChoices = AppraisalTierChoices
    ScopeChoices = AppraisalScopeChoices
    ...
    objects = AppraisalMasterManager()

    class Meta:
        db_table = 'eduflow_staff_appraisal_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Appraisal Master")
        verbose_name_plural = _("Appraisal Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_appra_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_appra_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_appra_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_appra_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'staff:appraisal_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/staff/appraisal/{self.pk}/"
...
class AppraisalItem(models.Model):
    """Subordinate component or line transaction item for Appraisal."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Appraisal Item")
        verbose_name_plural = _("Appraisal Items")
...
# -----------------------------------------------------------------------------
class AppraisalAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Appraisal."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_allocation'
        ordering = ['-start_time']
        verbose_name = _("Appraisal Allocation")
        verbose_name_plural = _("Appraisal Allocations")
...
# -----------------------------------------------------------------------------
class AppraisalMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Appraisal."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Appraisal Metric Record")
        verbose_name_plural = _("Appraisal Metric Records")
...
# -----------------------------------------------------------------------------
class AppraisalPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Appraisal."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_rule'
        ordering = ['rule_code']
        verbose_name = _("Appraisal Policy Rule")
        verbose_name_plural = _("Appraisal Policy Rules")
...
# -----------------------------------------------------------------------------
class AppraisalSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Appraisal."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Appraisal Schedule Period")
        verbose_name_plural = _("Appraisal Schedule Periods")
...
# -----------------------------------------------------------------------------
class AppraisalFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Appraisal."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_review'
...
# -----------------------------------------------------------------------------
class AppraisalWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Appraisal Workflow Transition")
        verbose_name_plural = _("Appraisal Workflow Transitions")
...
# -----------------------------------------------------------------------------
class AppraisalAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_access'
        verbose_name = _("Appraisal Access Rule")
        verbose_name_plural = _("Appraisal Access Rules")
...
# -----------------------------------------------------------------------------
class AppraisalConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_config'
        verbose_name = _("Appraisal Config Parameter")
        verbose_name_plural = _("Appraisal Config Parameters")
...
# -----------------------------------------------------------------------------
class AppraisalDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Appraisal Document Attachment")
        verbose_name_plural = _("Appraisal Document Attachments")
...
# -----------------------------------------------------------------------------
class AppraisalAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Appraisal."""
    master = models.ForeignKey(AppraisalMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_staff_appraisal_audit'
        ordering = ['-timestamp']
        verbose_name = _("Appraisal Audit Entry")
        verbose_name_plural = _("Appraisal Audit Entries")
