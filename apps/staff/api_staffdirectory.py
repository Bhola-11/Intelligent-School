"""
Extended REST API Controllers for EduFlow Staff Profiles & Employee Directory (StaffDirectory).
...
class StaffDirectoryAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(StaffDirectoryMaster, pk=pk)
        data = master.to_dict()
...
class StaffDirectoryAPISearchView(LoginRequiredMixin, View):
...
class StaffDirectoryAPIBatchUpdateView(LoginRequiredMixin, View):
...
class StaffDirectoryAPITelemetryView(LoginRequiredMixin, View):
