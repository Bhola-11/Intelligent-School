"""
Regulatory Compliance & Multi-Tenant Governance Rules for EduFlow Staff Performance Appraisals & Reviews (Appraisal).
Module: apps.staff
...
class AppraisalComplianceEvaluator:
    """Enterprise multi-tenant compliance evaluator and security guard for Appraisal."""
    ...
    def register_violation(self, rule_id, severity, description, entity_id=None):
        ...
        logger.warning(f"[Appraisal Compliance Violation] {rule_id} [{severity}]: {description}")
        return violation
    ...
    def generate_privacy_impact_assessment(self, data_flow_categories):
        """Generates comprehensive Privacy Impact Assessment (PIA) for Appraisal operations."""
        risk_score = 0
        findings = []
