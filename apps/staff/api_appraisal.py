"""
Extended REST API Controllers for EduFlow Staff Performance Appraisals & Reviews (Appraisal).
...
class AppraisalAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(AppraisalMaster, pk=pk)
        data = master.to_dict()
...
class AppraisalAPISearchView(LoginRequiredMixin, View):
...
class AppraisalAPIBatchUpdateView(LoginRequiredMixin, View):
...
class AppraisalAPITelemetryView(LoginRequiredMixin, View):
