"""
Models for EduFlow Staff Profiles & Employee Directory (StaffDirectory) in apps.staff.
...
# -----------------------------------------------------------------------------
class StaffDirectoryStatusChoices(models.TextChoices):
...
class StaffDirectoryPriorityChoices(models.TextChoices):
...
class StaffDirectoryTierChoices(models.TextChoices):
...
class StaffDirectoryScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class StaffDirectoryMasterQuerySet(models.QuerySet):
    def active(self):
...
class StaffDirectoryMasterManager(models.Manager):
    def get_queryset(self):
        return StaffDirectoryMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class StaffDirectoryMaster(models.Model):
    """
    Primary master configuration entity for Staff Profiles & Employee Directory.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = StaffDirectoryStatusChoices
    PriorityChoices = StaffDirectoryPriorityChoices
    TierChoices = StaffDirectoryTierChoices
    ScopeChoices = StaffDirectoryScopeChoices
    ...
    objects = StaffDirectoryMasterManager()

    class Meta:
        db_table = 'eduflow_staff_staffdirectory_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("StaffDirectory Master")
        verbose_name_plural = _("StaffDirectory Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'staff:staffdirectory_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/staff/staffdirectory/{self.pk}/"
...
# -----------------------------------------------------------------------------
class StaffDirectoryItem(models.Model):
    """Subordinate component or line transaction item for StaffDirectory."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("StaffDirectory Item")
        verbose_name_plural = _("StaffDirectory Items")
...
# -----------------------------------------------------------------------------
class StaffDirectoryAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for StaffDirectory."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_allocation'
        ordering = ['-start_time']
        verbose_name = _("StaffDirectory Allocation")
        verbose_name_plural = _("StaffDirectory Allocations")
...
# -----------------------------------------------------------------------------
class StaffDirectoryMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for StaffDirectory."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("StaffDirectory Metric Record")
        verbose_name_plural = _("StaffDirectory Metric Records")
...
# -----------------------------------------------------------------------------
class StaffDirectoryPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for StaffDirectory."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_rule'
        ordering = ['rule_code']
        verbose_name = _("StaffDirectory Policy Rule")
        verbose_name_plural = _("StaffDirectory Policy Rules")
...
# -----------------------------------------------------------------------------
class StaffDirectorySchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for StaffDirectory."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("StaffDirectory Schedule Period")
        verbose_name_plural = _("StaffDirectory Schedule Periods")
...
# -----------------------------------------------------------------------------
class StaffDirectoryFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for StaffDirectory."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_review'
        ordering = ['-submitted_at']
        verbose_name = _("StaffDirectory Review")
        verbose_name_plural = _("StaffDirectory Reviews")
...
# -----------------------------------------------------------------------------
class StaffDirectoryWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("StaffDirectory Workflow Transition")
        verbose_name_plural = _("StaffDirectory Workflow Transitions")
...
# -----------------------------------------------------------------------------
class StaffDirectoryAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_access'
        verbose_name = _("StaffDirectory Access Rule")
        verbose_name_plural = _("StaffDirectory Access Rules")
...
# -----------------------------------------------------------------------------
class StaffDirectoryConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_config'
        verbose_name = _("StaffDirectory Config Parameter")
        verbose_name_plural = _("StaffDirectory Config Parameters")
...
# -----------------------------------------------------------------------------
class StaffDirectoryDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("StaffDirectory Document Attachment")
        verbose_name_plural = _("StaffDirectory Document Attachments")
...
# -----------------------------------------------------------------------------
class StaffDirectoryAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on StaffDirectory."""
    master = models.ForeignKey(StaffDirectoryMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_staff_staffdirectory_audit'
        ordering = ['-timestamp']
        verbose_name = _("StaffDirectory Audit Entry")
        verbose_name_plural = _("StaffDirectory Audit Entries")
