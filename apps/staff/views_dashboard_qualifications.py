"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Staff Qualifications, Degrees & Certifications (Qualifications).
...
class QualificationsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for Qualifications."""
    template_name = 'staff/qualifications_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = QualificationsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'QualificationsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Staff Qualifications, Degrees & Certifications Operations Dashboard",
            'domain_name': 'Qualifications',
            'app_title': 'Staff',
...
class QualificationsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'staff/qualifications_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Staff Qualifications, Degrees & Certifications Analytical Insights",
