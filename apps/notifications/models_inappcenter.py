"""
Models for EduFlow In-App Notification Center & Alerts (InAppCenter) in apps.notifications.
...
# -----------------------------------------------------------------------------
class InAppCenterStatusChoices(models.TextChoices):
...
class InAppCenterPriorityChoices(models.TextChoices):
...
class InAppCenterTierChoices(models.TextChoices):
...
class InAppCenterScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class InAppCenterMasterQuerySet(models.QuerySet):
    def active(self):
...
class InAppCenterMasterManager(models.Manager):
    def get_queryset(self):
        return InAppCenterMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class InAppCenterMaster(models.Model):
    """
    Primary master configuration entity for In-App Notification Center & Alerts.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = InAppCenterStatusChoices
    PriorityChoices = InAppCenterPriorityChoices
    TierChoices = InAppCenterTierChoices
    ScopeChoices = InAppCenterScopeChoices
    ...
    objects = InAppCenterMasterManager()

    class Meta:
        db_table = 'eduflow_notifications_inappcenter_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("InAppCenter Master")
        verbose_name_plural = _("InAppCenter Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_inapp_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_inapp_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_inapp_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_inapp_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'notifications:inappcenter_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/notifications/inappcenter/{self.pk}/"
...
# -----------------------------------------------------------------------------
class InAppCenterItem(models.Model):
    """Subordinate component or line transaction item for InAppCenter."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("InAppCenter Item")
        verbose_name_plural = _("InAppCenter Items")
...
# -----------------------------------------------------------------------------
class InAppCenterAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for InAppCenter."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_allocation'
        ordering = ['-start_time']
        verbose_name = _("InAppCenter Allocation")
        verbose_name_plural = _("InAppCenter Allocations")
...
# -----------------------------------------------------------------------------
class InAppCenterMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for InAppCenter."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("InAppCenter Metric Record")
        verbose_name_plural = _("InAppCenter Metric Records")
...
# -----------------------------------------------------------------------------
class InAppCenterPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for InAppCenter."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_rule'
        ordering = ['rule_code']
        verbose_name = _("InAppCenter Policy Rule")
        verbose_name_plural = _("InAppCenter Policy Rules")
...
# -----------------------------------------------------------------------------
class InAppCenterSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for InAppCenter."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("InAppCenter Schedule Period")
        verbose_name_plural = _("InAppCenter Schedule Periods")
...
# -----------------------------------------------------------------------------
class InAppCenterFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for InAppCenter."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_review'
        ordering = ['-submitted_at']
        verbose_name = _("InAppCenter Review")
        verbose_name_plural = _("InAppCenter Reviews")
...
# -----------------------------------------------------------------------------
class InAppCenterWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("InAppCenter Workflow Transition")
        verbose_name_plural = _("InAppCenter Workflow Transitions")
...
# -----------------------------------------------------------------------------
class InAppCenterAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_access'
        verbose_name = _("InAppCenter Access Rule")
        verbose_name_plural = _("InAppCenter Access Rules")
...
# -----------------------------------------------------------------------------
class InAppCenterConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_config'
        verbose_name = _("InAppCenter Config Parameter")
        verbose_name_plural = _("InAppCenter Config Parameters")
...
# -----------------------------------------------------------------------------
class InAppCenterDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("InAppCenter Document Attachment")
        verbose_name_plural = _("InAppCenter Document Attachments")
...
# -----------------------------------------------------------------------------
class InAppCenterAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on InAppCenter."""
    master = models.ForeignKey(InAppCenterMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_notifications_inappcenter_audit'
        ordering = ['-timestamp']
        verbose_name = _("InAppCenter Audit Entry")
        verbose_name_plural = _("InAppCenter Audit Entries")
