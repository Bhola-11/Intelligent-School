"""
Extended REST API Controllers for EduFlow In-App Notification Center & Alerts (InAppCenter).
...
class InAppCenterAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(InAppCenterMaster, pk=pk)
        data = master.to_dict()
...
class InAppCenterAPISearchView(LoginRequiredMixin, View):
...
class InAppCenterAPIBatchUpdateView(LoginRequiredMixin, View):
...
class InAppCenterAPITelemetryView(LoginRequiredMixin, View):
