"""
Operational Dashboard and Realtime KPI Analytics for EduFlow In-App Notification Center & Alerts (InAppCenter).
...
class InAppCenterDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for InAppCenter."""
    template_name = 'notifications/inappcenter_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = InAppCenterAuditTrail.objects.all().order_by('-timestamp')[:10] if 'InAppCenterAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "In-App Notification Center & Alerts Operations Dashboard",
            'domain_name': 'InAppCenter',
...
class InAppCenterAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'notifications/inappcenter_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "In-App Notification Center & Alerts Analytical Insights",
