"""
Extended REST API Controllers for EduFlow Notification Engine & Channel Routing (Notifications).
...
class NotificationsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(NotificationsMaster, pk=pk)
        data = master.to_dict()
...
class NotificationsAPISearchView(LoginRequiredMixin, View):
...
class NotificationsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class NotificationsAPITelemetryView(LoginRequiredMixin, View):
