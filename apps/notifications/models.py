"""
Models for EduFlow Notification Engine & Channel Routing (Notifications) in apps.notifications.
...
class NotificationsPriorityChoices(models.TextChoices):
...
class NotificationsTierChoices(models.TextChoices):
...
class NotificationsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class NotificationsMasterQuerySet(models.QuerySet):
    def active(self):
...
class NotificationsMasterManager(models.Manager):
    def get_queryset(self):
        return NotificationsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class NotificationsMaster(models.Model):
    """
    Primary master configuration entity for Notification Engine & Channel Routing.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = NotificationsStatusChoices
    PriorityChoices = NotificationsPriorityChoices
    TierChoices = NotificationsTierChoices
    ScopeChoices = NotificationsScopeChoices
    ...
    objects = NotificationsMasterManager()

    class Meta:
        db_table = 'eduflow_notifications_notifications_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("Notifications Master")
        verbose_name_plural = _("Notifications Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_notif_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_notif_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_notif_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_notif_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'notifications:notifications_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/notifications/notifications/{self.pk}/"
...
class NotificationsItem(models.Model):
    """Subordinate component or line transaction item for Notifications."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("Notifications Item")
        verbose_name_plural = _("Notifications Items")
...
# -----------------------------------------------------------------------------
class NotificationsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for Notifications."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_allocation'
        ordering = ['-start_time']
        verbose_name = _("Notifications Allocation")
        verbose_name_plural = _("Notifications Allocations")
...
# -----------------------------------------------------------------------------
class NotificationsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for Notifications."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("Notifications Metric Record")
        verbose_name_plural = _("Notifications Metric Records")
...
# -----------------------------------------------------------------------------
class NotificationsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for Notifications."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_rule'
        ordering = ['rule_code']
        verbose_name = _("Notifications Policy Rule")
        verbose_name_plural = _("Notifications Policy Rules")
...
# -----------------------------------------------------------------------------
class NotificationsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for Notifications."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("Notifications Schedule Period")
        verbose_name_plural = _("Notifications Schedule Periods")
...
# -----------------------------------------------------------------------------
class NotificationsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for Notifications."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_review'
        ordering = ['-submitted_at']
        verbose_name = _("Notifications Review")
        verbose_name_plural = _("Notifications Reviews")
...
# -----------------------------------------------------------------------------
class NotificationsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("Notifications Workflow Transition")
        verbose_name_plural = _("Notifications Workflow Transitions")
...
# -----------------------------------------------------------------------------
class NotificationsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_access'
        verbose_name = _("Notifications Access Rule")
        verbose_name_plural = _("Notifications Access Rules")
...
# -----------------------------------------------------------------------------
class NotificationsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_config'
        verbose_name = _("Notifications Config Parameter")
        verbose_name_plural = _("Notifications Config Parameters")
...
# -----------------------------------------------------------------------------
class NotificationsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("Notifications Document Attachment")
        verbose_name_plural = _("Notifications Document Attachments")
...
# -----------------------------------------------------------------------------
class NotificationsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on Notifications."""
    master = models.ForeignKey(NotificationsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_notifications_notifications_audit'
        ordering = ['-timestamp']
        verbose_name = _("Notifications Audit Entry")
        verbose_name_plural = _("Notifications Audit Entries")
