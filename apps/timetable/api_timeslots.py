"""
Extended REST API Controllers for EduFlow Timetable Periods & Time Slot Definitions (TimeSlots).
...
class TimeSlotsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(TimeSlotsMaster, pk=pk)
        data = master.to_dict()
...
class TimeSlotsAPISearchView(LoginRequiredMixin, View):
...
class TimeSlotsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class TimeSlotsAPITelemetryView(LoginRequiredMixin, View):
