"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Room Allocation & Facility Optimization (RoomAllocator).
...
class RoomAllocatorDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for RoomAllocator."""
    template_name = 'timetable/roomallocator_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = RoomAllocatorAuditTrail.objects.all().order_by('-timestamp')[:10] if 'RoomAllocatorAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Room Allocation & Facility Optimization Operations Dashboard",
            'domain_name': 'RoomAllocator',
            'app_title': 'Timetable',
...
class RoomAllocatorAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'timetable/roomallocator_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Room Allocation & Facility Optimization Analytical Insights",
