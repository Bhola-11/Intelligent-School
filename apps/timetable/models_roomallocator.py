"""
Models for EduFlow Room Allocation & Facility Optimization (RoomAllocator) in apps.timetable.
...
# -----------------------------------------------------------------------------
class RoomAllocatorStatusChoices(models.TextChoices):
...
class RoomAllocatorPriorityChoices(models.TextChoices):
...
class RoomAllocatorTierChoices(models.TextChoices):
...
class RoomAllocatorScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class RoomAllocatorMasterQuerySet(models.QuerySet):
    def active(self):
...
class RoomAllocatorMasterManager(models.Manager):
    def get_queryset(self):
        return RoomAllocatorMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class RoomAllocatorMaster(models.Model):
    """
    Primary master configuration entity for Room Allocation & Facility Optimization.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = RoomAllocatorStatusChoices
    PriorityChoices = RoomAllocatorPriorityChoices
    TierChoices = RoomAllocatorTierChoices
    ScopeChoices = RoomAllocatorScopeChoices
    ...
    objects = RoomAllocatorMasterManager()

    class Meta:
        db_table = 'eduflow_timetable_roomallocator_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("RoomAllocator Master")
        verbose_name_plural = _("RoomAllocator Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_rooma_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_rooma_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_rooma_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_rooma_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'timetable:roomallocator_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/timetable/roomallocator/{self.pk}/"
...
# -----------------------------------------------------------------------------
class RoomAllocatorItem(models.Model):
    """Subordinate component or line transaction item for RoomAllocator."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("RoomAllocator Item")
        verbose_name_plural = _("RoomAllocator Items")
...
# -----------------------------------------------------------------------------
class RoomAllocatorAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for RoomAllocator."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_allocation'
        ordering = ['-start_time']
        verbose_name = _("RoomAllocator Allocation")
        verbose_name_plural = _("RoomAllocator Allocations")
...
# -----------------------------------------------------------------------------
class RoomAllocatorMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for RoomAllocator."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("RoomAllocator Metric Record")
        verbose_name_plural = _("RoomAllocator Metric Records")
...
# -----------------------------------------------------------------------------
class RoomAllocatorPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for RoomAllocator."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_rule'
        ordering = ['rule_code']
        verbose_name = _("RoomAllocator Policy Rule")
        verbose_name_plural = _("RoomAllocator Policy Rules")
...
# -----------------------------------------------------------------------------
class RoomAllocatorSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for RoomAllocator."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("RoomAllocator Schedule Period")
        verbose_name_plural = _("RoomAllocator Schedule Periods")
...
# -----------------------------------------------------------------------------
class RoomAllocatorFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for RoomAllocator."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_review'
        ordering = ['-submitted_at']
        verbose_name = _("RoomAllocator Review")
        verbose_name_plural = _("RoomAllocator Reviews")
...
# -----------------------------------------------------------------------------
class RoomAllocatorWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("RoomAllocator Workflow Transition")
        verbose_name_plural = _("RoomAllocator Workflow Transitions")
...
# -----------------------------------------------------------------------------
class RoomAllocatorAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_access'
        verbose_name = _("RoomAllocator Access Rule")
        verbose_name_plural = _("RoomAllocator Access Rules")
...
# -----------------------------------------------------------------------------
class RoomAllocatorConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_config'
        verbose_name = _("RoomAllocator Config Parameter")
        verbose_name_plural = _("RoomAllocator Config Parameters")
...
# -----------------------------------------------------------------------------
class RoomAllocatorDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("RoomAllocator Document Attachment")
        verbose_name_plural = _("RoomAllocator Document Attachments")
...
# -----------------------------------------------------------------------------
class RoomAllocatorAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on RoomAllocator."""
    master = models.ForeignKey(RoomAllocatorMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_timetable_roomallocator_audit'
        ordering = ['-timestamp']
        verbose_name = _("RoomAllocator Audit Entry")
        verbose_name_plural = _("RoomAllocator Audit Entries")
