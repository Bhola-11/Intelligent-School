"""
Models for EduFlow Timetable Schedule Matrix & Substitution (ScheduleMatrix) in apps.timetable.
...
# -----------------------------------------------------------------------------
class ScheduleMatrixStatusChoices(models.TextChoices):
...
class ScheduleMatrixPriorityChoices(models.TextChoices):
...
class ScheduleMatrixTierChoices(models.TextChoices):
...
class ScheduleMatrixScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ScheduleMatrixMasterQuerySet(models.QuerySet):
    def active(self):
...
class ScheduleMatrixMasterManager(models.Manager):
    def get_queryset(self):
        return ScheduleMatrixMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ScheduleMatrixMaster(models.Model):
    """
    Primary master configuration entity for Timetable Schedule Matrix & Substitution.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ScheduleMatrixStatusChoices
    PriorityChoices = ScheduleMatrixPriorityChoices
    TierChoices = ScheduleMatrixTierChoices
    ScopeChoices = ScheduleMatrixScopeChoices
    ...
    objects = ScheduleMatrixMasterManager()

    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("ScheduleMatrix Master")
        verbose_name_plural = _("ScheduleMatrix Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_sched_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_sched_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_sched_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_sched_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'timetable:schedulematrix_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/timetable/schedulematrix/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ScheduleMatrixItem(models.Model):
    """Subordinate component or line transaction item for ScheduleMatrix."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("ScheduleMatrix Item")
        verbose_name_plural = _("ScheduleMatrix Items")
...
# -----------------------------------------------------------------------------
class ScheduleMatrixAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for ScheduleMatrix."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_allocation'
        ordering = ['-start_time']
        verbose_name = _("ScheduleMatrix Allocation")
        verbose_name_plural = _("ScheduleMatrix Allocations")
...
# -----------------------------------------------------------------------------
class ScheduleMatrixMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for ScheduleMatrix."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("ScheduleMatrix Metric Record")
        verbose_name_plural = _("ScheduleMatrix Metric Records")
...
# -----------------------------------------------------------------------------
class ScheduleMatrixPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for ScheduleMatrix."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_rule'
        ordering = ['rule_code']
        verbose_name = _("ScheduleMatrix Policy Rule")
        verbose_name_plural = _("ScheduleMatrix Policy Rules")
...
# -----------------------------------------------------------------------------
class ScheduleMatrixSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for ScheduleMatrix."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("ScheduleMatrix Schedule Period")
        verbose_name_plural = _("ScheduleMatrix Schedule Periods")
...
# -----------------------------------------------------------------------------
class ScheduleMatrixFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for ScheduleMatrix."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_review'
        ordering = ['-submitted_at']
        verbose_name = _("ScheduleMatrix Review")
        verbose_name_plural = _("ScheduleMatrix Reviews")
...
# -----------------------------------------------------------------------------
class ScheduleMatrixWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("ScheduleMatrix Workflow Transition")
        verbose_name_plural = _("ScheduleMatrix Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ScheduleMatrixAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_access'
        verbose_name = _("ScheduleMatrix Access Rule")
        verbose_name_plural = _("ScheduleMatrix Access Rules")
...
# -----------------------------------------------------------------------------
class ScheduleMatrixConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_config'
        verbose_name = _("ScheduleMatrix Config Parameter")
        verbose_name_plural = _("ScheduleMatrix Config Parameters")
...
# -----------------------------------------------------------------------------
class ScheduleMatrixDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("ScheduleMatrix Document Attachment")
        verbose_name_plural = _("ScheduleMatrix Document Attachments")
...
# -----------------------------------------------------------------------------
class ScheduleMatrixAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on ScheduleMatrix."""
    master = models.ForeignKey(ScheduleMatrixMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_timetable_schedulematrix_audit'
        ordering = ['-timestamp']
        verbose_name = _("ScheduleMatrix Audit Entry")
        verbose_name_plural = _("ScheduleMatrix Audit Entries")
