"""
Extended REST API Controllers for EduFlow Room Allocation & Facility Optimization (RoomAllocator).
...
class RoomAllocatorAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(RoomAllocatorMaster, pk=pk)
        data = master.to_dict()
...
class RoomAllocatorAPISearchView(LoginRequiredMixin, View):
...
class RoomAllocatorAPIBatchUpdateView(LoginRequiredMixin, View):
...
class RoomAllocatorAPITelemetryView(LoginRequiredMixin, View):
