"""
Models for EduFlow Timetable Conflict Detection Engine (ConflictEngine) in apps.timetable.
...
# -----------------------------------------------------------------------------
class ConflictEngineStatusChoices(models.TextChoices):
...
class ConflictEnginePriorityChoices(models.TextChoices):
...
class ConflictEngineTierChoices(models.TextChoices):
...
class ConflictEngineScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ConflictEngineMasterQuerySet(models.QuerySet):
    def active(self):
...
class ConflictEngineMasterManager(models.Manager):
    def get_queryset(self):
        return ConflictEngineMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ConflictEngineMaster(models.Model):
    """
    Primary master configuration entity for Timetable Conflict Detection Engine.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ConflictEngineStatusChoices
    PriorityChoices = ConflictEnginePriorityChoices
    TierChoices = ConflictEngineTierChoices
    ScopeChoices = ConflictEngineScopeChoices
    ...
    objects = ConflictEngineMasterManager()

    class Meta:
        db_table = 'eduflow_timetable_conflictengine_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("ConflictEngine Master")
        verbose_name_plural = _("ConflictEngine Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_confl_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_confl_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_confl_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_confl_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'timetable:conflictengine_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/timetable/conflictengine/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ConflictEngineItem(models.Model):
    """Subordinate component or line transaction item for ConflictEngine."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("ConflictEngine Item")
        verbose_name_plural = _("ConflictEngine Items")
...
# -----------------------------------------------------------------------------
class ConflictEngineAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for ConflictEngine."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_allocation'
        ordering = ['-start_time']
        verbose_name = _("ConflictEngine Allocation")
        verbose_name_plural = _("ConflictEngine Allocations")
...
# -----------------------------------------------------------------------------
class ConflictEngineMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for ConflictEngine."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("ConflictEngine Metric Record")
        verbose_name_plural = _("ConflictEngine Metric Records")
...
# -----------------------------------------------------------------------------
class ConflictEnginePolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for ConflictEngine."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_rule'
        ordering = ['rule_code']
        verbose_name = _("ConflictEngine Policy Rule")
        verbose_name_plural = _("ConflictEngine Policy Rules")
...
# -----------------------------------------------------------------------------
class ConflictEngineSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for ConflictEngine."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("ConflictEngine Schedule Period")
        verbose_name_plural = _("ConflictEngine Schedule Periods")
...
# -----------------------------------------------------------------------------
class ConflictEngineFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for ConflictEngine."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_review'
        ordering = ['-submitted_at']
        verbose_name = _("ConflictEngine Review")
        verbose_name_plural = _("ConflictEngine Reviews")
...
# -----------------------------------------------------------------------------
class ConflictEngineWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("ConflictEngine Workflow Transition")
        verbose_name_plural = _("ConflictEngine Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ConflictEngineAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_access'
        verbose_name = _("ConflictEngine Access Rule")
        verbose_name_plural = _("ConflictEngine Access Rules")
...
# -----------------------------------------------------------------------------
class ConflictEngineConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_config'
        verbose_name = _("ConflictEngine Config Parameter")
        verbose_name_plural = _("ConflictEngine Config Parameters")
...
# -----------------------------------------------------------------------------
class ConflictEngineDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("ConflictEngine Document Attachment")
        verbose_name_plural = _("ConflictEngine Document Attachments")
...
# -----------------------------------------------------------------------------
class ConflictEngineAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on ConflictEngine."""
    master = models.ForeignKey(ConflictEngineMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_timetable_conflictengine_audit'
        ordering = ['-timestamp']
        verbose_name = _("ConflictEngine Audit Entry")
        verbose_name_plural = _("ConflictEngine Audit Entries")
