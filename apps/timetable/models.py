"""
Models for EduFlow Timetable Periods & Time Slot Definitions (TimeSlots) in apps.timetable.
...
# -----------------------------------------------------------------------------
class TimeSlotsStatusChoices(models.TextChoices):
...
class TimeSlotsPriorityChoices(models.TextChoices):
...
class TimeSlotsTierChoices(models.TextChoices):
...
class TimeSlotsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class TimeSlotsMasterQuerySet(models.QuerySet):
    def active(self):
...
class TimeSlotsMasterManager(models.Manager):
    def get_queryset(self):
        return TimeSlotsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class TimeSlotsMaster(models.Model):
    """
    Primary master configuration entity for Timetable Periods & Time Slot Definitions.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = TimeSlotsStatusChoices
    PriorityChoices = TimeSlotsPriorityChoices
    TierChoices = TimeSlotsTierChoices
    ScopeChoices = TimeSlotsScopeChoices
    ...
    objects = TimeSlotsMasterManager()

    class Meta:
        db_table = 'eduflow_timetable_timeslots_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("TimeSlots Master")
        verbose_name_plural = _("TimeSlots Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_times_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_times_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_times_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_times_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'timetable:timeslots_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/timetable/timeslots/{self.pk}/"
...
# -----------------------------------------------------------------------------
class TimeSlotsItem(models.Model):
    """Subordinate component or line transaction item for TimeSlots."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("TimeSlots Item")
        verbose_name_plural = _("TimeSlots Items")
...
# -----------------------------------------------------------------------------
class TimeSlotsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for TimeSlots."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_allocation'
        ordering = ['-start_time']
        verbose_name = _("TimeSlots Allocation")
        verbose_name_plural = _("TimeSlots Allocations")
...
# -----------------------------------------------------------------------------
class TimeSlotsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for TimeSlots."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("TimeSlots Metric Record")
        verbose_name_plural = _("TimeSlots Metric Records")
...
# -----------------------------------------------------------------------------
class TimeSlotsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for TimeSlots."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_rule'
        ordering = ['rule_code']
        verbose_name = _("TimeSlots Policy Rule")
        verbose_name_plural = _("TimeSlots Policy Rules")
...
# -----------------------------------------------------------------------------
class TimeSlotsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for TimeSlots."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("TimeSlots Schedule Period")
        verbose_name_plural = _("TimeSlots Schedule Periods")
...
# -----------------------------------------------------------------------------
class TimeSlotsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for TimeSlots."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_review'
        ordering = ['-submitted_at']
        verbose_name = _("TimeSlots Review")
        verbose_name_plural = _("TimeSlots Reviews")
...
# -----------------------------------------------------------------------------
class TimeSlotsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("TimeSlots Workflow Transition")
        verbose_name_plural = _("TimeSlots Workflow Transitions")
...
# -----------------------------------------------------------------------------
class TimeSlotsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_access'
        verbose_name = _("TimeSlots Access Rule")
        verbose_name_plural = _("TimeSlots Access Rules")
...
# -----------------------------------------------------------------------------
class TimeSlotsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_config'
        verbose_name = _("TimeSlots Config Parameter")
        verbose_name_plural = _("TimeSlots Config Parameters")
...
# -----------------------------------------------------------------------------
class TimeSlotsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("TimeSlots Document Attachment")
        verbose_name_plural = _("TimeSlots Document Attachments")
...
# -----------------------------------------------------------------------------
class TimeSlotsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on TimeSlots."""
    master = models.ForeignKey(TimeSlotsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_timetable_timeslots_audit'
        ordering = ['-timestamp']
        verbose_name = _("TimeSlots Audit Entry")
        verbose_name_plural = _("TimeSlots Audit Entries")
