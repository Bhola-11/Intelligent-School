"""
Extended REST API Controllers for EduFlow Timetable Schedule Matrix & Substitution (ScheduleMatrix).
...
class ScheduleMatrixAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ScheduleMatrixMaster, pk=pk)
        data = master.to_dict()
...
class ScheduleMatrixAPISearchView(LoginRequiredMixin, View):
...
class ScheduleMatrixAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ScheduleMatrixAPITelemetryView(LoginRequiredMixin, View):
