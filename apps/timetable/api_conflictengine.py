"""
Extended REST API Controllers for EduFlow Timetable Conflict Detection Engine (ConflictEngine).
...
class ConflictEngineAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ConflictEngineMaster, pk=pk)
        data = master.to_dict()
...
class ConflictEngineAPISearchView(LoginRequiredMixin, View):
...
class ConflictEngineAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ConflictEngineAPITelemetryView(LoginRequiredMixin, View):
