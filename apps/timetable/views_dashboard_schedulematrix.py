"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Timetable Schedule Matrix & Substitution (ScheduleMatrix).
...
class ScheduleMatrixDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for ScheduleMatrix."""
    template_name = 'timetable/schedulematrix_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = ScheduleMatrixAuditTrail.objects.all().order_by('-timestamp')[:10] if 'ScheduleMatrixAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Timetable Schedule Matrix & Substitution Operations Dashboard",
            'domain_name': 'ScheduleMatrix',
            'app_title': 'Timetable',
...
class ScheduleMatrixAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'timetable/schedulematrix_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Timetable Schedule Matrix & Substitution Analytical Insights",
