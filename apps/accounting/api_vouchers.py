"""
Extended REST API Controllers for EduFlow Expense Vouchers & Payment Approvals (Vouchers).
...
class VouchersAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(VouchersMaster, pk=pk)
        data = master.to_dict()
...
class VouchersAPISearchView(LoginRequiredMixin, View):
...
class VouchersAPIBatchUpdateView(LoginRequiredMixin, View):
...
class VouchersAPITelemetryView(LoginRequiredMixin, View):
