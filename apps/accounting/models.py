"""
Models for EduFlow Chart of Accounts & Financial Ledger (ChartOfAccounts) in apps.accounting.
...
# -----------------------------------------------------------------------------
class ChartOfAccountsStatusChoices(models.TextChoices):
...
class ChartOfAccountsPriorityChoices(models.TextChoices):
...
class ChartOfAccountsTierChoices(models.TextChoices):
...
class ChartOfAccountsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class ChartOfAccountsMasterQuerySet(models.QuerySet):
    def active(self):
...
class ChartOfAccountsMasterManager(models.Manager):
    def get_queryset(self):
        return ChartOfAccountsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class ChartOfAccountsMaster(models.Model):
    """
    Primary master configuration entity for Chart of Accounts & Financial Ledger.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = ChartOfAccountsStatusChoices
    PriorityChoices = ChartOfAccountsPriorityChoices
    TierChoices = ChartOfAccountsTierChoices
    ScopeChoices = ChartOfAccountsScopeChoices
    ...
    objects = ChartOfAccountsMasterManager()

    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("ChartOfAccounts Master")
        verbose_name_plural = _("ChartOfAccounts Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_chart_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_chart_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_chart_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_chart_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'accounting:chartofaccounts_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/accounting/chartofaccounts/{self.pk}/"
...
# -----------------------------------------------------------------------------
class ChartOfAccountsItem(models.Model):
    """Subordinate component or line transaction item for ChartOfAccounts."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("ChartOfAccounts Item")
        verbose_name_plural = _("ChartOfAccounts Items")
...
# -----------------------------------------------------------------------------
class ChartOfAccountsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for ChartOfAccounts."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_allocation'
        ordering = ['-start_time']
        verbose_name = _("ChartOfAccounts Allocation")
        verbose_name_plural = _("ChartOfAccounts Allocations")
...
# -----------------------------------------------------------------------------
class ChartOfAccountsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for ChartOfAccounts."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("ChartOfAccounts Metric Record")
        verbose_name_plural = _("ChartOfAccounts Metric Records")
...
# -----------------------------------------------------------------------------
class ChartOfAccountsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for ChartOfAccounts."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_rule'
        ordering = ['rule_code']
        verbose_name = _("ChartOfAccounts Policy Rule")
        verbose_name_plural = _("ChartOfAccounts Policy Rules")
...
# -----------------------------------------------------------------------------
class ChartOfAccountsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for ChartOfAccounts."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("ChartOfAccounts Schedule Period")
        verbose_name_plural = _("ChartOfAccounts Schedule Periods")
...
# -----------------------------------------------------------------------------
class ChartOfAccountsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for ChartOfAccounts."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_review'
        ordering = ['-submitted_at']
        verbose_name = _("ChartOfAccounts Review")
        verbose_name_plural = _("ChartOfAccounts Reviews")
...
# -----------------------------------------------------------------------------
class ChartOfAccountsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("ChartOfAccounts Workflow Transition")
        verbose_name_plural = _("ChartOfAccounts Workflow Transitions")
...
# -----------------------------------------------------------------------------
class ChartOfAccountsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_access'
        verbose_name = _("ChartOfAccounts Access Rule")
        verbose_name_plural = _("ChartOfAccounts Access Rules")
...
# -----------------------------------------------------------------------------
class ChartOfAccountsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_config'
        verbose_name = _("ChartOfAccounts Config Parameter")
        verbose_name_plural = _("ChartOfAccounts Config Parameters")
...
# -----------------------------------------------------------------------------
class ChartOfAccountsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("ChartOfAccounts Document Attachment")
        verbose_name_plural = _("ChartOfAccounts Document Attachments")
...
# -----------------------------------------------------------------------------
class ChartOfAccountsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on ChartOfAccounts."""
    master = models.ForeignKey(ChartOfAccountsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_accounting_chartofaccounts_audit'
        ordering = ['-timestamp']
        verbose_name = _("ChartOfAccounts Audit Entry")
        verbose_name_plural = _("ChartOfAccounts Audit Entries")
