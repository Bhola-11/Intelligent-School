"""
Extended REST API Controllers for EduFlow Financial Statements & Cash Registers (FinancialReports).
...
class FinancialReportsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(FinancialReportsMaster, pk=pk)
        data = master.to_dict()
...
class FinancialReportsAPISearchView(LoginRequiredMixin, View):
...
class FinancialReportsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class FinancialReportsAPITelemetryView(LoginRequiredMixin, View):
