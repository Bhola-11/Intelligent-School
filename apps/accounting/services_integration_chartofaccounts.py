"""
Enterprise Integration Adapter & Data Interchange for EduFlow Chart of Accounts & Financial Ledger (ChartOfAccounts).
...
class ChartOfAccountsIntegrationAdapter:
    """Enterprise external integration pipeline and ETL payload transformer for ChartOfAccounts."""
    ...
    def build_external_lms_export_payload(self, record_code, record_attributes):
        """Transforms internal ChartOfAccounts record into standard SCORM/LTI exchange dictionary."""
        payload = {
            "schema_version": "LTI_v1.3",
            "resource_link_id": f"eduflow-{code_prefix}-{record_code}",
            "domain": "ChartOfAccounts",
            "institution_id": self.institution_id,
            "context_label": "Chart of Accounts & Financial Ledger",
            "custom_claims": {
                "code": record_code,
    ...
    def dispatch_webhook_event(self, event_name, event_data, target_endpoint, shared_secret):
        ...
        headers = {
            ...
            "X-EduFlow-Domain": "ChartOfAccounts",
        }
        ...
        logger.info(f"[ChartOfAccounts] Webhook queued for {target_endpoint}: {event_name}")
    ...
    def construct_graphql_projection_query(self, entity_name, fields_requested, filter_params=None):
        ...
        return "query GetChartOfAccounts { " + str(entity_name) + str(filter_part) + " { " + str(fields_str) + " } }"
    ...
    def transform_edfi_interchange_record(self, edfi_payload_dict):
        ...
        return {
            "edfi_id": edfi_payload_dict.get("id", f"CHAR-EDFI"),
            "school_id": self.institution_id,
            "domain_context": "ChartOfAccounts",
            "normalized_record": {
                "code": edfi_payload_dict.get("code") or f"CHAR-EDFI",
