"""
Operational Dashboard and Realtime KPI Analytics for EduFlow General Ledger & Double-Entry Journals (GeneralLedger).
...
class GeneralLedgerDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for GeneralLedger."""
    template_name = 'accounting/generalledger_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = GeneralLedgerAuditTrail.objects.all().order_by('-timestamp')[:10] if 'GeneralLedgerAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "General Ledger & Double-Entry Journals Operations Dashboard",
            'domain_name': 'GeneralLedger',
...
class GeneralLedgerAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'accounting/generalledger_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "General Ledger & Double-Entry Journals Analytical Insights",
