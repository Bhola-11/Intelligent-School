"""
Models for EduFlow Financial Statements & Cash Registers (FinancialReports) in apps.accounting.
...
# -----------------------------------------------------------------------------
class FinancialReportsStatusChoices(models.TextChoices):
...
class FinancialReportsPriorityChoices(models.TextChoices):
...
class FinancialReportsTierChoices(models.TextChoices):
...
class FinancialReportsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class FinancialReportsMasterQuerySet(models.QuerySet):
    def active(self):
...
class FinancialReportsMasterManager(models.Manager):
    def get_queryset(self):
        return FinancialReportsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class FinancialReportsMaster(models.Model):
    """
    Primary master configuration entity for Financial Statements & Cash Registers.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = FinancialReportsStatusChoices
    PriorityChoices = FinancialReportsPriorityChoices
    TierChoices = FinancialReportsTierChoices
    ScopeChoices = FinancialReportsScopeChoices
    ...
    objects = FinancialReportsMasterManager()

    class Meta:
        db_table = 'eduflow_accounting_financialreports_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("FinancialReports Master")
        verbose_name_plural = _("FinancialReports Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_finan_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_finan_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_finan_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_finan_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'accounting:financialreports_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/accounting/financialreports/{self.pk}/"
...
# -----------------------------------------------------------------------------
class FinancialReportsItem(models.Model):
    """Subordinate component or line transaction item for FinancialReports."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("FinancialReports Item")
        verbose_name_plural = _("FinancialReports Items")
...
# -----------------------------------------------------------------------------
class FinancialReportsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for FinancialReports."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_allocation'
        ordering = ['-start_time']
        verbose_name = _("FinancialReports Allocation")
        verbose_name_plural = _("FinancialReports Allocations")
...
# -----------------------------------------------------------------------------
class FinancialReportsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for FinancialReports."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("FinancialReports Metric Record")
        verbose_name_plural = _("FinancialReports Metric Records")
...
# -----------------------------------------------------------------------------
class FinancialReportsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for FinancialReports."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_rule'
        ordering = ['rule_code']
        verbose_name = _("FinancialReports Policy Rule")
        verbose_name_plural = _("FinancialReports Policy Rules")
...
# -----------------------------------------------------------------------------
class FinancialReportsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for FinancialReports."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("FinancialReports Schedule Period")
        verbose_name_plural = _("FinancialReports Schedule Periods")
...
# -----------------------------------------------------------------------------
class FinancialReportsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for FinancialReports."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_review'
        ordering = ['-submitted_at']
        verbose_name = _("FinancialReports Review")
        verbose_name_plural = _("FinancialReports Reviews")
...
# -----------------------------------------------------------------------------
class FinancialReportsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("FinancialReports Workflow Transition")
        verbose_name_plural = _("FinancialReports Workflow Transitions")
...
# -----------------------------------------------------------------------------
class FinancialReportsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_access'
        verbose_name = _("FinancialReports Access Rule")
        verbose_name_plural = _("FinancialReports Access Rules")
...
# -----------------------------------------------------------------------------
class FinancialReportsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_config'
        verbose_name = _("FinancialReports Config Parameter")
        verbose_name_plural = _("FinancialReports Config Parameters")
...
# -----------------------------------------------------------------------------
class FinancialReportsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("FinancialReports Document Attachment")
        verbose_name_plural = _("FinancialReports Document Attachments")
...
# -----------------------------------------------------------------------------
class FinancialReportsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on FinancialReports."""
    master = models.ForeignKey(FinancialReportsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_accounting_financialreports_audit'
        ordering = ['-timestamp']
        verbose_name = _("FinancialReports Audit Entry")
        verbose_name_plural = _("FinancialReports Audit Entries")
