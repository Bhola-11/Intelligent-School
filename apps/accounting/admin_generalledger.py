"""Django Admin configuration for EduFlow General Ledger & Double-Entry Journals (GeneralLedger)."""
...
if 'GeneralLedgerMaster' in globals():
    @admin.register(GeneralLedgerMaster)
    class GeneralLedgerMasterAdmin(admin.ModelAdmin):
...
if 'GeneralLedgerItem' in globals():
    @admin.register(GeneralLedgerItem)
    class GeneralLedgerItemAdmin(admin.ModelAdmin):
...
if 'GeneralLedgerAllocation' in globals():
    @admin.register(GeneralLedgerAllocation)
    class GeneralLedgerAllocationAdmin(admin.ModelAdmin):
...
if 'GeneralLedgerMetricRecord' in globals():
    @admin.register(GeneralLedgerMetricRecord)
    class GeneralLedgerMetricRecordAdmin(admin.ModelAdmin):
        list_display = ['master', 'metric_name', 'target_value', 'actual_value', 'score_percentage', 'is_passing']
        list_filter = ['is_passing']

if 'GeneralLedgerPolicyRule' in globals():
    @admin.register(GeneralLedgerPolicyRule)
    class GeneralLedgerPolicyRuleAdmin(admin.ModelAdmin):
        list_display = ['master', 'rule_code', 'rule_name', 'threshold_value', 'is_enforced']
        list_filter = ['is_enforced']

if 'GeneralLedgerSchedulePeriod' in globals():
    @admin.register(GeneralLedgerSchedulePeriod)
    class GeneralLedgerSchedulePeriodAdmin(admin.ModelAdmin):
        list_display = ['master', 'period_title', 'day_of_week', 'start_time', 'end_time', 'room_number']
        list_filter = ['day_of_week', 'is_break_period']

if 'GeneralLedgerFeedbackReview' in globals():
    @admin.register(GeneralLedgerFeedbackReview)
    class GeneralLedgerFeedbackReviewAdmin(admin.ModelAdmin):
        list_display = ['master', 'reviewer_name', 'reviewer_role', 'rating', 'is_resolved', 'submitted_at']
        list_filter = ['rating', 'is_resolved']

if 'GeneralLedgerWorkflowTransition' in globals():
    @admin.register(GeneralLedgerWorkflowTransition)
    class GeneralLedgerWorkflowTransitionAdmin(admin.ModelAdmin):
        list_display = ['master', 'from_stage', 'to_stage', 'actor_username', 'is_approved', 'transition_timestamp']
        list_filter = ['is_approved']

if 'GeneralLedgerAccessRule' in globals():
    @admin.register(GeneralLedgerAccessRule)
    class GeneralLedgerAccessRuleAdmin(admin.ModelAdmin):
        list_display = ['master', 'role_allowed', 'can_read', 'can_write', 'can_delete', 'can_export']
        list_filter = ['role_allowed']

if 'GeneralLedgerConfigurationParameter' in globals():
    @admin.register(GeneralLedgerConfigurationParameter)
    class GeneralLedgerConfigurationParameterAdmin(admin.ModelAdmin):
        list_display = ['master', 'param_key', 'param_value', 'data_type', 'is_editable']

if 'GeneralLedgerDocumentAttachment' in globals():
    @admin.register(GeneralLedgerDocumentAttachment)
    class GeneralLedgerDocumentAttachmentAdmin(admin.ModelAdmin):
        list_display = ['master', 'title', 'mime_type', 'file_size_bytes', 'uploaded_by', 'uploaded_at']

if 'GeneralLedgerAuditTrail' in globals():
    @admin.register(GeneralLedgerAuditTrail)
    class GeneralLedgerAuditTrailAdmin(admin.ModelAdmin):
