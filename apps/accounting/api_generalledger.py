"""
Extended REST API Controllers for EduFlow General Ledger & Double-Entry Journals (GeneralLedger).
...
class GeneralLedgerAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(GeneralLedgerMaster, pk=pk)
        data = master.to_dict()
...
class GeneralLedgerAPISearchView(LoginRequiredMixin, View):
...
class GeneralLedgerAPIBatchUpdateView(LoginRequiredMixin, View):
...
class GeneralLedgerAPITelemetryView(LoginRequiredMixin, View):
