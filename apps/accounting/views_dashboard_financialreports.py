"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Financial Statements & Cash Registers (FinancialReports).
...
class FinancialReportsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for FinancialReports."""
    template_name = 'accounting/financialreports_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = FinancialReportsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'FinancialReportsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Financial Statements & Cash Registers Operations Dashboard",
            'domain_name': 'FinancialReports',
...
class FinancialReportsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'accounting/financialreports_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Financial Statements & Cash Registers Analytical Insights",
