"""
Models for EduFlow General Ledger & Double-Entry Journals (GeneralLedger) in apps.accounting.
...
# -----------------------------------------------------------------------------
class GeneralLedgerStatusChoices(models.TextChoices):
...
class GeneralLedgerPriorityChoices(models.TextChoices):
...
class GeneralLedgerTierChoices(models.TextChoices):
...
class GeneralLedgerScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class GeneralLedgerMasterQuerySet(models.QuerySet):
    def active(self):
...
class GeneralLedgerMasterManager(models.Manager):
    def get_queryset(self):
        return GeneralLedgerMasterQuerySet(self.model, using=self._db)
...
class GeneralLedgerMaster(models.Model):
    """
    Primary master configuration entity for General Ledger & Double-Entry Journals.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = GeneralLedgerStatusChoices
    PriorityChoices = GeneralLedgerPriorityChoices
    TierChoices = GeneralLedgerTierChoices
    ScopeChoices = GeneralLedgerScopeChoices
    ...
    objects = GeneralLedgerMasterManager()

    class Meta:
        db_table = 'eduflow_accounting_generalledger_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("GeneralLedger Master")
        verbose_name_plural = _("GeneralLedger Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_gener_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_gener_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_gener_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_gener_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'accounting:generalledger_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/accounting/generalledger/{self.pk}/"
...
# -----------------------------------------------------------------------------
class GeneralLedgerItem(models.Model):
    """Subordinate component or line transaction item for GeneralLedger."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("GeneralLedger Item")
        verbose_name_plural = _("GeneralLedger Items")
...
# -----------------------------------------------------------------------------
class GeneralLedgerAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for GeneralLedger."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_allocation'
        ordering = ['-start_time']
        verbose_name = _("GeneralLedger Allocation")
        verbose_name_plural = _("GeneralLedger Allocations")
...
# -----------------------------------------------------------------------------
class GeneralLedgerMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for GeneralLedger."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("GeneralLedger Metric Record")
        verbose_name_plural = _("GeneralLedger Metric Records")
...
# -----------------------------------------------------------------------------
class GeneralLedgerPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for GeneralLedger."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_rule'
        ordering = ['rule_code']
        verbose_name = _("GeneralLedger Policy Rule")
        verbose_name_plural = _("GeneralLedger Policy Rules")
...
# -----------------------------------------------------------------------------
class GeneralLedgerSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for GeneralLedger."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("GeneralLedger Schedule Period")
        verbose_name_plural = _("GeneralLedger Schedule Periods")
...
# -----------------------------------------------------------------------------
class GeneralLedgerFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for GeneralLedger."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_review'
        ordering = ['-submitted_at']
        verbose_name = _("GeneralLedger Review")
        verbose_name_plural = _("GeneralLedger Reviews")
...
# -----------------------------------------------------------------------------
class GeneralLedgerWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("GeneralLedger Workflow Transition")
        verbose_name_plural = _("GeneralLedger Workflow Transitions")
...
# -----------------------------------------------------------------------------
class GeneralLedgerAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_access'
        verbose_name = _("GeneralLedger Access Rule")
        verbose_name_plural = _("GeneralLedger Access Rules")
...
# -----------------------------------------------------------------------------
class GeneralLedgerConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_config'
        verbose_name = _("GeneralLedger Config Parameter")
        verbose_name_plural = _("GeneralLedger Config Parameters")
...
# -----------------------------------------------------------------------------
class GeneralLedgerDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("GeneralLedger Document Attachment")
        verbose_name_plural = _("GeneralLedger Document Attachments")
...
# -----------------------------------------------------------------------------
class GeneralLedgerAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on GeneralLedger."""
    master = models.ForeignKey(GeneralLedgerMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_accounting_generalledger_audit'
        ordering = ['-timestamp']
        verbose_name = _("GeneralLedger Audit Entry")
        verbose_name_plural = _("GeneralLedger Audit Entries")
