"""
Extended REST API Controllers for EduFlow Chart of Accounts & Financial Ledger (ChartOfAccounts).
...
class ChartOfAccountsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(ChartOfAccountsMaster, pk=pk)
        data = master.to_dict()
...
class ChartOfAccountsAPISearchView(LoginRequiredMixin, View):
...
class ChartOfAccountsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class ChartOfAccountsAPITelemetryView(LoginRequiredMixin, View):
