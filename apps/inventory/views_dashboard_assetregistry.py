"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Inventory Categories & Asset Registry (AssetRegistry).
...
class AssetRegistryDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for AssetRegistry."""
    template_name = 'inventory/assetregistry_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = AssetRegistryAuditTrail.objects.all().order_by('-timestamp')[:10] if 'AssetRegistryAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Inventory Categories & Asset Registry Operations Dashboard",
            'domain_name': 'AssetRegistry',
            'app_title': 'Inventory',
...
class AssetRegistryAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'inventory/assetregistry_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Inventory Categories & Asset Registry Analytical Insights",
