"""
Extended REST API Controllers for EduFlow Inventory Categories & Asset Registry (AssetRegistry).
...
class AssetRegistryAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(AssetRegistryMaster, pk=pk)
        data = master.to_dict()
...
class AssetRegistryAPISearchView(LoginRequiredMixin, View):
...
class AssetRegistryAPIBatchUpdateView(LoginRequiredMixin, View):
...
class AssetRegistryAPITelemetryView(LoginRequiredMixin, View):
