"""
Models for EduFlow Asset Allocations, Stock Movement & Depreciations (AssetAllocations) in apps.inventory.
...
# -----------------------------------------------------------------------------
class AssetAllocationsStatusChoices(models.TextChoices):
...
class AssetAllocationsPriorityChoices(models.TextChoices):
...
class AssetAllocationsTierChoices(models.TextChoices):
...
class AssetAllocationsScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class AssetAllocationsMasterQuerySet(models.QuerySet):
    def active(self):
...
class AssetAllocationsMasterManager(models.Manager):
    def get_queryset(self):
        return AssetAllocationsMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class AssetAllocationsMaster(models.Model):
    """
    Primary master configuration entity for Asset Allocations, Stock Movement & Depreciations.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = AssetAllocationsStatusChoices
    PriorityChoices = AssetAllocationsPriorityChoices
    TierChoices = AssetAllocationsTierChoices
    ScopeChoices = AssetAllocationsScopeChoices
    ...
    objects = AssetAllocationsMasterManager()

    class Meta:
        db_table = 'eduflow_inventory_assetallocations_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("AssetAllocations Master")
        verbose_name_plural = _("AssetAllocations Masters")
        indexes = [
            models.Index(fields=['code', 'status'], name='idx_asset_cd_st'),
            models.Index(fields=['category', 'tier'], name='idx_asset_cat_tr'),
            models.Index(fields=['effective_start_date', 'status'], name='idx_asset_dt_st'),
            models.Index(fields=['created_at', 'priority'], name='idx_asset_cr_pr'),
        ]
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'inventory:assetallocations_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/inventory/assetallocations/{self.pk}/"
...
# -----------------------------------------------------------------------------
class AssetAllocationsItem(models.Model):
    """Subordinate component or line transaction item for AssetAllocations."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("AssetAllocations Item")
        verbose_name_plural = _("AssetAllocations Items")
...
# -----------------------------------------------------------------------------
class AssetAllocationsAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for AssetAllocations."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_allocation'
        ordering = ['-start_time']
        verbose_name = _("AssetAllocations Allocation")
        verbose_name_plural = _("AssetAllocations Allocations")
...
# -----------------------------------------------------------------------------
class AssetAllocationsMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for AssetAllocations."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("AssetAllocations Metric Record")
        verbose_name_plural = _("AssetAllocations Metric Records")
...
# -----------------------------------------------------------------------------
class AssetAllocationsPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for AssetAllocations."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_rule'
        ordering = ['rule_code']
        verbose_name = _("AssetAllocations Policy Rule")
        verbose_name_plural = _("AssetAllocations Policy Rules")
...
# -----------------------------------------------------------------------------
class AssetAllocationsSchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for AssetAllocations."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("AssetAllocations Schedule Period")
        verbose_name_plural = _("AssetAllocations Schedule Periods")
...
# -----------------------------------------------------------------------------
class AssetAllocationsFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for AssetAllocations."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_review'
        ordering = ['-submitted_at']
        verbose_name = _("AssetAllocations Review")
        verbose_name_plural = _("AssetAllocations Reviews")
...
# -----------------------------------------------------------------------------
class AssetAllocationsWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("AssetAllocations Workflow Transition")
        verbose_name_plural = _("AssetAllocations Workflow Transitions")
...
# -----------------------------------------------------------------------------
class AssetAllocationsAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_access'
        verbose_name = _("AssetAllocations Access Rule")
        verbose_name_plural = _("AssetAllocations Access Rules")
...
# -----------------------------------------------------------------------------
class AssetAllocationsConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_config'
        verbose_name = _("AssetAllocations Config Parameter")
        verbose_name_plural = _("AssetAllocations Config Parameters")
...
# -----------------------------------------------------------------------------
class AssetAllocationsDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("AssetAllocations Document Attachment")
        verbose_name_plural = _("AssetAllocations Document Attachments")
...
# -----------------------------------------------------------------------------
class AssetAllocationsAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on AssetAllocations."""
    master = models.ForeignKey(AssetAllocationsMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetallocations_audit'
        ordering = ['-timestamp']
        verbose_name = _("AssetAllocations Audit Entry")
        verbose_name_plural = _("AssetAllocations Audit Entries")
