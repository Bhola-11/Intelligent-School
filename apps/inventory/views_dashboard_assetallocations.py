"""
Operational Dashboard and Realtime KPI Analytics for EduFlow Asset Allocations, Stock Movement & Depreciations (AssetAllocations).
...
class AssetAllocationsDashboardView(LoginRequiredMixin, TemplateView):
    """Executive operations dashboard for AssetAllocations."""
    template_name = 'inventory/assetallocations_dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        # Recent activities
        audit_trail = AssetAllocationsAuditTrail.objects.all().order_by('-timestamp')[:10] if 'AssetAllocationsAuditTrail' in globals() else []
        recent_records = records[:8] if records else []

        ctx.update({
            "page_title": "Asset Allocations, Stock Movement & Depreciations Operations Dashboard",
            'domain_name': 'AssetAllocations',
            'app_title': 'Inventory',
...
class AssetAllocationsAnalyticsView(LoginRequiredMixin, TemplateView):
    """Detailed analytics report view with chart telemetry data."""
    template_name = 'inventory/assetallocations_analytics.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            "page_title": "Asset Allocations, Stock Movement & Depreciations Analytical Insights",
