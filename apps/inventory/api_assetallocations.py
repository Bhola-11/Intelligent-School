"""
Extended REST API Controllers for EduFlow Asset Allocations, Stock Movement & Depreciations (AssetAllocations).
...
class AssetAllocationsAPIDetailView(LoginRequiredMixin, View):
    """Retrieves full nested JSON representation of a master record."""
    def get(self, request, pk, *args, **kwargs):
        master = get_object_or_404(AssetAllocationsMaster, pk=pk)
        data = master.to_dict()
...
class AssetAllocationsAPISearchView(LoginRequiredMixin, View):
...
class AssetAllocationsAPIBatchUpdateView(LoginRequiredMixin, View):
...
class AssetAllocationsAPITelemetryView(LoginRequiredMixin, View):
