"""
Models for EduFlow Inventory Categories & Asset Registry (AssetRegistry) in apps.inventory.
...
# -----------------------------------------------------------------------------
class AssetRegistryStatusChoices(models.TextChoices):
...
class AssetRegistryPriorityChoices(models.TextChoices):
...
class AssetRegistryTierChoices(models.TextChoices):
...
class AssetRegistryScopeChoices(models.TextChoices):
...
# -----------------------------------------------------------------------------
class AssetRegistryMasterQuerySet(models.QuerySet):
    def active(self):
...
class AssetRegistryMasterManager(models.Manager):
    def get_queryset(self):
        return AssetRegistryMasterQuerySet(self.model, using=self._db)
...
# -----------------------------------------------------------------------------
class AssetRegistryMaster(models.Model):
    """
    Primary master configuration entity for Inventory Categories & Asset Registry.
    Maintains governance rules, lifecycle states, budgetary allotments, and KPIs.
    """
    StatusChoices = AssetRegistryStatusChoices
    PriorityChoices = AssetRegistryPriorityChoices
    TierChoices = AssetRegistryTierChoices
    ScopeChoices = AssetRegistryScopeChoices
    ...
    objects = AssetRegistryMasterManager()

    class Meta:
        db_table = 'eduflow_inventory_assetregistry_master'
        ordering = ['-created_at', 'name']
        verbose_name = _("AssetRegistry Master")
        verbose_name_plural = _("AssetRegistry Masters")
        indexes = [
    ...
    def get_absolute_url(self):
        try:
            return reverse(f'inventory:assetregistry_detail', kwargs={'pk': self.pk})
        except Exception:
            return f"/inventory/assetregistry/{self.pk}/"
...
# -----------------------------------------------------------------------------
class AssetRegistryItem(models.Model):
    """Subordinate component or line transaction item for AssetRegistry."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='items')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_item'
        ordering = ['sequence_order', 'id']
        verbose_name = _("AssetRegistry Item")
        verbose_name_plural = _("AssetRegistry Items")
...
# -----------------------------------------------------------------------------
class AssetRegistryAllocation(models.Model):
    """Resource allocations (students, teachers, rooms, staff) for AssetRegistry."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='allocations')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_allocation'
        ordering = ['-start_time']
        verbose_name = _("AssetRegistry Allocation")
        verbose_name_plural = _("AssetRegistry Allocations")
...
# -----------------------------------------------------------------------------
class AssetRegistryMetricRecord(models.Model):
    """Captures KPI metrics, telemetry, and performance measurements for AssetRegistry."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='metrics')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_metric'
        ordering = ['-evaluated_at']
        verbose_name = _("AssetRegistry Metric Record")
        verbose_name_plural = _("AssetRegistry Metric Records")
...
# -----------------------------------------------------------------------------
class AssetRegistryPolicyRule(models.Model):
    """Institutional rules, thresholds, and governance policies for AssetRegistry."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='policy_rules')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_rule'
        ordering = ['rule_code']
        verbose_name = _("AssetRegistry Policy Rule")
        verbose_name_plural = _("AssetRegistry Policy Rules")
...
# -----------------------------------------------------------------------------
class AssetRegistrySchedulePeriod(models.Model):
    """Temporal schedules, time intervals, and recurrent sessions for AssetRegistry."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='schedule_periods')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_period'
        ordering = ['day_of_week', 'start_time']
        verbose_name = _("AssetRegistry Schedule Period")
        verbose_name_plural = _("AssetRegistry Schedule Periods")
...
# -----------------------------------------------------------------------------
class AssetRegistryFeedbackReview(models.Model):
    """Institutional evaluation, stakeholder feedback, and peer reviews for AssetRegistry."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='reviews')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_review'
        ordering = ['-submitted_at']
        verbose_name = _("AssetRegistry Review")
        verbose_name_plural = _("AssetRegistry Reviews")
...
# -----------------------------------------------------------------------------
class AssetRegistryWorkflowTransition(models.Model):
    """Auditable multi-stage approval workflow and transition state machine."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='workflow_transitions')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_workflow'
        ordering = ['-transition_timestamp']
        verbose_name = _("AssetRegistry Workflow Transition")
        verbose_name_plural = _("AssetRegistry Workflow Transitions")
...
# -----------------------------------------------------------------------------
class AssetRegistryAccessRule(models.Model):
    """Object-level access permissions and role-based viewing/editing grants."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='access_rules')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_access'
        verbose_name = _("AssetRegistry Access Rule")
        verbose_name_plural = _("AssetRegistry Access Rules")
...
# -----------------------------------------------------------------------------
class AssetRegistryConfigurationParameter(models.Model):
    """Runtime parameters, feature flags, and custom settings."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='config_parameters')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_config'
        verbose_name = _("AssetRegistry Config Parameter")
        verbose_name_plural = _("AssetRegistry Config Parameters")
...
# -----------------------------------------------------------------------------
class AssetRegistryDocumentAttachment(models.Model):
    """Associated official documentation, uploaded files, and certificates."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='attachments')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_doc'
        ordering = ['-uploaded_at']
        verbose_name = _("AssetRegistry Document Attachment")
        verbose_name_plural = _("AssetRegistry Document Attachments")
...
# -----------------------------------------------------------------------------
class AssetRegistryAuditTrail(models.Model):
    """Immutable audit trail capturing all state mutations on AssetRegistry."""
    master = models.ForeignKey(AssetRegistryMaster, on_delete=models.CASCADE, related_name='audit_records')
    ...
    class Meta:
        db_table = 'eduflow_inventory_assetregistry_audit'
        ordering = ['-timestamp']
        verbose_name = _("AssetRegistry Audit Entry")
        verbose_name_plural = _("AssetRegistry Audit Entries")
